...
請確定您更新了環境中的所有系統需求和相依性。 檢閱目標PHP版本和[必要的PHP設定](https://www.php.net/manual/en/appendices.php)的[PHP移轉附錄](../../installation/prerequisites/php-settings.md#php-settings)。
...
* [正在安裝Elasticsearch](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)
...
請參閱[升級Elasticsearch](https://www.elastic.co/docs/deploy-manage/upgrade/deployment-or-cluster)，以取得有關備份資料、偵測可能的移轉問題，以及在部署到生產環境之前測試升級的完整指示。 根據您目前的Elasticsearch版本，不一定需要重新啟動完整叢集。

Elasticsearch需要Java Development Kit (JDK) 1.8或更新版本。 請參閱[安裝Java Software Development Kit (JDK)](../../installation/prerequisites/search-engine/overview.md#install-the-java-software-development-kit)以檢查已安裝的JDK版本。
...
OpenSearch需要JDK 1.8或更新版本。 請參閱[安裝Java Software Development Kit (JDK)](../../installation/prerequisites/search-engine/overview.md#install-the-java-software-development-kit)以檢查已安裝的JDK版本。
...
>[!NOTE]
>
>這些步驟僅適用於Adobe Commerce 2.4.6和2.4.7。Adobe Commerce 2.4.8及更新版本不再支援Elasticsearch；請改用OpenSearch。

1. 將Elasticsearch 7.x伺服器升級至8.x，並確定已啟動且執行中。 請參閱[Elasticsearch檔案](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)。
...
1. 將Elasticsearch 8.x伺服器降級為7.x，並確定已啟動且執行中。 請參閱[Elasticsearch檔案](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)。
...
Adobe建議將開啟的檔案[ulimit](https://www.gnu.org/software/bash/manual/html_node/Bash-Builtins.html#index-ulimit)設定為大於或等於`65536`的值，但您可以視需要使用較大的值。 您可以在命令列上設定限制，也可以將其設為使用者殼層的永久設定。
