...
此參考包含130個命令，可透過 `bin/magento` 命令列工具。
...
在此例項的「Adobe I/O事件」中建立自訂事件提供者。 如果不指定標籤和說明選項，則必須在系統app/etc/event-types.json檔案中定義這些選項。
...
從配置清單（XML和應用程式配置）建立Adobe I/O中的元資料
...
事件訂閱的規則清單，其中每個規則的格式都為「欄位\|運算元\|值」。
...
將與結帳報價相關的表移動到單獨的資料庫伺服器。 自2.4.2起已棄用，並將移除
...
將與銷售相關的表移動到單獨的資料庫伺服器。 自2.4.2起已棄用，並將移除
