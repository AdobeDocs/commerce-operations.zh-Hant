...
#### 僅提供郵遞區號時，Luma (Rest API)和Graphql不會計算稅額。
...
#### 當「密碼重設請求的最大數量」設定大於0，例如： 3 ，「超過限制錯誤訊息會在重新達到限制之前傳送，即從第二次開始

沒有可用的說明。

_AC-13767_

#### 雖然「密碼重設要求最大數量」設為0 （已停用） ，但「自第2次傳送超過限制的錯誤訊息」
...
#### 在管理員中，當我們嘗試重新排序時，提交訂單按鈕無法點按。 （斷斷續續）
...
#### 在+0時區中返回客戶訂單查詢發運注釋created_at，該時區不在儲存配置的時區中

當使用客戶訂單查詢時，系統現在會從客戶配置的時區中的發運注釋中正確顯示「created_at」欄位。以前，「created_at」欄位顯示在+0時區中，與客戶配置的時區無關。

_AC-8109 - [GitHub問題](https://github.com/magento/magento2/issues/36947) - [GitHub代碼貢獻](https://github.com/magento/magento2/pull/37642)_

#### i18n:collect-phrases中斷了翻譯完整性

`bin/magento i18n:collect-phrases -o`命令現在可以正確收集和添加來自JavaScript和.phtml檔案的新短語，確保翻譯正確反映在翻譯檔案中。以前，系統未能將來自JavaScript檔案的多行翻譯短語和來自.phtml檔案的短語包括在翻譯檔案中，導致翻譯不完整或不正確。

_AC-9843 - [GitHub代碼貢獻](https://github.com/magento/magento2/commit/0c53bbf7)_

#### 訪問動態塊的權限問題

以前，用於限制管理員添加新動態塊時引發錯誤。實施此修復程式後，受限制的管理員可以成功添加動態塊，並編輯該塊而不出現任何錯誤
...
檔案驗證錯誤已更新為「檔案驗證失敗。 請驗證儲存配置中的影像處理設定。」以前，它只是「檔案驗證失敗」。

_ACP2E-2847 - [GitHub代碼貢獻](https://github.com/magento/magento2/commit/39d54c2d)_

#### PageBuilder中的庫顯示舊影像縮略圖，而不是新上載的影像

通過頁面生成器內容中的媒體集為刪除和重新上載的同名影像重新生成影像預覽。

_ACP2E-2957 - [GitHub代碼貢獻](https://github.com/magento/magento2-page-builder/commit/60140cd2) - [GitHub代碼貢獻](https://github.com/magento/magento2/commit/001e5188)_
...
#### GTM針對具有自訂選項的可設定產品，在dataLayer中缺少addToCart事件
...
#### 2.4.8-beta102 package Enterprise edition失敗，發生應用程式例外狀況

沒有可用的說明。

_AC-13501_

#### 執行部分索引時，分配給共用目錄的產品不會反映在前端

部分索引完成後，通過REST API分配給共用目錄的產品現在在店面立即可見。以前，只有在完全重新編製索引後，產品才可見。

_ACP2E-2139 - [GitHub代碼貢獻](https://github.com/magento/magento2/commit/7377de59)_

#### [雲]移動版和案頭版的價格顯示與「我的報價」中不同

在使用目錄總價部分時，Contigal Quote中不再顯示「不需要的包括稅」行。
...
#### [雲端]透過graphql呼叫建立公司時無法設定custom_attributes
...
#### 從禮品登入Magento 2.4.4 Enterprise/Commerce訂購時發生問題
...
#### 購買禮品登入專案時，客戶會看到其登入上沒有的專案

贈品登入更新不再包含不屬於贈品登入的專案。

_ACP2E-3495_

#### [雲端] 「移除全部」確認快顯視窗在未確認的情況下移除購物車專案的問題
...
#### YouTube Nocookie URL在頁面產生器中無法運作
...
#### 由於字元集和定序變更，MariaDB 11.4版安裝:upgrade失敗
...
#### 執行mview cron時發生DDL作業時找不到基底資料表或檢視的錯誤
...
#### 2.4.8-beta1 EE報告因magento例外狀況而無法產生
...
#### 在OrderAddress中查詢selected_options時發生錯誤
...
#### 從Mozilla Firefox瀏覽器的「管理員」建立訂單後，專案會重複
...
#### [Braintree]退款線上儲存交易為transactionid-refall

沒有可用的說明。

_BUNDLE-3394_

#### [Braintree] + [雲端] Braintree （信用卡）訂單無法分割費用

沒有可用的說明。

_BUNDLE-3421_

#### [Braintree] [雲端]Braintree SSL憑證將於6月30日前到期
...
#### ProductInterface中的only_x_left_in_stock屬性在可設定產品上不準確
...
#### CustomerOrder.items_eligible_for_return清單必須與訂單料號一致
...
#### 訪客使用者確認URL會重新導向至訂單狀態頁面，因為它缺少orderRef （針對GuestRMA）
...
#### 符合的註冊客戶功能在Boilerplate中無法運作，需要為來賓啟用trackViewedProduct突變。
...
#### UPS REST devdoc中的「沙箱」和「prod」設定指示更新
...
#### 整合測試失敗Magento\NegotiableQuote\Controller\Quote\DownloadTest：：testCompanyManagerDownloadWithNQSubPermission

沒有可用的說明。

_AC-13716_

#### [資料庫比較]如果資料庫包含無條件的Target規則相關記錄，會發生嚴重錯誤
