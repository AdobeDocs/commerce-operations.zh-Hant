...
* **ACSD-68759** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.9） — 修正使用阿拉伯語地區設定及「出生日期」(DOB)屬性設定為顯示在店面時，建立客戶帳戶失敗的問題。
...
* **ACSD-69129** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.7 || >=2.4.8 &lt;2.4.9） — 修正當嘗試透過REST API更新次要網站的層級價格時，刪除預設基本網站並使用次要網站作為預設導致錯誤的問題。
...
* **ACSD-67686** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p15 &lt;2.4.5 || >=2.4.5-p14 &lt;2.4.6 || >=2.4.6-p12 &lt;2.4.7） — 修正提交空白`Syntax Error: Unexpected <EOF>`請求時發生[!DNL GraphQL]錯誤的問題。
...
* **ACP2E-3977** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正設定&#x200B;**[!UICONTROL Cap Reward Points Balance At]**&#x200B;時[!UICONTROL Rewards Points Balance Redemption Threshold]欄位不可為空白而導致驗證錯誤的問題。
...
* **ACSD-64813** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正透過[!DNL B2B] API從[!DNL REST]共用目錄取消指派類別耗時過長或因大型目錄而逾時的問題。
...
* **ACP2E-3838** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p13 || >=2.4.5-p8 &lt;2.4.5-p12 || >=2.4.6-p6 &lt;2.4.6-p10 || >=2.4.7 &lt;2.4.7-p5） — 修正[!DNL Page Builder] CORS錯誤導致無法在生產模式中儲存管理員面板變更的問題。
...
* **ACSD-65100** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正在[!UICONTROL Maximum Width]設定中移除[!UICONTROL Maximum Height]和[!UICONTROL Media Gallery Image Optimization]的值，在影像最佳化程式期間導致錯誤的問題。
...
* **ACSD-48570** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正當[!UICONTROL Admin]將市集代碼新增至URL **已啟用**&#x200B;而先前導致登入頁面或顯示404頁面時，無法透過按一下&#x200B;*重設密碼連結來存取重設密碼頁面的問題。*
...
* **ACSD-62689** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.5.0） — 修正客戶在&#x200B;**[!UICONTROL Related Product Rules and Widgets]** depth 4 *後無法在*&#x200B;中新增類別的問題。
* **ACSD-62708** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6-p2 || >=2.4.6-p8 &lt;2.4.7-p1） — 修正管理員中[!DNL TinyMCE] 7編輯器字型大小顯示&#x200B;*PT*&#x200B;而非&#x200B;*PX的問題id=&#39;212&#39;/>。*&#x200B;現在，您也可以以&#x200B;*PX*&#x200B;設定字型大小，而非&#x200B;*PT*。
* **ACSD-62758** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.5.0） — 修正&#x200B;**[!UICONTROL Configurable Product]**&#x200B;包含所選選項時，[!DNL URL]詳細資料頁面上的產品影片無法正確呈現的問題。
* **ACSD-62951** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.5.0） — 修正傳送銷退折讓單電子郵件（不含專案和總計）的問題。
* **ACSD-62965** （適用於Adobe Commerce >=2.4.7 &lt;2.5.0） — 修正訂購位置`LocalizedException`回應中未包含[!DNL GraphQL]訊息的問題。
...
* **ACSD-63242** （適用於Adobe Commerce和Magento Open Source >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8） — 修正當新增包含超過10,000個專案的目錄產品時匯入緩慢的問題。
* **ACSD-63062** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 修正套用多個重疊規則時發生錯誤購物車折扣計算的問題。
* **ACSD-62979** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正在[!UICONTROL Store ID]標頭中使用錯誤[!DNL GraphQL]會造成嚴重記憶體錯誤的問題。
...
* **ACSD-61785** （適用於Adobe Commerce >=2.4.4 &lt;2.4.8） — 修正無法透過`reward_warning_notification`突變和[!DNL GraphQL]呼叫更新[!DNL REST API]屬性的問題，使其行為與`reward_update_notification`一致。
...
* **ACSD-61134** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正當購物者取消選取「[!DNL Braintree Vault]」核取方塊以更新其帳單地址時，在結帳工作流程中自動取消選取&#x200B;*[!UICONTROL My billing and shipping address are the same]*&#x200B;付款方式的問題。
...
* **ACSD-61133** （適用於Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.8） — 修正`sales_clean_quotes` [!DNL cron]從未核准的採購單中刪除報價的問題。
* **ACSD-61528** （適用於Adobe Commerce >=2.4.6 &lt;2.4.8） — 修正使用[!UICONTROL Admin]從[!DNL GraphQL]擷取角色時未傳回任何結果的問題。
* **ACSD-61553** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.7） — 修正將具有不同優先順序和&#x200B;**[!UICONTROL Cart Price Rule]**&#x200B;的多個折扣套用至產品時，**[!UICONTROL Maximum Qty Discount is Applied To]**&#x200B;折扣計算錯誤的問題。
...
* **ACSD-45049** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.4-p8 || >=2.4.5 &lt;2.4.6） — 修正客戶&#x200B;*[!UICONTROL Is required]*&#x200B;屬性設定無法按照「管理」中網站範圍正確運作的問題。
...
* **ACSD-54887** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p4 &lt;2.4.4-p9 || >=2.4.5-p3 &lt;2.4.5-p8 || >=2.4.6-p1 &lt;2.4.6-p6） — 修正客戶工作階段到期且啟用[!UICONTROL Persistent Shopping Cart]後，客戶購物車被清除的問題。
...
* **ACSD-60303** （適用於Adobe Commerce >=2.4.4-p9 &lt;2.4.5 || >=2.4.5-p8 &lt;2.4.6 || >=2.4.6-p6 &lt;2.4.8） — 修正啟用HTML縮制時，無法下達管理員訂單的問題。
...
* **ACSD-55566** （適用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.7） — 修正當合併具有相同組合專案的來源和目的地購物車時，`mergeCart`變異在&#x200B;*回應中失敗並出現「*&#x200B;內部伺服器錯誤[!DNL GraphQL]」的問題。
...
* **ACSD-56741** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.7） — 修正當資料庫包含與索引機制和&#x200B;*無關的自訂*&#x200B;觸發程式時，在`setup:upgrade`期間顯示的「[!DNL MySQL]嘗試存取型別為null[!DNL MView]的值上的陣列位移」錯誤訊息。
...
* **ACSD-57074** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.7） — 修正以&#x200B;*開頭且具有*&#x200B;的`attrbute_code`是/否`price_`自訂屬性無法正確搭配索引運作，且具有這類屬性的產品無法在前端取得的問題。
...
* **ACSD-53845** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正當[!DNL MySQL] = 0時的`consumer max_messages`連線逾時問題。
* **ACSD-54890** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正由於`aggregate_sales_report_bestsellers_data`磁碟空間不足而[!DNL MySQL]導致`/tmp`錯誤的問題。
* **ACSD-55112** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正可在&#x200B;*[!UICONTROL Submit review]*&#x200B;驗證的情況下多次點選[!DNL Google reCAPTCHA v3]按鈕的問題。
* **ACSD-54264** （適用於Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7） — 修正錯誤訊息&#x200B;*「您無法更新要求的屬性的問題。 資料列識別碼： store_id&quot;*&#x200B;會在客戶嘗試從其他商店檢視中取出可轉讓的報價時出現。
...
* **ACSD-55628** （適用於Adobe Commerce >=2.4.4-p2 &lt; 2.4.5 || >=2.4.5-p1 &lt; 2.4.6） — 修正了在公司登錄檔格上傳檔案和取代店面客戶屬性檔案的問題。
...
* **ACSD-54040** （適用於Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6） — 修正啟用B2B模組時，*[!UICONTROL Created]*&#x200B;欄位空白的訂單詳細資料問題。
...
* **ACSD-54989** （適用於Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7） — 修正當&#x200B;*[!UICONTROL Enable Purchase Orders]*&#x200B;設為&#x200B;*[!UICONTROL Yes]*&#x200B;且&#x200B;*[!UICONTROL Purchase Order]*&#x200B;設為&#x200B;*[!UICONTROL No]*&#x200B;時，公司管理員無法下訂單的問題。
...
* **ACSD-53998** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.7） — 修正登出客戶帳戶後，根據&#x200B;**[!UICONTROL Dynamic Block]**&#x200B;的&#x200B;**[!UICONTROL Customer Segment]**&#x200B;無法正常運作的問題。
...
* **ACSD-52606** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正使用者按一下&#x200B;*時，顯示錯誤訊息*&#x200B;您的訂單尚未準備好取貨&#x200B;**[!UICONTROL Notify Order is Ready for Pickup]**&#x200B;的問題。
...
* **ACSD-53750** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正多重執行緒&#x200B;*重新索引期間的*&#x200B;中斷的管道或關閉的連線`catalog_product_price`錯誤。
* **ACSD-49843** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.0 || >=2.4.1 &lt;2.4.7） — 修正啟用&#x200B;**[!UICONTROL Payment Action]** = **[!UICONTROL Sale]**&#x200B;設定的線上付款方式自動開立訂購專案發票後，無法使用產品下載連結的問題。
...
* **ACSD-51890** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正可在[!UICONTROL Submit review] v3驗證的情況下多次按一下[!DNL Google reCAPTCHA]按鈕的問題。
...
* **ACSD-51857** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正大型sales_order和`aggregate_sales_report_bestsellers_data`資料庫表格上`sales_order_item` cron報告因主要資料查詢寫入方式而效能緩慢的問題。
...
* **ACSD-50345** （適用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.4 || >=2.4.4-p1 &lt;2.4.6） — 修正Recaptcha v2在提交付款失敗後未重新載入的問題。
...
* **ACSD-49849** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.6） — 修正透過GraphQL向[!DNL PayPal]下達訂單時，客戶電子郵件被取代為[!DNL PayPal Express]電子郵件的問題。
...
* **ACSD-48216** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7） — 修正[!UICONTROL AUTO_INCREMENT]表格的[!UICONTROL inventory_source_item]在[!UICONTROL UPDATE]作業中增加的問題。
* **ACSD-47908** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7） — 修正結帳期間，在出貨步驟上選取來源和數量時，出現「值小於或等於0 」的錯誤。
...
* **ACSD-47937** （適用於Adobe Commerce和Magento Open Source 2.4.4 || >=2.4.5 &lt;2.4.6） — 修正因應用程式層級的快取而不一定傳送降價通知的問題。
...
* **MDVA-43451** （*用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.5*） — 修正錯誤&#x200B;*找不到要求的存放區的問題。 請驗證存放區，然後再試一次。為特定網站設定共用目錄時出現*。
...
* **MDVA-42326** （*適用於Adobe Commerce和Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*） — 修正即使已啟用永久性購物車，客戶在工作階段逾時後發生結帳錯誤的問題。
...
* **MDVA-29400** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;=2.3.7-p1 || >=2.4.0 &lt;=2.4.0-p1*） — 修正以[!DNL PayPal Express Checkout]下重複訂單的問題。
...
* **MDVA-37234** （*適用於Adobe Commerce和Magento Open Source >=2.3.5 &lt;2.4.0 || >=2.4.1 &lt;=2.4.2-p2*） — 修正多次將專案新增至購物車（平行請求）相同SKU的問題，此問題會產生相同購物車ID的重複條列專案。
...
* **MDVA-34948** （*適用於Adobe Commerce和Magento Open Source >=2.3.6 &lt;=2.3.6-p1 || >=2.4.0 &lt;=2.4.0-p1*） — 修正長期執行之查詢（例如`GET_LOCK`）的問題。
...
* **MDVA-38308** （*適用於Adobe Commerce >=2.3.5 &lt;=2.3.6-p1 || >=2.4.0 &lt;=2.4.1-p1 || >=2.4.2 &lt;2.4.4*） — 修正將[!DNL Vimeo]視訊新增至產品的問題。
...
* **MDVA-37874** （*適用於Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;=2.4.2-p1*） — 修正&#x200B;*整張購物車的固定折扣金額*&#x200B;不正確套用至包含多個選項的套件組合產品的問題。
...
* **MDVA-35773** （*適用於Adobe Commerce >=2.3.6 &lt;=2.3.6-p1 || >=2.4.1 &lt;=2.4.2*） — 修正發票上100%折扣訂單的總計未顯示為零的問題。
...
* **MDVA-32694** （*適用於Adobe Commerce >=2.3.0 &lt;2.3.6 || >= 2.4.0 &lt;2.4.2*） — 修正如果產品與非預設商店相關，管理員使用者無法新增產品至可轉讓報價的問題。
...
* **MDVA-30858** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.2*） — 修正[!DNL PayPal]報告&#x200B;**>**&#x200B;銷售&#x200B;**>**&#x200B;結算下未如預期提供&#x200B;**[!DNL PayPal]**&#x200B;結算報告的問題。
...
* **MDVA-20376** （*適用於Adobe Commerce >=2.3.2 &lt;2.3.4*） — 修正下單後登入客戶的&#x200B;*中*&#x200B;沒有customerId = 1`exception.log`這類實體錯誤的問題。
...
* **MDVA-28651** (*適用於Adobe Commerce >=2.3.0 &lt;2.3.6 || >2.4.0 （含B2B擴充功能）*) — 修正透過REST API載入可協商引號時出現效能問題的問題。
...
* **MDVA-29954** (*適用於Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0 || 2.4.2 （含B2B副檔名*）) — 修正&#x200B;*新公司註冊要求*&#x200B;與&#x200B;*您已連結至公司*&#x200B;電子郵件傳送錯誤地址的問題。
* **MDVA-28357** （*適用於Adobe Commerce >=2.3.2 &lt;2.3.6 || >=2.4.0 &lt;2.4.1*） — 針對[!DNL ElasticSearch]索引中的SKU欄位，以具有關鍵字tokenizer的自訂分析器取代標準分析器，讓萬用字元搜尋查詢可搭配包含連字型大小(&quot;-&quot;)的SKU運作。
...
* **MDVA-25028** （*適用於Adobe Commerce >=2.3.2 &lt;2.3.3 || >=2.3.5 &lt;2.3.6*） — 修正當觸發欺詐篩選器時，使用[!DNL PayPal Payflow Pro]下訂單未設定為疑似欺詐狀態的問題。
...
* **MDVA-30841** （*適用於Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*） — 修正分層導覽的問題，其中若將&#x200B;*用作搜尋引擎，則布林型產品屬性的* No[!DNL Elasticsearch]值未納入分層導覽中。
...
* **MDVA-28409** （*適用於Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*） — 修正當資料庫中的過期引號數目非常多時，`sales_clean_quotes` cron工作失敗並出現&#x200B;*記憶體不足*&#x200B;錯誤的問題。
...
* **MDVA-28656** （*適用於Adobe Commerce >=2.3.1 &lt;2.3.6 || >=2.4.0 &lt;2.4.2*） — 修正下列問題：如果下訂單時不需要付款資訊（例如，有100%折扣），且訂單建立了發票，則訂單狀態會變更為&#x200B;*已關閉*，而非「完成」。
...
* **MDVA-26694** （*適用於Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0*） — 修正產品和目錄快取每天過期的問題，不過排程方式不同。
...
* **MDVA-30131** （*適用於Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*） — 修正分層導覽的問題，其中若將&#x200B;*用作搜尋引擎，則布林型產品屬性的* No[!DNL Elasticsearch]值未納入分層導覽中。
