...
* **ACP2E-4698** （適用於Adobe Commerce和Magento Open Source >=2.4.7-p9 &lt;2.4.8） || >=2.4.8-p4 &lt;2.4.9) — 修正再次在頁面產生器文字內容中編輯影像時，可以儲存絕對媒體URL，而非保留可攜式媒體指示詞的問題。
* **ACP2E-4748** （適用於Adobe Commerce >=2.4.7 &lt;2.4.9） — 修正具有大量獎勵點歷史記錄的商店中，獎勵點過期程式緩慢的問題，導致獎勵點過期延遲。
* **ACP2E-4797** （適用於Adobe Commerce >=2.4.8 &lt;2.4.9） — 修正在WYSIWYG編輯器或管理員中的頁面產生器內容中輸入4位元組Unicode字元時，即使資料庫設定為支援utf8mb4，仍會遭到錯誤封鎖的問題。
* **ACP2E-4799** （針對Adobe Commerce，B2B >=1.5.0 &lt;1.5.3） — 修正GraphQL查詢傳回「total_count」值的問題，該值僅反映目前頁面的專案數，而非符合查詢條件的請購單清單總數。`requisition_lists`
* **ACP2E-4805** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正清單中第一個可銷售子產品延遲出現時，包含許多子產品的可設定產品之結帳API請求速度會顯著變慢的問題。
* **ACP2E-4840** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正「產品」GraphQL查詢中要求的數量值傳回null的問題。
* **ACP2E-4870** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.10） — 修正產品警報電子郵件通知忽略商店檢視電子郵件設定的問題。
* **ACP2E-4875** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.10） — 修正在Admin中檢視具有大型通訊錄的客戶帳戶可能會意外登出Admin使用者的問題。
* **ACP2E-4894** （適用於Adobe Commerce >=2.4.7 &lt;2.4.9） — 修正當在大量存放區上啟用非同步索引時，新訂單在管理訂單管理網格中出現的延遲問題。
* **ACP2E-4981** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正頁面產生器產品輪播以不正確的類別位置順序顯示產品的問題，以及在可個別顯示相符的子產品時包含可設定的產品。
* 已更新的版本： **ACSD-45255**，**ACSD-50849**
* 已取代的修補程式： **ACSD-49970-V3**、**ACSD-50260-V2**、**ACSD-54966**、**ACSD-63325**、**ACSD-64212**、**ACSD-66865**、**ACSD-69237**
* 已棄用的修補程式： **ACP2E-4815**
...
* **ACP2E-4626** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正部分店面JavaScript檔案被請求並執行兩次，而造成間歇性重複載入和不穩定行為的問題。
...
* **AC-10737** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7-p5） — 修正bin/magento setup:db:status命令無法辨識JSON資料型別的問題。
* **ACP2E-4402** (Adobe Commerce >=2.4.5 &lt;2.4.6) || >=2.4.8 &lt;2.4.9) — 修正啟用後，建立為停用的產品未新增回相關Target規則結果的問題。
* **ACP2E-4505** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正可能從重複的瀏覽器分頁中儲存過時資料的類別並建立循環相依性的問題。
* **ACP2E-4531** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正變更CMS頁面的URL金鑰未更新頁面階層式URL的問題。
* **ACP2E-4601** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正付款交易處理在某些情況下可能表現無效的問題。
* **ACP2E-4603** （適用於Adobe Commerce >=2.4.6 &lt;2.4.8） — 修正執行「目錄許可權」產品重新索引時，現有許可權索引列未變更，導致更新的類別許可權授予無法可靠地反映在產品上的問題。
* **ACP2E-4706** （適用於Adobe Commerce >=2.4.5 &lt;2.4.9） — 修正Target規則索引器略過Admin範圍中未啟用產品的問題。
* **ACP2E-4720** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正搭售產品與購物車折扣規則無法正確套用或移除免運費的問題。
* **ACP2E-4411** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正購物車頁面及多貨幣商店迷你購物車中，套件組合產品的價格顯示不正確的問題。
* **ACP2E-4475** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正啟用「顯示無庫存產品」選項時，產品清單頁面錯誤地篩選及依價格排序無庫存套件組合產品的問題。
* **ACP2E-4110** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正以非預設貨幣在PDP和PLP上以特殊價格捆綁產品顯示錯誤金額的問題。
* **AC-10698** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 修正系統在所有訂單層級傳送貨幣，而非將其與個別訂單建立關聯的問題。 現在，交易價格和總計會依訂單傳送至Google Tag，改善電子商務資料追蹤的準確性。
* 已更新的版本： **ACSD-67383**，**ACSD-69203**
* 已取代的修補程式： **ACSD-67370**，**ACSD-69319**

## v1.1.78 {#v1-1-78}

* **ACP2E-4416** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正在Admin中建立客戶獎勵點時未初始化的問題。
* **ACP2E-4419** （適用於Adobe Commerce >=2.4.6 &lt;2.4.8） — 修正成功在店面進行reCAPTCHA v2 （「我不是機器人」）驗證後，在結帳時未正確套用禮品卡的問題。
* **ACP2E-4431** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正重新索引程式期間刪除符合目標規則之相關產品的問題。
* **ACP2E-4448** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正Redis復原後，Redis中斷期間所做的設定變更未反映，而造成儲存過時值的問題。
* **ACP2E-4452** （針對Adobe Commerce，B2B >=1.5.1 &lt;1.5.3） — 修正「快速訂購」頁面上的產品價格包含稅捐（無論稅捐顯示組態為何）的問題。
* **ACP2E-4456** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正使用GraphQL突變取消訂單時，未將完全使用禮品卡支付的訂單轉變為「已關閉」狀態的問題。
* **ACP2E-4507** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正透過GraphQL變動提出的客戶密碼重設請求未套用密碼選項組態的問題。
* **ACP2E-4513** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正無法從系統刪除過期驗證碼影像的問題。
* **ACP2E-4522** （適用於Adobe Commerce >=2.4.7 &lt;2.4.9） — 修正同時執行多個購物車合併或報價儲存請求時，quote_coupons表格會發生間歇性重複金鑰錯誤的問題。
* **ACP2E-4528** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正客戶地址中的城市驗證問題，該問題現在允許正斜線(/)字元，並拒絕無效字元，例如！、&#39;、#和？。
* **ACP2E-4535** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正提交忘記密碼表單導致工作階段被摧毀或重新產生（PHPSESSID變更）並清除訪客購物車的問題。
* **ACP2E-4540** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正Fotorama程式庫未正確載入的問題，此問題會導致系統只顯示第一個附加的影像。
* **ACP2E-4555** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正包含「」的現代電話號碼的問題。 或「/」未正確驗證。
* **ACP2E-4565** （針對Adobe Commerce，B2B >=1.5.0 &lt;1.5.3） — 修正使用X-Adobe-Company標頭時，GraphQL公司查詢傳回「目前客戶未獲授權」的問題。
* **ACP2E-4591** （適用於Adobe Commerce >=2.4.4 &lt;2.4.8） — 修正當透過REST API下訂單時，根據訂單計數的客戶區段（例如「首次購買者」）未更新的問題。
* **ACP2E-4609** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.9） — 修正部分引號包含已刪除產品時，我的引號頁面未顯示引號的問題。
* **ACP2E-4613** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.9） — 修正大型媒體目錄結構導致gettree回應緩慢，導致Media Gallery目錄樹狀結構載入時間延長的問題。
* **ACP2E-4628** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正當「帳戶共用」設定為「全域」時，匯入具有大寫電子郵件地址的客戶會導致未定義陣列金鑰錯誤的問題。
* **ACP2E-4665** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正透過REST API提出請求時，產品相簿中含有視訊之可設定產品的子產品未列出的問題。
...
* **ACSD-68664** （適用於Adobe Commerce >=2.4.6 &lt;2.4.9） — 修正嘗試預覽具有自訂網域的存放區內容時，排程更新預覽中斷的問題。
* **ACSD-68759** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p2 &lt;2.4.5） || >=2.4.5-p1 &lt;2.4.9) — 修正使用阿拉伯語地區設定時，建立客戶帳戶失敗的問題，以及出生日期(DOB)屬性設定為顯示在店面的問題。
...
* **ACSD-69237** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正可透過sales_*_async_insert cron工作處理和插入的專案數限製為每次100個的問題。
* **ACSD-69311** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正從商業發票建立部份退款時，若先前銷退折讓單是從訂單檢視頁面建立的，貸項通知單中稅捐計算不正確的問題。
* **ACSD-69351** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正禮卡餘額和到期日顯示方式與所指派網站範圍不符的問題。
* **ACSD-69494** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正非同步退款作業的問題，此作業無法正確處理具有「is_online」引數的退款請求。
* 更新的版本： **ACSD-67250**
* 已取代的修補程式： **ACSD-62629**，**ACSD-66157**
* 已棄用的修補程式： **ACSD-66157**
...
* **ACSD-69086** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 修正cron工作無法清除changelog表格、導致在處理大量資料時蓋拉叢集當機的問題。
* **ACSD-69115** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正為指派給非預設網站的客戶管理購物車時，未向管理員使用者顯示購物車錯誤的問題。
* **ACSD-69129** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.7） || >=2.4.8 &lt;2.4.9) — 修正當嘗試透過REST API更新次要網站的層級價格時，刪除預設基本網站並使用次要網站作為預設值會導致錯誤的問題。
* **ACSD-69203** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正當類別條件中列出多個類別時，產品清單Widget傳回錯誤結果的問題。
* **ACSD-69261** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正因部分發票中`times_used`屬性的處理不正確及剩餘數量取消情況，而多次重複使用設定為每位客戶單次使用的購物車價格規則優惠券的問題。
* **ACSD-69308** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正僅於網站層級設定`special_price`時（非「所有商店檢視」），目錄價格規則不適用的問題。 修正後，請先檢查網站的預設商店，正確套用目錄價格規則。
* **ACSD-69319** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正當子產品有自訂來源下的存貨時，套件價格未正確編制索引的問題。
* **ACSD-69325** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正修改SKU案例導致產品在店面無存貨的問題。
* **ACSD-69331** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.9） — 修正媒體集中的內容建立者無法只建立具有`create_folder`許可權的資料夾的問題。 完成修正後，他們就能如預期建立資料夾。
* **ACSD-69333** （適用於Adobe Commerce >=2.4.7 &lt;2.4.9） — 修正具有作用中排程更新的產品允許SKU變更的問題。 修正後，SKU變更在作用中更新期間被禁止；儲存失敗並出現明確錯誤，管理SKU欄位被停用。 這可防止在預備倒回期間，SKU變更所導致的MSI詳細目錄不一致。
* **ACSD-69541** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正將管理員中的產品數量減少到低於購物車中已有的產品數量時，無法透過GraphQL編輯該購物車中的產品數量的問題。
* 已更新的版本： **ACSD-46541**、**ACSD-53750**、**ACSD-66404**
* 已取代的修補程式： **ACSD-66404**，**ACSD-68499**

## v1.1.75 {#v1-1-75}

* **ACSD-68289** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正如果所有可搜尋欄位整體符合最低相符條件，全文檢索搜尋現在會傳回相符產品的問題，而非要求條件由單一欄位滿足。
* **ACSD-68359** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正使用[!UICONTROL Pick in Store]在結帳時選取商店時，許多產品在購物車中時，不再因為URL過長而失敗的問題。 之前，這會在選取商店期間產生過長的URL，導致客戶無法完成結帳，進而觸發&#x200B;*414錯誤*。
...
* **ACSD-68810** （適用於Adobe Commerce和Magento Open Source >=2.4.8 &lt;2.4.9） — 修正將訂單指派給在其他網站上建立之客戶（無論是否使用&#x200B;**[!UICONTROL Customer Account Sharing]**&#x200B;設定）的問題。
...
* **ACSD-68206** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.9） — 修正使用GraphQL應用程式伺服器並安裝[!DNL PHP Redis]擴充功能時的錯誤。**[!UICONTROL Rate Limiting]**
...
* **ACSD-66807** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正`report_viewed_product_index`表格顯示錯誤產品頁面檢視計數的問題。
* **ACSD-67383** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正在同一工作階段中，使用具有兩個公司管理員帳戶的&#x200B;**[!UICONTROL Login as Customer]**&#x200B;會造成&#x200B;*沒有具有cartId*&#x200B;的實體錯誤的問題。
...
* **ACSD-67696** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正快取排清後，`media_gallery`專案沒有傳回購物車GraphQL產品節點的問題。
...
* **ACSD-67424** （針對Adobe Commerce，B2B >=1.5.0 &lt;1.5.3） — 修正使用可轉讓引號時，`GET /carts/search` [!DNL REST] API回應中的`updated_at`值不符合&#x200B;**[!UICONTROL Admin panel]**&#x200B;中顯示值的問題。
...
* **ACSD-60624** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正&#x200B;**[!UICONTROL Upload Image]**&#x200B;無法用於Page Builder中[!UICONTROL Image]、[!UICONTROL Banner]和[!UICONTROL Slider]區段的空白內容的問題。
...
* **ACSD-67102** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 修正Adobe Commerce後端載入&#x200B;**[!UICONTROL Categories]**&#x200B;非常緩慢的問題。
...
* **ACSD-67686** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p15 &lt;2.4.5） || >=2.4.5-p14 &lt;2.4.6 || >=2.4.6-p12 &lt;2.4.7) — 修正提交空的[!DNL GraphQL]請求時發生`Syntax Error: Unexpected <EOF>`錯誤的問題。
* **ACSD-67250** （適用於Adobe Commerce >=2.4.7-p4 &lt;2.4.8） — 修正&#x200B;**[!UICONTROL Shared Catalog]**&#x200B;儲存作業會更新所有專案，而非僅更新受影響的專案，並透過消除不必要的作業而提升效能的問題。
...
* **ACP2E-3731** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正具有&#x200B;**[!UICONTROL Catalog, Search]**&#x200B;可見度的產品匯出錯誤包含來自多商店環境中其他商店檢視的記錄的問題。
...
* **ACP2E-3977** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正設定[!UICONTROL Rewards Points Balance Redemption Threshold]時&#x200B;**[!UICONTROL Cap Reward Points Balance At]**&#x200B;欄位不可為空白而導致驗證錯誤的問題。
* **ACP2E-4050** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.8） — 修正當使用套件組合產品且使用子選取條件並啟用免費送貨時，購物車價格規則無法正確適用於多送貨產品的問題。
* **ACSD-56226** （適用於Adobe Commerce >=2.4.6 &lt;2.4.7） — 修正啟用`synchronous_replication`旗標時，從節點的「讀取」查詢傳回過期資料的問題。
...
* **ACSD-66149** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正IPN處理常式針對不支援或未知IPN型別傳回&#x200B;*500*&#x200B;錯誤的問題。
* **ACSD-66153** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正由於快取了不正確的配置結構而導致頁面傳回&#x200B;*500*&#x200B;錯誤的問題。
...
* **ACSD-66404** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正cron作業無法清除changelog表格、導致在處理大量資料時[!DNL Galera Cluster]當機的問題。
...
* **ACSD-67347** （適用於Adobe Commerce和Magento Open Source >=2.4.5-p11 &lt;2.4.6） — 修正當使用含特殊字元的優惠券且啟用檔案鎖定時，順序失敗並出現&#x200B;*無法取得鎖定*&#x200B;錯誤的問題。
* 已取代的修補程式： **ACP2E-3841**
...
* **ACSD-66179** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p11 &lt;2.4.5） || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.9) — 修正取消以「未擷取」付款型別建立的發票會導致404錯誤頁面的問題。
...
* **ACSD-66049** （適用於Adobe Commerce和Magento Open Source >=2.4.5-p3 &lt;2.4.6） || >=2.4.7 &lt;2.4.9) — 修正非英文店面因ICU資料庫版本而顯示不正確定價的問題。
...
* **ACSD-66139** （適用於Adobe Commerce >=2.4.7 &lt;2.4.8） — 修正GraphQL呼叫對不存在或非使用中的購物車下訂單時傳回&#x200B;*未定義的*&#x200B;錯誤碼的問題。
...
* **AC-14984** （適用於Adobe Commerce和Magento Open Source >=2.4.6-p10 &lt;2.4.7） || >=2.4.8 &lt;2.4.9) — 修正RabbitMQ SSL連線上的&#x200B;*無效的框架型別21*&#x200B;錯誤。
...
* **ACP2E-3789** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.9） — 修正提供媒體ID時，透過[!DNL WebAPI]重複的媒體檔案更新產品的問題。
...
* **ACSD-66041** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.9） — 修正因遺失`CountryID`而無法搜尋取車地點愛爾蘭(IE)郵遞區號的問題。
...
* **ACSD-64813** （適用於Adobe Commerce >=2.4.4 &lt;2.4.9） — 修正透過[!DNL REST] API從[!DNL B2B]共用目錄取消指派類別耗時過長或因大型目錄而逾時的問題。
...
* **ACSD-65100** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正在[!UICONTROL Media Gallery Image Optimization]設定中移除[!UICONTROL Maximum Width]和[!UICONTROL Maximum Height]的值，在影像最佳化程式期間導致錯誤的問題。
* **ACSD-65127** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正了在生產模式下啟用JavaScript縮制導致[!DNL TinyMCE] 6在瀏覽器主控台中產生錯誤，影響功能和使用者體驗的問題。
...
* **ACSD-65540** （適用於Adobe Commerce，B2B 1.5.2） — 修正更新`company_structure`表格時，因缺少`REGEXP_LIKE`函式而發生SQL語法錯誤的問題。
* **ACSD-65684** （適用於Adobe Commerce、B2B 1.5.2） — 修正更新為[!DNL B2B] 1.5.2後，處理`company_structure`表格中的大量記錄(~100,000+)時，耗用超長時間的效能問題。`Magento_Company`
...
* **ACSD-63406** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p9 &lt;2.4.5） || >=2.4.5-p8 &lt;2.4.6 || >=2.4.6-p6 &lt;2.4.8) — 修正當`persistent_clear_expired` cron作業執行時，任何cron作業都無法清除過期的永久性引號的問題。
...
* **ACSD-65164** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.8） — 修正錯誤訊息&#x200B;*部分選取的專案選專案前無法使用的問題*&#x200B;當使用單一選取的核取方塊自訂選項重新排序可設定產品時發生。
...
* **ACSD-64431** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 請求中包含優惠券代碼資訊的`placeOrder`變異不再擲回內部錯誤，而是顯示已成功下訂單。
* **ACSD-64467** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 修正儲存商店檢視層級的類別說明後，WYSIWYG編輯器顯示為空白的問題。
* **ACSD-64546** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正UI中出現一般錯誤訊息，且UPS傳送標籤建立期間，記錄中會儲存&#x200B;*陣列至字串轉換*&#x200B;例外狀況的問題，確保UI中顯示實際錯誤，且記錄中會儲存正確的錯誤訊息。
...
* **ACSD-48570** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正當&#x200B;**將市集代碼新增至URL**&#x200B;已啟用&#x200B;*而先前導致登入頁面或顯示404頁面時，無法透過按一下[!UICONTROL Admin]重設密碼連結來存取重設密碼頁面的問題。*
* **ACSD-62118** （適用於Adobe Commerce >=2.4.6 &lt;2.4.8） — 修正使用「採購單」方法下達[!DNL B2B]份訂單時，`sales_order_tax_item`表格未完全更新的問題。
* **ACSD-63067** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正所有產品數量皆未正確標示的問題，且僅有一個數量不正確時，訊息&#x200B;*[!DNL Please specify the quantity of product(s).]*&#x200B;會針對分組產品中的所有產品顯示。
* **ACSD-63090** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正將產品新增到購物車後，當產品被刪除時購物車專案被移除的問題。
* **ACSD-63182** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正儲存已啟用&#x200B;**[!DNL MSI]** *的重複套件組合產品時發生錯誤的問題*。
...
* **ACSD-63572** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.8） — 修正索引器處理序終止時，`CatalogRule`索引器暫存表格未清理的問題。
...
* **ACSD-58325** （適用於Adobe Commerce >=2.4.6 &lt;2.4.7） — 修正即使在驗證錯誤後，**[!UICONTROL Import]**&#x200B;按鈕仍可用的問題。
* **ACSD-59083** （適用於Adobe Commerce >=2.4.4 &lt;2.5.0） — 修正某些資料庫更新作業導致&#x200B;*找不到基底資料表或檢視的問題*&#x200B;錯誤（如果[!DNL mview]更新同時執行）。
...
* **ACSD-62481** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.5.0） — 修正即使&#x200B;**[!UICONTROL Persistence]**&#x200B;已啟用，客戶的購物車還是空白的問題。
* **ACSD-62629** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.5.0） — 修正&#x200B;**[!UICONTROL Widgets]**&#x200B;中使用的產品清單未反映類別條件的問題。
* **ACSD-62635** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.5.0） — 修正多商店相關產品在[!DNL GraphQL]產品查詢中無法正確顯示的問題。
* **ACSD-62671** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.5.0） — 修正[!DNL GraphQL]請求在第一次嘗試時未傳回最新位址資訊的問題。
* **ACSD-62689** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.5.0） — 修正客戶在&#x200B;*depth 4*&#x200B;後無法在&#x200B;**[!UICONTROL Related Product Rules and Widgets]**&#x200B;中新增類別的問題。
...
* **ACSD-62758** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.5.0） — 修正[!DNL URL]包含所選選項時，**[!UICONTROL Configurable Product]**&#x200B;詳細資料頁面上的產品影片無法正確呈現的問題。
* **ACSD-62951** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.5.0） — 修正傳送銷退折讓單電子郵件（不含專案和總計）的問題。
* **ACSD-62965** （適用於Adobe Commerce >=2.4.7 &lt;2.5.0） — 修正訂購位置[!DNL GraphQL]回應中未包含`LocalizedException`訊息的問題。
* **ACSD-63286** （適用於Adobe Commerce >=2.4.6 &lt;2.4.7） — 修正透過[!DNL API]指派給共用目錄的產品在執行手動重新索引前不會出現在店面的問題。
* **ACSD-63326** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.5.0） — 修正從後端下訂單後，[!UICONTROL Admin]被重新導向到中斷頁面的問題。
* 更新的版本： **ACSD-51739**
* 已取代的修補程式： **MDVA-43451**，**ACSD-62755**

## v1.1.56 {#v1-1-56}

* **ACSD-63244** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正[!DNL JavaScript]錯誤導致[!DNL Google Maps]無法正確呈現的問題。 修正許多&#x200B;*Uncaught TypeError的問題：這個。_each不是[!UICONTROL Admin]面板中主控台的*&#x200B;函式。
...
* **ACSD-62979** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正在[!DNL GraphQL]標頭中使用錯誤[!UICONTROL Store ID]會造成嚴重記憶體錯誤的問題。
* **ACSD-62971** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正匯入&#x200B;**quantity**&#x200B;欄中有非數值的庫存來源時，**數量**&#x200B;設定為&#x200B;*0*&#x200B;的問題。
* **ACSD-62872** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正唯一屬性驗證的問題，其中排程更新驗證不正確。
* **ACSD-62755** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p11 &lt;2.4.5） || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8) — 修正[!DNL TinyMCE] 7需要在編輯器初始化設定中明確新增字型大小和字型的問題。
...
* **ACSD-62475** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 修正購物車中[!UICONTROL Gift Card]產品不正確合併的問題。
...
* **ACSD-58735** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正受限管理員無法在相關網站的[!UICONTROL Admin]客戶帳戶頁面上檢視放棄的購物車的問題。
...
* **ACSD-60344** （適用於Adobe Commerce >=2.4.4 &lt;2.4.8） — 修正使用&#x200B;**[!UICONTROL Purchase Order]**&#x200B;自動核準時傳送重複訂單確認電子郵件的問題。
* **ACSD-61348** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正透過[!DNL GraphQL]顯示願望清單專案，但在多網站環境中不會顯示在店面的問題。
* **ACSD-61534** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 修正無法使用`bin/magento config:set`命令設定設計設定，且鎖定值可透過表單操控進行變更的問題。 現在無法更新從[!DNL CLI]設定的`--lock-env`或`--lock-conf`鎖定值。
* **ACSD-61785** （適用於Adobe Commerce >=2.4.4 &lt;2.4.8） — 修正無法透過[!DNL GraphQL]突變和[!DNL REST API]呼叫更新`reward_warning_notification`屬性的問題，使其行為與`reward_update_notification`一致。
...
* **ACSD-61134** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正當購物者取消選取「*[!UICONTROL My billing and shipping address are the same]*」核取方塊以更新其帳單地址時，在結帳工作流程中自動取消選取[!DNL Braintree Vault]付款方式的問題。
...
* **ACSD-61522** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正可能將電子郵件地址輸入客體客戶的&#x200B;*[!UICONTROL First Name]*&#x200B;和&#x200B;*[!UICONTROL Last Name]*&#x200B;欄位，並傳送無效訂單確認電子郵件的問題。
...
* **ACSD-61845** （適用於Adobe Commerce和Magento Open Source >=2.4.7-p1 &lt;2.4.8） — 修正只使用&#x200B;*text/html*&#x200B;接受標頭傳送請求時發生的錯誤。
* **ACSD-62056** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正安裝MSI時可設定產品影像上傳失敗的問題。
* **ACSD-62485** (適用於Adobe Commerce >=2.4.4 &lt;2.4.6-p8 || >=2.4.7 &lt;2.4.8) — 修正建立公司時`async.operations.all`消費者停止運作的問題。
...
* **ACSD-60804** （適用於Adobe Commerce >=2.4.4 &lt;2.4.8） — 修正編輯連結至已刪除公司的客戶時，導致在null *上呼叫member函式`getSuperUserId()`發生錯誤*&#x200B;的問題。
...
* **ACSD-61553** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.7） — 修正將具有不同優先順序和&#x200B;**[!UICONTROL Maximum Qty Discount is Applied To]**&#x200B;的多個折扣套用至產品時，**[!UICONTROL Cart Price Rule]**&#x200B;折扣計算錯誤的問題。
...
* **ACSD-59952** （適用於Adobe Commerce >=2.4.4 &lt;2.4.8） — 修正建立具有指派給現有[!UICONTROL Shared Catalog]之群組識別碼的[!UICONTROL Shared Catalog]時發生錯誤的問題。
* **ACSD-60590** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 改善針對大量已下訂單產生[!UICONTROL Bestsellers Aggregated Daily Reports]的效能。
* **ACSD-60673** （適用於Adobe Commerce >=2.4.4 &lt;2.4.8） — 修正結帳時多種付款方式的[!UICONTROL Cart Price Rule]無法適當套用至特定付款方式的問題。
* **ACSD-60684** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.7） — 修正按多個欄位排序的GraphQL產品無法如預期運作的問題。
* **ACSD-60788** （適用於Adobe Commerce >=2.4.7 &lt;2.4.8） — 修正由於內容安全性原則(CSP)錯誤而無法執行[!DNL Google Tag Manager]自訂指令碼的問題。
...
* **ACSD-61366** （適用於Adobe Commerce和Magento Open Source >=2.4.7 &lt;2.4.8） — 修正`setup:static-content:deploy --jobs 4`命令執行時多個工作失敗並出現&#x200B;*連線埠的問題，當為DB連線指定連線埠時，必須在主機引數*&#x200B;錯誤中設定連線埠。
...
* **ACSD-60234** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.8） — 修正付款方式套用折扣時，[!DNL PayPal]上顯示錯誤金額的問題。
...
* **ACSD-59280** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.5） — 修正安裝2.4.4-pX版本時發生的錯誤&#x200B;*呼叫未定義的方法ReflectionUnionType：：getName()*。
* **ACSD-45049** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.4-p8） || >=2.4.5 &lt;2.4.6) — 修正客戶&#x200B;*[!UICONTROL Is required]*&#x200B;屬性設定無法按照Admin網站範圍正確運作的問題。
* **ACSD-46938** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.6） — 修正在`setup:upgrade`期間重新建立DB觸發器的效能問題。
* **ACSD-48210** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正更新特定存放區檢視中的&#x200B;*[!UICONTROL Website Scope]*&#x200B;屬性會覆寫全域範圍中屬性值的問題。
* **ACSD-54887** （適用於Adobe Commerce和Magento Open Source >=2.4.4-p4 &lt;2.4.4-p9） || >=2.4.5-p3 &lt;2.4.5-p8 || >=2.4.6-p1 &lt;2.4.6-p6) — 修正啟用[!UICONTROL Persistent Shopping Cart]的客戶工作階段過期後，客戶購物車被清空的問題。
...
* **ACSD-59514** （適用於Adobe Commerce >=2.4.4 &lt;2.4.7-p2） — 修正具有[!DNL Page Builder]之管理區域中的表單擲回錯誤&#x200B;*[!DNL Page Builder]呈現5秒而未解除鎖定的問題。* 在瀏覽器主控台中提交表單後，變更無法儲存。
...
* **ACSD-58163** （適用於Adobe Commerce >=2.4.3 &lt;2.4.7） — 修正&#x200B;*[!UICONTROL Cart Price Rule]*&#x200B;不針對無優惠券代碼相符購物車的訪客客戶套用折扣的問題。*[!UICONTROL Customer Segment]*
...
* **ACSD-55566** （適用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.7） — 修正當合併具有相同組合專案的來源和目的地購物車時，`mergeCart`變異在[!DNL GraphQL]回應中失敗並出現「*內部伺服器錯誤*」的問題。
* **ACSD-56546** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正當&#x200B;**顯示產品組態外**&#x200B;為&#x200B;*已停用*&#x200B;時，可設定和套件產品在店面顯示為&#x200B;**無庫存**&#x200B;的問題。
* **ACSD-56635** （適用於Adobe Commerce >=2.4.6 &lt;2.4.7） — 修正當匯入用於&#x200B;**帳戶共用**&#x200B;設定為&#x200B;*全域*&#x200B;時，匯入的客戶會以相同的電子郵件地址重複的問題。
* **ACSD-56741** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.7） — 修正當資料庫包含與索引機制和[!DNL MView]無關的自訂[!DNL MySQL]觸發程式時，在`setup:upgrade`期間顯示的「*嘗試存取型別為null*&#x200B;的值上的陣列位移」錯誤訊息。
...
* **ACSD-55241** （適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.7） — 修正下列問題：在多個位址結帳期間，*[!UICONTROL Used]*&#x200B;和&#x200B;*[!UICONTROL Times Used]*&#x200B;屬性針對產生的抵用券顯示不正確的值。
...
* **ACSD-57074** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.7） — 修正以`price_`開頭且具有`attrbute_code`的&#x200B;*是/否*&#x200B;自訂屬性無法正確搭配索引運作，且具有這類屬性的產品無法在前端取得的問題。
...
* **ACSD-56415** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.7） — 修正當資料庫有大量要編制索引的區域性價格資料時，由於`DELETE`查詢，導致部分價格索引效能變慢的問題。
...
* **ACSD-54885** （適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.7） — 修正當管理員使用者使用&#x200B;*登入作為Customer*&#x200B;功能時，多重地址簽出的問題。
...
* **ACSD-54983** （適用於Adobe Commerce >=2.4.2 &lt;2.4.7） — 修正當使用者狀態設為&#x200B;*[!UICONTROL Inactive]*&#x200B;時，無法透過GraphQL要求取得公司使用者UID的問題。
...
* **ACSD-53845** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正當`consumer max_messages` = 0時的[!DNL MySQL]連線逾時問題。
* **ACSD-54890** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正由於`/tmp`磁碟空間不足而`aggregate_sales_report_bestsellers_data`導致[!DNL MySQL]錯誤的問題。
* **ACSD-55112** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正可在[!DNL Google reCAPTCHA v3]驗證的情況下多次點選&#x200B;*[!UICONTROL Submit review]*&#x200B;按鈕的問題。
...
* **ACSD-55628** (適用於Adobe Commerce >=2.4.4-p2 &lt; 2.4.5 || >=2.4.5-p1 &lt; 2.4.6) — 修正了在公司登錄檔單上傳檔案，以及取代店面中客戶屬性的檔案。
...
* **ACSD-54040** (Adobe Commerce >=2.4.4-p5 &lt;2.4.5) || >=2.4.5-p4 &lt;2.4.6) — 修正啟用B2B模組時，順序詳細資料中&#x200B;*[!UICONTROL Created]*&#x200B;欄位空白的問題。
...
* **ACSD-55004** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.7） — 修正上傳大於`php.ini`中設定的值的匯入檔案時，驗證器當機的問題。
* **ACSD-54989** (Adobe Commerce >=2.4.4-p5 &lt;2.4.5) || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7) — 修正&#x200B;*[!UICONTROL Enable Purchase Orders]*&#x200B;設為&#x200B;*[!UICONTROL Yes]*&#x200B;且&#x200B;*[!UICONTROL Purchase Order]*&#x200B;設為&#x200B;*[!UICONTROL No]*&#x200B;時，公司管理員無法下訂單的問題。
* **ACSD-54007** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正匯入客戶資料時的錯誤&#x200B;*「未定義的陣列機碼&quot;_scope&quot;*。
* **ACSD-55031** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.6） — 修正&#x200B;*型別「mixed」在編譯期間不可為nullable*&#x200B;錯誤。
...
* **ACSD-54342** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正錯誤訊息&#x200B;*資料結構中的錯誤：匯入沒有有效資料的CSV檔案時，值會混合*。
* **ACSD-54660** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.6） — 已新增輸入屬性&#x200B;*sort*，以便依`sort_field`和`sort_direction`在GraphQL中排序客戶訂單。
...
* **ACSD-52606** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正使用者按一下&#x200B;**[!UICONTROL Notify Order is Ready for Pickup]**&#x200B;時，顯示錯誤訊息&#x200B;*您的訂單尚未準備好取貨*&#x200B;的問題。
...
* **ACSD-53795** （適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.7） — 修正`indexer_update_all_views` cron作業中資料型別無效的問題。
* **ACSD-52143** （適用於Adobe Commerce和Magento Open Source >=2.4.6 &lt;2.4.7） — 修正產品匯入後移除自訂選項的問題。
* **ACSD-53750** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正多重執行緒`catalog_product_price`重新索引期間的&#x200B;*中斷的管道或關閉的連線*&#x200B;錯誤。
...
* **ACSD-50887** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正產品屬性屬性&#x200B;*[!UICONTROL Use in Search Results Layered Navigation]*&#x200B;可設為&#x200B;*是*，而&#x200B;*[!UICONTROL Use in search]*&#x200B;選項未設為&#x200B;*是*&#x200B;的問題。
* **ACSD-51846** （適用於Adobe Commerce和Magento Open Source >=2.4.3-p2 &lt;2.4.6） — 修正所發生的&#x200B;*內部錯誤*&#x200B;問題，因為並非所有層級的REST API承載都已驗證。
* **ACSD-52906** （適用於Adobe Commerce >=2.3.7 &lt;2.4.7） — 修正屬於相同客戶區段的登入客戶若未正確設定X-Magento-Vary Cookie，會導致某些頁面快取不當的問題。
* **ACSD-52736** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.6） — 修正&#x200B;*購物車價格規則* （包含可設定產品數量需求）無法如預期運作的問題。
* **ACSD-47875** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.7） — 修正管理員使用者無法針對具有詳細目錄管理的特定商店檢視範圍，從管理員將產品新增到客戶購物車的問題。
* **ACSD-53176** （適用於Adobe Commerce >=2.3.7 &lt;2.4.5） — 修正含&#x200B;*的*&#x200B;相關產品規則&#x200B;*為*&#x200B;條件之一時，與產品不符的問題。
...
* **ACSD-52041** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 修正錯誤訊息： *[錯誤] [!DNL Page Builder]呈現5秒而未解除鎖定的問題。* 儲存使用[!DNL Page Builder]編輯的內容時顯示在Chrome瀏覽器中。
...
* **ACSD-51890** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正可在[!DNL Google reCAPTCHA] v3驗證的情況下多次按一下[!UICONTROL Submit review]按鈕的問題。
...
* **ACSD-52398** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正錯誤&#x200B;*要求的數量無法使用*，此錯誤發生在嘗試更新店面購物車中捆綁產品的數量時。
* **ACSD-52786** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.6） — 修正目錄規則條件&#x200B;*SKU為*&#x200B;的問題，此問題會從指定的SKU開始套用至所有產品。
...
* **ACSD-51265** （適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.7） — 修正當系統中捆綁產品過多時，`catalog_product_price`重新索引效能的問題。
...
* **ACSD-50813** （適用於Adobe Commerce >=2.4.5 &lt;2.4.7） — 修正管理員訂單無法新增在SKU中含有斜線的套件產品的問題，該斜線具有[!UICONTROL Add Products by SKU]功能。
...
* **ACSD-51739** （適用於Adobe Commerce >=2.4.6 &lt;2.4.7） — 修正在CompanyTeam GraphQL請求中請求`structure_id`時傳回錯誤的問題。
* **ACSD-51857** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.7） — 修正大型sales_order和`sales_order_item`資料庫表格上`aggregate_sales_report_bestsellers_data` cron報告因主要資料查詢寫入方式而效能緩慢的問題。
* **ACSD-48448** （適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.7） — 修正訂單取消期間發生競爭條件問題，而導致`inventory_reservation`表格中出現重複專案的問題。
...
* **ACSD-50478** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.5） — 修正DB傾印包含觸發程式和&#x200B;*分隔符號* SQL命令時的資料庫復原命令。
...
* **ACSD-50895** （適用於Adobe Commerce >=2.4.5 &lt;2.4.7） — 修正若未設定[!DNL Google Analytics] 4 GTM，則不會引發[!DNL Google Analytics] 3 GTM標籤的問題。
...
* **ACSD-51431** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.7） — 修正即使變更記錄檔中沒有新專案，索引子狀態為&#x200B;*運作中*&#x200B;的問題。
...
* **ACSD-51379** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.7） — 修正無法儲存透過[!UICONTROL Page Builder]對頁面文字內容進行變更的問題。
...
* **ACSD-49877** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.7） — 修正視訊直接連結至遠端視訊檔案而非串流服務時，視訊自動播放在行動裝置[!DNL Safari]上無法運作的問題。
...
* **ACSD-46683** （適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.7） — 修正運送價格顯示&#x200B;*尚未計算的問題*。
...
* **B2B-2598** （適用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.7） — 新增快取功能至[!UICONTROL availableStores]、[!UICONTROL countries]、[!UICONTROL country]、[!UICONTROL currency]和[!UICONTROL storeConfig]個GraphQL查詢。
...
* **ACSD-48771** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.7） — 修正從舊版[!DNL Page Builder]升級欄區塊內容型別的問題。
...
* **ACSD-50234** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.5） — 修正以[!DNL PayPal]下訂單時，確認電子郵件中客戶名稱錯誤的問題。
* **ACSD-49960** （適用於Adobe Commerce和Magento Open Source >=2.4.5 &lt;2.4.7） — 修正客戶訂單格線無法依日期篩選的問題。
* **ACSD-49849** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.4.6） — 修正透過GraphQL向[!DNL PayPal Express]下達訂單時，客戶電子郵件被取代為[!DNL PayPal]電子郵件的問題。
...
* **ACSD-48059** （適用於Adobe Commerce >=2.3.7 &lt;2.4.7） — 修正商家無法儲存類別中的「[!UICONTROL Match product by rule]」的問題。
* **ACSD-48216** （適用於Adobe Commerce和Magento Open Source >=2.3.7 &lt;2.3.8） || >=2.4.0 &lt;2.4.7) — 修正[!UICONTROL UPDATE]作業中[!UICONTROL inventory_source_item]表格的[!UICONTROL AUTO_INCREMENT]增加的問題。
...
* **ACSD-46617** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.6） — 修正&#x200B;*繼續結帳*&#x200B;按鈕呈現灰色的問題，即使小計大於設定的&#x200B;*最低訂購量*&#x200B;亦然。
...
* **ACSD-47336** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.6） — 修正&#x200B;*發生錯誤。* 在Commerce管理員中關閉通知時發生錯誤。
* **ACSD-47559** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.6） — 修正預覽電子郵件範本區域未完全可見的問題。
* **ACSD-47920** （適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.6） — 修正在&#x200B;*允許訪客結帳*&#x200B;關閉時，仍可透過Rest API以訪客使用者身分下訂單的問題。
...
* **ACSD-46520** （*適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.5*） — 修正使用者使用商店信用退款時收到錯誤訂單狀態的問題。
...
* **ACSD-46869** （*用於Adobe Commerce和Magento Open Source >=2.4.4 &lt;2.4.6*） — 修正購物車中的可設定產品未透過&#x200B;*PUT REST API*&#x200B;請求更新而不變更產品數量的問題。
...
* **ACSD-44938** （*適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.4*） — 修正無法在訪客使用者的GraphQL要求中套用`VAT_ID`的問題。
* 更新修補程式：MDVA-43417。

## v1.1.17 {#v1-1-17}

* **ACSD-45241** （*用於Adobe Commerce和Magento Open Source >=2.3.5 &lt;2.4.4*） — 修正建立銷退折讓單後虛擬產品庫存數量計算錯誤的問題。
* **ACSD-43887** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.5*） — 修正啟用公司採購訂單時，結帳付款頁面上顯示不正確詳細資料的問題。
* **ACSD-45143** （*適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.5*） — 修正`setShippingAddressesOnCart`突變不允許將數值區碼設為&#x200B;*region*&#x200B;的問題。
* **ACSD-44591** （*用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.6*） — 修正未通過驗證碼確認下訂單時發生的錯誤。
...
* **MDVA-44940** （*用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.4*） — 修正從管理員儲存類別時發生的SQL錯誤。
* **MDVA-44562** （*用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.2-p2*） — 修正預設商店ID覆寫報價專案的非預設商店ID的問題，儘管該GraphQL請求源自非預設商店檢視。
...
* **MDVA-43605** （*適用於Adobe Commerce和Magento Open Source >=2.3.1 &lt;2.4.5*） — 修正使用Rest API時，訂單資料傳回列總數負值的問題。
* **MDVA-43102** （*適用於Adobe Commerce和Magento Open Source >=2.3.1 &lt;2.4.5*） — 修正透過REST API進行退款時，未正確更新可售數量的問題。
* **MDVA-43178** （*適用於Adobe Commerce和Magento Open Source >=2.4.3-p2 &lt;2.4.5*） — 修正無法在GraphQL中擷取自訂商店的客戶權杖的問題。
* **MDVA-43859** （*適用於Adobe Commerce和Magento Open Source >=2.4.1 &lt;2.4.5*） — 修正錯誤&#x200B;*當已刪除的客戶嘗試登入時，不會記錄任何具有customerId =*&#x200B;的這類實體問題。
...
* **MDVA-39605** （*用於Adobe Commerce和Magento Open Source >=2.3.4 &lt;2.4.5*） — 修正Redis快取TTL （到期日）具有錯誤值的問題。
* **MDVA-43862** （*用於Adobe Commerce和Magento Open Source >=2.3.3 &lt;2.4.5*） — 修正由於GraphQL *UpdateCartItems突變*&#x200B;錯誤，客戶無法更新購物車專案的問題。
* **MDVA-43824** (*適用於Adobe Commerce和Magento Open Source >=2.3.6 &lt;=2.3.7-p3 || >=2.4.1 &lt;2.4.5*) — 修正取消具有折扣的訂單時出現錯誤的問題。
* **MDVA-43451** （*用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.5*） — 修正錯誤&#x200B;*找不到要求的存放區的問題。 請驗證存放區，然後再試一次。* 會在設定特定網站的共用目錄時顯示。
...
* **MDVA-44188** （*用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.5*） — 修正系統核發的電子郵件地址中包含`.-`的電子郵件未傳送的問題。
...
* **MDVA-42645** （*適用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.5*） — 修正當市集信用功能停用時，管理員無法退還獎勵點數的問題。
...
* **MDVA-41628** （*適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.5*） — 修正當新增模組時，現有的受限制管理員使用者取得新資源存取權的問題。
...
* **MDVA-43731** （*用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.4*） — 修正&#x200B;*搜尋同義字*&#x200B;在值新增至&#x200B;*相符的最少字詞數*&#x200B;時不再運作的問題。
* **MDVA-43232** （*用於Adobe Commerce和Magento Open Source >=2.3.4 &lt;2.4.5*） — 修正在[!DNL Visual Merchandiser]中依「特殊價格至底部/頂端」排序產品時，導致儲存類別時發生錯誤的問題。
...
* **MDVA-42269** （*適用於Adobe Commerce和Magento Open Source >=2.4.3-p1 &lt;2.4.5*） — 修正管理員使用者因&#x200B;*TypeError而無法登入管理員的問題： strtotime()預期引數1為字串，但指定*&#x200B;錯誤則為null。
* **MDVA-40830** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.5*） — 修正下單期間多次套用商店點數的問題。
* **MDVA-42237** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.5*） — 修正可設定產品特殊價格在變更其子產品價格後未更新的問題。
...
* **MDVA-43091** （*適用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.4*） — 修正從後端管理員訂購套件組合產品會造成錯誤&#x200B;*的問題。您無法對此產品使用小數數量*。
* **MDVA-40816** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.5*） — 修正相關產品規則顯示規則條件中未定義類別之產品的問題。
* **MDVA-41305** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.5*） — 修正GraphQL突變在加入願望清單後未傳回可設定產品選項的問題。
...
* **MDVA-40175** （*適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.3*） — 修正&#x200B;*按一下以變更送貨方法*&#x200B;在重新訂購期間未在Admin中顯示選擇送貨方法的選項按鈕的問題。
* **MDVA-42768** （*適用於Adobe Commerce和Magento Open Source >=2.3.4 &lt;2.4.5*） — 修正當&#x200B;*顯示無庫存*&#x200B;為「是」時，可設定產品將一般價格顯示為0的問題。
* **MDVA-43201** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.4*） — 修正搭配特定地區設定使用DOB屬性時，客戶登入發生錯誤的問題。
* 更新修補程式：MDVA-35092、MDVA-33970。

## v1.1.9 {#v1-1-9}

* **MDVA-38346** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.5*） — 修正Adobe Commerce時區與本機環境時區不同時日期篩選器無法正常運作的問題。
* **MDVA-42657** （*適用於Adobe Commerce和Magento Open Source >=2.4.1 &lt;2.4.5*） — 修正管理員使用者無法在客戶區段條件中選取類別的問題。
* **MDVA-42806** （*用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.4*） — 修正每次透過REST API更新現有公司時傳送&#x200B;*新公司註冊*&#x200B;電子郵件的問題。
...
* **MDVA-42507** （*適用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.5*） — 修正套用購物車規則的測試更新後，整頁快取遭到清理的問題。
...
* **MDVA-38526** （*適用於Adobe Commerce和Magento Open Source >=2.4.1 &lt;2.4.4*） — 修正管理員使用者無法存取[!DNL Site-Wide Analysis tool]的問題。
* 更新修補程式：MDVA-40101。

## v1.1.8 {#v1-1-8}

* **MDVA-41215** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.4*） — 修正使用者在設定&#x200B;*mage-messages* Cookie （如果已經存在）後收到500錯誤的問題，但沒有任何新訊息。
* **MDVA-41139** （*適用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;2.4.4*） — 修正產品匯入後，可設定產品在其其中一個來源的數量為0時無存貨的問題。
* **MDVA-42326** (*適用於Adobe Commerce和Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) — 修正即使永續性購物車已啟用，客戶在工作階段逾時後結帳時仍發生錯誤的問題。
* **MDVA-42341** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.4*） — 修正當要求具有市集標頭時，`categoryList` GraphQL查詢無法篩選結果的問題。
* **MDVA-38393** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.4*） — 修正簡單產品重新命名時，目錄規則無法對可設定產品運作的問題。
* **MDVA-39153** （*用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.4*） — 修正在Admin中重新排序期間折扣金額計算錯誤的問題。
* 更新修補程式：MDVA-28993、MDVA-41061、MDVA-35984。

## v1.1.7 {#v1-1-7}

* **MDVA-39711** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.3*） — 修正管理員使用者在刪除網站後無法存取客戶格線的問題。
...
* **MDVA-27239** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.3.6*） — 修正無法顯示交叉銷售產品的問題。
...
* **MDVA-39031** （*適用於Adobe Commerce和Magento Open Source >=2.4.1 &lt;2.4.4*） — 修正即使未指派至目標網站，透過GraphQL將產品新增至購物車的問題。
* **MDVA-41597** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.4*） — 修正使用者將多個可設定產品新增至購物車時，發生錯誤的問題GraphQL。
* **MDVA-27456** （*適用於Adobe Commerce和Magento Open Source >=2.3.5 &lt;2.3.7*） — 修正使用者嘗試載入[!DNL Swagger]時發生錯誤的問題。
* **MDVA-32776** （*適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.2*） — 修正下訂單但未送貨時存貨狀態未更新的問題。
...
* **MDVA-41236** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.4*） — 修正先前已移除「結束日期」時，無法建立新的或編輯產品現有排程更新的問題。
* **MDVA-41046** （*用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.4*） — 修正簡單產品具有自訂選項時可指派至可設定/群組產品的問題。
* **MDVA-40545** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.4*） — 修正即使同一頁面有多個節點，僅擷取頁面第一個節點的問題。
* **MDVA-41164** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.3-p1*） — 修正管理員使用者無法儲存或編輯具有檔案或影像型別自訂客戶屬性的公司的問題。
* **MDVA-39229** （*用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.4*） — 修正更新目錄規則中繼更新開始時間後出現下列錯誤的問題： *Cron工作staging_synchronize_entities_period發生錯誤：無法刪除作用中的更新。*
* **MDVA-40619** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.4*） — 修正嘗試在CMS頁面上執行內嵌編輯時，CMS頁面階層變更會導致500錯誤的問題。
...
* **MDVA-40399** （*用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.4*） — 修正無法透過REST API同時建立相同訂單部分發票的問題。
...
* **MDVA-40537** （*用於Adobe Commerce和Magento Open Source >=2.3.4 &lt;=2.4.0-p1*） — 修正使用者在建立存放區檢視時，若存在數個具有相同URL索引鍵的CMS頁面時，會發生錯誤的問題。
...
* **MC-42528** （*適用於Adobe Commerce和Magento Open Source >=2.4.3 &lt;=2.4.3-p1*） — 修正`categoryList` GraphQL查詢傳回指派和未指派類別的問題。
* **MDVA-29400** (*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;=2.3.7-p1 || >=2.4.0 &lt;=2.4.0-p1*) — 修正[!DNL PayPal Express Checkout]中重複訂購的問題。
* **MDVA-26005** （*適用於Adobe Commerce和Magento Open Source >=2.3.4 &lt;=2.3.5-p2*） — 修正無法在購物車價格規則條件的類別樹狀結構中選取類別的問題。
* **MDVA-25631** （*用於Adobe Commerce和Magento Open Source >=2.3.3 &lt;=2.3.5-p2*） — 改善編輯和儲存包含大量客戶的客戶區段的效能。

## v1.1.3 {#v1-1-3}

* **MDVA-40262** （*用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.4*） — 修正GraphQL搜尋查詢未顯示在Admin中常用搜尋詞的問題。
...
* **MDVA-33606** （*適用於Adobe Commerce和Magento Open Source >=2.4.1 &lt;=2.4.2-p2*） — 修正使用者在儲存指派給階層的CMS頁面時收到&#x200B;*唯一條件約束違規*&#x200B;錯誤的問題。
* **MDVA-31590** （*適用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;=2.4.1-p1*） — 修正使用者無法使用MySQL非同步佇列大量更新屬性的問題。
* **MDVA-36309** （*用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;=2.4.2-p2*） — 修正管理格線中依屬性搜尋產品速度緩慢的問題。

## v1.1.2 {#v1-1-2}

* **MDVA-38929** （*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;2.4.4*） — 修正從商店信用支付訂單時，帶有FPT的商業發票顯示錯誤總金額的問題。
* **MDVA-37364** （*用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;=2.4.3*） — 修正日期型別的自訂客戶屬性破壞客戶網格UI的問題。
* **MDVA-39195** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;=2.4.2-p2*） — 修正當重新導向至購物車已啟用時，*加入購物車*&#x200B;按鈕在類別頁面上的非作用中問題。
...
* **MDVA-39043** （*適用於Adobe Commerce和Magento Open Source >=2.3.4 &lt;=2.4.3*） — 修正具有有限存取權的管理員使用者嘗試將&#x200B;*產品* Widget新增至CMS頁面時發生錯誤的問題。
* **MDVA-39966** (*適用於Adobe Commerce和Magento Open Source >=2.3.0 &lt;=2.3.5-p2 || >=2.4.0 &lt;=2.4.0-p1*) — 修正部署不正確地區設定的問題。
...
* **MDVA-38447** （*用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.4*） — 修正兩個問題：在GraphQL回應中傳回&#x200B;*無法個別顯示*&#x200B;可設定的子產品，並使用類別篩選器最佳化GraphQL產品查詢的MySQL查詢。
* **MDVA-40134** （*適用於Adobe Commerce和Magento Open Source >=2.4.2 &lt;2.4.3*） — 修正GraphQL在啟用共用目錄時未傳回相關產品的問題。
* **MDVA-39935** （*適用於Adobe Commerce和Magento Open Source >=2.4.1 &lt;2.4.4*） — 修正GraphQL傳回可在網站層級停用的可設定子產品的問題。

## v1.1.1 {#v1-1-1}

* **MDVA-36021** （*用於Adobe Commerce和Magento Open Source >=2.4.0 &lt;2.4.4*） — 修正Admin中訂單詳細資料頁面上顯示&#x200B;*呼叫成員函式getId()*&#x200B;錯誤的問題。
...
* **MDVA-38468** （*適用於Adobe Commerce >=2.3.2 &lt;=2.3.5-p2*） — 修正儲存CMS頁面時的錯誤： *已有相同識別碼PAGE_ID的專案*。
* **MDVA-34680** (*適用於Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;2.4.3*) — 修正客戶格線中客戶帳戶建立時間未正確篩選的問題。
...
* **MDVA-37082** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.3*） — 修正儲存分組產品的自訂庫存時導致產品在前端無存貨的問題。
* **MDVA-36572** （*適用於Adobe Commerce >=2.3.5 &lt;2.4.3*） — 修正銷退折讓單更新不再導致資料庫中重複產品預訂更新的問題。
...
* **MDVA-37779** （*適用於Adobe Commerce >=2.4.2 &lt;=2.4.4*） — 修正當網站ID與商店ID不一致時，透過GraphQL將可設定產品加入購物車的問題。
...
* **MDVA-37478** （*適用於Adobe Commerce >=2.3.0 &lt;=2.3.7*） — 修正針對透過REST API以&#x200B;*帳戶付款*&#x200B;付款方式下的訂單，Adobe Commerce建立部分發票時擲回錯誤的問題。
...
* **MDVA-30186** （*適用於Adobe Commerce >=2.3.4 &lt;=2.3.5-p2， >=2.4.0 &lt;=2.4.0-p1， >=2.4.2 &lt;=2.4.2-p1*） — 修正GraphQL回應中，屬性選項是依選項值排序而非屬性專案計數的問題。

## v1.0.22 {#v1-0-22}

* **MDVA-36718** （*適用於Adobe Commerce >=2.3.0 &lt;=2.4.2*） — 修正透過API變更舊自訂選項後仍留存的問題。
* **MDVA-35773** (*適用於Adobe Commerce >=2.3.6 &lt;=2.3.6-p1 || >=2.4.1 &lt;=2.4.2*) — 修正發票上100%折扣訂單的「總計」未顯示為零的問題。
...
* **MDVA-36464** （*適用於Adobe Commerce >=2.4.0 &lt;=2.4.2*） — 修正電子郵件通知設定在商店檢視層級無法運作的問題。
...
* **MDVA-36170** （*適用於Adobe Commerce >=2.3.4 &lt;2.4.2*） — 這修正了使用類別快取標籤，GraphQL查詢未快取的問題。
...
* **MDVA-11189** （*適用於Adobe Commerce >=2.3.0 &lt;2.3.5*） — 修正匯入CSV檔案以更新產品庫存後，`cataloginventory_stock`表格中的列會遭到刪除的問題。
* **MDVA-26639** （*適用於Adobe Commerce >=2.3.3-p1 &lt;2.3.6*） — 修正若建立新的訂單確認電子郵件範本，訂單郵件中遺失訂單專案的問題。
* **MDVA-15546** （*適用於Adobe Commerce >=2.3.0*） — 修正建立訂單後，例外狀況記錄中顯示&#x200B;*Column entity_id where子句模稜兩可的*&#x200B;錯誤的問題。
* **MDVA-21095** （*適用於Adobe Commerce >=2.3.0 &lt;2.3.5*） — 修正大量屬性值更新後查詢`INSERT INTO search_tmp`未結束的問題。
...
* **MDVA-33289** （*適用於Adobe Commerce >=2.4.0 &lt;2.4.3*） — 修正結帳期間帳單地址UI顯示原始JavaScript程式碼（如果已啟用[!DNL Google Tag Manager]）的問題。
* **MDVA-35982** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.3*） — 修正管理員使用者受限於特定網站時，無法為相同網站上的訂單建立出貨的問題。
...
* **MDVA-34591** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.3*） — 修正&#x200B;*最大數量折扣套用至*&#x200B;和&#x200B;*折扣數量步驟（購買X）*&#x200B;的購物車規則折扣計算錯誤的問題。
* **MDVA-33704** （*適用於Adobe Commerce >=2.4.0 &lt;2.4.3*） — 修正&#x200B;*店內提貨*&#x200B;送貨選項未顯示的問題，儘管已設定為可用。
* **MDVA-34928** （*適用於Adobe Commerce >=2.3.5 &lt;2.3.5-p2*） — 修正從付款中移除市集點數後，頁面載入器會無限期顯示的問題。
* **MDVA-35254** （*適用於Adobe Commerce >=2.3.1 &lt;2.4.3*） — 修正簽出期間驗證碼的問題。
* **MDVA-35569** （*適用於Adobe Commerce >=2.3.4 &lt;2.4.2*） — 修正指定狀態時GraphQL回應中未填入&#x200B;*已修正產品稅捐*&#x200B;欄位的問題。
* **MDVA-35847** （*適用於Adobe Commerce >=2.4.1 &lt;2.4.3*） — 修正使用自訂客戶屬性時「公司使用者」表單中斷的B2B問題。
* **MDVA-31307** （*適用於Adobe Commerce >=2.4.0 &lt;2.4.2*） — 修正因快取區塊的動態CSP白名單發生問題，導致某些類別出現&#x200B;*記憶體不足*&#x200B;錯誤的問題。

## v1.0.18 {#v1-0-18}

* **MDVA-32655** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.3*） — 刪除數個產品後，將消費者`quoteItemCleaner`的不正確&#x200B;*進行中*&#x200B;訊息狀態修正為正確的&#x200B;*完成*&#x200B;訊息。
* **MDVA-34102** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.3*） — 修正「產品網格」和「管理」區域中「編輯產品」頁面上已停用產品的預設庫存量為零。
...
* **MDVA-34189** （*適用於Adobe Commerce >=2.3.4 &lt;2.4.3*） — 載入管理類別頁面時，修正[!DNL Visual Merchandiser]個查詢的503個第一個位元組逾時。
...
* **MDVA-35064** （*適用於Adobe Commerce >=2.3.3 &lt;2.4.3*） — 修正無法針對透過API建立的可設定產品產生URL重寫的問題。
...
* **MDVA-34867** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.3*） — 修正排程更新之條件欄位集值未儲存的問題。
...
* **MDVA-33393** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.2*） — 修正錯誤&#x200B;*所提供的國家/地區識別碼不存在*。
...
* **MDVA-33976** （*適用於Adobe Commerce >=2.3.0 &lt;2.3.7*） — 修正從套件產品移除第二個選項後，在店面顯示套件產品無存貨的問題。
* **MDVA-33894** (*適用於Adobe Commerce >=2.4.0 &lt;2.4.1 （含B2B擴充功能）*) — 修正快速訂購功能的多個問題，包括新增和移除多個產品以及SKU區分大小寫。
* **MDVA-27664** （*適用於Adobe Commerce >=2.3.4 &lt;2.3.6*） — 修正客戶登錄檔單中顯示錯誤的問題： *錯誤 — 出生日期不應晚於今天。*
...
* **MDVA-34100** (*適用於Adobe Commerce >=2.3.4 &lt;2.4.2 （具有B2B副檔名）*) — 修正無法從公司結構頁面建立公司帳戶的問題。
...
* **MDVA-28511** （*適用於Adobe Commerce >=2.3.5 &lt;2.3.6*） — 修正Name欄位包含特定字元（如重音大寫字母）時無法完成[!DNL PayPal]結帳的問題。
...
* **MDVA-24201** （*適用於Adobe Commerce >=2.3.0 &lt;2.3.5*） — 修正除非手動重新編列索引，否則價格無法反映排程購物車價格規則的問題。
...
* **MDVA-30858** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.2*） — 修正&#x200B;**報告** > **銷售** > **[!DNL PayPal]**&#x200B;結算下未如預期提供[!DNL PayPal]結算報告的問題。
...
* **MDVA-20376** （*適用於Adobe Commerce >=2.3.2 &lt;2.3.4*） — 修正下單後登入客戶的`exception.log`中&#x200B;*沒有customerId = 1*&#x200B;這類實體錯誤的問題。
...
* **MDVA-32545** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.2*） — 修正從管理員建立訂單時未自動送出發票的問題。
* **MDVA-32714** （*適用於Adobe Commerce >=2.3.4 &lt;2.4.1*） — 修正GraphQL產品查詢中客戶群組價格無法正常運作的問題。
...
* **MDVA-30845** （*適用於Adobe Commerce >=2.3.5 &lt;2.3.7*） — 修正&#x200B;*很抱歉，此訂單目前沒有報價的問題*&#x200B;當無法連線至UPS XML/USPS/DHL時顯示，且沒有其他送貨方法可用。
...
* **MDVA-31363** （*適用於Adobe Commerce >=2.3.2 &lt;2.4.2*） — 修正使用&#x200B;*整個購物車*&#x200B;動作的固定金額折扣時，未透過GraphQL套用含抵用券購物車價格規則的問題。
...
* **MDVA-32313** （*適用於Adobe Commerce >=2.3.0 &lt;2.3.4*） — 修正可設定產品會以錯誤的設定選項新增至願望清單的問題。
...
* **MDVA-28651** (*適用於Adobe Commerce >=2.3.0 &lt;2.3.6 || >2.4.0搭配B2B擴充功能*) — 修正透過REST API載入可轉讓引號時出現效能問題的問題。
...
* **MDVA-30428** （*適用於Adobe Commerce >=2.3.4 &lt;2.3.5*） — 修正客戶無法新增產品至願望清單的問題（若此產品已指派給自訂詳細目錄來源）。
...
* **MDVA-30837** （*適用於Adobe Commerce >=2.3.1 &lt;2.3.5*） — 已在免運費方式設定中新增組態設定&#x200B;*包含稅金至金額：是/否*。 當&#x200B;*含稅金額*&#x200B;設定為&#x200B;*是*&#x200B;時，最小訂購金額會以小計+稅捐計算。 當&#x200B;*含稅金額*&#x200B;設定為&#x200B;*否*&#x200B;時，最小訂購金額會以小計計算
...
* **MDVA-30102** （*適用於Adobe Commerce >=2.3.2 &lt;=2.4.0*） — 修正Redis快取快速成長的問題，因為配置快取沒有TTL。
...
* **MDVA-30357** （*適用於Adobe Commerce >=2.3.2 &lt;=2.4.0*） — 修正當cron產生Sitemap時，建立錯誤影像URL的問題。
* **MDVA-30565** （*適用於Adobe Commerce >=2.3.2 &lt;=2.3.3-p1*） — 修正問題：如果已啟用永久性購物車，店面結帳時不會針對訪客客戶顯示具有cartid = 0 *錯誤的實體。*
* **MDVA-29787** （*適用於Adobe Commerce >=2.3.0 &lt;=2.4.0*） — 修正當&#x200B;*是*&#x200B;條件之一用來定義要顯示的產品時，相關產品的目標規則無法運作的問題。
* **MDVA-30977** （*適用於Adobe Commerce >=2.3.4 &lt;=2.3.5-p2*） — 修正重新索引後類別中隨機產品遺失的問題。
...
* **MDVA-30841** (*適用於Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) — 修正階層導覽的問題，若使用[!DNL Elasticsearch]做為搜尋引擎，則階層導覽不會包含布林型別產品屬性的&#x200B;*No*&#x200B;值。
* **MDVA-28191** （*適用於Adobe Commerce >=2.3.3 &lt;2.4.2*） — 修正透過Admin建立訂單期間未載入付款方式的問題。
* **MDVA-29959** (*適用於Adobe Commerce >=2.3.0 &lt;=2.3.3-p1 （具有B2B擴充功能）*) — 修正具有&#x200B;*公司*&#x200B;許可權的受限制管理員使用者無法刪除公司帳戶的問題。
* **MDVA-30265** （*適用於Adobe Commerce >=2.3.3 &lt;2.4.2*） — 修正發票建立後出貨追蹤連結停止運作的問題。
* **MDVA-28409** (*適用於Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) — 當資料庫中的過期引號數目非常多時，修正`sales_clean_quotes` cron作業失敗並出現&#x200B;*記憶體不足*&#x200B;錯誤的問題。
* **MDVA-30593** （*適用於Adobe Commerce >=2.3.0 &lt;2.3.4*） — 修正未清除根據Quote Lifetime設定過期的報價的問題。
...
* **MDVA-30284** （*適用於Adobe Commerce >=2.3.0 &lt;2.4.2*） — 修正目錄搜尋索引子因下列&#x200B;*[!DNL Elasticsearch]錯誤而失敗的問題：已超過索引中欄位總數的限制。*
* **MDVA-29042** (*適用於Adobe Commerce >=2.3.3 &lt;=2.3.4-p2 （具有B2B擴充功能）*) — 修正將新產品新增至共用目錄後，目錄許可權自動變更為&#x200B;*允許*&#x200B;的問題。
* **MDVA-30428** （*適用於Adobe Commerce >=2.3.3 &lt;2.4.2*） — 修正客戶無法新增產品至願望清單的問題（若此產品已指派給自訂詳細目錄來源）。
* **MDVA-28661** (*適用於Adobe Commerce >=2.3.0 &lt;2.4.2 （具有B2B副檔名）*) — 修正公司管理員變更後，「公司使用者」公司帳戶區段中擲回錯誤的問題。
...
* **MDVA-28656** (*適用於Adobe Commerce >=2.3.1 &lt;2.3.6 || >=2.4.0 &lt;2.4.2*) — 修正下列問題：如果下訂單時不需要付款資訊（例如，有100%折扣），且已針對訂單建立發票，則訂單狀態會變更為&#x200B;*已關閉*，而非「完成」。
...
* **MDVA-30164** （*適用於Adobe Commerce >=2.3.1 &lt;2.4.2*） — 修正當存在自訂客戶屬性時，無法從客戶格線匯出客戶記錄的問題。
...
* **MDVA-30232** （*適用於Adobe Commerce >=2.3.1 &lt;2.4.1*） — 修正原始訂單含有禮品卡時無法重新訂購的問題。
* **MDVA-29965** （*適用於Adobe Commerce >=2.3.3 &lt;2.4.0*） — 修正客戶將產品加入購物車時出現無效表單金鑰錯誤的問題。
...
* **MC-35984** （*適用於Adobe Commerce >=2.4.0 &lt;2.4.1*） — 修正商家為退貨授權(RMA)建立運送標籤後，無法在「退貨」頁面上與任何頁面元素互動的問題。
...
* **MDVA-29835** （*適用於Adobe Commerce >2.3.1 &lt;2.4.2*） — 修正禮卡訂單包含兩個代碼而非一個代碼的問題。
* **MDVA-30052** （*適用於Adobe Commerce >=2.3.2-p2 &lt;2.3.5*） — 修正未正確填入私人內容（本機儲存）而導致效能問題的問題。
* **MDVA-30131** (*適用於Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) — 修正階層導覽的問題，若使用[!DNL Elasticsearch]做為搜尋引擎，則階層導覽不會包含布林型別產品屬性的&#x200B;*No*&#x200B;值。
* **MDVA-35514** （*適用於Adobe Commerce >=2.4.0 &lt;2.4.1*） — 修正在「建立封裝」強制回應視窗中建立送貨標籤並將訂購的產品新增至封裝的問題。
