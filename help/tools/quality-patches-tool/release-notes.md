...
* **ACSD-50345** (適用於Adobe Commerce和Magento Open Source>=2.4.3 &lt;2.4.4 || >=2.4.4-p1 &lt;2.4.6) — 修正提交失敗付款後Recaptcha v2未重新載入的問題。
* **ACSD-50817** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 最佳化Cron工作 `sales_clean_quotes` 跑得更快。
* **ACSD-49392** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.0 || >= 2.4.1 &lt;2.4.7) — 修正套件產品的部分退款後訂單狀態變更為已關閉的問題。
* **ACSD-51036** (適用於Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.5) — 修正同時REST API呼叫期間的競爭條件導致中的運送狀態資訊覆寫的問題 [!UICONTROL Items Ordered] 表格。
* **ACSD-50858** (適用於Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.7) — 改善載入橫幅內容的效能。
* 新增MDVA-39305-v2、ACSD-45169的新版本。
* 更新修補程式ACSD-50260-v2。
...
* **ACSD-48362** (適用於Adobe Commerce >=2.4.1 &lt;2.4.7) — 修正使用可轉讓報價下單時使用預設運送地址而非新地址的問題。
* **ACSD-48059** (適用於Adobe Commerce >=2.3.7 &lt;2.4.7) — 修正商戶無法儲存「[!UICONTROL Match product by rule]」。
* **ACSD-48216** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) — 修正 [!UICONTROL AUTO_INCREMENT] 的 [!UICONTROL inventory_source_item] 表格在 [!UICONTROL UPDATE] 操作。
* **ACSD-47908** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) — 修正結帳期間於裝運步驟上選取來源和數量時，出現「值小於或等於0」錯誤。
* **ACSD-49497** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.6) — 修正發運後訂單仍處於處理狀態，且套用部分退款的問題。
* **ACSD-48694** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.3.8 || >=2.4.1 &lt;2.4.7) — 修正「請求無效狀態變更」錯誤導致客戶無法下訂單的問題。
* **ACSD-49013** (適用於Adobe Commerce和Magento Open Source>=2.4.3 &lt;2.4.7) — 修正使用大量API建立客戶時，電子郵件確認未翻譯為網站地區設定的問題。
* **ACSD-48164** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修正受限管理員無法儲存網站層級值的問題。
* **ACSD-48404** (適用於Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.4) — 修正按瀏覽器的上一步按鈕時，「記住類別分頁=是」會導致錯誤的問題。
* **ACSD-48634** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修正測試更新頁面上，當「[!UICONTROL Google Analytics Content Experiments]」。
* **ACSD-49042** (適用於Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.5) — 修正無法從Storefront訂購無限延購產品的問題。
* 更新的修補程式：ACSD-48366、ACSD-48661。

## v1.1.26 {#v1-1-26}

* **ACSD-47937** (適用於Adobe Commerce和Magento Open Source2.4.4 || >=2.4.5 &lt;2.4.6) — 修正應用程式層級快取不一定會傳送降價通知的問題。
* **ACSD-48661** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修正公司信用限制大於999時，逗號分隔符號會因驗證錯誤而阻止公司儲存的問題。
* **ACSD-48773** (適用於Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.7) — 修正從錯誤商店擷取獎勵點電子郵件範本的問題。
* **ACSD-48587** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修正產品介面工具集比對規則中HTML特殊字元時，無法顯示相符產品的問題。
* **ACSD-48212** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.6) — 修正產品匯入將產品指派至錯誤來源的問題。
* **ACSD-47988** (適用於Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.6) — 修正產品匯出從頁面產生器產品說明中刪除HTML標籤的問題。
* **ACSD-48366** (適用於Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.6) — 修正產品影像未顯示在「返回股票」電子郵件範本的問題。
* **ACSD-48417** (針對Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.7) — 修正在建立產品排程變更並儲存其他產品後，出現SQL錯誤的問題。

## v1.1.25 {#v1-1-25}

* **ACSD-48058** (適用於Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.6) — 修正如果套件產品未指派給任何網站，產品價格重新索引無法運作的問題。
* **ACSD-48262** (適用於Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.6) — 修正當「每頁允許所有產品」設定設為「是」時，前端無法顯示產品的問題。
* **ACSD-48293** (針對Adobe Commerce和Magento Open Source>=2.4.3 &lt;2.4.4) — 修正當售出的子產品傳回至存貨時，複合產品無存貨的問題。
* **ACSD-47520** (適用於Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.6) — 修正建立貸項通知單時客戶失去獎勵點的問題。
* **ACSD-48044** (適用於Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.4) — 修正將多份禮品卡套用至具有多份運送的單份訂單時，無法下單的問題。
* **ACSD-48300** (適用於Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.6) — 修正移除可設定產品時無法建立傳回的問題。
* **ACSD-47910** (適用於Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.6) — 修正相關實體網格中缺少訂單、發票、發運和貸項通知單的問題。
* **ACSD-47292** (適用於Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.6) — 修正如果「顯示無庫存產品」設為「是」，GraphQL回應中無法提供無庫存套件產品的問題。
* **ACSD-48234** (適用於Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.6) — 修正啟用「顯示無存貨」選項時，目錄搜尋結果顯示錯誤類別項目計數的問題。
* **ACSD-48313** (針對Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.5) — 修正如果屬性值包含逗號，則不會剖析「configurable_varations」欄的問題。 「additional_attributes」使用相同的解析算法。
* **ACSD-48627** (適用於Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.6) — 修正當傳送GraphQL請求以取得購物車詳細資訊時，無庫存可設定產品造成錯誤的問題。
* 更新的修補程式：MDVA-39384。
...
* **ACSD-46519** (適用於Adobe Commerce和Magento Open Source>=2.4.1 &lt;2.4.6) — 修正GraphQL categoryList要求針對錨點類別傳回錯誤product_count的問題。
* **ACSD-47027** (適用於Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.6) — 修正緩慢的updateCompanyRole GraphQL要求。
...
* **MDVA-44562** (*適用於Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.2-p2*) — 修正儘管GraphQL請求源自非預設商店檢視，但報價項目的非預設商店ID仍以預設商店ID覆寫的問題。
...
* **MDVA-41631** (*適用於Adobe Commerce和Magento Open Source>=2.4.1 &lt;2.4.4*) — 修正使用者嘗試擷取訂單資訊時，未選用 *電話* 價值。
...
* **MDVA-40120** (*適用於Adobe Commerce和Magento Open Source>=2.4.1 &lt;2.4.4*) — 修正GraphQL依DESC/ASC排序無法搭配具有相同關聯性或價格的產品使用的問題。
...
* **MDVA-39031** (*適用於Adobe Commerce和Magento Open Source>=2.4.1 &lt;2.4.4*) — 修正即使產品未指派給目標網站，仍可透過GraphQL將產品新增至購物車的問題。
...
* **MDVA-40262** (*適用於Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.4*) — 修正GraphQL搜尋查詢未顯示於「管理員」熱門搜尋詞的問題。
* **MDVA-40601** (*適用於Adobe Commerce和Magento Open Source>=2.3.1 &lt;=2.4.2-p2*) — 修正使用者嘗試取得類別相關資訊時，透過GraphQL排程更新所變更的問題。
...
* **MDVA-39521** (*適用於Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.4*) — 修正使用者無法透過GraphQL，以空電話號碼在購物車上設定運送地址的問題。
...
* **MDVA-38447** (*適用於Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.4*) — 修正兩個問題：其中 *不單獨顯示* 可在GraphQL回應中傳回可設定的子產品，並使用類別篩選器最佳化GraphQL產品查詢的MySQL查詢。
* **MDVA-40134** (*適用於Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.3*) — 修正啟用共用目錄時，GraphQL未傳回相關產品的問題。
* **MDVA-39935** (*適用於Adobe Commerce和Magento Open Source>=2.4.1 &lt;2.4.4*) — 修正GraphQL傳回可設定子產品時，網站層級已停用的問題。
...
* **MDVA-38270** (*適用於Adobe Commerce >=2.4.1 &lt;2.4.3*) — 修正GraphQL中訂購總計中遺失禮品卡資訊的問題。
...
* **MDVA-37362** (*適用於Adobe Commerce >=2.3.4 &lt;=2.4.2-p1*) — 修正可設定產品選項值和變體屬性值在GraphQL回應中空白的問題。
* **MDVA-37288** (*適用於Adobe Commerce 2.4.2*) — 修正GraphQL要求後傳回錯誤階層價格的問題。
...
* **MDVA-35569** (*適用於Adobe Commerce >=2.3.4 &lt;2.4.2*) — 修正 *固定產品稅* 指定狀態時，不會在GraphQL回應中填入欄位。
...
* **MDVA-35312** (*適用於Adobe Commerce >=2.4.1-p1 &lt;2.4.2*) — 修正空的GraphQL要求時的回應代碼500。
...
* **MDVA-33975** (*適用於Adobe Commerce >=2.3.4 &lt;2.4.2*) — 修正GraphQL要求中與價格計算相關的多個問題。
...
* **MDVA-32714** (*適用於Adobe Commerce >=2.3.4 &lt;2.4.1*) — 修正客戶群組價格在GraphQL產品查詢中無法運作的問題。
...
* **MDVA-31363** (*適用於Adobe Commerce >=2.3.2 &lt;2.4.2*) — 修正具有抵用券的購物車價格規則，在 *整個購物車的固定金額折扣* 動作。
...
* **MDVA-30123** (*適用於Adobe Commerce >=2.3.4 &lt;2.4.2*) — 修正屬性選項標籤無法針對GraphQL查詢正確轉譯的問題。
...
* **MDVA-30444** (*適用於Adobe Commerce >=2.3.0 &lt;2.4.1*) — 修正使用GraphQL下訂單時未傳送任何確認電子郵件的問題。
