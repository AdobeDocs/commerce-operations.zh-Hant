...
*期間出現錯誤訊息*&#x200B;嘗試存取型別null`setup:upgrade`值的陣列位移，因為資料庫中的自訂MySQL觸發程式與索引和[!DNL MView]無關。
