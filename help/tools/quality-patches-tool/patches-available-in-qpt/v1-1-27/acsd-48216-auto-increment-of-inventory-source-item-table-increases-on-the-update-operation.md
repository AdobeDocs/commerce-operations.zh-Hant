...
ACSD-48216修補程式修正了&#x200B;*inventory_source_item*&#x200B;資料表的&#x200B;*AUTO_INCREMENT*&#x200B;在&#x200B;*UPDATE*&#x200B;作業中增加的問題。 安裝[[!DNL Quality Patches Tool (QPT)]](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/tools/quality-patches-tool/quality-patches-tool-to-self-serve-quality-patches) 1.1.27時，即可使用此修補程式。 修補程式ID為ACSD-48216。 請注意，此問題已排程在Adobe Commerce 2.4.7中修正。
...
`AUTO_INCREMENT`資料表的`inventory_source_item`會在`UPDATE`作業中增加。

<u>要再現的步驟</u>：

1. 檢查`AUTO_INCREMENT`資料表中目前的`inventory_source_item`值：
...
1. 再次檢查`AUTO_INCREMENT`資料表的`inventory_source_item`值。

<u>預期結果</u>：

`AUTO_INCREMENT`資料表的`inventory_source_item`值在每次更新作業後都不會增加。

<u>實際結果</u>：

每次更新作業後，`AUTO_INCREMENT`資料表的`inventory_source_item`值都會增加。
