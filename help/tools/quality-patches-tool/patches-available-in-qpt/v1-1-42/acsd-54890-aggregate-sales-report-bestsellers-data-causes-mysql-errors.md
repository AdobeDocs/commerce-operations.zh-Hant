...
ACSD-54890修補程式修正`aggregate_sales_report_bestsellers_data`造成[!DNL MySQL]錯誤（因為`/tmpdisk`空間不足）的問題。 安裝[[!DNL Quality Patches Tool (QPT)]](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/tools/quality-patches-tool/quality-patches-tool-to-self-serve-quality-patches) 1.1.42時，即可使用此修補程式。 修補程式ID為ACSD-54890。 請注意，此問題已排程在Adobe Commerce 2.4.7中修正。
...
由於`aggregate_sales_report_bestsellers_data`空間不足，**[!DNL MySQL]**&#x200B;導致`/tmpdisk`個錯誤。

<u>要再現的步驟</u>：

當`aggregate_sales_report_bestsellers_data`表格有大量的記錄（如數千萬筆記錄）時，執行`sales_bestsellers_aggregated_daily` cron工作。
