...
# ACSD-67089：設定`page_size`時，`inventory/export-stock-salable-qty` API傳回不正確的`total_count`
...
設定`page_size`時，`inventory/export-stock-salable-qty` API傳回不正確的`total_count`值。
...
當`page_size`設定為&#x200B;*5*&#x200B;時，`inventory/export-stock-salable-qty` API傳回不正確的&#x200B;*5*&#x200B;的`total_count`，而不是實際的&#x200B;*22*&#x200B;總數。
