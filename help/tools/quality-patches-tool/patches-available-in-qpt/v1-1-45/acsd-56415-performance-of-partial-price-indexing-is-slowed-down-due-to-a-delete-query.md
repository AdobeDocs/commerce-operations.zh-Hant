...
# ACSD-56415：由於[!UICONTROL Partial Price Indexing]查詢，`DELETE`的效能變慢

ACSD-56415修補程式修正了資料庫含有大量部分價格資料索引時，[!UICONTROL Partial Price Indexing]由於`DELETE`查詢而效能變慢的問題。 安裝[[!DNL Quality Patches Tool (QPT)]](https://experienceleague.adobe.com/zh-hant/docs/commerce-operations/tools/quality-patches-tool/quality-patches-tool-to-self-serve-quality-patches) 1.1.45時，即可使用此修補程式。 修補程式ID為ACSD-56023。 請注意，此問題已排程在Adobe Commerce 2.4.7中修正。
...
快速執行來自`main_table`的SQL查詢DELETE `catalog_product_index_price`。

<u>實際結果</u>：

來自`main_table`的SQL查詢DELETE `catalog_product_index_price`執行速度非常慢。
