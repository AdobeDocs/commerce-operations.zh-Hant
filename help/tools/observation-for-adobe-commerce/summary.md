...
**Web:** 事務是使用HTTP請求啟動的。 對於大多陣列織來說，這些事務代表以客戶為中心的交互，因此是最重要的要監控的事務。

**非Web:** 非Web事務不是使用Web請求啟動的。 它們可以包括非Web工作進程、後台進程、指令碼、消息隊列活動和其他任務。

看 **[!UICONTROL Transaction Overview]** 框架中，APDEX平均得分為0.76的交易接近5.3萬次，其中95%的交易是在2.313秒內發生的。 如果在較短的時間段內出現APDEX命中，那麼較短的時間段可能顯示與當前平均值的偏差。

## [!UICONTROL 404 page errors frame]

![404頁錯誤幀](../../assets/tools/404-page-errors.jpg)

的 **[!UICONTROL 404 page errors]** 框列出 [URI](https://en.wikipedia.org/wiki/Uniform_Resource_Identifier) 以及在所選時間段內404頁錯誤的計數。

## [!UICONTROL % of Storage Free frame]

![可用儲存幀的百分比](../../assets/tools/percent-of-storage-free.jpg)

的 **[!UICONTROL % of Storage Free]** frame顯示群集所有節點上儲存裝載的平均空閒百分比。 例如，如果有三個節點群集，則此圖將顯示\&lt;mount point=&quot;&quot;>, \&lt;environment name=&quot;&quot;>。 如果跨三個節點存在差異，則此幀可能具有欺騙性。 超差的示例是： `/data/mysql` 掛載點free是三個節點群集中的另一個值。 下面有個框架 [!UICONTROL MySQL] 按節點名稱切分裝入點以更準確地查看 `/data/mysql` 每個節點上的儲存是免費的。

## [!UICONTROL % of system memory that is free frame]

![空閒幀的系統記憶體百分比](../../assets/tools/percent-of-system-memory-that-is-free.jpg)

此幀按節點顯示每個節點上可用的系統記憶體量。
...
所有環境和節點的聚合顯示在 **[!UICONTROL CPU % by host]** 框。 您應取消選擇非生產環境。 您應注意生產環境的所有節點都不存在的任何實例。 本文就CPU利用率高提供了一些技巧： [使用New Relic在Adobe Commerce診斷效能](https://support.magento.com/hc/en-us/articles/360042149832#high_cpu_usage)。

## [!UICONTROL Alerts during timeframe]

![警報](../../assets/tools/alerts-during-timeframe.jpg)

的 **[!UICONTROL Alerts during timeframe]** 顯示所有警報，包括 [!UICONTROL Managed Alerts] Adobe Commerce支援補充說。

## [!UICONTROL CPU Usage]

![CPU使用率](../../assets/tools/cpu-usage.jpg)

如果 **[!UICONTROL CPU Usage]** 框架為空，它表示基礎架構應用程式 [!DNL New Relic] 未啟用。 如果您的站點在Starter上，您將看不到此資訊。 如果您的站點位於Pro上，請開啟支援票證以 [!DNL New Relic Infrastructure] 為您的站點啟用。
...
的 **[!UICONTROL Response Code]** frame是Web流量和請求響應代碼的一個很好的指示。 是 [!DNL New Relic] 事務資料，並且它由多面 `httpResponseCode` 的子菜單。

## [!UICONTROL Web Traffic volume compared with one week ago Magento Managed Alerts Information]

![Web流量與一週前相比](../../assets/tools/web-traffic-volume-compared.jpg)

此框架顯示一週前與當前卷相比的Web通信量。
...
的 **[!UICONTROL Deployment State]** 從部署日誌中框架facets特定部署短語。 以下是日誌和方面名稱中計算的短語示例：
...
的 **[!UICONTROL IP Frequency]** 幀計數來自IP的每個IP的（「MISS」和「PASS」）狀態 [!DNL Fastly] 日誌。 具有這些狀態的Web請求將到達源伺服器，並將向伺服器添加負載。 它以頻率顯示前20個地址。 此框架可用於檢測網站上的IP攻擊或重負載源。

## [!UICONTROL IP Response – top 20 URLs in duration]

![ip響應 — 持續時間前20個url](../../assets/tools/ip-response-top-20-urls.jpg)

此框架顯示響應持續時間最長的URL。 它可能指示具有最長響應持續時間的大型影像檔案或頁面、API或頁面。
...
的 **[!UICONTROL Potential Bots]** frame會查看請求名稱為request_user_agent的請求，如NULL或「%bot%」。 通常，「%bot%」request_user_agent將遵循中的策略設定 `robots.txt` 的子菜單。
...
的 **[!UICONTROL Nginx access by node]** 框查看來自 `access.log` 按鈕。 瞭解載荷是否均勻分佈是很有幫助的。 它經常顯示節點何時下降。 這還將顯示整個站點的負載。

## [!UICONTROL Galera Log]

![加萊拉日誌](../../assets/tools/galera-log.jpg)

[加萊拉](https://galeracluster.com/library/galera-documentation.pdf) 用於資料庫群集。 此幀聚焦於來自 [!UICONTROL Galera] 群集。 這些信號主要針對進入和退出群集的節點，這是維護資料庫資料完整性的正常行為。 節點保持同步，因為 [!UICONTROL Galera] 群集狀態更改。
...
[雷迪斯](https://redis.io/docs/about/) 是用作資料庫、快取和消息代理的開源（BSD許可）記憶體中資料結構儲存。 如果已配置，它可以執行資料庫和會話快取。 的 **[!UICONTROL Redis synchronization from Log]** 幀聚焦 [Redis同步](https://redis.io/docs/manual/replication/)。 越大 [!DNL Redis] 資料集，同步問題（要保持同步的資料越多）的可能性越大。
...
PHP進程的行為方式取決於 [配置](https://www.php.net/manual/en/install.fpm.configuration.php)。 配置複雜，有許多變數和選項。 的 **[!UICONTROL PHP process states]** 框架有助於瞭解何時終止和重新啟動PHP進程。

### [!UICONTROL PHP errors]

![php錯誤](../../assets/tools/php-errors.jpg)

的 **[!UICONTROL PHP errors]** frame顯示選定時間範圍內工作程式出現的PHP錯誤數。 有關詳細資訊，請參閱 [Adobe CommercePHP設定](https://devdocs.magento.com/guides/v2.4/install-gde/prereq/php-settings.html)。
...
[PHP-FPM](https://php-fpm.org/)的 [!UICONTROL FastCGI Process Manager] 使用 [!DNL Nginx]。 要瞭解系統要求，請參閱 [映射到Adobe Commerce版本的PHP版本要求](https://devdocs.magento.com/guides/v2.4/install-gde/system-requirements.html)。 的 **[!UICONTROL PHP processes]** 幀顯示選定時間線中特定時間運行的PHP進程數。

## [!UICONTROL Secondary processes]

![輔助進程](../../assets/tools/secondary-processes.jpg)

輔助進程可能會影響站點響應。 的 **[!UICONTROL Secondary processes]** 框架可以指示可能向站點添加負載的進程。 資料庫主要運行的輔助進程數最多。

## [!UICONTROL Traffic vs Week Ago]

![流量與一週](../../assets/tools/traffic-vs-week-ago.jpg)

的 **[!UICONTROL Traffic vs Week Ago]** frame查看來自 [!DNL Fastly] 具有(「MISS」、「PASS」)快取狀態的日誌。 這些請求將載入到源伺服器。 此框架顯示Web請求卷，而一週前在同一時間段顯示。

## [!UICONTROL Fastly Cache]

![快速快取](../../assets/tools/fastly-cache.jpg)

的 **[!UICONTROL Fastly Cache]** frame顯示來自「快速」日誌的請求的快取狀態的聚合視圖。 如果按一下「ERROR（錯誤）」 ，則它將顯示請求中的錯誤百分比。 當源伺服器對頁面請求的響應速度不夠快時，這一比例通常會增加。

## [!UICONTROL Page Rendering]

![頁面呈現](../../assets/tools/page-rendering.jpg)

的 **[!UICONTROL Page Rendering]** frame顯示頁面視圖源的平均頁面呈現持續時間 [!DNL New Relic] 與前一週的同一時間段相比。
...
的 **[!UICONTROL Order transactions (default?)]** 框架查找交易記錄 `request.headers.host` 從名稱=「WebTransaction/Action/checkout/onepage/success」的事務中。 如果訂單成功URL不同，則此框架將沒有資料。
