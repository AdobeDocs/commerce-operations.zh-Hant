...
description: 本文提供當您在 [!DNL Apdex] 分數中收到Adobe Commerce的 [!DNL New Relic]. The [!DNL Apdex] 警告警示時，用於測量使用者對Web應用程式和服務回應時間的滿意度的疑難排解步驟。 需要立即採取行動來解決問題。
...
本文提供當您在[!DNL Apdex]中收到Adobe Commerce的[!DNL New Relic]警告警示時的疑難排解步驟。 [!DNL Apdex]分數會測量使用者對Web應用程式與服務回應時間的滿意度。 需要立即採取行動來解決問題。 根據您選取的警報通知通道，警報看起來類似以下內容。
