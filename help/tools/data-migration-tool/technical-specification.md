---
title: '"[!DNL Data Migration Tool] 技術規範'
description: 瞭解Oracle Server的實施詳細資訊 [!DNL Data Migration Tool] 以及在Magento1和Magento2之間傳輸資料時如何擴展。
...
的 [系統要求](../../installation/system-requirements.md) 為 [!DNL Data Migration Tool] 與第2Magento相同。
...
運行步驟的可視化由Symfony的ProgressBar元件提供(請參見 [進度欄](https://symfony.com/doc/current/components/console/helpers/progressbar.html))。 以LogLevelProcessor的形式在步驟中訪問此元件。
...
映射步驟負責將大部分資料從Magento1傳輸到Magento2。 此步驟從map.xml檔案中讀取說明(位於 `etc/` )。 該檔案描述了源(Magento1)和目標(Magento2)的資料結構之間的差異。 如果Magento1包含屬於Magento2中不存在的某個擴展的表或欄位，則可以將這些實體放在此處，以便通過映射步驟忽略它們。 否則，它將顯示錯誤消息。
...
為了實現遷移過程的輸出，並控制所有可能級別的PSR記錄器，該記錄器用於Magento。 `\Migration\Logger\Logger` 類已實現以提供日誌記錄功能。 要使用記錄器，應通過建構子依賴項注入來注入它。
