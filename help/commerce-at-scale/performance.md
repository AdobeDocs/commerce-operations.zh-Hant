...
以下章節以高層級方式說明建議的技術重點區域，請務必檢閱這些內容，以便在CIF/Adobe Commerce環境中啟用AEM上的有效快取。
...
預設Dispatcher設定使用/statfilelevel &quot;0&quot;設定 — 這表示單一「.stat」檔案放置在htdocs目錄的根目錄（檔案根目錄）中。 如果對AEM中的頁面或檔案進行變更，則此single stat檔案的修改時間會更新為變更時間。 如果時間比資源的修改時間新，則Dispatcher會考慮所有資源都失效，並且任何後續對失效資源的請求都將觸發對Publish執行個體的呼叫。 所以基本上，使用此設定時，每次啟用都會讓整個快取失效。

對於任何網站，尤其是具有大量負載的商務網站，這會在AEM Publish層級上增加不必要的負載，使得整個網站結構在僅需單一頁面更新時失效。
...
>[!NOTE]
>
> [aem-dispatcher-experiments](https://github.com/adobe/aem-dispatcher-experiments/tree/main/experiments/gracePeriod) GitHub存放庫中有更多有關此主題的詳細閱讀。

## CIF — 透過元件的GraphQL快取

AEM中的個別元件可設定為快取，這表示GraphQL要求Adobe
Commerce會呼叫一次，之後在設定的時間限制內，系統會從AEM快取中擷取後續請求，而不會進一步將負載載入Adobe Commerce。 例如，根據每個頁面上顯示的類別樹狀結構網站導覽，以及多面向搜尋功能中的選項，這些只是需要在Adobe Commerce上建立大量資源查詢的兩個區域，但不太可能定期變更，因此是快取很好的選擇。 舉例來說，如此一來，即使發佈商正在重建PDP或PLP，導覽組建的資源密集GraphQL請求也不會命中Adobe Commerce，而可從AEM CIF上的GraphQL快取中擷取。
...
上述快取選項可在「GraphQL使用者端」中使用AEM OSGi設定主控台設定
Configuration Factory」。 每個快取設定專案都可以以下列格式指定：
...
也可以使用混合方法快取頁面：CIF頁面可以包含永遠會直接從客戶瀏覽器向Adobe Commerce要求最新資訊的元件。 這對於範本中頁面的特定區域非常有用，這些區域對於即時資訊保持最新非常重要：例如PDP中的產品價格。 如果價格因動態價格比對而經常變更，則該資訊可以設定為在Dispatcher上不快取，而是在客戶瀏覽器中透過AEM CIF網頁元件的GraphQL API，直接從Adobe Commerce擷取使用者端價格。
...
>[!NOTE]
>
>[aem-dispatcher-experiments](https://github.com/adobe/aem-dispatcher-experiments/tree/main/experiments/ignoreUrlParams) GitHub存放庫中有提供設定`ignoreUrlParams`重要性的進一步閱讀。
...
MPM Worker設定是進階的Apache HTTP伺服器設定，需要根據您的Dispatcher可用的CPU和RAM進行徹底測試以最佳化。 不過，在本白皮書的範圍內，我們建議ServerLimit和MaxRequestWorker應增加到伺服器可用的CPU和RAM支援的層級，然後MinSpareThreads和MaxSpareThreads應同時增加到符合MaxRequestWorker的層級。
