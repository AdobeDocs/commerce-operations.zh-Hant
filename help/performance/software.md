...
使用您正在安裝的Adobe Commerce發行版本所支援的PHP版本，如[系統需求](../installation/system-requirements.md)中所列。 配置PHP以獲得請求處理的最高速度和效率時，需要考慮幾個因素。
...
若要讓[!DNL Commerce]達到最大速度，您必須啟動OpCache模組並正確加以設定。 建議將下列設定用於模組：
...
| 網頁伺服器 | 屬性名稱 | 位置 | 相關資訊 |
|--- | --- | --- | ---|
| Nginx | `worker_connections` | `/etc/nginx/nginx.conf` (Debian) | [調整NGINX的效能](https://www.f5.com/company/blog/nginx/tuning-nginx) |
...
最近的[!DNL MySQL]發行版本包含許多效能改善，且一般會以良好的預設設定發佈[!DNL MySQL]。 最關鍵的設定如下：
...
[!DNL Commerce]為支援的[!DNL Varnish]版本散發範例組態檔，其中包含所有建議的效能設定。 其中最關鍵的效能包括：
...
若要將您的資產儲存在[!DNL Varnish]，請在`default.vcl`產生的[!DNL Commerce]檔案中新增下列VCL專案。
