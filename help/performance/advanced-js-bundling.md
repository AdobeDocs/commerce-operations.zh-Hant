...
如需協力廠商工具、HTTP/2以及已棄用的JS和CSS合併的相關指引，請參閱&#x200B;*組態最佳實務*&#x200B;中的[套件提示](configuration.md#bundling-tips)。
...
若要取得[!DNL RequireJS]以使用您的組合，請在`build.js`檔案中的`modules`節點之後新增`onModuleBundleComplete`回呼：
...
開啟`pub/static/frontend/Magento/luma/en_US`目錄中的`requirejs-config.js`，以驗證[!DNL RequireJS]是否已附加檔案與套件組合組態呼叫：
