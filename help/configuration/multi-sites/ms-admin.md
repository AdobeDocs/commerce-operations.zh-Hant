...
>[!INFO]
>
>僅供範例用途，我們使用法文網站搭配網站程式碼 `french` 在此主題中。 如需逐步教學課程，請參閱 [教學課程：使用Apache設定多個網站](ms-apache.md) 和 [教學課程：使用nginx設定多個網站](ms-nginx.md)
...
1. 在 _商店_ 頁面，按一下 **建立網站**.
   ...
   - **名稱** — 輸入名稱以識別網站。
   - **程式碼** — 輸入唯一代碼；例如，如果您有法式商店，則可輸入 `french`
   - **排序順序** — 輸入選用的數字排序順序。
...
使用類似以下的唯一URL存取網站： `http://french.magento.mg`，您必須變更Admin中每個網站的基底URL。

若要變更網站基底URL：

1. 在 _管理員_ 面板，按一下 **商店** > **設定** > **設定** > **一般** > **Web**.
1. 從 **存放區檢視** 在頁面頂端清單中，按一下其中一個網站的名稱，如下圖所示。

   ![選取範圍](../../assets/configuration/multi-site-scope.png)

1. 在右窗格中，展開 **基本URL**.
1. 在 _基本URL_ 部分，清除 **使用系統值**.
1. 輸入 `http://french.magento.mg` 中的URL **基礎URL** 和 **基礎連結URL** 欄位。

1. 在中重複上述步驟 _基礎URL （安全）_ 區段。

   >[!INFO]
   >
   >如果您正在設定雲端基礎結構上部署Adobe Commerce的基礎URL，則必須將第一個句號取代為三個破折號。 例如，如果您的基底URL為 `french.branch-sbg7pPa-f3dueAiM03tpy.us.magentosite.cloud`，輸入 `http://french---branch-sbg7pPa-f3dueAiM03tpy.us.magentosite.cloud`. 如果您設定基底URL以進行本機測試，請使用句點。
...
Commerce可讓您將商店程式碼新增至網站基底URL，簡化設定多個商店的程式。 使用此選項，您不需要在Commerce檔案系統上建立目錄來儲存 `index.php` 和 `.htaccess`.

這可避免 `index.php` 和 `.htaccess` 避免在未來的升級中與Commerce程式碼基底不同步。
...
1. 在 _管理員_ 面板，按一下 **商店** > **設定** > **設定** > **一般** > **Web**.
1. 從 **存放區檢視** 清單頁面頂端，按一下 **預設設定** 如下圖所示。

   ![選取預設設定範圍](../../assets/configuration/multi-site-default.png)

1. 在右窗格中，展開 **Url選項**.
1. 清除 **使用系統值** 核取方塊旁的 _將存放區代碼新增至Url_.
...
1. 從 _存放區檢視_ 清單頁面頂端，按一下 **預設設定**.

   ![選取預設設定範圍](../../assets/configuration/multi-site-default.png)

1. 在右窗格中，展開 **基本URL**.
1. 在 _基本URL_ 部分，清除 **使用系統值**.
1. 輸入 `http://magento.mg` 中的URL **基礎URL** 和 **基礎連結URL** 欄位。

1. 在中重複上述步驟 **基礎URL （安全）** 區段。
