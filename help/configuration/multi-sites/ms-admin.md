...
>[!INFO]
>
>網站、商店和商店檢視程式碼只能包含字母（a-z或A-Z）、數字(0-9)和底線(_)。 此外，第一個字元必須是字母。 如果使用大寫或駝峰字元，內部比對不區分大小寫，以透過環境變數來適應設定覆寫。 另請參閱 [使用環境變數來覆寫組態設定](../reference/override-config-settings.md#environment-variables).