...
使用靜態視圖檔案部署命令可以寫入 [靜態檔案](https://glossary.magento.com/static-files) 將Commerce軟體設定為 [生產模式](../bootstrap/application-modes.md#production-mode)。
...
>[!WARNING]
>
>_僅開發人員模式_:安裝或啟用新模組時，它可能會載入新的JavaScript、CSS、佈局等。 要避免靜態檔案出現問題，必須清除舊檔案，以確保獲得新模組的所有更改。 可以通過多種方式清除生成的靜態視圖檔案。 請參閱 [清除靜態檔案快取主題以瞭解詳細資訊](https://devdocs.magento.com/guides/v2.4/frontend-dev-guide/cache_for_frontdevs.html#clean_static_cache) 的子菜單。

**部署靜態視圖檔案**:

1. 以或 [切換到檔案系統所有者](https://devdocs.magento.com/guides/v2.4/install-gde/prereq/file-sys-perms-over.html)。
...
| 選項 | 說明 | 需要？ |
| ------ | ----------- | --------- |
| `<languages>` | 空格分隔清單 [ISO-639](http://www.loc.gov/standards/iso639-2/php/code_list.php) 要輸出靜態視圖檔案的語言代碼。 (預設值為 `en_US`。)<br>通過運行： `bin/magento info:language:list` | 否 |
...
| `--theme <theme>` | 為其部署靜態內容的線程。 預設值為 **全部**。<br>例如： `--theme Magento/blank --theme Magento/luma` | 否 |
...
[首先安裝Commerce軟體](https://devdocs.magento.com/guides/v2.4/install-gde/bk-install-guide.html);否則，無法運行靜態視圖檔案部署工具。
...
1. 以或 [切換到](https://devdocs.magento.com/guides/v2.4/install-gde/prereq/file-sys-perms-over.html)，檔案系統所有者。
1. 刪除 `<magento_root>/pub/static` 目錄，但 `.htaccess` 的子菜單。 不要刪除此檔案。
1. 部署靜態視圖檔案： `bin/magento setup:static-content:deploy`
