...
的責任 `config.xml` 舊版Commerce中使用的設定檔案現在分為多個檔案，位於不同的模組目錄中。 只有當模組請求特定設定型別時，Commerce才會視需要載入多個設定檔案。
...
- **設定階段** — 階段定義為 _主要_， _全域_、和 _區域_. 每個階段會決定何時載入組態型別並與相同名稱的組態型別合併。 例如， `module.xml` 檔案會與其他檔案合併 `module.xml` 檔案。

- **設定範圍** — 範圍是組態階段的補充，定義組態型別模型。 例如， `adminhtml` 是在階段與其他模組一起載入的區域範圍 `adminhtml` 設定。 如需詳細資訊，請參閱 [模組和區域](https://developer.adobe.com/commerce/php/architecture/modules/areas/).
...
Commerce會依照以下順序載入設定檔（所有路徑都是相對於Commerce安裝目錄）：
...
- 來自模組的區域特定設定(`<your component base dir>/<vendorname>/<component-type>-<component-name>/etc/<area>/*.xml`)。 從所有模組收集組態檔，並將其合併至全域組態。 某些區域特定設定可能會覆寫或擴充全域設定。
...
組態檔中的節點會根據其完整限定的XPath進行合併，XPath具有定義於 `$idAttributes` 宣告為其識別碼的陣列。 此識別碼對巢狀內嵌於相同父項節點下的所有節點而言必須是唯一的。
...
>[!INFO]
>
>您可以使用 [\Magento\Framework\Config\Reader\Filesystem](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/Reader/Filesystem.php) 用於偵錯和瞭解背後邏輯的類別 [組態檔載入器](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/Reader/Filesystem.php#L125) 和 [合併設定](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/Reader/Filesystem.php#L144) 程式。
...
| `persistent.xml` | [Magento持續](https://developer.adobe.com/commerce/php/module-reference/module-persistent/) 組態檔 | 全域 | [\Magento\Persistent\Helper\Data](https://github.com/magento/magento2/blob/2.4/app/code/Magento/Persistent/Helper/Data.php) |
...
您可以使用下方的介面與組態檔互動 [Magento\框架\設定](https://github.com/magento/magento2/tree/2.4/lib/internal/Magento/Framework/Config).

如果符合下列條件，您可以使用這些介面： [建立設定型別](../reference/config-create-types.md#create-configuration-types).
...
- [Framework\Config\ReaderInterface](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/ReaderInterface.php)，會從儲存體讀取設定資料，並選取從中讀取的儲存體。
...
- [Framework\Config\ScopeListInterface](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/ScopeListInterface.php)，這會傳回範圍清單。
