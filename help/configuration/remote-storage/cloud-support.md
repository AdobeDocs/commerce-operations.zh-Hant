...
`REMOTE_STORAGE`變數用於雲端基礎結構專案的[部署階段](https://experienceleague.adobe.com/zh-hant/docs/commerce-on-cloud/user-guide/develop/deploy/process)。
...
將`REMOTE_STORAGE`變數設為[環境層級變數](https://experienceleague.adobe.com/zh-hant/docs/commerce-on-cloud/user-guide/configure/env/variable-levels)，這樣就不會在生產、測試和整合環境之間共用檔案。 在環境層級設定變數，可讓您靈活地在選取的環境上僅使用遠端儲存，例如排除使用遠端儲存的整合環境。
...
`key`和`secret`是選用專案。 當您建立變數時，可以選取`sensitive`選項以隱藏`key`和`secret`。 使用此設定時，網頁介面中不會顯示值。 請參閱&#x200B;_雲端基礎結構上的Commerce指南_&#x200B;中的[變數可見度](https://experienceleague.adobe.com/zh-hant/docs/commerce-on-cloud/user-guide/configure/env/variable-levels#visibility)。
...
準備您的[Fastly認證](https://experienceleague.adobe.com/zh-hant/docs/commerce-on-cloud/user-guide/cdn/setup-fastly/fastly-configuration#get-fastly-credentials)。 在Pro專案上，使用SSH連線到您的伺服器並從`/mnt/shared/fastly_tokens.txt`檔案取得Fastly認證。 測試和生產環境都有獨特的認證。 您必須取得每個環境的認證。
