...
Commerce提供命令列選項來設定Redis頁面和預設快取。 雖然您可以編輯`<Commerce-install-dir>app/etc/env.php`檔案來設定快取，但建議使用命令列的方法，尤其是對於初始設定。 命令列會提供驗證，以確保組態語法正確。
...
- **Zend快取（2.4.8和更早版本）** — Commerce 2.4.8和更早版本的舊版Redis後端：
   - **舊版Redis後端** — 使用完整類別路徑(`Magento\Framework\Cache\Backend\Redis`)
...
- **Symfony快取(2.4.9+)** — 從Commerce 2.4.9開始，Symfony快取為Redis提供符合PSR 6規範的現代化快取實作，並大幅改善效能：
   - **自動Redis流水線** — 將多個作業批次化為單一請求，減少延遲
...
>[!NOTE]
>
>從Commerce 2.4.9開始，請使用簡化的後端型別`'backend' => 'redis'`，而非完整的類別路徑。 指定簡化名稱時，會自動使用Symfony快取。
