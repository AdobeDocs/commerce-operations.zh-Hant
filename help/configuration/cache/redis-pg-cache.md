...
`--cache-backend-redis-<parameter_name>=<parameter_value>` 是配置預設快取的鍵和值對的清單：

| 命令行參數 | 參數 | 意義 | 預設值 |
|--- |--- |--- |--- |
| 快取 — 後端 — 紅色伺服器 | 伺服器 | 完全限定的主機名、 IP地址或UNIX套接字的絕對路徑。 預設值127.0.0.1表示Redis安裝在Commerce伺服器上。 | 127.0.0.1 |
| 快取 — 後端 — Redis埠 | 埠 | Redis伺服器偵聽埠 | 6379 |
| 快取後端 — redis-db | 資料庫 | 如果將Redis用於預設和全頁快取，則此為必需欄位。 必須指定其中一個快取的資料庫編號；預設情況下，其他快取使用0。<br><br>重要提示：如果將Redis用於多種類型的快取，則資料庫編號必須不同。 建議將預設的快取資料庫編號分配給0，將頁快取資料庫編號分配給1，將會話儲存資料庫編號分配給2。 | 0 |
| 快取後端密碼 | 密碼 | 配置Redis密碼可啟用其內置安全功能之一：這樣 `auth` 命令，它要求客戶端進行身份驗證才能訪問資料庫。 口令直接在Redis的配置檔案中配置， `/etc/redis/redis.conf`。 |  |
...
`--page-cache-redis-<parameter_name>=<parameter_value>` 是配置頁快取的參數/值對的清單：

| 命令行參數 | 參數 | 意義 | 預設值 |
|--- |--- |--- |--- |
| 頁面快取 — Redis伺服器 | 伺服器 | 完全限定的主機名、 IP地址或UNIX套接字的絕對路徑。 預設值127.0.0.1表示Redis安裝在Commerce伺服器上。 | 127.0.0.1 |
| page-cache-redis埠 | 埠 | Redis伺服器偵聽埠 | 6379 |
| page-cache-redis-db | 資料庫 | 如果將Redis用於預設和全頁快取，則此為必需欄位。 必須指定其中一個快取的資料庫編號；預設情況下，其他快取使用0。<br/>重要提示：如果將Redis用於多種類型的快取，則資料庫編號必須不同。 建議將預設的快取資料庫編號分配給0，將頁快取資料庫編號分配給1，將會話儲存資料庫編號分配給2。 | 0 |
| 頁快取密碼 | 密碼 | 配置Redis密碼可啟用其內置安全功能之一：這樣 `auth` 命令，它要求客戶端進行身份驗證才能訪問資料庫。 在Redis配置檔案/etc/redis/redis.conf中配置密碼。 |  |
...
由於Commerce在Redis快取中儲存了大量配置資料，因此可以預載入在頁之間重用的資料。
要查找必須預裝的鍵，您需要分析從Redis傳輸到Commerce的資料。 我們建議將載入到每頁的資料載入到每個頁面上，常見示例包括 `SYSTEM_DEFAULT`。 `EAV_ENTITY_TYPES`。 `DB_IS_UP_TO_DATE`。
Redis使用 `pipeline` 以組合載入請求。
請注意，鍵應包括資料庫前置詞，例如，如果資料庫前置詞是 `061_`，預載入鍵看起來 `061_SYSTEM_DEFAULT`。
...
如果您正在將預載入功能與L2快取一起使用，請不要忘記將「：hash」尾碼添加到鍵中，因為L2快取只傳輸資料的哈希，而不是資料本身：
