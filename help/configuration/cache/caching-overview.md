...
[清漆快取](config-varnish.md)是快取記憶體中完整頁面的HTTP加速器。 Adobe強烈建議在生產環境中使用Varnish，因為它比內建的全頁快取快得多得多。
