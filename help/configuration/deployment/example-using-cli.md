...
- **傳送電子郵件至** 敏感設定值的ID。
   ...
   此專案位於 **商店** >設定> **設定** >一般> **連絡人**.

- **預設電子郵件網域** 系統特定組態值。

   此專案位於 **商店** >設定> **設定** >客戶> **客戶設定** > **建立新帳戶選項**.

您可以使用本範例中所示的相同程式來設定下列參照中的任何設定：
...
1. 在右窗格中，展開 **存放區資訊**.
1. 如有必要，請清除 **使用預設值** 核取方塊旁的 **VAT編號** 和 **存放區名稱** 欄位。
1. 在欄位中輸入數字(例如， `12345`)。
1. 在 **存放區名稱** 欄位，輸入值(例如 `My Store`)。
1. 按一下 **儲存設定**.
1. 在左側導覽列的「一般」底下，按一下 **連絡人**.
1. 在右窗格中，展開 **電子郵件選項**.
1. 如有必要，請清除 **使用預設值** 核取方塊旁的 **傳送電子郵件至** 欄位。
1. 在欄位中輸入電子郵件地址。
1. 按一下 **儲存設定**.
...
現在您已變更Admin中的設定，請使用下列步驟將共用設定寫入檔案：

{{$include /help/_includes/config-save-config.md}}

即使 `app/etc/env.php` （系統特定設定）已更新，請勿將其簽入原始檔控制。
稍後在本程式中，您將在生產系統上建立相同的組態設定。

## 步驟3：更新您的建置系統並產生檔案

現在您已確認將共用設定的變更提交至原始檔控制，您可以將這些變更提取至組建系統、編譯程式碼，並產生靜態檔案。
...
- 每個設定的範圍
   ...
   如果您依照步驟1中的指示進行， **傳送電子郵件至** 是網站和範圍 **預設電子郵件網域** 是全域的（即預設設定範圍）。

   您需要網站程式碼來設定 **傳送電子郵件至** 設定值。

   如需尋找此值的詳細資訊，請參閱： [使用環境變數來覆寫組態設定](../reference/override-config-settings.md#environment-variables).
...
- 此範例中使用的設定的設定路徑：
   ...
   如需所有敏感和系統專屬的設定路徑，請參閱： [敏感和系統專屬設定路徑參考](../reference/config-reference-sens.md).
...
若要設定系統特定設定 **預設電子郵件網域**，位於預設範圍內，請使用以下命令：
...
若要設定下列專案的值： **傳送電子郵件至**&#x200B;但是，您必須知道範圍型別(`website`)和範圍代碼，每個網站上的範圍代碼可能不同。
...
本節說明如何將您在開發及建置系統上所做的所有變更提取至生產環境，這會更新共用組態設定（商店名稱和VAT編號）。
