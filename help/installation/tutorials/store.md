---
title: 設定存放區
description: 瞭解在部署設定和資料庫架構設定（包括安全安裝選項）後，如何從命令列設定Adobe Commerce存放區。
