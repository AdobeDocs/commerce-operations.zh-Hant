...
Adobe Commerce支援nginx 1.x （或[最新的主線版本](https://nginx.org/en/linux_packages.html#mainline)）。 您也必須針對要安裝的Adobe Commerce發行版本所支援的PHP版本安裝`php-fpm`。
...
您也可以從來源[建置nginx](https://nginx.org/en/docs/install.html)
...
1. 安裝`php-fpm`和`php-cli`：
   ...
   >[!NOTE]
   >
   >將`<supported-php-version>`取代為您正在安裝的Adobe Commerce發行版本的[系統需求](../../system-requirements.md)中列出的PHP次要版本。 在下列步驟中，在檔案路徑、服務名稱和通訊端路徑中使用相同的值。
...
1. 在編輯器中開啟`/etc/php.ini`檔案。
   ...
   >[!NOTE]
   >
   >安裝為要安裝的Adobe Commerce發行版本所支援的PHP版本提供`php-fpm`的套件名稱。 套件名稱會因存放庫和作業系統而異。
