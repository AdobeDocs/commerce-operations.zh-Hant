...
description: 了解2.4.4 Adobe Commerce版本的新功能、增強功能、錯誤修正和已知問題。
...
Adobe Commerce 2.4.4引入了對PHP 8.1的支援。所有項目庫和依賴項都已更新，以與PHP 8.1相容。核心撰寫器依賴項和第三方庫也已升級到與PHP 8.1相容的最新版本。此版本還提供了對OpenSearch 1.2的支援。
...
雖然這些功能的程式碼與每季發行的Adobe Commerce核心程式碼搭配使用，但其中幾個專案(例如B2B、Page Builder和Progressive Web Application(PWA)Studio)也會獨立發行。 這些專案的錯誤修正會記錄在每個專案的檔案中，提供個別專案的發行資訊。

>[!NOTE]
>
>除了Braintree以外，所有隨附的擴充功能已從Adobe Commerce 2.4.4中移除。
...
Adobe Commerce 2.4.4包括解決針對Adobe Commerce和Magento Open Source2.4.3、2.4.3-p1和2.3.7-p2提供的下列Hotfix解決的所有問題：
...
建議您在安裝Adobe Commerce 2.4.4後套用下列修補程式：

* `braintree-disabled-partial-capture-for-applepay-googlepay.patch`.  商戶無法針對透過Braintree以Apple支付支付的訂單提交部分退款。 當商家嘗試為訂單發票的部分退款建立貸項通知單時， **要退款的數量** 欄位無法編輯。 此Hotfix可解決該問題。 請參閱 [Adobe Commerce 2.4.4:無法建立部分發票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知識庫文章。  <!-- BUNDLE-3088-->

## Adobe Commerce 2.4.4亮點

查看此版本中的下列重點。

### 安全性增強功能

此版本包含一項安全性修正和平台安全性改善。 此安全性修正已背向Adobe Commerce 2.4.3-p2和Adobe Commerce 2.3.7-p3。
...
* Adobe Commerce店面現在預設會啟用HTTPS。 此 **在店面上使用安全URL** 和 **在管理中使用安全URL** 預設會啟用設定，且所有內建cookie現在都設為安全。  <!-- AC-1173-->

* 所有Adobe Commerce安裝現在都需要相依性混淆外掛程式。 以前，只有以撰寫器為基礎的安裝才需要此外掛程式。 現在允許使用受信任的版本。 商家可以略過此外掛程式中設定的限制，禁止某些撰寫器安裝組合。 您可以針對信任的版本略過這些限制，而Adobe Commerce現在會在繼續安裝前顯示警告。  <!-- AC-501 970-->
...
* Adobe Commerce 2.4.4支援Elasticsearch7.16和OpenSearch 1.2。部署由內部部署托管的Adobe Commerce商戶可使用任一解決方案。 不過，OpenSearch現在是托管於雲端之Adobe Commerce 2.4.4部署的預設搜尋引擎。 升級至2.4.4版的所有雲端部署客戶都必須使用OpenSearch。 請參閱 [在Cloud 2.4.4上切換為OpenSearch for Adobe Commerce](https://support.magento.com/hc/en-us/articles/4419942355725-Switching-to-OpenSearch-for-Adobe-Commerce-on-Cloud-2-4-4)
...
Adobe Commerce效能增強功能可提升高吞吐量順序處理和訊息佇列最佳化。 此版本推出的非同步訂購功能支援每小時建立大約60,000筆訂購。 舊版Adobe Commerce支援每小時約10,000筆訂單的處理作業，這可能造成快閃記憶體銷售的瓶頸。 新的多重使用者功能支援調整單一雲端例項上訊息佇列使用者的數量，並增加每小時處理的訂單數量。

此版本的效能增強功能：

* AsyncOrder功能支援比同步執行提供更快的訂單放置。 啟用AsyncOrder時，購物者在店面完成其他工作時，就會在背景執行訂單放置。

* 新 **對購物車載入啟用庫存檢查** 配置選項提供在購物車中載入產品時可切換的庫存檢查。 預設會啟用。 當您停用此選項時，Adobe Commerce會在報價載入時略過庫存檢查，進而加快結帳速度，尤其是對包含許多項目的購物車。

* 新 `multiple_processes` 配置選項支援在多個進程中運行並行使用者。 以前， `cron` 視需要執行單一消費者。 啟動多個用戶以並行運行進程可以提高任務執行速度。 若要啟用此功能，請新增 `multiple_processes` 到 `app/etc/env.php` 檔案。

* 包含750多項可設定產品的購物車操作已透過提高 `max_input_vars` 在 `php.ini` 檔案，以支援輸入變數卷。

* 透過延遲總計計算，最佳化結帳期間的銷售規則處理。 通常，每當購物者新增產品至購物車、更新產品數量或點按  **下單** 按鈕。 商戶可借由設定 `checkout/deferred_total_calculating` 變數 `env.php` 檔案。 或者，您也可以執行 `bin/magento setup:config:set --deferred-total-calculating 1|0`.  <!-- MCP-573-->

* 改善非同步下單期間受購物車價格規則影響的訂單驗證程式。 <!-- MCP-304-->
...
* **完成GraphQL報價**.  B2B公司使用者現在可以使用GraphQL完成與可轉讓報價相關的所有工作。 舊版本的此API支援協商流程，但不支援結帳。 <!-- PWA-2101-->

#### 新突變

* [`assignCustomerToGuestCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/assign-customer-to-guest-cart.html) 變異

* [`placeNegotiableQuoteOrder`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/place-negotiable-quote-order.html) 變異

* [`setNegotiableQuoteBillingAddress`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-negotiable-quote-billing-address.html) 變異

* [`setNegotiableQuotePaymentMethod`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-negotiable-quote-payment-method.html) 變異<!-- PWA-2114-->

* [`setNegotiableQuoteShippingMethods`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-negotiable-quote-shipping-methods.html) 變異

* [`setNegotiableQuoteShippingAddress`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-negotiable-quote-shipping-address.html) 變異

* **效能改善**:

   * GraphQL購物車作業的效能已改善。 此 `collectQuoteTotals()` 現在，在GraphQL要求期間只呼叫方法一次，可縮短回應時間。 <!-- PWA-2110-->

   * Storefront效能已因變更GraphQL請求的快取方式而改善。 現在，Ampley and Vilesh會快取伴隨傳送的GraphQL請求 `auth` 代號。 <!-- PWA-1817-->

* **新店面相關管理配置設定**. 此 `storeConfig` 查詢現在會傳回零小計結帳和支票/貨幣訂單付款方法的組態設定。 <!-- PWA-1576-->

* **更新核心GraphQL程式庫**. 此 `webonyx` 程式庫可讓核心GraphQL運作，已升級至^14.9版。 <!-- PWA-2137 2184 -->

* **修正GraphQL中多網站和多語言商店的翻譯問題**. GraphQL解析程式現在會根據預期的存放區範圍傳回翻譯的字串。 <!-- PWA-1946-->

* GraphQL現在提供New Relic描述性交易名稱，有助於除錯。 [GitHub-30915](https://github.com/magento/magento2/issues/30915) <!-- PWA-1311-->
...
除 [Braintree](https://docs.magento.com/user-guide/payment/braintree.html)，所有廠商套件的擴充功能已從Adobe Commerce 2.4.4程式碼基底中移除。 商戶應移轉至官方擴充功能，這些功能可在 [Commerce Marketplace](https://marketplace.magento.com/extensions.html). <!-- AC-1165-->
...
PWA Studio12.3.0版與Adobe Commerce 2.4.4相容。此版本包含reCaptcha、頁面產生器內容最佳化和個人化內容支援。 如需增強功能和錯誤修正的詳細資訊，請參閱 [PWA Studio發行](https://github.com/magento/pwa-studio/releases). 請參閱 [版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 以取得PWA Studio版本及其相容Adobe Commerce核心版本的清單。
...
新 `Magento_ElasticsearchCatalogPermissionsGraphQl` 模組在更新的Live Search期間已停用 [安裝](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/onboard/install.html). 現有的Live Search安裝必須升級至Live Search 2.0.0，才能利用這些功能。 如需詳細資訊，請參閱即時搜尋 [發行說明](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html).

### 協助工具更新

此版本提高了標準協助工具准則的一致性。 這些增強功能可改善視力不足或語言、認知和學習能力有限的使用者的體驗。  螢幕Reader工具效能也有所改善。

協助工具增強功能包括：

* 工具提示連結中包含文字，以及輸入內容的準確、以任務為重點、可見的標籤
* 編輯按鈕現在具有唯一文字。
* 管理員按鈕現在具有符合AA標準的獨特、可存取和描述性用途
* 傳達含義的圖示影像現在提供文字替代。
* 增強管理按鈕和表單欄位中的對比度，對比度至少為3:1，並搭配相鄰的顏色。
...
我們正在修正Adobe Commerce 2.4.4核心程式碼中的數百個問題。 以下是這些已修正問題的子集。

### 安裝、升級、部署

<!-- MC-42026-->

* Adobe Commerce現在會如預期般在建置記錄檔中記錄靜態內容部署錯誤。
...
* 商家現在可以透過Klarna成功從Adobe Commerce 2.4.2部署升級至Adobe Commerce 2.4.3。 [GitHub-33760](https://github.com/magento/magento2/issues/33760)

<!-- ACP2E-77-->

* 現在可以從命令列成功變更Sitemap產生器設定。 以前，當您嘗試在「管理員」外部變更Sitemap設定時，Adobe Commerce會顯示此錯誤： `Import failed: Notice: Trying to access array offset on value of type null in app/code/Magento/Cron/Model/Config/Backend/Sitemap.php on line 78`.  [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* 擁有受限權限的管理員現在可以如預期般建立回訪。

### 後端

<!-- MC-43161-->

* 管理員現在可以登入已設定自訂管理員路徑且已啟用機密金鑰的部署。 以前，Adobe Commerce顯示此錯誤： `Invalid security or form key. Please refresh the page`.

<!-- MC-41949-->

* 當多商店階層變更時，Adobe Commerce不再重新導向至「所有商店檢視」範圍。 先前，儲存階層後，購物者會重新導向至錯誤的商店URL，而且在多商店部署中選取了「所有商店檢視」範圍。

### 捆綁產品

<!-- MC-42885-->

* 購物者現在可以新增附有「固定產品稅」和兩個選項的套件產品至購物車。 以前，Adobe Commerce未新增產品並顯示此錯誤： `We can't add this item to your shopping cart right now`.
...
* `cron` 現在會如預期清除產品類別快取，期間 `indexer_update_all_views` 執行。 以前，在大量更新類別中的許多產品後，Adobe Commerce在類別頁面上顯示錯誤的產品計數。

<!-- MC-42811-->

* Adobe Commerce現在會在套件產品重新出現時，如預期般更新類別產品快取。
...
* 在啟用永久性購物車的部署中，當工作階段逾時後，購物者現在可以下單而無錯誤。 以前，Adobe Commerce會在工作階段逾時後顯示付款錯誤。
...
* 如果更新的數量無效，Adobe Commerce現在會將購物車產品數量回復為上一個值。
...
* Adobe Commerce現在會在啟用表格費率時，正確將免運費臨界值套用至訂單。 [GitHub-21832](https://github.com/magento/magento2/issues/21832)
...
* Adobe Commerce不再在購物者點按時擲回錯誤 **更新購物車** 在迷你購物車中變更產品數量後。 以前，Adobe Commerce顯示此錯誤：  `The quote item isn't found. Verify the item and try again`.
...
* 當購物者新增缺少街道欄位的計費地址時，Adobe Commerce不再擲回此錯誤： `Uncaught TypeError: Unable to process binding "text: function(){return currentBillingAddress().street.join(', ') }"`. [GitHub-33826](https://github.com/magento/magento2/issues/33826)

<!-- AC-1625-->

* Adobe Commerce在已移除迷你購物車的商店結帳期間，不再擲回主控台錯誤。 [GitHub-34513](https://github.com/magento/magento2/issues/34513)
...
* Adobe Commerce在您使用 `addConfigurableProductsToCart` 變更商店檢視後，直接將可設定產品新增至購物車的變異。

<!-- ACP2E-176-->

* Adobe Commerce在您使用 `addConfigurableProductsToCart` 突變，將可配置產品的禁用子項添加到購物車。 以前，Adobe Commerce會擲出完整性限制違反錯誤。

<!-- ACP2E-206-->

* 當 **顯示購物車中的交叉銷售項目** 配置設定已禁用。
...
* 已新增驗證至結帳工作流程，以比較客戶的街道地址與預設值。 如果驗證程式識別變更，則會設定 `customer_address_id` to `null` 將其視為新客戶地址。 以前，如果客戶在結帳時未進行驗證便儲存該位址，而在驗證套用至結帳欄位後，同一位客戶嘗試使用相同位址下單，Adobe Commerce便不接受編輯過的位址，並擲回例外狀況。

### 購物車價格規則

<!-- MC-42288-->

* 現在會封鎖產生抵用券，直到相關購物車價格規則與 `auto` 選項啟用。 如果您嘗試儲存此規則，但未啟用 `auto` 選項，Adobe Commerce會顯示此訊息： `Rule is not saved with auto generate option enabled. Please save the rule and try again`.
...
* 現在，當Adobe Commerce設定為 **整個購物車的固定金額折扣** 購物車價格規則。 以前，Adobe Commerce顯示此錯誤： `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`.
...
* GraphQL查詢現在會根據子產品的可見性篩選設定，傳回可設定產品的子產品的結果。

<!-- MC-41853-->

* 當可設定產品屬性變更時，Adobe Commerce不再修改相關產品價格。 以前，可配置產品的最低廣告價格(MAP)覆蓋了商店前端的相關產品的價格。

<!-- MC-42659-->

* 管理員現在可以重新指派類別中剩餘的最後一個產品，並儲存空白類別。
...
* 新增驗證，說明路由器無法實例化從URL收集的動作類別。 Adobe Commerce現在會顯示標準錯誤訊息。 以前，Adobe Commerce顯示此錯誤：  `PHP Error: Cannot instantiate abstract class Magento\Catalog\Controller\Product\Compare`.

<!-- ACP2E-10-->

* 現在，當兩個購物車規則套用至符合下列條件的訂單時，Adobe Commerce會正確計算折扣：一個購物車規則套用了抵用券代碼和 **整個購物車的固定價格**  條件和第二個購物車價格規則沒有抵用券和 **產品價格折扣百分比** 條件。 以前，當先前的購物車規則已對某些產品套用折扣時，相關演算法無法將折扣適當地分割到購物車產品中。
...
* 在部分索引過程中，更新子產品不再刪除從其父可配置產品繼承的目錄規則折扣。 以前，當我們將可配置產品分配給某個類別，但未將其關聯的簡單產品，然後使用該類別建立目錄規則時，Adobe Commerce在部分索引過程中刪除了分配給該簡單產品的目錄規則。
...
* 在部分重新索引期間，Adobe Commerce不再移除可設定產品的目錄價格規則。 以前，在可配置產品重新索引時，只會重新索引子產品。
...
* 現在，可設定的產品選項標籤名稱是根據建立管理訂單期間選取的商店檢視而定。 以前，Adobe Commerce會使用預設商店檢視的標籤名稱。
...
* Adobe Commerce現在會在建立訂單時考量管理區域設定的網站範圍。 以前，訂單詳細資訊（例如客戶地址屬性）只能在多網站部署中，才能在一個網站上正常運作。 [GitHub-23254](https://github.com/magento/magento2/issues/23254)

### 客戶區段

<!-- MC-42241-->

* 現在當您建立公司帳戶時，新客戶區段的「相符客戶」計數會自動更新。 之前，您必須重新整理區段資料才能取得正確的計數。
...
* Adobe Commerce從管理員和 **通知客戶** 核取方塊已啟用。 之前，運送確認電子郵件包含以下訊息： `We're sorry, an error has occurred while generating this content`.
...
* Adobe Commerce現已包含 `Content-Disposition: inline` 電子郵件中的標題。 [GitHub-29258](https://github.com/magento/magento2/issues/29258)
...
* 按一下任何電子郵件範本的預覽，現在會如預期在個別視窗中開啟範本預覽。  之前，Adobe Commerce會從「電子郵件範本」格線和快顯視窗中同時開啟範本預覽。
...
* 幫手 `Magento\Payment\Helper\Data` 不再在建構子中建立新佈局。 以前，當自訂命令中使用此協助程式而未指定區域代碼時，Adobe Commerce擲回錯誤。 [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* Adobe Commerce現在會在引導檔案中發生例外狀況時傳回500回應代碼。 之前，Adobe Commerce會傳回200 OK狀態代碼。 [GitHub-22196](https://github.com/magento/magento2/issues/22196)
...
* 新增必要選項至 `.htaccess` 和 `nginx.conf` 隨Adobe Commerce分發的檔案，以支援PHP 8.x。 已過時的選項也已移除。 [GitHub-34358](https://github.com/magento/magento2/issues/34358)

<!-- MC-42448-->

* 商家現在可以上傳多種產品的影片。 現在，只有當上傳的檔案名稱包含副檔名時，檔案產生程式才會將副檔名附加至檔案名稱。 以前，當商家嘗試上傳，然後針對兩種不同產品儲存相同的視訊時，Adobe Commerce擲回此錯誤： `Notice: Undefined index: extension in /app/7ha7zds7wvqys_stg/vendor/magento/framework/File/Uploader.php on line 699`. 發生此問題是因為Adobe Commerce嘗試將副檔名附加至名稱，而不論上傳的檔案名稱是否有副檔名。

<!-- magento/magento2/pull/34106-->

* 修正 `sprintf(__())` 和 `sprintf(Magento\Framework\Phrase())` 構建整個代碼庫與PHP 8.x的相容性。 以前，Adobe Commerce會擲出此類錯誤： `Expected parameter of type 'string', 'Magento\Framework\Phrase' provided`. [GitHub-34085](https://github.com/magento/magento2/issues/34085)
...
* 執行中 `bin/magento support:backup:code` 現在會如預期建立備份。 之前，Adobe Commerce建立了備份，但擲回錯誤。
...
* 此 `staging_synchronize_entities_period` `cron` 工作現在如預期般運作。 之前，第一次測試更新已成功套用，但Adobe Commerce隨後更新擲回此錯誤： `report.ERROR: Cron Job staging_synchronize_entities_period has an error: The active update can't be deleted`.
...
* Adobe Commerce現在複製產品時，會根據其定義的範圍複製所有產品欄位。 之前， **名稱** 和 **說明** 欄位重設為 `global` 範圍。

<!-- MC-42612-->

* 當產品價格為0時，Adobe Commerce不再套用增量捨入至折扣。 以前，Adobe Commerce會計算負折扣金額。

<!-- MC-42215-->

* 產品 **設為新** 已指派空白開始日期和過期結束日期的屬性現在可成功儲存。 以前，當您使用下列設定儲存產品時，Adobe Commerce會擲出此錯誤： `Make sure the To Date is later than or the same as the From Date`.

<!-- MC-41803-->

* 當您變更SEO URL識別碼時，Adobe Commerce不再複製CMS階層中的現有CMS頁面。 以前，Adobe Commerce會在CMS階層中建立新節點，複製現有節點。
...
* 類別 `BundleDiscountPrice` in `magento2/app/code/Magento/Bundle/Pricing/Price/ConfiguredPrice.php` 現在已如預期般宣告。 以前，Adobe Commerce會擲回此錯誤： `Class Magento\Bundle\Pricing\Price\BundleDiscountPrice not found`. [GitHub-33334](https://github.com/magento/magento2/issues/33334)
...
* 當您從儲存檢視層級的格線內嵌編輯CMS頁面時，Adobe Commerce現在會成功儲存該頁面 **使用父節點層次結構** 配置設定已禁用。 以前，Adobe Commerce會顯示此錯誤，且未儲存您的CMS頁面編輯： `A technical problem with the server created an error. Try again to continue what you were doing. If the problem persists, try again later`.

<!-- ACP2E-171-->

* 包含相同頁面的節點現在會正確顯示階層連結。 以前，Adobe Commerce會針對屬於多個節點的頁面顯示相同的節點名稱。 Adobe Commerce僅擷取頁面的第一個節點，即使每個頁面有多個節點亦然。

<!-- ACP2E-202-->

* URL重寫內嵌編輯事件現在會記錄在報表中。

<!-- ACP2E-290-->

* 此 `mage-messages` 如果沒有新訊息，則不會重設cookie。

### 禮品卡

<!-- MC-42327-->

* 新增購物者進入無效禮品卡時提供更多資訊的錯誤訊息。
...
* 此 `setBillingAddressToCart` 變異現在正確使用 `same_as_shipping` 參數，以設定與發運地址相符的開單地址。 以前，Adobe Commerce顯示此錯誤： `The shipping method is missing. Select the shipping method and try again`. [GitHub-30924](https://github.com/magento/magento2/issues/30924)

<!-- PWA-1980-->

* GraphQL購物車操作現在只會在查詢要求時計算購物車總量。 以前，購物車作業一律會計算總計，這是耗用大量資源的作業。
...
* GraphQL現在提供New Relic描述性交易名稱，有助於除錯。 [GitHub-30915](https://github.com/magento/magento2/issues/30915)

<!-- MC-42903-->

* 此 `setShippingAddressesOnCart` 變異現在支援在購物車上設定運送地址，且當 **顯示電話** 管理配置設定設為可選。 以前，Adobe Commerce會擲回此錯誤： `Field CartAddressInput.telephone of required type String! was not provided`.
...
* MySQL查詢已針對GraphQL最佳化 `products` 使用搜尋參數來調整搜尋結果的查詢。
...
* GraphQL查詢現在會在選用電話欄位的值設為空白字串時，如預期傳回帳單地址。 以前，查詢返回了空地址值。 [GitHub-30218](https://github.com/magento/magento2/issues/30218)
...
* Adobe Commerce在店面上同時載入影像時，排清影像快取時不再擲回錯誤。
...
* Adobe Commerce現在會在排程匯出後，將匯出檔案名稱中的時間戳記轉換為使用者的時區。 以前這些值不會轉換，而Adobe Commerce會顯示UTC時間戳記。

<!-- MC-42330-->

* Adobe Commerce匯入相同CSV檔案多次時，不再在遠端儲存區中建立重複影像。
...
* Adobe Commerce現在會在重新索引後，如預期般在店面上顯示產品。 以前，當前重新索引的前500個產品有存貨，而後500個產品無存貨時，店面沒有顯示任何其他產品。
...
* Adobe Commerce現在會在管理員具有唯讀狀態時顯示資訊錯誤 `pubs/media` 嘗試存取包含影像之產品的產品詳細資訊頁面。 之前，Adobe Commerce擲回PHP錯誤。 [GitHub-32819](https://github.com/magento/magento2/issues/32819)
...
* 將返回類型聲明添加到內部方法中，其返回類型不完整，以便與PHP 8.1相容。 [GitHub-34498](https://github.com/magento/magento2/issues/34498) <!-- AC-1606-->

<!-- AC-1147-->

* 更新根 `composer.json` 元包和  `composer.json` 檔案轉換為PHP 8.1。 [GitHub-34009](https://github.com/magento/magento2/issues/34009)

* 第三方 `jquery.tabs` 程式庫（最新版本）

* NPM套件（最新版本） [GitHub-33512](https://github.com/magento/magento2/issues/33512), [GitHub-33972](https://github.com/magento/magento2/issues/33972) <!-- magento/magento2/pull/33515 33998-->

* `jquery.cookie` 協力廠商程式庫（最新版本） [GitHub-34427](https://github.com/magento/magento2/issues/34427) <!-- AC-101-->

#### 撰寫器相依性更新

* 所有撰寫器相依性已更新為與PHP 8.1相容的版本。
...
* 下列程式庫已移除，因為Adobe Commerce 2.4.x支援的所有瀏覽器都已內建支援此功能：
...
* Adobe Commerce不再為執行大量動作的失敗API呼叫建立記錄項目 **系統** > **大量動作**. 之前，失敗的API呼叫的永久項目已新增至大量動作記錄。
...
* 購物者現在可以從店面和管理員，透過類型檔案和類型下拉式清單的自訂選項組合，成功重新訂購包含產品的現有訂單。 之前，Adobe Commerce擲回錯誤，且未處理重新排序。

<!-- MC-42746-->

* 按商店貸項支付的訂單的發票，以及已應用固定產品稅(FPT)的訂單，現在包括正確的總額。

<!-- MC-42332-->

* 管理訂單詳細資料頁面現在會如預期般載入。 以前，Adobe Commerce在載入含特定稅的訂單的訂單詳細資料頁面時擲出下列錯誤： `Call to a member function getId() on array`.

<!-- MC-38521-->

* 具有限制權限（包括訂單和發運權限）的管理員現在可以按預期方式查看訂單頁。 以前，Adobe Commerce會在擁有這些權限的管理員嘗試檢視訂單時擲回錯誤。 [GitHub-14633](https://github.com/magento/magento2/issues/14633)
...
* `bin/magento setup:upgrade` 從Adobe Commerce 2.4.2-p1升級時，現在會如預期運作。 以前，Adobe Commerce會擲回此錯誤： `Unable to apply data patch Magento\Paypal\Setup\Patch\Data\UpdateBmltoPayLater for module Magento_Paypal`. [GitHub-33678](https://github.com/magento/magento2/issues/33678)
...
* 禮品卡金額的PayPal信用顯示異常已解決。 以前，當啟用PayPal信用並配置了多張禮品卡金額時，如果購物者更改了禮品卡價值的金額，則店面不會更新分期付款的金額。 這是Adobe Commerce 2.4.3中的已知問題。

#### PayPal

<!-- MC-42154-->

* Adobe Commerce在使用PayPal Express付款方法結帳期間不再顯示錯誤。 以前，雖然結帳程式已完成，但Adobe Commerce仍顯示此錯誤： `Something went wrong`.

<!-- AC-722-->

* PayPal Express現在可如預期般從購物車運作。 之前，當您按一下 **PayPal** 按鈕從購物車開始快速結帳，Adobe Commerce擲回此錯誤：  `To check out, please sign in with your email address`. [GitHub-33445](https://github.com/magento/magento2/issues/33445)
...
* 將索引新增至 `magento_giftcardaccount.code` 來改善效能。
...
* Adobe Commerce依預設會在產品儲存期間同步調整影像大小。 商戶現在可以對 `di.xml` 檔案。
...
* 已最佳化搭配相關產品載入產品詳細資料頁面的程式。 相關產品條件查詢也已最佳化。
...
* 如果「類別」頁面上的價格篩選不含第二個值，Adobe Commerce不再擲回錯誤。 以前，Adobe Commerce會在記錄檔中擲回例外狀況。

### ProductAlert

<!-- MC-41917-->

* 新 `product_alert` 消費者透過建立佇列訊息、執行消費者以及縮短執行時間來改善客戶警報的傳送。 之前，Adobe Commerce會在傳送超過100,000個產品警報時擲出記憶體不足例外狀況。 Adobe Commerce也花了20多小時傳送所有警報。

### 產品影片

<!-- MC-42105-->

* Adobe Commerce現在會啟用 **儲存** 在新增視訊時輸入不完整的URL時，按鈕並自動完成欄位(管理員 **目錄**  > **產品** > **新增影片**)。 之前， **儲存** 按鈕已停用，且欄位未填入。

### 退貨授權(RMA)

<!-- MC-42987-->

* 「管理員建立退貨產品」網格現在顯示稅，包括僅針對配置為顯示帶有價格的稅的產品的價格。 以前，Adobe Commerce不會檢查 `tax/calculation/price_includes_tax` 標籤。
...
* 「獎勵點平衡」和「獎勵點歷史記錄」現在顯示相同的平衡。 過期的點數現在可正確扣除。 以前，「獎勵點數平衡」無法準確反映過期的點數。

<!-- ACP2E-158-->

* 過期的獎勵點數現在會標示為 **過期** 在店面獎勵點歷史記錄網格的「原因」部分。
...
* Adobe Commerce現在會如預期從店面移除HTML標籤 **帳戶** > **我的訂單** > **檢視順序** 頁面。 以前，Adobe Commerce會在店面客戶訂單註解區段中顯示HTML標籤。
...
* 價格範圍的分層導覽選項現在可在自訂價格屬性中正常運作。 Adobe Commerce會在篩選自訂價格屬性時使用價格導覽步驟的設定。 之前，Adobe Commerce會使用手動步驟設定。
...
* `category_name` 已新增至系統保留的產品屬性代碼。 Adobe Commerce現在會在使用者嘗試使用屬性代碼建立產品屬性時顯示錯誤 `category_name`. 之前，產品未列在預期的類別中，且記錄了Elasticsearch錯誤。
...
* Adobe Commerce現在在Elasticsearch價格指數對應中新增正確的價格欄位。 以前，索引中新增的價格欄位有商店ID而非網站ID不正確。
...
* Adobe Commerce現在會在多次運送模式中，當購物者從購物車中刪除產品，然後切換至單一運送地址時，成功更新迷你購物車。 之前，產品價格未如預期更新。

<!-- AC-267-->

* 使用S3儲存適配器時，現在可正常匯入表格率。 以前，Adobe Commerce顯示此錯誤： `File "https://[bucket].s3.eu-central-1.amazonaws.com/[prefix]/tmp/phpLjGmHf" not found`. [GitHub-33072](https://github.com/magento/magento2/issues/33072)
...
* Adobe Commerce現在會在「管理員」和「店面發票」頁面總計中顯示免運費(0)。 以前，當訂單的發運為零時，Adobe Commerce不會在發票頁發運合計的合計中顯示發運金額。

<!-- ACP2E-120-->

* 對線上運輸承運人的費率請求已從數個請求減少到一個。 現在可正確計算每個項目/包的包保險和處理價格。 以前，Adobe Commerce對FedEx運送方法顯示的運送費率不正確。

<!-- ACP2E-174-->

* Adobe Commerce現在會在為從波多黎各發貨建立發運標籤時顯示UPS費率。 以前，Adobe Commerce不會顯示這些UPS費率。
...
* 產品已排程的更新影像和視訊現已正確儲存。 過去，在建立新的排程更新後，視訊、URL、標題和說明會遭到移除。

<!-- MC-42501-->

* Adobe Commerce現在會在「計畫新更新」表單中，針對已啟用屬性的商店檢視顯示「最低廣告價格」屬性。

<!-- MC-42590-->

* Adobe Commerce現在只會如預期般在產品「已排程更新」表單中顯示一個「影像」索引標籤。 之前，Adobe Commerce已複製此索引標籤。

<!-- MC-23994-->

* 現在，「排程更新」預覽中可正確顯示產品庫存狀態。 以前，在建立「排程更新」期間啟用先前停用的產品時，產品狀態會顯示為無存貨。

### 商店

<!-- MC-42884-->

* 「以客戶身分登入」功能現在可如預期，在包含不同URL上多個存放區的部署中運作。 以前，即使存取正確的存放區網域，Adobe Commerce也未載入正確的存放區。

### 網站範圍分析工具

<!-- ACP2E-240-->

* 商戶現在可以如預期般存取網站範圍分析工具。 以前，中斷的金鑰組會讓某些商戶無法存取工具。

### 稅

<!-- MC-41924-->

* Adobe Commerce現在會在客戶選擇送至多個地址後刪除項目時，正確計算小計車。
...
* 整合測試記憶體消耗已降低，這提高了測試效能並減少了測試完成所需的時間。
...
* 修正錯誤，具有 `StorefrontPOWorkflowVerifyApprovalFlowTabTest`. [GitHub-34378](https://github.com/magento/magento2/issues/34378)
...
* 現在，當Adobe Commerce的基本URL中已設定子路徑時，會正確顯示通知訊息。
...
* Adobe Commerce載入具有預先存在之自訂主題之商店的首頁時，不再顯示錯誤訊息。 以前，Adobe Commerce會顯示此訊息：  `The store will not work correctly in the case when cookies are disabled`.

### 翻譯和地區設定

<!-- AC-285 MC-41400-->

* 此 `UserExpiration` 驗證器不再失敗 `de_DE` 和 `uk_UA` 地區設定。 以前，當管理員從區域設定為的管理員建立新使用者時，Adobe Commerce會嘗試設定到期日時擲回錯誤 `de_DE` 或 `uk_UA`. [GitHub-32497](https://github.com/magento/magento2/issues/32497)
...
* 在設為葡萄牙語（葡萄牙）地區的商店中建立新帳戶時，您現在可以成功輸入客戶的出生日期。 之前，Adobe Commerce會在您嘗試建立帳戶時擲回錯誤。 [GitHub-34130](https://github.com/magento/magento2/issues/34130)
...
* Adobe Commerce現在會從「聯絡我們」表單電子郵件輸入欄位中修剪非斷字元。 以前，提交表單時未從電子郵件輸入中移除非中斷空格字元（若有），這會導致記錄檔中發生錯誤。 此 **回覆** 已生成聯繫人電子郵件消息中也缺少欄位給儲存管理員。

<!-- MC-42793-->

* Adobe Commerce現在會根據其在管理員中的位置顯示相關產品、向上銷售產品和交叉銷售產品。
...
* 管理員客戶格線現在會顯示所有客戶資料，包括新增的資料 `date` 自訂屬性，如預期。 以前，Adobe Commerce會擲回錯誤，且在 `date` 屬性集被設定為列。
...
* 停用Cookie限制模式時，Adobe Commerce在升級後不再顯示此訊息： `The store will not work correctly in the case when cookies are disabled`. [GitHub-33811](https://github.com/magento/magento2/issues/33811)
...
* Adobe Commerce在購物者點按時不再顯示重複的位址 **變更地址** 在部署的「複核和付款」頁面上， **地址搜索** 組態設定已啟用。
...
* 檢視分界點現在在Adobe Commerce中都一致。

<!-- ACP2E-199-->

* Adobe Commerce現在可以在「管理員客戶」頁面上，以自訂地區設定正確顯示自訂客戶日期屬性。

<!-- MC-41760-->

* 以768像素寬的瀏覽器視窗顯示時，使用「頁面產生器」新增的影像不再在店面上重複。 以前，Adobe Commerce會透過顯示行動版和案頭版影像，利用此瀏覽器視窗寬度複製店面上的影像。
...
* 視覺商品商 **從庫存到底** 當所有簡單產品無存貨時，自動排序選項現在可設定產品可如預期運作。

<!-- ACP2E-61-->

* 若未新增任何條件，則不再儲存「視覺商品商」規則。
...
* 您現在可以使用的子類建立對象 `\Magento\Framework\Api\AbstractSimpleObjectBuilder` PHP 7.3。之前， `preg_match` 當Adobe Commerce以PHP 7.3在Redhat上托管時，擲回此警告訊息： `Warning: preg_match(): Compilation failed: unrecognized character follows...`.
...
* Adobe Commerce現在會產生 `customertoken` GraphQL或REST API要求，如預期。 之前，Adobe Commerce不會檢查 `lock_expires_at` in `oauth_token_request_log` 大於目前的日期和時間，且一律會傳回失敗的嘗試次數，使客戶無法登入。 [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* Adobe Commerce不再於 **在側欄中顯示** 「願望清單」選項已禁用。 之前，Adobe Commerce會忽略此選項。
...
**問題**:商戶無法針對透過Braintree以Apple支付支付的訂單提交部分退款。 當商家嘗試為訂單發票的部分退款建立貸項通知單時， **要退款的數量** 欄位無法編輯。  **因應措施**:應用修補程式 `braintree-disabled-partial-capture-for-applepay-googlepay.patch`. 請參閱 [Adobe Commerce 2.4.4:無法建立部分發票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知識庫文章。  <!-- BUNDLE-3088-->

**問題**:若未在「管理員」中選取任何選項，購物者便無法將產品新增至購物車 **商店** >  **設定** > **一般** > **國家/地區選項** > **允許國家/地區**. Adobe Commerce會改為顯示下列主控台錯誤： `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->
