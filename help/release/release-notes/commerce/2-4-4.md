...
description: 瞭解2.4.4 Adobe Commerce版本的新功能、增強功能、錯誤修正和已知問題。
...
>[!NOTE]
>
>除了Braintree之外，所有廠商隨附的擴充功能均已從Adobe Commerce 2.4.4中移除。
...
Adobe Commerce 2.4.4包含針對Adobe Commerce和Magento Open Source2.4.3、2.4.3-p1及2.3.7-p2提供的下列Hotfix解決的所有問題的解決方案：
...
建議您安裝Adobe Commerce 2.4.4後套用下列修補程式：

* `braintree-disabled-partial-capture-for-applepay-googlepay.patch`.  商家無法針對透過Braintree以Apple Pay支付的訂單，提交部份退款。 當商家嘗試從訂單商業發票建立部份退款的銷退折讓單時， **要退款的數量** 欄位不可編輯。 此Hotfix可解決此問題。 請參閱 [Adobe Commerce 2.4.4：無法建立部分發票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知識庫文章。  <!-- BUNDLE-3088-->
...
* Adobe Commerce店面現已預設啟用HTTPS。 此 **在店面上使用安全URL** 和 **在管理員中使用安全URL** 設定預設為啟用，且所有內建Cookie現在均設為安全。  <!-- AC-1173-->

* 所有Adobe Commerce安裝現在都需要相依性混淆外掛程式。 之前，只有Composer型安裝需要此外掛程式。 它現在允許信任的版本。 商家可以略過此外掛程式中設定的限制，這些限制會禁止某些組合的Composer安裝。 對於受信任的版本，可略過這些限制，Adobe Commerce現在會在繼續安裝之前顯示警告。  <!-- AC-501 970-->
...
* Adobe Commerce 2.4.4支援Elasticsearch 7.16和OpenSearch 1.2。部署於內部部署託管的Adobe Commerce商家可使用任一解決方案。 不過，OpenSearch現在是雲端託管之Adobe Commerce 2.4.4部署的預設搜尋引擎。 所有使用雲端部署的客戶若升級至2.4.4版，都必須使用OpenSearch。 另請參閱 [在Cloud 2.4.4上切換至Adobe Commerce的OpenSearch](https://support.magento.com/hc/en-us/articles/4419942355725-Switching-to-OpenSearch-for-Adobe-Commerce-on-Cloud-2-4-4)
...
Adobe Commerce效能增強功能可大幅提升高輸送量訂單處理和訊息佇列最佳化。 此版本中引入的非同步訂購功能支援每小時建立約60,000份訂單。 舊版Adobe Commerce支援每小時約10,000份訂單的處理作業，因此可能成為快閃銷售瓶頸。 新的多使用者功能支援在單一雲端例項上調整訊息佇列使用者的數量，並增加每小時處理的訂單數。

此版本中的效能增強功能：

* AsyncOrder功能支援比同步執行更快的順序放置。 啟用AsyncOrder時，當購物者完成店面上的其他工作時，會在背景執行下單。

* 新 **在購物車載入時啟用詳細目錄檢查** 設定選項在購物車中載入產品時提供可切換的詳細目錄檢查。 預設為啟用。 當您停用此選項時，Adobe Commerce會在報價載入時略過存貨檢查，這會加快結帳速度，尤其是包含許多專案的購物車。

* 新 `multiple_processes` 組態選項支援在多個處理程式中執行平行取用者。 先前， `cron` 需要時已執行單一消費者。 啟動多個使用者以並行執行程式可以提高任務執行速度。 若要啟用此功能，請新增 `multiple_processes` 至 `app/etc/env.php` 檔案。

* 透過增加設定的記憶體限制，已改善購物車操作，購物車內含超過750種可設定產品 `max_input_vars` 在 `php.ini` 檔案以支援輸入變數數量。

* 透過延遲總計計算來最佳化結帳期間的銷售規則處理。 通常，每當購物者新增產品至購物車、更新產品數量或按一下  **下訂單** 按鈕。 商戶可透過設定 `checkout/deferred_total_calculating` 中的變數 `env.php` 檔案。 或者，您可以執行 `bin/magento setup:config:set --deferred-total-calculating 1|0`.  <!-- MCP-573-->

* 非同步下單期間，受購物車價格規則影響的訂單驗證程式的改善。 <!-- MCP-304-->

另請參閱 [高輸送量訂單處理](../../../performance/high-throughput-order-processing.md).

### GraphQL

此版本包含下列GraphQL增強功能：

* **可轉讓報價的完整GraphQL涵蓋範圍**.  B2B公司使用者現在可以使用GraphQL完成與可協商報價相關的所有任務。 此API的先前版本支援交涉流程，但不支援簽出。 <!-- PWA-2101-->
...
以下專案除外： [Braintree](https://docs.magento.com/user-guide/payment/braintree.html)，所有廠商隨附的擴充功能已從Adobe Commerce 2.4.4程式碼庫中移除。 商戶應移轉至官方的擴充功能，這些擴充功能可在 [Commerce Marketplace](https://marketplace.magento.com/extensions.html). <!-- AC-1165-->
...
PWA Studio v.12.3.0與Adobe Commerce 2.4.4相容。其中包含對reCaptcha、Page Builder內容最佳化和個人化內容的支援。 如需有關增強功能和錯誤修正的資訊，請參閱 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 另請參閱 [版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 以取得PWA Studio版本及其相容Adobe Commerce核心版本的清單。
...
新 `Magento_ElasticsearchCatalogPermissionsGraphQl` 模組在更新的即時搜尋期間停用 [安裝](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/onboard/install.html). 現有的Live Search安裝必須升級至Live Search 2.0.0，才能利用這些功能。 如需詳細資訊，請參閱即時搜尋 [發行說明](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html).

### 協助工具更新

此版本提高了對標準無障礙指引的符合程度。 這些增強功能可改善視力有限或語言、認知和學習能力有限的使用者體驗。  熒幕Reader工具的效能也有所改善。

協助工具增強功能包括：

* 工具提示連結包含文字，以及準確、以工作為中心的可見輸入標籤
* 編輯按鈕現在具有唯一文字。
* 現在，管理員按鈕具有符合AA標準的唯一、可存取和描述性目的
* 傳達意義的圖示影像現在提供替代文字。
* 增強管理按鈕和表單欄位的對比，使用相鄰顏色時對比度至少為3:1。
...
我們正在修正Adobe Commerce 2.4.4核心程式碼中的數百個問題。 以下說明這些已修正問題的子集。
...
* 現在可以從命令列成功變更Sitemap產生器設定。 之前，當您嘗試在管理員外部變更Sitemap設定時，Adobe Commerce會顯示此錯誤： `Import failed: Notice: Trying to access array offset on value of type null in app/code/Magento/Cron/Model/Config/Backend/Sitemap.php on line 78`.  [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* 具有受限制許可權的管理員現在可以按預期建立傳回。

### 後端

<!-- MC-43161-->

* 管理員現在可以在已設定自訂管理員路徑且已啟用秘密金鑰的部署中登入管理員。 Adobe Commerce先前會顯示此錯誤： `Invalid security or form key. Please refresh the page`.

<!-- MC-41949-->

* 多存放區階層變更時，Adobe Commerce不再重新導向至所有存放區檢視範圍。 先前，儲存階層後，購物者會重新導向至不正確的商店URL，而多商店部署中會選取「所有商店檢視」範圍。

### 套裝產品

<!-- MC-42885-->

* 購物者現在可以新增包含固定產品稅和兩個選項的套件產品至購物車。 之前，Adobe Commerce並未新增產品，且顯示以下錯誤： `We can't add this item to your shopping cart right now`.
...
* 當套件產品重新上架時，Adobe Commerce現在會依預期更新類別產品快取。
...
* 在啟用永久購物車的部署中，購物者現在可以在工作階段逾時後下訂單，而不會發生錯誤。 過去，Adobe Commerce會在工作階段逾時後顯示付款錯誤。
...
* 如果更新的數量無效，Adobe Commerce現在會將購物車產品數量回覆為先前的值。
...
* 現在，啟用表格費率時，Adobe Commerce可正確將免費送貨臨界值套用至訂單。 [GitHub-21832](https://github.com/magento/magento2/issues/21832)
...
* 購物者點按時，Adobe Commerce不再擲回錯誤 **更新購物車** 變更迷你購物車中的產品數量後。 Adobe Commerce先前會顯示此錯誤：  `The quote item isn't found. Verify the item and try again`.
...
* 在已移除mini cart的商店中結帳時，Adobe Commerce不再擲回主控台錯誤。 [GitHub-34513](https://github.com/magento/magento2/issues/34513)
...
* 使用Adobe Commerce後，不會再擲回錯誤 `addConfigurableProductsToCart` 變異：在變更商店檢視後直接將可設定產品新增至購物車。

<!-- ACP2E-176-->

* Adobe Commerce不再在您使用 `addConfigurableProductsToCart` 變異，以將可設定產品的已停用子項新增至購物車。 之前，Adobe Commerce擲回完整性條件約束違規錯誤。
...
* 已將驗證新增至結帳工作流程，以比較客戶的街道地址與預設值。 如果驗證程式識別變更，則會設定 `customer_address_id` 至 `null` 將其視為新的客戶地址。 先前，如果客戶在結帳時儲存地址而未進行驗證，且在對結帳欄位套用驗證後，同一客戶嘗試使用相同地址下訂單，則Adobe Commerce不接受編輯的地址並擲回例外狀況。

### 購物車價格規則

<!-- MC-42288-->

* 優惠券產生現在會遭到封鎖，直到將相關的購物車價格規則與一併儲存 `auto` 選項已啟用。 如果您嘗試儲存此規則，但未啟用 `auto` 選項，Adobe Commerce會顯示此訊息： `Rule is not saved with auto generate option enabled. Please save the rule and try again`.
...
* 現在當使用Adobe Commerce設定時，可以成功將價格為0的產品新增到購物車 **整個購物車的固定金額折扣** 購物車價格規則。 Adobe Commerce先前會顯示此錯誤： `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`.
...
* 當產品具有「 」時，在產品網格上執行大量屬性更新動作時，Adobe Commerce不再擲回例外狀況 `datetime` 屬性。
...
* 新增路由器無法具現化從URL收集之動作類別情況下的驗證。 Adobe Commerce現在顯示標準錯誤訊息。 Adobe Commerce先前會顯示此錯誤：  `PHP Error: Cannot instantiate abstract class Magento\Catalog\Controller\Product\Compare`.

<!-- ACP2E-10-->

* 當兩個購物車規則套用至符合以下條件的訂單時，Adobe Commerce現在可以正確計算折扣：一個購物車規則套用優惠券代碼和 **整個購物車的固定價格**  條件和第二個購物車價格規則沒有優惠券和 **產品價格折扣的百分比** 條件。 先前，當先前的購物車規則已對某些產品套用折扣時，相關演演算法無法在購物車產品之間適當地按比例分割折扣。
...
* 更新子產品時，不再移除在部分索引程式期間繼承自其父可設定產品的目錄規則折扣。 先前，當我們將可設定產品指派給類別，但沒有其關聯的簡單產品，然後使用該類別建立目錄規則時，Adobe Commerce在部分索引過程中移除了指派給簡單產品的目錄規則。
...
* 可設定的產品選項標簽名稱現在會根據管理員訂單建立期間選取的商店檢視。 之前，Adobe Commerce使用預設商店檢視中的標簽名稱。
...
* Adobe Commerce現在會在建立訂單時考量管理員地區設定的網站範圍。 過去，訂單詳細資料（例如客戶地址屬性）只有在多網站部署中，才能在一個網站上正常運作。 [GitHub-23254](https://github.com/magento/magento2/issues/23254)

### 客戶區段

<!-- MC-42241-->

* 現在當您建立公司帳戶時，新客戶區段的「相符客戶」計數會自動更新。 之前，您必須重新整理區段資料才能取得準確計數。
...
* Adobe Commerce當從管理員和 **通知客戶** 核取方塊已啟用。 以前，送貨確認電子郵件會包含此訊息： `We're sorry, an error has occurred while generating this content`.
...
* Adobe Commerce現在包含 `Content-Disposition: inline` 電子郵件中的標題如預期。 [GitHub-29258](https://github.com/magento/magento2/issues/29258)
...
* 按一下任何電子郵件範本的預覽，現在會依預期在個別視窗中開啟範本預覽。  之前，Adobe Commerce會從「電子郵件範本」格線和在快顯視窗中同時開啟範本預覽。
...
* 協助程式 `Magento\Payment\Helper\Data` 不再於建構函式中建立新版面。 先前，當自訂命令中使用此Helper而未指定區碼時，Adobe Commerce會擲回錯誤。 [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* 現在，當bootstrap檔案中發生例外狀況時，Adobe Commerce會傳回500回應代碼。 之前，Adobe Commerce會傳回200 OK狀態代碼。 [GitHub-22196](https://github.com/magento/magento2/issues/22196)
...
* 新的必要選項已新增至 `.htaccess` 和 `nginx.conf` 使用Adobe Commerce發佈的檔案以提供PHP 8.x支援。 過時的選項也已移除。 [GitHub-34358](https://github.com/magento/magento2/issues/34358)

<!-- MC-42448-->

* 商戶現在可以上傳多個產品的影片。 現在，只有當上傳的檔案名稱包含副檔名時，檔案產生程式才會將副檔名附加至檔案名稱。 先前，當商家嘗試上傳，然後為兩個不同的產品儲存相同的視訊時，Adobe Commerce擲回此錯誤： `Notice: Undefined index: extension in /app/7ha7zds7wvqys_stg/vendor/magento/framework/File/Uploader.php on line 699`. 這是因為Adobe Commerce嘗試將副檔名附加至檔案名稱，不論上傳的檔案名稱是否有副檔名。

<!-- magento/magento2/pull/34106-->

* 修正的問題 `sprintf(__())` 和 `sprintf(Magento\Framework\Phrase())` 在整個程式碼基底中建構與PHP 8.x相容性。 之前，Adobe Commerce擲回這種型別的錯誤： `Expected parameter of type 'string', 'Magento\Framework\Phrase' provided`. [GitHub-34085](https://github.com/magento/magento2/issues/34085)
...
* 執行中 `bin/magento support:backup:code` 現在會依預期建立備份。 之前，Adobe Commerce曾建立備份，但擲回錯誤。
...
* 此 `staging_synchronize_entities_period` `cron` 工作現在如預期般運作。 之前，第一個測試更新已成功套用，但Adobe Commerce在後續更新中擲回此錯誤： `report.ERROR: Cron Job staging_synchronize_entities_period has an error: The active update can't be deleted`.
...
* 複製產品時，Adobe Commerce現在會根據其定義的範圍複製所有產品欄位。 先前， **名稱** 和 **說明** 欄位已重設為 `global` 範圍。

<!-- MC-42612-->

* 當產品價格為0時，Adobe Commerce不再將差數舍入套用至折扣。 之前，Adobe Commerce計算負折扣金額。

<!-- MC-42215-->

* 產品具有 **設為新增** 現在可以成功儲存指派了空白開始日期和過期結束日期的屬性。 之前，當您使用以下設定儲存產品時，Adobe Commerce會擲回此錯誤： `Make sure the To Date is later than or the same as the From Date`.
...
* 類別 `BundleDiscountPrice` 在 `magento2/app/code/Magento/Bundle/Pricing/Price/ConfiguredPrice.php` 現在會按預期宣告。 之前，Adobe Commerce擲回此錯誤： `Class Magento\Bundle\Pricing\Price\BundleDiscountPrice not found`. [GitHub-33334](https://github.com/magento/magento2/issues/33334)
...
* 現在，當您從商店檢視層級的內嵌格線中編輯CMS頁面時，Adobe Commerce會成功儲存CMS頁面。 **使用父節點階層** 組態設定已停用。 之前，Adobe Commerce會顯示此錯誤，且沒有儲存您的CMS頁面編輯： `A technical problem with the server created an error. Try again to continue what you were doing. If the problem persists, try again later`.

<!-- ACP2E-171-->

* 現在，包含相同頁面的節點可正確顯示階層連結。 之前，Adobe Commerce會為屬於多個節點的頁面顯示相同的節點名稱。 Adobe Commerce僅會擷取頁面的第一個節點，即使每個頁面有多個節點亦然。
...
* 此 `setBillingAddressToCart` 變異現在正確使用 `same_as_shipping` 引數來設定帳單地址，以如預期符合送貨地址。 Adobe Commerce先前會顯示此錯誤： `The shipping method is missing. Select the shipping method and try again`. [GitHub-30924](https://github.com/magento/magento2/issues/30924)
...
* 此 `setShippingAddressesOnCart` 突變現在支援在下列情況下，以空白電話號碼值設定購物車的送貨地址： **顯示電話** 管理員組態設定已設為選用。 之前，Adobe Commerce擲回此錯誤： `Field CartAddressInput.telephone of required type String! was not provided`.
...
* 在排定的匯出後，Adobe Commerce現在會將匯出檔案名稱中的時間戳記轉換為使用者的時區。 以前，這些值不會轉換，而Adobe Commerce會顯示UTC時間戳記。
...
* Adobe Commerce現在會在重新索引後，依預期在店面顯示產品。 先前，當第一個重新編列索引的500個產品有庫存，接著的500個產品無庫存，店面沒有顯示任何其他產品。
...
* Adobe Commerce現在會在管理員使用唯讀時顯示資訊錯誤 `pubs/media` 嘗試存取包含影像之產品的產品詳細資訊頁面。 之前，Adobe Commerce擲回PHP錯誤。 [GitHub-32819](https://github.com/magento/magento2/issues/32819)
...
* 為了與PHP 8.1相容，已將傳回型別宣告新增至具有不完整傳回型別的內部方法。 [GitHub-34498](https://github.com/magento/magento2/issues/34498) <!-- AC-1606-->

<!-- AC-1147-->

* 已更新根 `composer.json` 中繼封裝和  `composer.json` PHP 8.1的每個模組的檔案。 [GitHub-34009](https://github.com/magento/magento2/issues/34009)
...
* 所有Composer相依性已更新至與PHP 8.1相容的版本。
...
* Adobe Commerce不再為在中執行大量動作的失敗API呼叫建立記錄專案 **系統** > **大量動作**. 之前，失敗的API呼叫的永久專案會新增至大量動作記錄檔。
...
* 購物者現在可以成功重新訂購包含產品的現有訂單，並搭配店面和管理員中型別檔案和型別下拉式清單的自訂選項。 之前，Adobe Commerce會擲回錯誤，且不會處理重新排序。

<!-- MC-42746-->

* 依商店貸方支付且已沖銷固定產品稅捐(FPT)的訂單商業發票，現在包含正確的總計。

<!-- MC-42332-->

* 管理員訂單詳細資訊頁面現在會如預期載入。 之前，針對具有特定稅捐的訂單，Adobe Commerce在載入訂單詳細資訊頁面時擲回下列錯誤： `Call to a member function getId() on array`.

<!-- MC-38521-->

* 具有包括訂單和送貨許可權的受限制許可權的管理員現在可以按預期檢視訂單頁面。 之前，當具有這些許可權的管理員嘗試檢視訂單時，Adobe Commerce會擲回錯誤。 [GitHub-14633](https://github.com/magento/magento2/issues/14633)
...
* `bin/magento setup:upgrade` 從Adobe Commerce 2.4.2-p1升級時，現在會如預期般執行。 之前，Adobe Commerce擲回此錯誤： `Unable to apply data patch Magento\Paypal\Setup\Patch\Data\UpdateBmltoPayLater for module Magento_Paypal`. [GitHub-33678](https://github.com/magento/magento2/issues/33678)
...
* 已解決禮品卡金額的PayPal信用卡顯示異常。 先前，啟用PayPal信用卡並設定多個禮品卡金額時，如果購物者變更禮品卡金額值，店面不會更新分期付款的金額。 這是Adobe Commerce 2.4.3中的已知問題。
...
* PayPal Express現在可從購物車正常運作。 先前，當您按一下 **PayPal** 按鈕以從購物車開始快速結帳，Adobe Commerce擲回此錯誤：  `To check out, please sign in with your email address`. [GitHub-33445](https://github.com/magento/magento2/issues/33445)
...
* 新增索引至 `magento_giftcardaccount.code` 以提升效能。
...
* Adobe Commerce預設會在產品儲存期間同步調整影像大小。 商戶現在可以透過對進行微幅編輯將影像大小調整為背景非同步程式 `di.xml` 檔案。
...
* 已最佳化載入具有相關產品的產品詳細資料頁面。 相關產品條件查詢也已最佳化。
...
* 如果「類別」頁面上的價格篩選不含第二個值，Adobe Commerce就不會再擲回錯誤。 之前，Adobe Commerce在記錄檔中擲回例外狀況。

### 產品警報

<!-- MC-41917-->

* 新 `product_alert` consumer透過建立佇列訊息、執行使用者並改善執行時間，來改善客戶警報的傳送。 之前，Adobe Commerce在傳送超過100,000個產品警報時擲回記憶體不足例外狀況。 Adobe Commerce也花了20多個小時來傳送所有警報。

### 產品影片

<!-- MC-42105-->

* Adobe Commerce現在會啟用 **儲存** 當您在新增影片時輸入不完整的URL時，按鈕和自動完成欄位（管理員） **目錄**  > **產品** > **新增視訊**)。 先前， **儲存** 按鈕已停用，且欄位未填入。

### 退貨授權(RMA)

<!-- MC-42987-->

* 「管理員建立退貨產品」網格現在會顯示稅捐，包括僅設定為隨價格顯示稅捐的產品價格。 之前，Adobe Commerce不會檢查上的組態顯示設定 `tax/calculation/price_includes_tax` 標幟。
...
* 獎勵積分餘額與獎勵積分歷史記錄現在會顯示相同的餘額。 已過期的點數現在已正確扣除。 先前，獎勵積分結餘並未準確反映過期的積分。

<!-- ACP2E-158-->

* 過期的獎勵積分現在標籤為 **已過期** 位於店面「獎勵點記錄」格線的「原因」區段。
...
* Adobe Commerce現在會如預期從店面移除HTML標籤 **帳戶** > **我的訂單** > **檢視訂單** 頁面。 之前，Adobe Commerce會在storefront客戶訂單評論區段中顯示HTML標籤。
...
* 價格範圍的分層導覽選項現在與自訂價格屬性可如預期運作。 在篩選自訂價格屬性時，Adobe Commerce會使用價格導覽步驟的設定。 過去，Adobe Commerce使用手動步驟設定。
...
* Adobe Commerce現在將正確的價格欄位新增至Elasticsearch價格指數對應。 以往，系統會使用商店ID而非網站ID將不正確的價格欄位新增至索引。
...
* Adobe Commerce現在會在購物者於多送貨模式從購物車中刪除產品，然後切換至單一送貨地址時，成功更新迷你購物車。 以前，產品價格未按預期更新。

<!-- AC-267-->

* 使用S3儲存配接卡時，現在匯入表格速率可如預期運作。 Adobe Commerce先前會顯示此錯誤： `File "https://[bucket].s3.eu-central-1.amazonaws.com/[prefix]/tmp/phpLjGmHf" not found`. [GitHub-33072](https://github.com/magento/magento2/issues/33072)
...
* Adobe Commerce現在會在管理員和店面發票頁總數上顯示免運費(0)。 以前，當訂單的出貨量為零時，Adobe Commerce不會在發票頁面上的出貨總金額中顯示出貨總金額。

<!-- ACP2E-120-->

* 對線上出貨承運人的費率要求已從數個要求減少至一個。 現在可以正確計算每個專案/套件的套件保險與處理價格。 以前，Adobe Commerce對FedEx送貨方法顯示不正確的運費。

<!-- ACP2E-174-->

* Adobe Commerce現在在建立從波多黎各出貨的運送標籤時顯示UPS費率。 之前，Adobe Commerce不會顯示這些UPS速率。
...
* 現在可正確儲存產品排程更新影像和影片。 先前，在建立新的排程更新後，會移除影片、URL、標題和說明。
...
* Adobe Commerce現在在產品排程更新表單中只按預期顯示一個影像索引標籤。 之前，Adobe Commerce會複製此標籤。

<!-- MC-23994-->

* 產品庫存狀態現在正確顯示在排程更新預覽中。 以前，在建立排程更新期間啟用先前停用的產品時，產品狀態會顯示為無庫存。

### 儲存

<!-- MC-42884-->

* 「以客戶身分登入」功能現在可在不同URL上包含多個存放區的部署中正常運作。 過去，即使存取正確的商店網域，Adobe Commerce也不會載入正確的商店。

### 全網站分析工具

<!-- ACP2E-240-->

* 商戶現在可以如預期存取「全網站分析工具」。 過去，中斷的索引鍵配對會使某些商家無法存取該工具。

### 稅金

<!-- MC-41924-->

* 當客戶選擇送貨至多個地址後刪除專案時，Adobe Commerce現在可以正確計算迷你購物車小計。
...
* 整合測試記憶體耗用量已減少，可改善測試效能，並減少完成測試所需的時間。
...
* 已更正的錯誤，包括 `StorefrontPOWorkflowVerifyApprovalFlowTabTest`. [GitHub-34378](https://github.com/magento/magento2/issues/34378)
...
* 當Adobe Commerce的基本URL中已設定子路徑時，通知訊息現在可以正確顯示。
...
* 載入具有預先存在的自訂主題的存放區首頁時，Adobe Commerce不再顯示錯誤訊息。 Adobe Commerce先前會顯示此訊息：  `The store will not work correctly in the case when cookies are disabled`.

### 翻譯與地區設定

<!-- AC-285 MC-41400-->

* 此 `UserExpiration` 驗證器不再失敗 `de_DE` 和 `uk_UA` 地區設定。 之前，管理員從管理員建立新使用者（地區設定已設為）時，Adobe Commerce會嘗試設定到期日時擲回錯誤 `de_DE` 或 `uk_UA`. [GitHub-32497](https://github.com/magento/magento2/issues/32497)
...
* 在設定為葡萄牙文（葡萄牙）地區設定的商店中建立新帳戶時，您現在可以成功輸入客戶出生日期。 之前，當您嘗試建立帳戶時，Adobe Commerce會擲回錯誤。 [GitHub-34130](https://github.com/magento/magento2/issues/34130)
...
* Adobe Commerce現在會修剪「聯絡我們」表單電子郵件輸入欄位中的不斷行空格字元。 以前，提交表單時沒有從電子郵件輸入中移除不間斷的空格字元（如果給定），這會造成記錄檔案中的錯誤。 此 **回覆** 產生的連絡人電子郵件訊息中亦缺少欄位給商店管理員。

<!-- MC-42793-->

* Adobe Commerce現在會根據相關產品、向上銷售產品和交叉銷售產品在管理員中的位置，顯示這些產品。
...
* Admin客戶格線現在顯示所有客戶資料，包括新新增的資料 `date` 自訂屬性（如預期）。 之前，Adobe Commerce擲回錯誤，且當 `date` 屬性集已設定為欄。
...
* 停用Cookie限制模式時，升級後Adobe Commerce不再顯示此訊息： `The store will not work correctly in the case when cookies are disabled`. [GitHub-33811](https://github.com/magento/magento2/issues/33811)
...
* 購物者點按時，Adobe Commerce不再顯示重複的地址 **變更地址** 在部署中的「複查與付款」頁面上， **地址搜尋** 已啟用組態設定。
...
* Adobe Commerce現在會在「管理客戶」頁面上正確顯示自訂客戶日期屬性與自訂地區設定。

<!-- MC-41760-->

* 以瀏覽器視窗寬度768px顯示時，使用「頁面產生器」新增的影像不再在店面中重複。 以往，使用此瀏覽器視窗寬度時，Adobe Commerce會同時顯示行動版和案頭版的影像，以複製店面的影像。
...
* Visual Merchander **從庫存移至底部** 當所有簡單產品都無存貨時，可設定產品的「自動排序」選項現在可如預期運作。

<!-- ACP2E-61-->

* 未新增任何條件時，不再儲存Visual Merchandiser規則。
...
* 您現在可以使用以下的子類別來建立物件： `\Magento\Framework\Api\AbstractSimpleObjectBuilder` 在PHP 7.3上。先前， `preg_match` 當Adobe Commerce在Redhat上使用PHP 7.3託管時，拋出此警告訊息： `Warning: preg_match(): Compilation failed: unrecognized character follows...`.
...
* Adobe Commerce現在產生 `customertoken` 在多次連續失敗的登入嘗試後，依預期執行GraphQL或REST API要求。 之前，Adobe Commerce不會檢查的 `lock_expires_at` 在 `oauth_token_request_log` 大於目前的日期和時間，且一律會傳回失敗嘗試次數，這會阻止客戶登入。 [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* 當「 」出現在類別側邊欄中時，「 」Adobe Commerce不再呈現 **在側欄中顯示** 希望清單選項已停用。 Adobe Commerce先前已忽略此選項。
...
**問題**：商家無法針對透過Braintree以Apple Pay支付的訂單提交部分退款。 當商家嘗試從訂單商業發票建立部份退款的銷退折讓單時， **要退款的數量** 欄位不可編輯。  **因應措施**：套用修補程式 `braintree-disabled-partial-capture-for-applepay-googlepay.patch`. 請參閱 [Adobe Commerce 2.4.4：無法建立部分發票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知識庫文章。  <!-- BUNDLE-3088-->

**問題**：在「管理員」中未選取任何選項時，購物者無法將產品新增至購物車 **儲存** >  **設定** > **一般** > **國家/地區選項** > **允許國家/地區**. Adobe Commerce改為顯示下列主控台錯誤： `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->
...
您可以使用以下專案安裝Adobe Commerce 2.4.4： [Composer](../../../installation/composer.md).
