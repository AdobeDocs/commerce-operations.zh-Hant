---
title: Adobe Commerce 2.4.7-beta3發行說明
description: 瞭解2.4.7-beta3 Adobe Commerce版本的新功能、增強功能、錯誤修正和已知問題。
...
# Adobe Commerce 2.4.7-beta3發行說明
...
Adobe Commerce 2.4.7-beta3推出對PHP 8.3的支援。此版本引進了超過150項品質修正和增強功能。 核心撰寫器相依性和協力廠商程式庫已升級至最新可用版本。 此版本包含顯著的效能改善，包括針對具有許多選項的複雜產品，更快速地載入產品清單頁面。

{{bics}}

## 其他發行資訊

雖然這些功能的程式碼與Adobe Commerce核心程式碼的發行版本搭配使用，但其中幾個專案(例如B2B和Progressive Web Application (PWA) Studio)也獨立發行。 這些專案的錯誤修正記錄在每個專案檔案中提供的個別專案特定發行資訊中。

## Adobe Commerce 2.4.7-beta3測試環境

Adobe Commerce 2.4.7-beta3已針對下列元件版本進行測試：

* Chrome 118
* Composer 2.6或與PHP 8.3相容的最新版本
...
此發行版本包含與Adobe Commerce 2.4.6-p5、2.4.5-p7和2.4.4-p8中包含的相同安全性修正和平台安全性改善。 另請參閱 [Adobe安全性公告](https://helpx.adobe.com/security/products/magento/apsb24-03.html) 以取得這些已修正問題的最新討論。
...
* **自動產生優惠券代碼數目的限制**. Commerce現在會限制自動產生的優惠券代碼數量。 預設最大值為250,000。 商家可以使用新的 **[!UICONTROL Code Quantity Limit]** 組態選項(**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL Customers]** > **[!UICONTROL Promotions]**)，以防止許多優惠券讓系統不堪重負。 <!-- AC-8753 -->
...
Adobe Commerce 2.4.7-beta3包含下列平台升級：

* **php 8.3相容性**. 此發行版本引入對PHP 8.3的支援。Commerce現在同時支援PHP 8.3和8.2。PHP 8.2將受支援直到其服務結束(EOS)日期為2025年12月。 2025年12月後，所有執行2.4.7部署的商家都應遷移至PHP 8.3。 <!-- AC-8988 -->

Adobe Commerce 2.4.7仍與PHP 8.1相容，僅供升級之用。 PHP 8.1不受支援，也不建議使用。 Adobe Commerce 2.4.7-beta3核心程式碼、所有隨附的擴充功能，以及所有Adobe擁有的擴充功能和SaaS服務都與PHP 8.3相容。 <!-- AC-9668 -->

* **RabbitMQ 3.11支援**. 此版本與最新版RabbitMQ 3.11相容。RabbitMQ 3.9仍支援相容性至2023年8月，但我們建議僅將Adobe Commerce 2.4.7-beta3與RabbitMQ 3.11搭配使用。

* **Composer 2.6.x**. 與Composer 2.2.x的相容性保持不變。 <!-- AC-9663 -->

* **支援清漆快取7.4**. 此版本相容於最新版的Varnish Cache 7.4。6.0.x和7.2.x版本仍維持相容性，但我們建議僅將Adobe Commerce 2.4.7-beta3與Varnish Cache 7.4或6.0版LTS搭配使用。 <!-- AC-9662 -->
...
Adobe Commerce核心程式碼中的所有JavaScript程式庫和NPM相依性均已更新至最新可用版本。 所有Laminas程式庫相依性都已更新至與PHP 8.3相容的最新版本。 <!-- AC-9667 9665 -->

#### 其他升級

* **支援每筆訂單多張優惠券**. 商戶現在可以設定每張訂單可以套用新優惠券的最大數量 **[!UICONTROL Maximum number of coupons per order]** 組態選項。 此值預設為1。 此版本已新增此功能的REST支援，GraphQL支援即將推出。

* Commerce UPS XML API閘道已移轉至新的Commerce UPS REST API，以支援UPS對其進行的更新 [API安全性模型](https://developer.ups.com/oauth-developer-guide). (UPS正在針對所有API實作OAuth 2.0安全性模型（持有人權杖）。) 所有先前的Commerce UPS XML API已從Adobe Commerce 2.4.7程式碼基底中移除。 <!-- AC-8918 -->
...
Commerce 2.4.7-beta3包括以下增強Commerce效能和擴充性：

* 企業商家現在最多可設定 **一百萬個使用中、以優惠券為基礎的購物車價格規則** 在Adobe Commerce中，購物車和結帳作業的效能不會大幅降低。 <!-- ACPT-1198 -->
...
* **針對許多商店的部署，更快速地儲存商店層級設定的作業**. 在擁有超過500個存放區的部署中儲存組態設定可能很耗時。 新的「非同步設定」模組會透過執行cron作業來啟用非同步設定儲存操作，該作業會使用取用者來處理訊息佇列中的儲存操作。 AsyncConfig預設為停用。
...
適用於GraphQL API的應用程式伺服器可讓Adobe Commerce維護Commerce GraphQL API請求中的狀態，並免除啟動載入需求，因為此需求會導致所有GraphQL API的輸送量增加、延遲降低且資源使用更有效率。 透過在程式之間共用應用程式狀態，GraphQL API要求的效率大幅提高，而GraphQL API回應的速度平均可加快30%。 <!-- ACPT-27 -->

### Adobe Commerce擴充功能中繼資料

此發行版本推出Adobe Commerce擴充功能中繼套件v2.0.0-beta3，這會自動將選取的Adobe Commerce擴充功能與此核心版本整合。 此中繼套件中包含的這些擴充功能版本，會在下列情況下安裝： `composer update` 會執行，在升級至最新核心版本時，可簡化升級這些擴充功能的程式。 這些擴充功能會維護獨立的發行排程。

Adobe Commerce 2.4.7-beta3的Adobe Commerce擴充功能中繼資料包含下列擴充功能：
...
Commerce Webhook (v1.2.0)可讓開發人員設定同步邏輯，以在Adobe Commerce事件觸發時執行對外部系統的呼叫。 當Commerce必須立即使用協力廠商端點計算或驗證訂單總計、稅額或付款等值時，則需要同步呼叫，並隨後將結果寫回Adobe Commerce。

Commerce webhook現已預設安裝。 另請參閱 [Adobe Commerce Webhooks概觀](https://developer.adobe.com/commerce/extensibility/webhooks/)

### Adobe Stock

Adobe Stock套件(`adobe-stock-integration`)現在封裝在Commerce擴充功能中繼套件(`extensions-metapackage`)，以支援更頻繁的更新。 <!-- CABPI-630 -->
...
* REST和GraphQL的更新
   ...
   * 新增端點，以提供本發行版本中推出的賣家起始報價功能的網頁API涵蓋範圍： `POST /V1/negotiableQuote/draft`. 另請參閱 [管理可協商的報價](https://developer.adobe.com/commerce/webapi/rest/b2b/negotiable-manage/).
...
* 3DS的裝置資料現在已驗證並顯示在Braintree控制面板上。 此 `collectDeviceData` 引數現在傳遞為 `true` 在 `verifyCard` 呼叫3DS。 <!-- BUNDLE-3327 -->

* 「管理員付款」設定頁面現在包含發行說明和Braintree擴充功能頁面的連結。 <!-- BUNDLE-3325 -->

* Braintree SDK已更新至最新版本。 <!-- BUNDLE-3323 -->

### 商務與Adobe IMS整合

Adobe IMS整合套件(`adobe-commerce/adobe-ims-metapackage`)現在封裝在Commerce擴充功能中繼套件(`extensions-metapackage`)，以支援更頻繁的更新。 <!-- CABPI-630 -->

### GraphQL

Commerce 2.4.7包含增強的GraphQL快取能力、對自訂屬性的GraphQL架構的支援、對Headless訂單取消的支援，以及改進的解析器快取。
...
* [attributeList](https://developer.adobe.com/commerce/webapi/graphql/schema/attributes/queries/attributes-list/) 查詢<!-- LYNX-150 -->
* [cancelerorder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) 突變<!-- LYNX-172 -->
* `clearCart` 突變 <!-- LYNX-254 -->
* `createGuestCart` 突變
...
* [attributeMetadata](https://developer.adobe.com/commerce/webapi/graphql/schema/attributes/queries/attributes-metadata/) 查詢
...
Inventory management (v1.2.7-beta3)提供管理產品詳細目錄的工具。 這項社群開發的功能與Adobe Commerce和Magento Open Source核心程式碼整合。

### 付款

* 已針對除Venmo外的所有支付方法新增核心作業的GraphQL支援。 有關支付服務的新GraphQL端點的說明，請參見 [簽出](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->

* 已針對所有付款方式（Venmo除外）加強信用卡儲存機制。 購物者現在可以儲存或儲存其ACH付款，並從其帳戶頁面檢視或刪除這些付款。 商戶可從「管理員」啟用或停用ACH存放的付款。 <!-- BUNDLE-3324 -->

* 結帳工作流程現在包含快速付款區段，提供PayPal、Google Pay和Apple Pay **[!UICONTROL Express]** 按鈕。 <!-- BUNDLE-3322 -->

[付款選項](https://experienceleague.adobe.com/docs/commerce-merchant-services/payment-services/payments-checkout/payments-options.html) 介紹支援的付款方法。

### PWA Studio

PWA Studio v.13.3.x與Adobe Commerce 2.4.7-beta3相容。 其中包含多項改善協助工具的增強功能。 如需有關錯誤修正的資訊，請參閱 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 另請參閱 [版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 以取得PWA Studio版本及其相容Adobe Commerce核心版本的清單。

### Web API框架

**使用每個購物車多張優惠券**

新的REST端點支援檢視、新增和刪除與購物車關聯的多個優惠券。 <!-- LYNX-208 -->
...
我們已修正Adobe Commerce 2.4.7-beta3核心程式碼中的數百個問題。 此版本中包含的已修正問題子集說明如下。 也說明舊版Commerce 2.4.7 beta中所包含的修正。

### 安裝、升級、部署

<!-- AC-10396 -->

* 已從設定程式中移除不必要的快取操控。 以前，Commerce在下列情況下將其設定寫入磁碟： `bin/magento setup:db-data:upgrade` 或 `bin/magento setup:upgrade` 在設定期間導致某些模組發生問題。 [GitHub-38124](https://github.com/magento/magento2/issues/38124)
...
* 「管理格線動作」下拉式選單現在可以如預期暫留在選取的連結上 **[!UICONTROL Content]** > **[!UICONTROL Pages]** （和B2B模組中的共用目錄）。 以前，此功能表大多處於隱藏狀態，管理員使用者必須捲動才能檢視選項。 [GitHub-36710](https://github.com/magento/magento2/issues/36710)
...
* 「購物車中產品」格線的「報表」區段現在會在以下情況顯示正確的產品價格： **[!UICONTROL Catalog Price Scope]** 設為 **[!UICONTROL Website]**. 以前，將產品指派至預設網站以外的網站時，產品價格會顯示為零。
...
* 停用Google Analytics4標籤時，Google Analytics3標籤現在會如預期觸發。
...
* 現在，類別產品計數在下列情況下可正確說明套件組合產品： **[!UICONTROL Display Out Of Stock*]** 已啟用。
...
* 產品詳細資料頁面現在會顯示已套用100%折扣的套件組合產品的正確價格。 先前，Commerce並未對捆綁產品價格套用100%折扣。
...
* Commerce現在會在產生整頁可快取頁面(例如產品詳細資料頁面(PDP)和CMS頁面)時（當啟用永久購物車時），透過不載入使用者特有的報價資料來最佳化效能。 以前，系統會在產生這些頁面期間不必要地載入使用者特定工作階段和報價資料，進而影響效能。
...
* Commerce現在會在觸發購物車新增動作時顯示資訊訊息。 先前， `The requested qty is not available` 並不一定都會視需要顯示在店面上。

<!-- ACP2E-2166 -->

* 擁有大型通訊錄的客戶的結帳頁面載入時間已有所改善。 Commerce現在只會處理 **[!UICONTROL Customer Addresses Limit]** 設定。 之前，Commerce會載入整個通訊錄。
...
* 您現在可以使用成功儲存新的購物車價格規則 `Magento_OfflineShipping` 擴充功能已停用。 之前，Adobe Commerce擲回此錯誤： `report.ERROR: Warning: Undefined array key "simple_free_shipping"`. [GitHub-37580](https://github.com/magento/magento2/issues/37580)
...
* 商家現在可以使用產品輪播在下列情況下建立CMS區塊： `catalog_product_price dimensions-mode` 設為 `website`. 先前，Commerce沒有儲存區塊，並擲回此SLQ錯誤： `report.CRITICAL: PDOException: SQLSTATE[42S02]: Base table or view not found: 1146 Table`.
...
* Commerce不再將產品說明文字插入產品詳細資料頁面 **[!UICONTROL Meta Description]** 此欄位留空時的欄位。 先前，當產品說明包含PageBuilder或其他HTML編輯器產生的HTML程式碼時，HTML會顯示在中繼資料中 **[!UICONTROL Description]** 欄位，因此您無法將此欄位留空。
...
* 其餘的 `V1/products/<sku>/media` 端點現在可以處理多個媒體影像的同時上傳。 先前，當同時處理多個請求時，會建立不一致的資料，且當此資料儲存至資料庫時，Commerce會擲回錯誤。
...
* 商家現在可以選擇 **[!UICONTROL Skip quantity at this time]** 和 **[!UICONTROL Skip image uploading at this time]** 在設定可設定的產品時不影響相關簡單產品的來源。 先前，選取  **[!UICONTROL Skip quantity at this time]** 導致產品來源消失。
...
* 中的擱置記錄數 `cron_schedule` 已藉由防止cron工作嘗試取得不必要的cron工作鎖定而減少資料表。 以前，cron工作嘗試在太晚而無法執行工作時取得鎖定。 中的Cron工作 `cron_schedule` 未標籤為已錯過，即使他們已超過其排程時間。 Commerce也會在錯誤記錄中重複顯示此錯誤： `report.WARNING: Could not acquire lock for cron job: indexer_update_all_views`.
...
* 包含變音符號或重音符號的電子郵件（即utf8電子郵件）現在支援客戶和公司，以及來賓和註冊客戶的訂單下單。 以前，Commerce不會儲存包含變音符號的客戶記錄並顯示此錯誤： `Something went wrong while saving the customer`.
...
* 在整個程式碼基底中重構程式碼，該程式碼基底是在程式碼實務中要求任何包含在中的快取區段時建立的。 `app/etc/env.php` 檔案會包含前端區段。 此重構可解決升級2.4.4之前版本的Commerce部署時的潛在問題。 [GitHub-38363](https://github.com/magento/magento2/issues/38363)
...
* 此 `var/log/system.log` 檔案不再包含 `main.INFO error` 訊息。 功能表專案記錄檔已從INFO層級移至DEBUG。 先前， `var/log/system.log` 檔案中滿是類似以下的訊息： `Add of item with id Magento_Theme::design_config was processed … `.
...
* 發生嚴重資料庫讀取失敗時 `synchronous_replication` 已為Adobe Commerce Cloud上的從屬連線啟用，不會再發生。
...
* 中的JSON序列化程式 `User` 模組已更換為更精確的新序列化程式(`JsonHexTag`)。 以前，Commerce在嘗試使用由規則序列化程式序列化的資料時（當資料包含未逸出的元素時），有時會當機。 [GitHub-31377](https://github.com/magento/magento2/issues/31377)
...
* 相關產品規則的 **為其中之一** 現在當使用設定時，條件可正確運作 **要比對的產品** 設定。 之前，此規則不會識別任何產品。

<!-- ACP2E-2164 -->

* CMS頁面格線現在會根據指派給管理員使用者的存放區檢視來篩選頁面。 以前不套用角色範圍。
...
* 當您先取消選取，然後選取新建立的CMS頁面時，Adobe Commerce不再擲回違反限制錯誤。
...
* 商戶現在可以通知購物者，訂單已準備好提貨，當 **[!UICONTROL No Manage Stock]** 已為產品啟用設定。 先前，啟用此設定時，Commerce會顯示此錯誤訊息： `Your order is not ready for pickup`.

<!-- ACP2E-2227 -->

* 商戶現在可以為訂單建立銷退折讓單，其中已刪除與可設定產品相關聯的簡單產品。 之前，商家無法建立銷退折讓單，因此Commerce擲回 `TypeError` 例外。
...
* 商家現在可以將同名但大小寫不同（混合大小寫字元）的影像上傳到媒體集。 以前，影像檔案名稱在頁面產生器中不區分大小寫，而名稱與大小寫不同的檔案會覆寫在區分大小寫的檔案系統上託管的Commerce媒體儲存體中。
...
* Commerce現在會將正確的折扣金額套用至部分取消的訂單。 以前，在中計算的折扣金額 `base_discount_cancelled` 變更訂單時欄計算不正確。 折扣訂單的商業發票已新增對負值的支援。

<!-- ACP2E-2585 -->

* 其餘的 `V1/orders` 端點現在會在匯入訂單時如預期套用禮品卡。 之前，系統不會儲存此端點中禮卡擴充功能屬性的變更。
...
* 當商家從此頁面起始搜尋時，「管理銷售訂單網格」頁面現在會如預期般載入。 先前，當商家嘗試搜尋此表格中的訂單時，頁面會凍結，而Commerce會顯示 `Request-URI Too Long` 瀏覽器主控台中的錯誤。
...
* 現在，Commerce可防止在此錯誤發生時建立訂單，正確處理PayPal的502錯誤。 以往，儘管PayPal發生502錯誤，導致Commerce記錄和PayPal記錄不一致，訂單仍會建立。
...
* 購物者現在可以在結帳時套用優惠券代碼，同時選擇配送方式。 以前，在這些條件下，Commerce會擲回錯誤，並將統一費率送貨指派給訂單。
...
* 獎勵點數過期後，系統不會再傳送獎勵點數過期電子郵件。 此 `magento_reward_balance_warning_notification` 和 `magento_reward_expire_points` cron工作已重新建構，以提高彈性和電子郵件傳送容量。 以前，商家無法傳送許多由這些cron任務在短時間內產生的電子郵件。 這可能會造成傳送時效性要求高的通知時發生延遲。
...
* 其餘的 `POST /V1/returns` 請求現在會針對每個產品建立一個RMA。 以往，此請求可略過驗證，而針對每項產品傳回多個RMA。

<!-- ACP2E-2450 -->

* RMA屬性現在會自動新增至預設EAV表單，當 **[!UICONTROL Show on Storefront]** RMA屬性已設定為 **否**. 先前，建立自訂傳回屬性時，RMA專案詳細資料快顯視窗為空白，且 **[!UICONTROL Show on Storefront]** RMA屬性已設定為 **否**.

<!-- ACP2E-2211 -->

* 的值 **[!UICONTROL Enable RMA]** 屬性現在符合設定中指定的值，當 **[!UICONTROL Use Config]** 「產品編輯」頁面上的核取方塊已啟用。
...
* 建立訂單的部分出貨後，Commerce現在會如預期轉換為新的出貨頁面。 先前，建立部分出貨後，Commerce並未針對訂單的其餘產品建立完整的出貨頁面。 [GitHub-38063](https://github.com/magento/magento2/issues/38063)
...
* 當管理員使用者使用登入作為客戶功能時，Commerce不再在具有多個運送地址的結帳期間擲回例外狀況。
...
* 現在，如果購物者於排程更新進行中時將套件產品新增至購物車，即可正確更新套件產品。 先前，如果在進行排程任務時，將具有選項的套件組合產品新增至購物車，Commerce會擲回此例外狀況： `Some of the products below do not have all the required options. Please select all required options`.
...
* 若未指定結束日期，更新排定的更新就不會再移除暫存專案的先前版本。 先前，更新暫存專案時，如果未指定結束日期，則會移除專案的倒回版本。 刪除前，Commerce未擲回錯誤或警告管理員使用者。
...
* Commerce現在為顯示正確的值 **[!UICONTROL Regular Price]** 當針對具有下列專案的產品選取自訂選項時的值 **[!UICONTROL Special Price]** 值條件 **[!UICONTROL Price Display Settings]** 設為包含稅捐。
...
* 修正贈品登入中的游標行為 **[!UICONTROL Message]** 文字欄位。
...
* 改善範本轉譯期間發生錯誤的傳訊功能。 Commerce現在會顯示資訊訊息。 以前，發生PHP錯誤時，Commerce顯示HTML洩漏。
...
* 現在，當單一商店模式啟用時，管理員使用者可以變更店面標誌。 以前，Commerce顯示預設的遞補標誌。 [GitHub-35952](https://github.com/magento/magento2/issues/35952)
...
* Commerce不再為根類別和設為的產品產生產品URL重寫 **[!UICONTROL Not Visible Individually]** 多存放區設定中，產生自訂存放區遺失URL重寫時。
...
* 當您嘗試在下列情況下為產品URL建立301/302重新導向，Commerce不再擲回錯誤 **產生類別/產品URL重寫** 已啟用。 建立產品並指派給類別時，就會建立自動化URL重新寫入：一個用於直接URL，另一個用於透過類別路徑的產品。 建立新產品URL重寫時，會檢查產品、類別和商店之間的關聯。 此檢查現在以關聯的實體屬性（類別和產品）為基礎。 之前，這項檢查會對預先存在的URL重寫執行。
...
* Commerce不再顯示 `Integration not secure` 管理員中的警告 **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** 當 **[!UICONTROL Callback URL]** 和 **[!UICONTROL Identity link URL]** 欄位是空的。 [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* 其餘的 `V1/carts/mine/estimate-shipping-methods` 方法現在會驗證所有層級的裝載欄位資料，並傳回400個HTTP程式碼中的無效資料。 先前，僅驗證第一層的API裝載，而如果在未驗證的裝載層級中傳回無效資料，則Commerce會傳回500狀態代碼。
...
下列已知問題將在Adobe Commerce 2.4.7 GA中修正：

**問題**：AWS雲端原生服務與PHP 8.3不相容。Adobe Commerce 2.4.7 GA將引入與PHP 8.3的相容性。 <!-- AC-11395 -->

**問題**：在快取設定為Fastly CDN (**[!UICONTROL Caching Application=Fastly CDN]**)。 這同時會影響客人購物者和註冊客戶。  <!-- AC-11411 -->

**問題**：將指派給產品的共用目錄從產品中移除後，產品不會如預期從迷你購物車中移除。 <!-- AC-8935 -->
...
您可使用以下程式碼安裝Adobe Commerce 2.4.7-beta3： [作曲者](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
