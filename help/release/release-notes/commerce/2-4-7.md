...
* **清漆快取7.4支援**。 此版本與最新版本的 Varnish Cache 7.4 相容。相容性保留在 6.0.x 和 7.2.x 版本中，但我們建議僅將 Adobe Systems Commerce 2.4.7 與 Varnish Cache 版本 7.4 或版本 6.0 LTS 一起使用。 <!-- AC-9662 -->

* **Elasticsearch 8.11** 相容性 <!-- AC-9661 -->

* 支援 Opensearch 2.12 和 OpenSearch 1.3 <!-- AC-9659 -->

* **Redis 7.2** <!-- AC-10811 -->

* 此 `extjs` 資料庫已被最新版本的 `jsTree`取代。 <!-- AC-9320 -->

* `jquery/fileUpload` 資料庫已被移除。 <!-- AC-10569 -->
...
* 添加了對新的 USPS 陸運優勢配送方式的支援。 這是與USPS的新運輸方式USPS地面優勢的開箱即用集成，該方式於2023年7月發佈。 這項新整合可用於透過USPS配送服務擷取運費、排程交貨與退貨。 USPS Ground Advantage送貨方法取代這些在USPS Ground Advantage送貨方法發行時已淘汰的送貨方法：
...
* **增強的索引器管理**。 新 `indexer:set-status` 命令支援索引器狀態的動態管理。 管理員使用者可以使用此指令將索引器狀態 `suspended`變更為、 `invalid`或 `valid`。 此功能對於在廣泛的批量作（如產品導入或更新）期間管理系統性能特別有用，因為它允許控制索引器何時由系統的cron作業自動觸發。 請參閱管理 [索引器](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/cli/manage-indexers.html)。

* **具有多種選擇**&#x200B;的複雜產品的產品清單頁面。 產品清單頁面的載入時間有所改善，其中包含具有100多個選項的複雜產品。 按類別清單產品的 GraphQL 請求的性能也有所提高。 <!-- ACPT-1683 -->

* **REST [匯入 API](https://developer.adobe.com/commerce/webapi/rest/modules/import/)** 現在支援 JSON 格式。 商家現在可以使用 JSON 格式，每分鐘將多達 100,000 條記錄導入 Adobe Systems Commerce。 <!-- ACPT-943 -->

* **銷售規則績效改進**。 通過許多（大約100,000個）活動銷售規則提高企業部署的性能。 大量實施促銷活動的企業部署通常部署許多活動購物車規則。 運行 Commerce 2.4.7 的這些類型的企業部署在結帳作期間不會看到與配置的購物車價格規則的數量相關的任何性能下降。 <!-- ACPT-1198 -->

* **為具有許多存儲**&#x200B;的部署更快地保存商店級配置的作。 在具有500個以上存儲的部署中保存配置設置可能非常耗時。 新的[非同步設定](../../../performance/configuration.md#asynchronous-configuration-save)模組會執行使用消費者處理訊息佇列中儲存作業的cron工作，以啟用非同步設定儲存作業。 AsyncConfig預設為停用。
...
GraphQL Application Server 僅適用於 Adobe Systems Commerce。 它不適用於Magento Open Source。 您必須 [提交 Adobe Systems Commerce 支援](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/help-center-guide/magento-help-center-user-guide) 票證才能在 Pro 項目上啟用 GraphQL 應用程式伺服器。

>[!NOTE]
>
>GraphQL 應用程式伺服器目前與 不 [[!DNL Amazon Simple Storage Service (AWS S3)]](https://aws.amazon.com/s3/)相容。 Adobe Systems Commerce on 雲端基礎結構 當前用於[!DNL AWS S3][遠端儲存](../../../configuration/remote-storage/cloud-support.md)的客戶無法使用 GraphQL 應用程式伺服器，直到 Adobe Systems 在 2024 年晚些時候發佈修補程式之前。<!-- ACPT-1840 -->
...
* [Adobe Systems商務Adobe Systems的 I/O 事件](https://developer.adobe.com/commerce/extensibility/events/)
...
### Adobe Systems Commerce Webhooks

Commerce Webhooks （v1.2.0 ） 使開發人員能夠配置同步邏輯，以便在觸發 Adobe Systems Commerce 事件 時執行對外部系統的調用。 當 Commerce 必須立即使用 協力廠商 終端節點計算或驗證訂單總額、稅金或付款等值，然後將結果寫回 Adobe Systems Commerce 時，需要同步調用。

現在預設會安裝 Commerce Webhooks。 請參閱 [Adobe Systems Commerce Webhooks 概覽](https://developer.adobe.com/commerce/extensibility/webhooks/)

### Adobe Stock

Adobe Stock 包 （`adobe-stock-integration`） 現在打包在商務擴展元包 （`extensions-metapackage`） 中，以支援更頻繁的更新。 <!-- CABPI-630 -->
...
* **GraphQL 支援所有 [!DNL Braintree] 付款方式，但[!DNL Venmo]**— GraphQL API 中公開了更多配置。 這對於無頭應用程式特別有用。

* **[!DNL Vaulting]帳戶區域中**&#x200B;的付款 - 已登錄的客戶現在可以 [在“客戶帳戶”區域中保險存儲/商店](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree#step-3-complete-the-advanced-settings) 新的信用卡/轉帳卡和 [!DNL PayPal] 帳戶。 以前，客戶只能在保存付款以供以後在結帳時完成交易時使用時進行保險庫/商店，現在他們可以保險庫新的信用卡/轉帳卡並PayPal帳戶，而無需創建新交易。

* **無摩擦交易** — 無摩擦交易通過減少完成線上貸方/轉帳卡卡片交易的客戶點擊/步驟量來加快支付流程。 以前（啟用 3DS 時），每個客戶都接受 3DS 質詢。 借助新的無摩擦交易功能，客戶只有在銀行要求時才會收到 3DS 的挑戰。 這減少了購物車放棄率，提高了轉換率，並帶來了更多的銷售。
...
* **訂單專案現在包含產品圖片**。 `OrderItemInterface` 顯示產品圖像，這允許圖像與訂購的產品相關聯並更有效地載入。  [GitHub-32369](https://github.com/magento/magento2/issues/32369) <!-- AC-7918 -->

* **擴展了對解析程式快取**&#x200B;的支援。 以下 GraphQL 查詢解析器現在可以緩存在 GraphQL 解析器結果緩存中，這提高了使用POST請求提交查詢時的性能：
...
* **對自訂屬性的增強支援**。 GraphQL已透過豐富API資料來支援所有屬性型別，進而增強自訂屬性支援。 GraphQL EAV屬性方案現在支援在管理員中擴充客戶屬性和客戶位址物件，以及使用GraphQL擷取它們。 具體的增強領域包括： <!-- LYNX-61 110-->

   * 擴展/新增自定義屬性支持至特定區域，例如客戶和客戶地址
   * 新增自訂屬性的快取
   * 增強產品的現有自訂屬性支援
...
* **已改善GraphQL剖析器效能**。 通過減少每次請求調用解析方法的次數，GraphQL 解析器性能得到了提高。 現在稱為一次。 以前，解析器至少被調用三次。

#### 新現有突變的欄位

* 已將該欄位新增至 [`quickorder_active` storeConfig](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/) 和 [availableStores](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/available-stores/) 查詢。此欄位指示是否啟用了快捷訂購功能。

* 新增下列欄位至「 `setBillingAddressOnCart` 和 `setShippingAddressesOnCart` 」突變：
...
* 生成資料庫備份現在可從命令行和命令行按預期工作。 之前，Adobe Systems Commerce 會拋出這個錯誤： `Syntax error or access violation: 1064 You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'delimiter' at line 1, query was: delimiter ;;`。

<!-- ACP2E-1604 -->

* 未指定`--lock-db-prefix`參數而執行`setup:config:set`將不再從`env.php`檔中刪除當前值。
...
* 管理員用戶現在可以在為相應的簡單產品啟用時為 **[!UICONTROL Qty Uses Decimal]** 新添加的捆綁選項設置十進位預設值。 以前，小數值只能針對儲存的選擇範圍設定。
...
* 現在， `POST V1/products` 當目錄價格範圍設置為 時 `website`，建立的捆綁產品會成功。 以前，發生完整性限制違規。 [GitHub-35595](https://github.com/magento/magento2/issues/35595)
...
* 商家現在可以從產品詳細資訊頁面中成功刪除現有圖片，然後上傳同名的新圖片。 以前在這些情況下，店面產品頁面無法顯示正確的圖像。

<!-- ACP2E-2128 -->

* 現在， `productDetail` 當產品影像 `alt` 屬性設置為空時，查詢將返回產品名稱，而不是 null。

<!-- ACP2E-2215 -->

* 現在，同時包含可下載產品和實物產品的訂單會在訂單完成後立即包含可下載產品的工作連結。 以前，在發貨同一訂單中的實物產品之前，無法獲得可下載產品的連結。

<!-- ACP2E-2343 -->

* 現在，當類別從一個商店/根類別移動到另一個商店/根類別時，會更新URL重寫。 現在，當子類別轉移到新的父類別時，將刪除過時的資料庫條目。 資料庫現在僅包含相關條目。 之前， `url_path` 並未更新正確的 商店 ID。

<!-- ACP2E-2152 -->

* REST `V1/products/<sku>/media`端點現在可以同時處理多個媒體影像的上傳。 以前，在同時處理多個請求時，會產生不一致的數據，當這些數據保存到資料庫中時，Commerce 會拋出錯誤。
...
* 已改善類別產品和產品類別索引器的部分重新索引效能。 以前，cron 作業針對 `indexer_update_all_views` 單個產品或類別多次運行重新索引器。
...
* Adobe Systems Commerce 現在根據組態設定，在编辑客戶頁面上顯示 **[!UICONTROL Date of Birth]**、 **[!UICONTROL Telephone]****[!UICONTROL Gender]****[!UICONTROL Company]****[!UICONTROL Tax/VAT Number]**&#x200B;和 **[!UICONTROL Fax]** 字段。[GitHub-36196](https://github.com/magento/magento2/issues/36196)

### 客戶區段

<!-- ACP2E-2471 -->

* 客戶區段條件現在會如預期般載入所有符合的客戶。 以前，客戶區段匹配客户清單不包含所有匹配的客户。
...
* 電子郵件寄件者名稱不能再包含冒號字元。 新的驗證規則現在會阻止保存包含冒號字符的發件人姓名，這些發件者姓名來自“管理員 **[!UICONTROL Stores]** > **[!UICONTROL General]**”的“商店電子郵件地址”部分。 以前，商家可能會儲存包含冒號字元的寄件者名稱，這會導致郵件伺服器錯誤。
...
* 該檔 `var/log/system.log` 不再包含 `main.INFO error` 消息。 功能表項記錄已從 INFO 級別移至 DEBUG。 以前， `var/log/system.log` 該檔充斥著按讚以下消息： `Add of item with id Magento_Theme::design_config was processed … `。

<!-- AC-8809 -->

* 當返回 false 時 `exec()` ，Redis 預載入不再失敗。 驗證已添加到預載入過程中，以防止 `array_combine` 作為 `false` 第二個參數接收。 因此，當`exec()`傳回`false`時，預先載入將繼續執行。 [GitHub-37509](https://github.com/magento/magento2/issues/37509)
...
* 該 `bin/magento setup:upgrade` 命令不再因 `$schemaPatch`數據類型變數而失敗。 之前，字串值被傳遞給`get_class`函數，這導致了錯誤。 [GitHub-37545](https://github.com/magento/magento2/issues/37545)
...
* 代理代碼生成現在按預期為在生產模式下運行的部署生成代理和工廠檔。 [GitHub-35252](https://github.com/magento/magento2/issues/35252)

<!-- AC-1477 -->

* 印度拉達克邦現在作為選項列在管理員位址 **[!UICONTROL State]** 下拉功能表中。 [GitHub-33698](https://github.com/magento/magento2/issues/33698)

<!-- AC-8388 -->

* 事件倒計時自動收報機現在可正常顯示在類別頁面上。

<!-- AC-8369 8368 8365 8317 8351 8357 8377 8357 8351 8323-->

* 禁止標記`@author`已從代碼庫中刪除，包括以下模組：`Magento_user-variable-wee`、`Magento_Catalog`、和`Magento_Customer``Magento_Wishlist`、、 `Magento_Review` `Tax`以及整個框架。[GitHub-37245](https://github.com/magento/magento2/issues/37245)，[GitHub-37246](https://github.com/magento/magento2/issues/37246)，[GitHub-37249](https://github.com/magento/magento2/issues/37249)，[GitHub-37263](https://github.com/magento/magento2/issues/37263)
...
* 管理員用戶現在可以在其商務執行個體連接到 S3 儲存 時生成發票。以前，Commerce 顯示此錯誤： `Cannot create image resource. File not found`。 [GitHub-35706](https://github.com/magento/magento2/issues/35706)
...
* 新增 `AddDataForCostaRica.php` 以提供哥斯大黎加的州資訊。 [GitHub-37382](https://github.com/magento/magento2/issues/37382)。

<!-- AC-8577 -->

* 改善用戶嘗試從管理員存取無效URL時出現的錯誤訊息。 [GitHub-35682](https://github.com/magento/magento2/issues/35682)。
...
* 在Adobe Commerce和Magento Open Source 2.4.6之前的版本中，透過延伸TinyMCE編輯器所建立的自訂樣式格式，現在可以如預期在Page Builder `text`內容型別中使用。 以前，自定義樣式格式被忽略，在升級到 Adobe Systems Commerce 2.4.6-p1 後僅載入預設格式。 [GitHub-36950](https://github.com/magento/magento2/issues/36950)

<!-- AC-8576 -->

* 在檔中添加了以下資訊性註釋 `quote.php` ： `Important: This method also copies customer data to quote and removes quote addresses`。 [GitHub-37400](https://github.com/magento/magento2/issues/37400)
...
* 現在儲存產品后，表格中的 `catalog_product_entity_varchar` 自動遞增值會正確遞增。 以前此值會增加十。
...
* 從不需要此空格的屬性移除 `id` 不必要的空格。 （此錯誤是由 Adobe Systems Commerce 2.4.6 代碼變更引入的。）

<!-- AC-7923 -->

* 當商店處於單商店模式時，商店代碼不再顯示在商店URL中。 [GitHub-36831](https://github.com/magento/magento2/issues/36831)

<!-- AC-7988 -->

* 已對 `around` 外掛程式進行微幅更新。 [GitHub-31443](https://github.com/magento/magento2/issues/31443)

<!-- AC-7989 -->

* 色票 `Helper` 已經過重構，以刪除誤導性的導入別名。 [GitHub-31373](https://github.com/magento/magento2/issues/31373)
...
* 查詢 `products` 現在返回所有可用的聚合 （篩選器） 及其正確翻譯的標籤。 之前，價格和類別匯總標籤未按預期轉換。 [GitHub-36140](https://github.com/magento/magento2/issues/36140)
...
* 更正了在處理驗證的方法之前執行的錯誤，在該錯誤中，在 GraphQL 突變中設置當前商店的方法。 [GitHub-31336](https://github.com/magento/magento2/issues/31336)
...
* 價格索引器性能問題已得到解決。 索引器效能不再逐漸降低。 索引器已重構，以透過變更建立臨時表格的方式，提高索引器處理速度。 這些臨時表格現在已重新建立，而非由更耗時的DELETE陳述式處理。
...
* 已解決店面產品詳細資訊頁面由於目錄許可權索引而導致的顯示問題。 之前，Adobe Commerce會先清除索引表格，然後在完整`catalogpermissions_product`或`catalogpermissions_category`索引程式期間重新索引。 在完成重新索引之前，目錄許可權在店面中無法如預期運作。
...
* 現在，REST `V1/products/<sku>` 會在更新產品的庫存狀態時觸發自動重新編製索引。 以前，不會為正在更新的產品觸發股票重新索引。

<!-- ACP2E-1653 -->

* 現在，當子產品由 更新 `V1/inventory/source-items`時，可在店面上更新可配置的產品庫存狀態。

<!-- ACP2E-2114 -->

* `POST <store_code>/V1/inventory/source-items` REST 調用現在驗證返回的庫存源項中空白的有效負載，並根據需要發送有關驗證的錯誤消息。

<!-- ACP2E-2072 -->

* 商家現在可以在為產品啟用設置 **[!UICONTROL No Manage Stock]** 時通知購物者訂單已準備好取貨。 以前，啟用此設置時，Commerce 會顯示以下錯誤消息： `Your order is not ready for pickup`。

<!-- ACP2E-2227 -->

* 商家現在可以為已刪除與可配置產品關聯的簡單產品的訂單創建貸項通知單。 之前，商家無法建立銷退折讓單，Commerce擲回`TypeError`例外狀況。

<!-- ACP2E-2310 -->

* 應用篩選器后，管理網格源計數現在返回有效記錄。 以前，該方法 `getSourcesCount` 總是在應用篩選器後返回源的總數。
...
* `AUTO_INCREMENT``inventory_source_item`的表不再隨著每個`UPDATE`作而增加。以前，每次更新都會增加此表的 ， `AUTO_INCREMENT` 最終導致 `AUTO_INCREMENT` 在向 添加新 `inventory_source_item table`記錄時該值超出範圍。 因此，管理員使用者嘗試從管理員建立新產品時，收到下列錯誤： `Numeric value out of range...`。
...
* 商家現在可以按預期創建貸項通知單，以部分退回部分由獎勵積分支付的訂單。 以前，當商家創建部分退貨時，訂單狀態被錯誤地標識為已關閉，並且管理員訂單頁面不顯示貸項通知單選項。

<!-- ACP2E-2632 -->

* 當輪換日誌存檔頻率設置為每周或每月間隔時，管理員作日誌輪換現在按預期工作。 以前，在設置每周或每月間隔時，不會輪換日誌。

<!-- ACP2E-2583 -->

* 商務部現在會對部分取消的訂單套用正確的折扣金額。 以前，更改訂單時，列中 `base_discount_cancelled` 計算的折扣金額計算不正確。 對負值的支援已添加到折扣訂單的發票中。

<!-- ACP2E-2585 -->

* REST `V1/orders` 端點現在在導入訂單時按預期應用禮品卡。 之前，不會儲存此端點中贈與卡片擴充功能屬性的變更。

<!-- ACP2E-2412 -->

* Commerce 現在根據管理員用戶在管理員中創建訂單時提供的排序順序對自定義客戶地址屬性進行排序。

<!-- ACP2E-2400 -->

* 現在，當商家從此頁面發起搜尋時，管理員銷售訂單網格頁面按預期載入。 以前，當商家嘗試從此表中搜尋訂單時，頁面凍結，Commerce 在瀏覽器控制台中顯示 `Request-URI Too Long` 錯誤。

<!-- ACP2E-2294 -->

* 現在，當使用信用額度和其他付款方式退還全部訂單金額時，訂單狀態將標記為已關閉商店而不是完成。 以前，訂單狀態被標記為完成是因為退款金額計算錯誤。

<!-- ACP2E-2213 -->

* 透過減少從資料庫載入訂單的次數，改進了方法的效能 `OrderRepository::get()` 。 以前，此方法會多次載入訂單。 [GitHub-36636](https://github.com/magento/magento2/issues/36636)

<!-- ACP2E-1918 -->

* 商家現在可以為包含某些商品的訂單生成貸項通知單，而其他訂單商品的總數為零。 以前，當客戶對訂單中的某些料號有100%折扣時，商家無法執行多重退款。
...
* 當您嘗試上傳的影像大於指定的檔案大小上限時，Page Builder現在會視需要顯示錯誤訊息。 以前，Commerce 會針對小於指定最大值的上傳圖像文件顯示警報彈出消息。

<!-- ACP2E-1992 -->

* CMS 頁面產品滑塊現在包含已選擇包含在滑桿中的任何子產品的父產品。 先前，父產品不在滑桿內。
...
* 頁面 Builder 頁面元素現在在 鉻黃 瀏覽器 （v112） 中會以可接受的速度呈現。 之前，在Chrome瀏覽器中執行頁面產生器來儲存內容的速度很慢，Commerce在瀏覽器主控台中記錄此錯誤： `[ERROR] Page Builder was rendering for 5 seconds without releasing locks`。
...
* 多次點擊銷售>**訂單**&#x200B;頁面&#x200B;****&#x200B;的批量作下拉功能表不再生成多個POST請求。[GitHub-37997](https://github.com/magento/magento2/issues/37997)

<!-- AC-10944 -->

* 只有當購物者導航到觸發 301 重新導向的頁面時，才會 `200` 緩存 或 `404` 回應，除非頁面的 Cache-Control 標頭包含 `private `。 先前，會快取Cache-Control標頭中具有`private`的HTTP狀態200，而且會快取HTTP狀態301 （除非其Cache-Control標頭中包含`private`）。 [GitHub-36492](https://github.com/magento/magento2/issues/36492)
...
* 停用動態價格屬性時，將套件產品新增到購物車時，購物車價格規則現在會如預期運作。 無論啟用或停用動態屬性，都會套用免費運費和正確的表格費率。 之前，免費傳送不會如預期般套用。

<!-- AC-9729 -->

* 商務部現在在為訂單創建部分貨件后，按預期轉換為新的貨件頁面。 以前，在創建分批發貨后，Commerce 不會為訂單的其餘產品創建完整的發貨頁面。 [GitHub-38063](https://github.com/magento/magento2/issues/38063)

<!-- AC-8795 -->

* 現在可以正確計算將訂單運送到多個位址的總金額。 以前，當報價單包含多個發貨位址時，總收集器會更新給定`$quote->getShippingAddress()`位址的發貨金額，而不是按預期傳遞`$shippingAssignment` 。 [GitHub-37283](https://github.com/magento/magento2/issues/37283)
...
* 現在，在應用啟用產品的暫存更新后，產品按預期顯示在店面上。 以前，產品在管理員中啟用，但當目錄清單的索引器模式設置為“按計劃更新”時，在店面類別中不可見。 完全重新索引修復了此問題。

<!-- ACP2E-1647 -->

* `staging.synchronize_entity_period`當沒有要處理的暫存更新時，表中的`magento_bulk`消費者佇列不再增加。以前，當配置為每分鐘運行一次時 `cron` ，與消費者相關的 `staging.synchronize_entity_period` 新記錄每分鐘添加到此表中。

<!-- ACP2E-1415 -->

* 當您在保存產品時為產品創建計劃更改時，Adobe Systems商務不再引發 SQL 錯誤。 Adobe Systems Commerce 先前顯示此錯誤： `The product with the SKU "%1" couldn't be added to the current update`。

### Target規則

<!-- ACP2E-1438 -->

* Target規則現在可搭配不同的存放區檢視正常運作。 以前，相關產品規則緩存的問題導致無法在多商店部署中顯示建議的產品。
...
* `crossorigin`已將`linkTypes``HEAD`屬性添加到 for 元素。[GitHub-32509](https://github.com/magento/magento2/issues/32509)
...
* 在為自定義商店生成缺失的URL商店重寫時，Commerce 不再為根類別和設置為在多設置中的產品 **[!UICONTROL Not Visible Individually]** 生成產品URL重寫。
...
* 啟用&#x200B;**產生類別/產品URL重寫**&#x200B;時，當您嘗試為產品URL建立301/302重新導向時，Commerce不再擲回錯誤。 創建產品並將其分配給類別時，將創建自動重寫URL：一個用於直接URL，另一個用於通過類別路徑的產品。 創建新產品URL重寫時，將檢查產品、類別和商店之間的關聯。 此檢查現在以關聯的實體屬性 （類別 和 產品） 為基礎。 之前，這項檢查會對預先存在的URL重寫執行。
...
* 將、 與擴充功能屬性新增到 `taxes``Magento\Sales\Api\Data\OrderInterface``additional_itemized_taxes` `itemized_taxes`這些屬性允許您使用訂單 REST 調用保存應用的稅款。
...
* 嘗試將註解新增至擱置訂單的狀態時，REST `V1/orders/{id}/comments`端點不再變更訂單的狀態。 它現在給出了一個資訊性錯誤。 以前，此端點在添加註釋時更改訂單狀態。
...
* 缺少 `media_gallery_entries` 的欄位已添加到 REST `V1/configurable-products/sku/children` 端點。 之前，此函數缺少商店和顯示 `media_gallery_entries`所需的代碼。 [GitHub-37607](https://github.com/magento/magento2/issues/37607)

<!-- ACP2E-1986 -->

* REST `V1/carts/mine/estimate-shipping-methods` 方法現在驗證所有級別的有效負載欄位數據，併為無效數據返回 400 HTTP 代碼。 之前，僅驗證第一層的API裝載，如果以未驗證的裝載層級傳回無效資料，Commerce會傳回500狀態代碼。
...
* 現在， `POST /V1/products/tier-prices` 在創建層價格期間使用無效客戶群組名稱時，終端節點會返回資訊更豐富的錯誤消息。 之前，Adobe Commerce傳回類似以下的錯誤： `Notice: Undefined index: XXXXX in ~/public_html/vendor/magento/module-catalog/Model/Product/Price/Validation/TierPriceValidator.php on line 474"`。 [GitHub-36431](https://github.com/magento/magento2/issues/36431)

<!-- ACP2E-1776 -->

* 您現在可以執行授權的異步批量 API 請求，以使用非預設 群組 ID 创建客戶。 以前，這些請求會將群組ID重設為預設值。

<!-- ACP2E-1543 -->

* 使用批量 API 创建的客户現在會收到客戶 註冊 封電子郵件，這些電子郵件使用其分配的網站中配置的語言。 以前，所有電子郵件都會傳送到預設地區設定，而不是客戶被指派的網站的地區設定。

<!-- ACP2E-1339 AC-7577-->

* 禁用時&#x200B;**[!UICONTROL Allow Guest Checkout]**，`POST V1/guest-carts`呼叫不再提交來賓訂單。

<!-- ACP2E-1336 -->

* 您現在可以從管理員處開立訂單，其狀態 `Received` 為「未啟用時 `AsyncOrder` 」。 以前，當訂單狀態由 頁面 更改 `POST V1/orders`時，您無法從管理員打開訂單詳細資訊。
...
我們的技術棧疊是以PHP和MySQL為基礎所建置。 有關詳細信息，請参閱 [系統需求](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/system-requirements.html)。
