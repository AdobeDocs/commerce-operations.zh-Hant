...
* **支援清漆快取7.3**. 此版本相容於最新版的Varnish Cache 7.3。6.0.x和7.2.x版本仍維持相容性，但我們建議僅將Adobe Commerce 2.4.7-beta1與Varnish Cache 7.3或6.0版LTS搭配使用。

* **RabbitMQ 3.11支援**. 此版本與最新版RabbitMQ 3.11相容。RabbitMQ 3.9仍支援相容性至2023年8月，但建議您僅將Adobe Commerce 2.4.7-beta1與RabbitMQ 3.11搭配使用。
...
* **針對許多商店的部署，更快速地儲存商店層級設定的作業**. 在擁有超過500個存放區的部署中儲存組態設定可能很耗時。 新的 [`Async Config`](../../../performance/configuration.md#asynchronous-configuration-save-badge-247-beta1typeinformative-urlhelpreleaserelease-notescommerce2-4-7md-tooltipavailable-in-247-beta1-only) 模組會透過執行 `cron` 使用取用者處理訊息佇列中儲存作業的工作。 `AsyncConfig` 是 **已停用** 依預設。  <!-- ACPT-831 -->
...
此發行版本引進了在Swoole PHP擴充功能上實作的新PHP應用程式伺服器。 此應用程式伺服器可讓Adobe Commerce維護Commerce GraphQL API請求之間的狀態，並免除啟動程式。 透過在程式之間共用應用程式狀態，API要求會顯著更有效率，而API回應時間可能會減少50至60毫秒。

此 `ApplicationServer` 模組會實作PHP應用程式伺服器，這僅適用於內部部署。 想要實作此伺服器的開發人員也必須安裝並設定Swool和Nginx。

此應用程式伺服器的此Beta版僅供內部部署使用；不支援雲端部署。 尚不支援B2B GraphQL功能。 當設定此版本的PHP應用程式伺服器時，GraphQL要求可能無法如預期般在內部部署中運作。 另請參閱 [GraphQL API的應用程式伺服器](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/performance-best-practices/application-server.html). <!-- ACPT-27 -->

### Adobe Commerce擴充功能中繼資料

此發行版本推出Adobe Commerce擴充功能中繼套件v2.0.0-beta1，這會自動將選取的Adobe Commerce擴充功能與此核心版本整合。 此中繼套件中包含的這些擴充功能版本，會在下列情況下安裝： `composer update` 會執行，在升級至最新核心版本時，可簡化升級這些擴充功能的程式。 這些擴充功能會維護獨立的發行排程。
...
此發行版本包含B2B議價報價、REST API更新以及多項錯誤修正的新功能和增強功能。

* **賣方起始的報價**. B2B銷售代表現在可以直接從「管理員」中的「報價單」與「客戶」網格啟動報價單。 另請參閱 [建立報價](https://experienceleague.adobe.com/docs/commerce-admin/b2b/quotes/quote-request.html)
...
* REST API的更新
   ...
   * 新增端點，以提供本發行版本中介紹之賣家起始報價功能的網頁API涵蓋範圍。 另請參閱 [管理可協商的報價](https://developer.adobe.com/commerce/webapi/rest/b2b/negotiable-manage/).

     `POST /V1/negotiableQuote/draft`

   * 針對共用目錄提供的店面功能包含增強的網頁API涵蓋範圍，包括指派和重設自訂價格動作的新端點：
...
* 此 `addProductsToCart` 查詢現在已成功新增禮品卡到購物車。 以前，此查詢需要禮品卡收件者名稱，但會傳回寄件者名稱。 Adobe Commerce擲回此錯誤： `Please specify a recipient name`. [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
**問題**：當一個或多個子產品透過排程更新被停用時，可設定的產品在店面上錯誤地列為無庫存。 <!-- ACP2E-2003 -->
...
您可以使用以下程式碼安裝Adobe Commerce 2.4.7-beta1： [作曲者](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
