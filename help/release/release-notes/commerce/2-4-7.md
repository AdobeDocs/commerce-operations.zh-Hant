...
* 當此欄位留空時，Commerce不再將產品說明文字插入產品詳細資料頁面&#x200B;**[!UICONTROL Meta Description]**&#x200B;欄位中。 先前，當產品說明包含頁面產生器或其他HTML編輯器產生的HTML程式碼時，HTML會顯示在Meta **[!UICONTROL Description]**&#x200B;欄位中，而您無法將此欄位留空。
...
* 您現在可以在查詢`ProductCard`上的`related_products`、`upsell_products`和`crosssell_products`時使用片段`ProductInterface`。 [GitHub-29769](https://github.com/magento/magento2/issues/29769)
...
* 當產品編輯頁面上的&#x200B;**[!UICONTROL Enable RMA]**&#x200B;核取方塊啟用時，**[!UICONTROL Use Config]**&#x200B;屬性的值現在會符合組態中指定的值。
...
* 當沒有要處理的臨時更新時，`staging.synchronize_entity_period`表格中的`magento_bulk`取用者佇列不再增加。 先前，當`cron`設定為每分鐘執行時，每分鐘都會將與`staging.synchronize_entity_period`消費者相關的新記錄新增到此資料表。
...
* 針對具有&#x200B;**[!UICONTROL Regular Price]**&#x200B;值的產品選取自訂選項時（當&#x200B;**[!UICONTROL Special Price]**&#x200B;設為含稅時），Commerce現在會顯示&#x200B;**[!UICONTROL Price Display Settings]**&#x200B;值的正確值。
...
* 當`Integration not secure`和&#x200B;**[!UICONTROL System]**&#x200B;欄位空白時，Commerce不再在「管理員&#x200B;**[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** > **[!UICONTROL Callback URL]**」中顯示&#x200B;**[!UICONTROL Identity link URL]**&#x200B;警告。 [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* 當`Received`未啟用時，您現在可以從管理員開啟狀態為`AsyncOrder`的訂單。 之前，當訂單狀態由`POST V1/orders`變更時，您無法從Admin開啟訂單詳細資料頁面。

<!-- ACP2E-1526 -->

* Adobe Commerce現在已成功在`content`產品媒體API回應中傳回`GET V1/products/sku/media`屬性（base64影像代碼）。 `content`屬性先前未出現在回應中。
