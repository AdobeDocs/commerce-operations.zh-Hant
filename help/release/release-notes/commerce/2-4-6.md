...
此版本包含八項安全性修正和平臺安全性改進。

迄今為止，尚未發生與這些問題相關的確認攻擊。 但是，某些漏洞可能會被利用來訪問客戶信息或接管管理員會話。 這些問題大多需要攻擊者先取得Admin的存取權。 因此，我們提醒您採取一切必要步驟來保護您的管理員，包括但不限於這些工作：
...
如需這些已修正問題的最新討論，請參閱[Adobe安全性公告](https://helpx.adobe.com/security/products/magento/apsb23-17.html)。
...
* 已從程式碼基底中移除已到期的Zend架構(ZF1)元件。 請參閱[不向後相容的變更標示](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/).<!-- AC-1512 -->
...
* **透過負載平衡改善高輸送量訂單處理效能**。 雲端基礎結構上的Adobe Commerce 2.4.6客戶如果同時有大量訂單（每分鐘1000個訂單），現在可以平衡不同節點上資料庫的負載，以改善訂單處理效能。 商家可以為MySQL資料庫和Redis執行個體啟用次要連線，以重新平衡此負載。 檢視[高輸送量訂單處理](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/high-throughput-order-processing.html#load-balancing) <!-- ACPT-495 -->
...
適用於Adobe Commerce的Adobe I/O Events可擷取Commerce事件，並將資料從Adobe Commerce傳輸至其他Adobe系統。開發人員可以在 Adobe Systems Developer 應用程式 Builder 中版本編號訂閱商務事件（如庫存更新、訂單狀態更改和價格更新）的雲端原生應用。 開發人員可以使用此功能交換數據，以便在其任務關鍵系統中編排整個買方旅程。

### 輔助功能更新

此版本聚焦是在 Venia （PWA） 上創建一個更具可感知性、可作性、可理解性和健壯性的店面體驗。 這些增強功能包括：

<!-- AC-2479-->

* 按鈕的 **[!UICONTROL Sign in]** 可視文本標籤現在與其輔助名稱匹配。 （最佳做法建議UI元素的輔助名稱以可見的標籤文本開頭。
...
* 僅使用鍵盤的用戶現在可以訪問店面上的所有頁面功能。 以前，購物者無法僅使用擊鍵來訪問子導覽連結。


### Adobe Systems Commerce 與 Adobe Systems IMS 整合

以下 Adobe Systems IMS 模組已與 Adobe Systems Commerce 2.4.6 核心代碼分離，現在打包為單獨的元包：

* `adobe-ims`
* `adobe-ims-api`
* `admin-adobe-ims`

新的 `adobe-ims-metapackage` v2.2.0 與 Adobe Systems Commerce 2.4.6 捆綁在一起。此版本未變更任何功能。<!-- CABPI-505-->
...
* 採購訂單GraphQL結構涵蓋B2B公司使用者購買Order Management功能，例如建立、編輯和刪除採購訂單。 請參閱[採購訂單 （B2B）。](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order/)
* 採購訂單規則 GraphQL 綱要提供了創建、編輯和刪除審批規則以及手動驗證採購訂單的功能。 請參閱[採購訂單審核規則 （B2B）。](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order-rule/)

此版本包含多個錯誤修正。 請參閱 [B2B發行說明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html)。
...
* 現在，迷你購物車在運行 `bin/magento catalog:image:resize`后顯示正確的產品圖像。 以前，所有圖像都由預設產品佔位元圖像替換。 [GitHub-35535](https://github.com/magento/magento2/issues/35535)

<!-- AC-2376-->

* 現在當送貨位址包含擴展屬性時，可以成功完成結帳。 之前，Adobe Systems Commerce 會拋出這個錯誤： `Object of class Magento\Quote\Api\Data\AddressExtension could not be converted to string`。 [GitHub-34202](https://github.com/magento/magento2/issues/34202)

<!-- AC-2793-->

* `addBundleProductsToCart`如果所需的產品選項沒有價值，則突變不再將產品添加到報價單。[GitHub-25676](https://github.com/magento/magento2/issues/25676)
...
* 突变 `setGiftOptionsOnCart` 現在需要客戶授權才能進行客戶購物車作。

### 購物車價格規則

<!-- AC-2242-->

* 在創建規則期間設置的購物車價格規則免費運輸選項現在可按預期應用於訂單。 以前，在結帳時會忽略在創建規則期間應用免費配送選項，並且在應用規則時不允許免費配送。 [GitHub-35013](https://github.com/magento/magento2/issues/35013)

<!-- ACP2E-1074-->

* `discount_percent`現在，當未設置物料折扣數量或大於或等於訂購物料數量時，將按預期使用折扣百分比填充產品的表列`sales_order_item`。之前將購物車價格規則套用至此產品時，不會填入此欄。
...
* 「比較產品清單」功能現在可在多網站部署中正常運作，且取決於網站。

<!-- ACP2E-900-->

* 當您重新載入產品詳細數據頁面時，Adobe Systems商務」不再顯示上一頁面的數據。 以前，重新載入產品詳細信息頁面時，第一個標籤中的數據會在一瞬間與新載入的數據合併。

<!-- ACP2E-816-->

* 現在可在行動裝置裝置的主功能表中按預期展開具有子類別的類別。 以前，購物者無法在移動部署中打開或展開具有子類別的類別。 [GitHub-35329](https://github.com/magento/magento2/issues/35329)

<!-- ACP2E-789-->

* 現在，當商品被分配到新網站或商店時，類別商品信息會按預期更新。 之前，產品價格索引子未標示為無效，並隨後在`cron`執行如預期般重新觸發。 因此，價格指數表中遺漏了產品的價格專案，而且不會顯示在新的商店或網站上。
...
* 色票選項顏色和尺寸屬性現在在產品詳細信息中預先選擇，頁面當購物者從“我的購物車”頁面修改可配置產品時。 （jQuery 3.x 版本現在支援 `swatchRenderer` 小部件名稱，現在是駝峰大小寫。之前，不會預先選取「色票」選項。
...
* **[!UICONTROL Grid Filter Condition Type]**&#x200B;禁用 s **設置后，**[!UICONTROL Use in Filter Option]&#x200B;客戶 和客戶地址屬性的字段在屬性编辑頁面上不再可見。

<!-- AC-6020-->

* 如果商家在創建產品時未能輸入可選的SKU屬性值，Adobe Systems商務現在會顯示一條資訊性錯誤消息。 以前，當屬性為可選時，應用程式會引發此異常： `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/html/mg244/project-community-edition/vendor/magento/module-eav/Model/Entity/AbstractEntity.php on line 932`。 [GitHub-35810](https://github.com/magento/magento2/issues/35810)

<!-- ACP2E-716-->

* **[!UICONTROL Show my quotes]**&#x200B;頁面上的&#x200B;**[!UICONTROL My Quotes]**&#x200B;篩選器現在如預期般運作。以前，篩選結果包含其他用戶的報價。
...
* 當禮品包裝已在訂單層次套用至包含實體與虛擬產品的訂單時，禮品包裝價格現在會如預期顯示在訂單摘要區塊中。 之前，Adobe Systems商務不會顯示禮品包裝價格。
...
* 該`customer`查詢現在可以正確篩選器指定 `from`和`to`範圍的訂單。 [GitHub-31537](https://github.com/magento/magento2/issues/31537)

<!-- AC-5984-->

* 查詢現在會在 `product` 聚合中傳回正確的類別數據。 先前， `attribute_code.category_uid` 傳回`category_id``category_uid`的值 取代 .

<!-- AC-6084-->

* `customerOrders`查詢不再引發`\"model\" value should be specified`錯誤。[GitHub-35852](https://github.com/magento/magento2/issues/35852)
...
* `categoryList`查詢現在會傳回錨定和未錨定父類別的準確產品計數。 新的`is_anchor`屬性已新增至類別集合選取資料行清單。 先前的請求返回 0 個產品計數。 [GitHub-36300](https://github.com/magento/magento2/issues/36300)
...
* 購物車重新載入現在僅在 GraphQL 突變初始化購物車時按預期觸發一次。 以前，購物車載載會觸發不必要的庫存購物車重新驗證，每次載載購物車時都會消耗大量資源。
...
* 已新增資料庫傾印中的`DATE`和`DATETIME`值清理以支援Adobe Commerce。 以前，不會對資料庫備份表中的客戶/報價/訂單表清理日期時間類型字段。

<!-- ACP2E-804-->

* 該`escaper.js``escapeHtml`函數現在將 &amp; 符號 （&amp;） 特殊字元`"$amp;"`轉換為 per `InnerHtml` 規則。此通用解決方案解決了標題輸入框中的特殊字元和字元引用轉換問題，並將插入的確切值保存。 先前的 &amp; 符號 （&amp;） 字元在 內容 階層中被錯誤編碼為節點名稱。
...
* 舊版媒體集現在預設為禁用。 之前，**[!UICONTROL Enable old Media gallery]**&#x200B;設定為&#x200B;**[!UICONTROL Yes]**。

<!-- ACP2E-1302 -->

* 新媒體庫已經過優化，以改進類別頁面中目錄樹的載入。

### 伐木

<!-- ACP2E-1312-->

* 您現在可以為 0 位元組的紀錄檔產生系統報告。 以前，當一個或多個檔為空時，Adobe Systems Commerce 在為日誌檔集合生成報告時會引發錯誤。
...
* 訪客現在可以訂購數量少於一個的產品，而且產品數量現在被嚴格輸入為`float`。 以前，由於產品數量嚴格鍵入為 `int`，因此少於 1 的產品數量訂單失敗，並顯示以下消息： `Some of the products are disabled`。 [GitHub-36060](https://github.com/magento/magento2/issues/36060)

<!-- ACP2E-1313-->

* 現在，當配置選項設置為 時&#x200B;**[!UICONTROL including taxes]**，可以正確計算&#x200B;**[!UICONTROL Apply discount on prices]**&#x200B;運費退款。

<!-- ACP2E-1199-->

* 訂單存檔后，發票、裝運和貸項通知單網格現在會異步更新。

<!-- ACP2E-1194-->

* 重複訂單不再產生於併發、相同的 GraphQL 請求。 報價現在被鎖定長達 10 秒以防止重複訂單創建，並且第二次請求失敗並出現以下錯誤： `The cart isn't active` 或 `The order has already been placed and is currently processing`。 此鎖定值可以通過類`lockWaitTimeout`參數的 `Magento\Quote\Model\PlaceOrderMutex` di 進行配置。[GitHub-36274](https://github.com/magento/magento2/issues/36274)
...
* `customer_address_id`現在，當選中複選框時，**[!UICONTROL My billing and shipping address are the same]**&#x200B;新帳單地址的值將按預期設置。引入了一個附加條件，用於在為送貨地址設置客戶地址 ID 之前確定帳單地址是否與送貨地址相同。

<!-- AC-6057-->

* 訂單刊登的併發問題已得到解決。 以前，如果在同一或幾乎相同的時間從不同的瀏覽器下達兩個訂單，則兩個訂單都會分配相同的訂單 ID。 [GitHub-35833](https://github.com/magento/magento2/issues/35833)

<!-- AC-6908-->

* 在啟用異步索引時為免費產品下自動開票訂單現在按預期工作。 對於這些訂單，表格中 `sales_order_grid or sales_invoice_grid` 不會新增任何專案。 以前，Adobe Systems Commerce 會引發異常，因為應用程式嘗試在訂單刊登期間為網格編製索引，而不是依賴 `cron`。 [GitHub-36334](https://github.com/magento/magento2/issues/36334)
...
* 頁面 Builder 滑桿 CSS 規則的問題已解決。 [GitHub-34527](https://github.com/magento/magento2/issues/34527)

<!-- ACP2E-978-->

* `Dropdown`、 `Text area`和 `Text field` 屬性現在可在 頁面 Builder 產品屬性清單中使用。 以前只有 `Dropdown` 和 `Text area` 屬性可用。

### 付款

<!-- ACP2E-862-->

* 當貨到付款地址和帳單位址中指定的國家/地區免除貨到付款時，不再在店面結帳工作流程上呈現貨到付款方式。 以前，此功能的管理員配置設置並未實現。
...
* 訂單ID現在已添加到付款交易的結算請求中。

<!-- AC-3030-->

* 當商家按兩下 **[!UICONTROL Reports]** >時，Adobe Systems **[!UICONTROL Braintree Settlement]**&#x200B;商務不再拋出以下錯誤： `Error: Undefined constant Braintree\PaymentInstrumentType::ANDROID_PAY_CARD in /var/www/html/vendor/paypal/module-braintree-core/Ui/Component/Report/Listing/Column/PaymentType.php:49`。 [GitHub-35434](https://github.com/magento/magento2/issues/35434)

<!-- BUNDLE-3043-->

* `customerPaymentTokens`在管理員上關閉保管庫時，查詢不再返回Braintree令牌。

<!-- BUNDLE-3107-->

* 已為 **[!UICONTROL Pay Later]** 義大利和西班牙啟用按鈕和橫幅。
...
* RMA註解欄位(**[!UICONTROL Orders]** > **[!UICONTROL Return]**)不再包含多餘的空格。 `<textarea>`（和 `<?php` 標記現在位於一行。

<!-- ACP2E-1279-->

* 現在， **[!UICONTROL Use Config Settings]** 當儲存產品時，字段 **[!UICONTROL Enable RMA]** 的複選框會按預期儲存其值。
...
* **[!UICONTROL Display Zero Tax Subtotal]**&#x200B;配置設置現在按預期應用於提供多種運輸方式的部署中的訂單。以前，在選擇送貨方式之前平均禁用此設置時，結賬期間購物車頁面上會顯示零稅字段。 [GitHub-35561](https://github.com/magento/magento2/issues/35561)

<!-- AC-3157-->

* Adobe Systems Commerce 現在可以在運行 PHP 8.1 的部署中的店面類別頁面顯示含稅的分組產品。以前，Adobe Systems Commerce 不會顯示產品清單並引發此異常： `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`。 [GitHub-35500](https://github.com/magento/magento2/issues/35500)

<!-- ACP2E-1258-->

* 商家無法再將計劃更新移動到包含相同 規則 ID 的另一個更新。 以前，當更新移動到包含相同規則 ID 的另一個更新時，購物車價格規則的計劃更新重複計劃。

<!-- ACP2E-1295-->

* 當沒有與客戶地址相關聯的VAT ID時，自動客戶群組指派現在可運作。 如果地址不包含增值稅號，則被視為屬於非加值稅註冊客戶，Adobe Systems商務部會將客戶群組更改為非加值稅群組。 先前，啟用&#x200B;**[!UICONTROL Enable Automatic Assignment to Customer Group]**&#x200B;時，從客戶帳戶刪除VAT ID時，不會自動將客戶帳戶指派給`customerGroup`。

<!-- ACP2E-493-->

* 現在，會為處於暫存預覽模式的分階段目錄規則顯示正確的動態塊。 以前，會顯示不相關的動態塊。

<!-- ACP2E-1080-->

* 現在，在結賬期間應用多裝運時，發票和貸項通知單的“稅務摘要”部分中會正確呈現多個稅。

<!-- ACP2E-1076-->

* 當含稅和固定商品稅金額的訂單小計超過購物車頁面的購物車規則條件時，不再應用折扣。 以前，折扣平均總計超過購物車規則條件時應用。
...
* 日文字符現在可直接輸入到產品頁面屬性集 （組合方塊）。 之前，在MacOS上執行Firefox瀏覽器時，直接在此下拉式方塊中輸入日文字元會導致字元變更或刪除。 在所有瀏覽器中可以更妥善處理日文字元轉換的`textInput` HTML屬性已取代`Value`和`valueUpdate`屬性。
...
* 主題頁首設定 `Welcome Text` 現在可以包含單引號字元。 Adobe Systems Commerce 之前會拋出這個JavaScript錯誤： `Uncaught SyntaxError: Unable to process binding "ifnot: function(){return customer().fullname }"`。

### URL重寫

<!-- ACP2E-808-->

* 將產品可見性 **[!UICONTROL Not Visible Individually]** 更改為全域“所有商店視圖”範圍現在僅刪除未覆蓋可見性屬性的商店視圖的URL重寫。 以這種方式更改產品可見性還會為所有商店檢視生成URL重寫，包括那些覆蓋了URL鍵屬性（但不是可見性設置）的檢視。 以前，當可見性設置更改時，不會應用URL重寫規則。
...
3. 在升級期間套用此修補程式於管理套件階段之後、更新中繼資料階段之前。請參閱[如何套用Adobe提供的撰寫器修補程式](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/how-to/how-to-apply-a-composer-patch-provided-by-magento.html) 。
4. 完成升級程式。
...
**因應措施**：此問題的Hot Fix現已推出。 移至[品質修補程式工具：搜尋修補程式](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html)並搜尋修補程式ACSD-50814。
...
您可以使用[撰寫器](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html)安裝Adobe Commerce 2.4.6。
