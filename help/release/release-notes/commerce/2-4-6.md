...
# Adobe Commerce2.4.6發行說明

Adobe Commerce2.4.6推出對八點二菲律賓比索的支援。仍完全支援PHP 8.1，但已取消對PHP 7.4的支援。 它包括顯著的效能和可擴充性增強。 已優化了用於批量購物車操作和類別樹呈現響應的GraphQL操作。 本版介紹了Adobe Commerce擴展元包。

此版本包括300多種質量修復和增強功能。 Core Composer依賴項和第三方庫已升級到與PHP 8.2相容的最新版本。

{{bics}}

## 其他發佈資訊

儘管這些功能的代碼與Adobe Commerce核心代碼的版本捆綁在一起，但其中幾個項目(例如B2B、頁面生成器和Progressive Web Application(PWA)Studio)也是獨立發佈的。 這些項目的錯誤修復程式都記錄在每個項目文檔中提供的獨立、特定於項目的發行資訊中。
...
* IP允許
* [雙因素身份驗證](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication/)
* VPN的使用
* 使用唯一位置，而不是 `/admin`
* 良好的密碼衛生
...
Adobe Commerce2.4.6推出對八點二菲律賓比索的支援。仍完全支援PHP 8.1。 <!-- AC-6598 -->

已從此版本中刪除PHP 7.4支援。 您不能在PHP 2.4.6上運行Adobe Commerce。

Adobe Commerce2.4.6現在支援：

* 作曲家2.2.x已刪除作曲家1.x。 <!-- AC-2839 -->

* 雷迪斯7.0.x儘管與Redis 6.2的相容性仍然存在，但我們建議將此版本與Redis 7.0.x一起部署，因為Redis 6.2預計將於2024年壽終正寢。 <!-- AC-6606 -->

* OpenSearch現在支援作為Adobe Commerce本地和雲部署的預設搜索引擎。 它支援作為獨立的搜索引擎進行Magento Open Source。 此版本支援OpenSearch v2.x，並已通過OpenSearch 2.5測試。儘管仍與OpenSearch 1.x相容，但建議將此版本與OpenSearch 2.x一起使用。您可以在雲和內部部署上安裝並運行Adobe Commerce和Magento Open Source2.4.6（使用OpenSearch 2.x） [搜索引擎先決條件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html)。 <!-- AC-449 3555 -->
...
* `jquery-migrate` 已從商業代碼庫中刪除。 [GitHub-21406](https://github.com/magento/magento2/issues/21406) <!-- AC-1199 -->
...
* **改善了影響許多客戶細分市場的運營效能**。 商家可以通過啟用新的配置設定來改進涉及100多個客戶段的操作的效能。 的 **[!UICONTROL Real-time Check if Customer is Matched by Segment]** 可以根據需要禁用設定，以通過減少針對客戶的每個客戶段的驗證來縮短處理時間。 請參閱 [客戶細分驗證](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#customer-segments-validation)。 <!-- AC-6832 ACPT-835-->

* **用於限制產品網格中顯示的產品數量的新系統配置設定**。 商家可以使用新 **[!UICONTROL Limit Number of Products in Grid]** 配置設定以提高產品網格效能。 預設情況下，此限制配置為20000，僅影響UI元件使用的產品集合。 限制所顯示的產品數量可以提高填充了200,000多種產品的網格的操作效能。 請參閱 [限制網格中的產品數](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#limit-number-of-products-in-grid)<!-- ACPT-42 -->

* **改進的導入效能（每分鐘最多100,000條記錄）**。 商家可以使用新 `POST /rest/<store_view_code>/V1/import/csv` 將資料導入Adobe Commerce的REST API終結點。 此終結點提供與管理導入功能相同的功能，並支援使用CSV檔案建立、更新和刪除產品、產品定價和客戶實體。 <!-- ACPT-6 -->

* **通過負載平衡提高高吞吐量訂單處理效能**。 Adobe Commerce2.4.6雲基礎架構客戶的同時訂單量較大（每分鐘1000個訂單），現在可以通過平衡不同節點上資料庫的負載來提高訂單處理效能。 商戶可以通過為MySQL資料庫和Redis實例啟用輔助連接來重新平衡此負載。 請參閱 [高吞吐量訂單處理](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/high-throughput-order-processing.html#load-balancing) <!-- ACPT-495 -->


### Adobe Commerce擴展元包

此版本引入了Adobe Commerce擴展整合包v1.1.0 ，它自動將選定的Adobe Commerce擴展與此核心版本捆綁在一起。 此集合包中包含的這些擴展的版本在 `composer update` 運行，簡化升級到最新核心版本時升級這些擴展的過程。 這些擴展仍將保持獨立的發佈計畫。

Adobe Commerce的Adobe Commerce擴展集合2.4.6包括以下兩個擴展：

* [螺栓快速檢出](https://experienceleague.adobe.com/docs/commerce-merchant-services/quick-checkout/overview.html)
...
Adobe Commerce的Adobe I/O活動捕獲商業活動並將資料從Adobe Commerce傳輸到其他Adobe系統。 開發人員可以在Adobe Developer應用程式構建器中構建雲本地應用程式，以訂閱庫存更新、訂單狀態更改和價格更新等商務事件。 開發人員可以使用此功能交換資料，以協調整個關鍵系統的採購過程。
...
以下Adobe IMS模組已與Adobe Commerce2.4.6核心代碼分離，現在打包成單獨的元包：

* `adobe-ims`
* `adobe-ims-api`
* `admin-adobe-ims`

新 `adobe-ims-metapackage` v2.2.0與Adobe Commerce2.4.6。此版本中沒有更改任何功能。<!-- CABPI-505-->


### B2B

「公司採購訂單」功能現在由GraphQLAPI完全公開。 本版本介紹了兩個GraphQL模式：

* 採購訂單GraphQL架構涵蓋B2B公司用戶採購訂單管理功能，如建立、編輯和刪除採購訂單。 請參閱 [採購訂單(B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order/)。
* 採購訂單規則GraphQL方案提供建立、編輯和刪除審批規則以及人工驗證採購訂單的能力。 請參閱 [採購訂單審批規則(B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order-rule/)。
...
Page Builder v.1.7.3與Adobe Commerce2.4.6相容。


### PWA Studio

PWA Studiov.13.0.x與Adobe Commerce2.4.6相容。它包括多種增強功能，以改進可訪問性。 有關錯誤修復的資訊，請參見 [PWA Studio版本](https://github.com/magento/pwa-studio/releases)。 請參閱 [版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 清單中的「PWA Studio」版本及其相容的Adobe Commerce核心版本。
...
我們解決了Adobe Commerce核心代碼中的數2.4.6問題。 下文介紹了Adobe Commerce2.4.6所列的一些固定問題。
...
* Adobe Commerce現在在您使用命令行設定不帶密碼的Redis連接時拋出參數驗證錯誤。
...
* 現在，可以從管理員成功篩選捆綁產品 **[!UICONTROL Catalog]** > **[!UICONTROL Products]** 使用荷蘭語區域設定時。 以前，Adobe Commerce在您嘗試篩選時顯示此錯誤 `Type: Bundle Product`: `Something went wrong with processing the default view and we have restored the filter to its original state`。 [GitHub-35704](https://github.com/magento/magento2/issues/35704)

<!-- ACP2E-1038-->

* 當捆綁產品的特殊價格超過1000時，產品網格不再引發異常。 以前，當您按SKU搜索產品或導航到 **[!UICONTROL Catalog]** > **[!UICONTROL Products]**,Adobe Commerce拋出了此錯誤： `Non Numeric value encountered`。 [GitHub-36284](https://github.com/magento/magento2/issues/36284))
...
* Adobe Commerce現在正確計算訂單退款的總和，並為發運提供折扣。

<!-- ACP2E-876-->

* Google反碼頭現在能如預期那樣運作。 以前，Adobe Commerce在購物者首次嘗試下訂單之前未首次選擇GoogleReCAPTCHA時顯示了以下錯誤： `ReCaptcha validation failed, please try again` 和 `No such cart with id = ID`。
...
* Adobe Commerce現在會像預期的那樣顯示確認彈出窗口，當購物者在不更新購物車的情況下更改購物車頁面上的產品數量，然後嘗試導航到下一頁時，將購物者重定向到下一頁。

<!-- AC-5987-->

* 購物者現在可以在管理員中減少購物車的數量。 以前，當購物者試圖減少產品數量時，Adobe Commerce拋出了以下錯誤： `The requested qty is not available`。 [GitHub-35780](https://github.com/magento/magento2/issues/35780)

<!-- AC-6744-->

* 當產品數量超過多次發運簽出期間配置的最大值時，Adobe Commerce不再在JavaScript控制台中引發錯誤。 相反，Adobe Commerce現在顯示一條資訊性錯誤消息。 [GitHub-36184](https://github.com/magento/magento2/issues/36184)
...
* 購物者無法再將產品從更改購物者所屬的客戶組後禁止的類別添加到購物車。 Adobe Commerce現在在權限配置設定已更改時檢查特定類別的客戶組權限訪問。
...
* 當發運地址包含擴展屬性時，現在可以成功完成簽出。 此前，Adobe Commerce拋出了這個錯誤： `Object of class Magento\Quote\Api\Data\AddressExtension could not be converted to string`。 [GitHub-34202](https://github.com/magento/magento2/issues/34202)
...
* Adobe Commerce現在會在 `addProductsToCart` 在將產品添加到手推車時，如果權限不足，則變異失敗。

<!-- ACP2E-1242-->

* Adobe Commerce在使用快速訂單以超出可用庫存的數量添加產品時，不再在購物車頁上引發錯誤。

<!-- AC-6312-->

* 的 `setGiftOptionsOnCart` 變異現在需要客戶對客戶購物車操作的授權。
...
* Adobe Commerce在重新載入產品詳細資訊頁面時不再顯示上一頁的資料。 以前，當重新載入產品詳細資訊頁面時，第一個頁籤中的資料與新載入的資料合併一秒。
...
* 分配給批發組的客戶現在可以按類別權限將分組產品添加到其購物車。 如果未為客戶組定義此權限，則優先配置設定。
...
* 現在，當由字母數字產品屬性的相關產品規則生成時，相關產品將在店面產品詳細資訊頁面上成功獲取。 [GitHub-36285](https://github.com/magento/magento2/issues/36285)
...
* Adobe Commerce不再錯誤地假定一個類別是基於類別路徑的另一個類別的父類別。 以前，當兩個類別相鄰且一個類別ID以另一個類別的類別ID開頭時，Adobe Commerce假定這些類別具有相同的父類別。 [GitHub-32464](https://github.com/magento/magento2/issues/32464)
...
* 當管理員用戶將產品手動添加到已分配了許多簡單產品的可配置產品時，Adobe Commerce不再拋出錯誤。 以前，Adobe Commerce顯示了以下錯誤： `Something went wrong with processing the default view and we have restored the filter to its original state`。

<!-- AC-2877-->

* 在禁用背訂單的部署中，購物者不能再將可售數量為0的可配置產品添加到購物車中。 Adobe Commerce現在表示產品已庫存不足。 [GitHub-35319](https://github.com/magento/magento2/issues/35319)
...
* 可配置產品SKU和產品名稱現在可準確顯示。 以前，在將屬性組移到產品詳細資訊組上方後，名稱和SKU將顯示為未定義 **[!UICONTROL Stores]** > **[!UICONTROL Attribute set]**。 [GitHub-36309](https://github.com/magento/magento2/issues/36309)
...
* 的 `catalog_index_refresh_price` cron作業現在成功執行。 以前，Adobe Commerce在克隆執行期間記錄了此錯誤： `report.ERROR: Cron Job catalog_index_refresh_price has an error: SQLSTATE[HY093]: Invalid parameter number: number of bound variables does not match number of tokens`。
...
* 在建立新購物車價格規則期間，管理客戶細分網格上的「是」/「否」篩選器現在按預期工作。

### 電子郵件

<!-- ACP2E-1063-->

* Adobe Commerce在從管理員建立訂單後不再發送客戶重複的電子郵件。
...
* 管理員用戶現在可以成功開啟管理員 **[!UICONTROL Store]** > **[!UICONTROL Configuration]** 頁中沒有國家/地區 **歐盟國家** 設定&#x200B;**[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL General]** > **[!UICONTROL Country options]** > **[!UICONTROL European Union Countries]**)。 以前，當管理員用戶嘗試開啟此頁時，Adobe Commerce拋出了以下錯誤： `TypeError: explode(): Argument #2 ($string) must be of type string, null given in /var/www/php/folder/vendor/magento/module-config/ViewModel/CountryFilter.php:46`。 [GitHub-35901](https://github.com/magento/magento2/issues/35901)
...
* 現在，在運行Adobe Commerce2.4.3-p2的部署中，可以按照預期重新授權OAuth整合。 以前，重新授權失敗，錯誤為： `The attempt to post data to consumer failed due to an unexpected error. Please try again later`。 Adobe Commerce還試圖解密未加密的值。 [GitHub-35313](https://github.com/magento/magento2/issues/35313)
...
* AMQP使用者不再因已棄用而失敗 `trim()` 運行PHP 8.1的部署上的函式。以前，當AMQP使用者在安裝後啟動時，Adobe Commerce拋出了以下錯誤： `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in .../vendor/magento/framework-amqp/Config.php on line 143`。 [GitHub-35380](https://github.com/magento/magento2/issues/35380)

<!-- AC-2882-->

* 的 `getFilteredTimerIds` 方法現在按預期工作。  類 `Magento\Framework\Profiler\Driver\Standard\Stat` 現在相容於PHP 8.1。以前，Adobe Commerce在調用此方法時拋出異常。 [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* Adobe Commerce現在顯示管理員用戶選擇的準確時間範圍 **[!UICONTROL Last 24 Hours]** 選項。 以前，Adobe Commerce顯示的時間範圍不正確。 發生此情況是因為資料庫的時區保持不變。
...
* 訪問受限於一個網站的管理員用戶現在可以使用「作為客戶登錄」功能成功登錄到管理員。
...
* 禮品卡產品金額現在與導入期間在CSV檔案中指定的金額保持相同。 已添加對現有選項的檢查，並且以前的值現在基於唯一鍵進行更新。 此前，當多次導入禮品卡時，禮品卡金額翻了一番。

### 禮品包裝

<!-- AC-3061-->

* 在將禮品包裝應用於包含物理和虛擬產品的訂單時，禮品包裝價格現在會按預期顯示在訂單摘要塊中。 此前，Adobe Commerce沒有展示禮品包裝價格。
...
* 索引器 `batch_size` 為 `catalogpermissions_category` 索引現在更改為 `env.php` 檔案。 以前，索引器路徑中的錯誤 `batch_size` 覆蓋阻止將批大小更改為中提供的值 `env.php`。 [GitHub-652](https://github.com/magento/partners-magento2ee/issues/652)
...
* 添加的清洗 `DATE` 和 `DATETIME` 資料庫轉儲中的值，用於Adobe Commerce支援。 以前，資料庫備份表中的customer/quote/order表沒有清理日期時間類型欄位。

<!-- ACP2E-804-->

* 的 `escaper.js` `escapeHtml` 函式現在將&amp;符特殊字元轉換為 `"$amp;"` 按 `InnerHtml` 規則。 此通用解決方案解決了標題輸入框中的特殊字元和字元引用轉換問題，並將插入的準確值保存。 以前，內容層次結構中的節點名稱中對&amp;符號字元進行了錯誤編碼。
...
* 現在，預設情況下已禁用舊媒體集。 以前， **[!UICONTROL Enable old Media gallery]** 已設定為 **[!UICONTROL Yes]**。

<!-- ACP2E-1302 -->

* 「新建媒體集」已優化，以改進類別頁面中目錄樹的載入。

### 記錄

<!-- ACP2E-1312-->

* 現在，您可以為0位元組的日誌檔案生成系統報告。 以前，當一個或多個檔案為空時，Adobe Commerce為日誌檔案集合生成報告時出錯。

### 訂單

<!-- ACP2E-1107-->

* 「管理銷售訂單貸項通知單」頁現在顯示使用「儲存貸項」和多張發票建立的訂單的正確總計。 以前，總計計算不正確，用戶無法生成貸項通知單。
...
* 向已存檔訂單添加註釋不再將其返回到未存檔訂單網格，同時保持其存檔狀態。
...
* 啟用非同步索引時為免費產品自動開票訂單現在可以按預期方式工作。 未向 `sales_order_grid or sales_invoice_grid` 表格。 以前，Adobe Commerce曾提出一個例外，因為應用程式試圖在訂單放置期間對網格編製索引，而不是依賴 `cron`。 [GitHub-36334](https://github.com/magento/magento2/issues/36334)

### 頁面生成器

<!-- ACP2E-956-->

* 現在，您可以使用頁面生成器拖放功能將影像批量導入庫。

<!-- AC-2074-->

* 已解決頁面生成器滑塊CSS規則的問題。 [GitHub-34527](https://github.com/magento/magento2/issues/34527)

<!-- ACP2E-978-->

* `Dropdown`。 `Text area`, `Text field` 屬性現在可在頁面生成器產品屬性清單中找到。 以前，僅 `Dropdown` 和 `Text area` 屬性可用。
...
* Adobe Commerce不再在商家點擊時拋出以下錯誤 **[!UICONTROL Reports]** > **[!UICONTROL Braintree Settlement]**: `Error: Undefined constant Braintree\PaymentInstrumentType::ANDROID_PAY_CARD in /var/www/html/vendor/paypal/module-braintree-core/Ui/Component/Report/Listing/Column/PaymentType.php:49`。 [GitHub-35434](https://github.com/magento/magento2/issues/35434)
...
* 管理員用戶現在可以在啟用Braintree付款方法時建立訂單或重新排序。 此前，Adobe Commerce拋出了這個錯誤： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`。

<!-- BUNDLE-3139-->

* 啟用BraintreePayPal並成功完成使用Braintree付款方法的結帳 **[!UICONTROL Enable Card Payments]** 已禁用。 以前，進度欄從未解決，Adobe Commerce顯示了以下控制台錯誤： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`。 [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* 管理員用戶現在可以在啟用Braintree付款方法時為客戶建立訂單或重新排序。 此前，Adobe Commerce拋出了這個錯誤： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`

<!-- BUNDLE-3139-->

* 購物者現在可以在店面使用Braintree支付方式 **[!UICONTROL Enable Card Payments]** 已禁用。 以前，Adobe Commerce拋出此JavaScript錯誤： `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* 管理日誌報告現在顯示準確 **[!UICONTROL Value before change]** 更新類別規則時的值。
...
* RMA注釋欄位(**[!UICONTROL Orders]** > **[!UICONTROL Return]**)不再包含冗餘空格。 ( `<textarea>` 和 `<?php` 標籤現在在一行上。

<!-- ACP2E-1279-->

* 的 **[!UICONTROL Use Config Settings]** 複選框 **[!UICONTROL Enable RMA]** 欄位現在會在保存產品時按預期保存其值。
...
* 搜索引號內的字串時，現在可以按預期工作 **[!UICONTROL Minimum Terms to Match]** 配置設定已啟用。 此前，Adobe Commerce拋出了這個錯誤： `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`。
...
* Adobe Commerce現在尊敬管理員客戶 **[!UICONTROL Show Company]** 顯示發運或客戶地址時的配置設定。 禁用此設定後，發運公司將不再包括在發運、客戶地址或資料庫訂單記錄中。 以前，儘管設定了禁用，但仍顯示該資訊。 [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* Adobe Commerce在將折扣應用於運輸成本時不再拋出最小訂單金額錯誤。
...
* 重新計畫轉移更新的結束日期不會在更新完成後導致錯誤。 以前，管理員無法開啟產品頁面和產品網格，Adobe Commerce拋出了以下錯誤： `report.CRITICAL: Exception: Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "1" already exists`。

<!-- ACP2E-776-->

* 現在，轉移儀表板會顯示正確的記錄總數，儀表板分頁現在可以按預期工作。 前情提要。 儀表板最多顯示20條記錄。

<!-- ACP2E-1213-->

* 現在，在具有許多儲存的部署中，內容暫存預覽的載入速度更快。 預覽重新索引現在只重新索引選定的儲存。

<!-- ACP2E-919-->

* 現在，可以在計畫預覽模式下將產品添加到購物車。 使用以下參數在計畫預覽模式下更新Breadcrumbs連結更正了此問題： `version`。 `timestamp`, `signature`。
...
* Adobe Commerce現在可以在運行PHP 8.1的部署中在店麵類別頁上顯示帶稅的分組產品。此前，Adobe Commerce沒有顯示產品清單，並拋出了此例外： `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`。 [GitHub-35500](https://github.com/magento/magento2/issues/35500)
...
* 當沒有與客戶地址關聯的增值稅標識時，自動客戶組分配現在有效。 如果地址不包含增值稅ID，則該地址被視為屬於非增值稅註冊客戶，而Adobe Commerce將客戶組更改為非增值稅組。 以前， **[!UICONTROL Enable Automatic Assignment to Customer Group]** 已啟用，未自動將客戶帳戶分配給 `customerGroup` VAT ID從客戶帳戶中刪除時。
...
* 用戶角色網格的篩選器功能(管理 **[!UICONTROL System]** > **[!UICONTROL Permissions]** > **[!UICONTROL User roles]** > **[!UICONTROL Administrators]**)現在按預期工作。 陣列值現在單獨處理。 以前，當您嘗試在此網格中輸入查詢時，Adobe Commerce拋出了此控制台錯誤，並且該頁未載入： `Uncaught TypeError: Cannot read properties of null`。 [GitHub-35993](https://github.com/magento/magento2/issues/35993)
...
* 主題標題設定 `Welcome Text` 現在可以包含單引號字元。 以前，Adobe Commerce拋出此JavaScript錯誤： `Uncaught SyntaxError: Unable to process binding "ifnot: function(){return customer().fullname }"`。
...
* 在按規則匹配產品時，您不能再手動更改可視採購商類別清單中的產品位置。

<!-- ACP2E-985-->

* 僅限於一個商店的管理員用戶無法再在Visual Clotcler中將產品拖放到類別中。 相反，他們現在可以 **[!UICONTROL Position]** 欄中。 以前，這些用戶只能通過拖放操作來更改類別中的產品位置。

<!-- ACP2E-953-->

* 「類別編輯」頁的「類別中的產品」部分現在顯示正確的可配置產品數量。 現在，正確的表列用於檢索可配置產品數量。
...
* 商家現在可以使用REST API為總計為零的訂單建立貸項通知單。 以前，Adobe Commerce顯示了以下錯誤： `The order does not allow a credit memo to be created. The credit memo's total must be positive. We can't create a credit memo for the invoice/order`。
...
* 現在，Fulecher貨幣服務在通過ApiLayer生成的密鑰中可以按預期工作，並且貨幣匯率已正確導出。 以前，Adobe Commerce在商家試圖通過管理員導入匯率時出錯。
...
我們的技術堆棧構建在PHP和MySQL上。 有關詳細資訊，請參見 [系統要求](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/system-requirements.html)。

## 安裝和升級說明

您可以使用 [作曲家](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html)。
