...
Adobe Commerce 2.4.6引入了對PHP 8.2的支援。PHP 8.1仍完全受支援，但已不再支援PHP 7.4。 其中包括顯著的效能和可擴充性增強功能。 已最佳化大量購物車作業和類別樹狀轉譯回應的GraphQL作業。
...
Adobe Commerce 2.4.6-beta2已針對下列元件版本進行測試：
...
Adobe Commerce 2.4.6-beta2引入了對PHP 8.2的支援。PHP 8.1仍完全受支援。  <!-- AC-6598 -->

此版本已移除PHP 7.4支援。 不能在PHP 7.4上運行Adobe Commerce。

Adobe Commerce 2.4.6-beta2現在支援：

* 撰寫器2.2.x <!-- AC-2839 -->

* Redis 7.0.x。雖然與Redis 6.2的相容性仍維持不變，但建議您使用Redis 7.0.x部署此版本，因為Redis 6.2將於2024年4月終止服務。 <!-- AC-6606 -->
...
* 採購訂單GraphQL結構涵蓋B2B公司用戶採購訂單管理功能，如建立、編輯和刪除PO。
...
**改善已啟用類別權限查詢類別時的回應時間**. 查詢類別中已啟用類別權限且包含許多客戶群組或共用目錄的產品時，回應時間已有所改善。 <!-- B2B-2179-->

**GraphQL中類別樹呈現的最佳效能**. 此 `categoryList` 查詢已最佳化，可支援具有大量類別的客戶。 使用此查詢來搜尋超過1500個類別（含最多五個層級的巢狀子類別）的商戶，將會發現效能有所改善。
<!-- B2B-2484-->
...
我們正在修正Adobe Commerce 2.4.6核心程式碼中的數百個問題。 以下說明Adobe Commerce 2.4.6-beta2中包含的一些已修正問題。
...
* GraphQL查詢回應現在包含套件產品的子產品相關資訊，位於 `product` 區段 `options` 封鎖。
...
* 現在，在「目錄」頁面上進行篩選後，當 **無存貨** 啟用。 以前，無存貨產品的店面計數不正確。
...
* 客戶 `order` 查詢現在會如預期傳回所有指定網站、商店和商店檢視的訂單。 此修正包括建立新請求引數。
...
* 載入目錄Widget產品清單時，頁面載入效能已有所改善。 以前，非全域屬性是直接從資料庫載入。
