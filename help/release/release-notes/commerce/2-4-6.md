...
**客戶查詢現在支援排序功能**。 `orders`查詢的`customer`輸出物件現在包含排序欄位。 您現在可以依訂單編號或建立日期，以遞增或遞減順序來排序訂單。<!-- AC-6903-->
...
* 當料號折扣數量未設定或大於或等於訂購料號數量時，產品之`discount_percent`表格的`sales_order_item`欄現在會依預期填入折扣百分比。 先前，此產品套用購物車價格規則時未填入此欄。
...
* 管理優惠券代碼工作流程已新增驗證邏輯，以在產生優惠券時強制實施優惠券代碼設定。 先前，當&#x200B;**[!UICONTROL Code Length]**&#x200B;抵用券數量&#x200B;**值大於2500時，抵用券程式碼產生會忽略**&#x200B;設定。
...
* 當&#x200B;**[!UICONTROL Store]**&#x200B;歐盟國家&#x200B;**[!UICONTROL Configuration]**&#x200B;設定中沒有國家/地區(**>** > **[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL General]**)時，管理員使用者現在可以成功開啟「管理員&#x200B;**[!UICONTROL Country options]** > **[!UICONTROL European Union Countries]**」頁面。 之前，當管理員使用者嘗試開啟此頁面時，Adobe Commerce擲回此錯誤： `TypeError: explode(): Argument #2 ($string) must be of type string, null given in /var/www/php/folder/vendor/magento/module-config/ViewModel/CountryFilter.php:46`。 [GitHub-35901](https://github.com/magento/magento2/issues/35901)
...
* 現在當為具有子類別的類別儲存類別`rowId`時，會傳遞正確的`urlKey`引數。 先前儲存類別`categoryId`時傳遞了錯誤的`URLKey`。
...
* 已更新`recordLogin`方法，以在使用者模型上設定登入值，並保留`logdate`資料表的`lognum`和`admin_user`欄位中的正確值。 之前，系統不會記錄更新後Admin使用者的登入嘗試。
...
* 停用&#x200B;**[!UICONTROL Grid Filter Condition Type]** s **[!UICONTROL Use in Filter Option]設定時，客戶與客戶地址屬性的**&#x200B;欄位不再顯示在屬性編輯頁面上。
...
* **[!UICONTROL Show my quotes]**&#x200B;頁面上的&#x200B;**[!UICONTROL My Quotes]**&#x200B;篩選器現在可如預期般運作。 先前，篩選結果包含其他管理員使用者的引號。
...
* 在GraphQL程式碼中實作多個介面的`Types`現在可以用`&`符號分隔，以及根據GraphQL `,`規格的`@doc`。
...
* `batch_size`索引的索引子`catalogpermissions_category`現在已變更為如預期在`env.php`檔案中提供的值。 先前，索引子`batch_size`覆寫路徑中的錯字會防止將批次大小變更為`env.php`中提供的值。 [GitHub-652](https://github.com/magento/partners-magento2ee/issues/652)
...
* `escaper.js` `escapeHtml`函式現在根據`"$amp;"`規則將&amp;特殊字元轉換為`InnerHtml`。 此通用解決方案解決標題輸入方塊中的特殊字元和字元參考轉換問題，並儲存插入時的確切值。 先前，在內容階層中的節點名稱中，&amp;字元編碼不正確。
...
* 公司屬性值現在會如預期顯示在結帳工作流程的帳單和送貨地址區段中。 以前，由於建立訂單期間未正確設定商店，因此訂單詳細資料頁面上無法正確顯示公司屬性。 只有在多存放區部署中，**主要網站**&#x200B;的&#x200B;**[!UICONTROL optional]**&#x200B;顯示公司&#x200B;**屬性設定為**&#x200B;時，屬性值才會正確儲存。

<!-- ACP2E-1117-->

* 勾選`customer_address_id`核取方塊時，**[!UICONTROL My billing and shipping address are the same]**&#x200B;值現在已設定為新帳單地址。 設定送貨地址的客戶地址ID之前，已引入另一個條件來識別帳單地址是否與送貨地址相同。
...
* 已從管理員樣式區段中的所有&#x200B;**[!UICONTROL Tagline]**&#x200B;按鈕中移除&#x200B;**[!UICONTROL Layout]**&#x200B;和&#x200B;**[!UICONTROL PayPal]**&#x200B;設定欄位。
...
* `ANDROID_PAY_CARD`中的`Braintree/Ui/Component/Report/Listing/Column/PaymentType.php`常數已取代`GOOGLE_PAY_CARD`。 這可解決Braintree報表的問題。
...
* 現在儲存產品時，**[!UICONTROL Use Config Settings]**&#x200B;欄位的&#x200B;**[!UICONTROL Enable RMA]**&#x200B;核取方塊會如預期儲存其值。
...
* 啟用&#x200B;**[!UICONTROL Display Out Of Stock Products]** GitHub-36055[時，搜尋結果現在包含所有可用產品的無庫存選項輸出](https://github.com/magento/magento2/issues/36055)
...
* 在管理員`Search Weight`上的`Storefront Properties`屬性&#x200B;**[!UICONTROL Stores]**&#x200B;輸入欄位下方新增資訊性文字  > [!UICONTROL Attributes] > **[!UICONTROL Product]**。
...
* 當購物者嘗試將具有產品`addProductsToWishlist`自訂選項無效值的專案新增到願望清單時，`select`查詢現在會傳回資訊性錯誤訊息。 先前，下拉式清單自訂選項值的伺服器驗證遺失，導致資料庫中的值無效。 因此，端點傳回此內部伺服器錯誤： `GraphQL endpoint returns Internal server error with "Call`。
