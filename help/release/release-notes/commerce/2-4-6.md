...
Adobe Commerce 2.4.6測試版已針對下列元件版本進行測試：
...
此版本包含八項安全性修正和平台安全性改善。 這些安全性修正已支援移植至Adobe Commerce 2.4.5-p2和Adobe Commerce 2.4.4-p3。
...
Adobe Commerce 2.4.6-beta5現在支援：

* 撰寫器2.2.x <!-- AC-2839 -->

* Redis 7.0.x。雖然與Redis 6.2的相容性仍然保持不變，但我們建議使用Redis 7.0.x部署此版本，因為Redis 6.2預計於2024年終止服務。 <!-- AC-6606 -->

* 現在支援OpenSearch作為Adobe Commerce內部部署和雲端部署的預設搜尋引擎。 支援作為獨立的Magento Open Source搜尋引擎。 此版本支援OpenSearch v2.x，並已與OpenSearch 2.4進行測試。雖然與OpenSearch 1.x的相容性仍然存在，但建議將此版本與OpenSearch 2.x搭配使用。您可以在雲端和內部部署上，透過OpenSearch 2.x安裝及執行Adobe Commerce和Magento Open Source2.4.6 [搜尋引擎必要條件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html). <!-- AC-449 3555 -->
...
* **改善影響許多客戶區段之作業的效能**. 商家可以啟用新的配置設定，從而改善涉及100多個客戶段的操作的效能。 此 **即時檢查客戶是否符合區段** 您可以視需要停用設定，減少每個客戶區段對客戶的驗證，以縮短處理時間。 請參閱 [客戶區段驗證](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#customer-segments-validation). <!-- AC-6832 ACPT-835-->

* **用於限制產品網格中顯示的產品數量的新系統配置設定**. 商家可以使用 **網格中的產品數限制** 配置設定，以改善產品網格效能。 此限制預設為20000，只會影響UI元件使用的產品集合。 限制顯示的產品數量可以提高包含200,000多種產品的網格的操作效能。 請參閱 [網格中的產品數限制](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#limit-number-of-products-in-grid)<!-- ACPT-42 -->
...
* 採購訂單規則GraphQL結構涵蓋採購訂單批准規則功能，如添加批准規則。 根據公司角色和權限，公司使用者可以建立、編輯、刪除或檢視核准規則。

此版本包含多項錯誤修正。 本主題包含B2B測試版發行說明。 您將可在以下位置找到Adobe Commerce 2.4.6 GA的B2B發行說明： [B2B發行說明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
**改善已啟用類別權限查詢類別時的回應時間**. 查詢類別中已啟用類別權限且包含許多客戶群組或共用目錄的產品時，回應時間已有所改善。 GraphQL解析器操作也已最佳化。 <!-- B2B-2179 2404 2329-->

**類別樹呈現的最佳效能**. 此 `categoryList` 查詢已最佳化，可支援具有大量類別的客戶。 使用此查詢來搜尋超過1500個類別（含最多五個層級的巢狀子類別）的商戶，將會發現效能有所改善。 通過刪除不必要的方法調用、改進類別樹的快取、遞歸地載入類別資料，改進了通過重構代碼載入類別子項的方法。 <!-- B2B-2484 2469 2472-->
...
我們正在修正Adobe Commerce 2.4.6核心程式碼中的數百個問題。 以下說明Adobe Commerce 2.4.6-beta版中包含的一些已修正問題。
...
此版本包含多項錯誤修正。 此處提供B2B測試版發行說明。 您將可在以下位置找到Adobe Commerce 2.4.6 GA的B2B發行說明： [B2B發行說明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
* 「我的報價」頁上的「顯示我的報價」篩選器現在可以正常工作。 以前，篩選結果包含其他管理員使用者的引號。
...
* 未指派給類別但是屬於套件或分組產品之產品選項的產品，現在會在回應中傳回 **允許類別瀏覽** 已為傳送查詢的客戶啟用配置設定。
...
* 匯出的值 **購物車中允許的最大數量** 現在符合「管理配置」設定中設定的預設值。
...
* 在多次出貨結帳期間，當產品數量超過設定的上限時，Adobe Commerce不再在JavaScript主控台中擲回錯誤。 現在，Adobe Commerce會顯示資訊性錯誤訊息。 [GitHub-36184](https://github.com/magento/magento2/issues/36184)
...
* 包含可設定產品數量需求的購物車規則現在可如預期運作。 `attribute_scope` for `quote_item_qty` 條件已變更為 `parent`. [GitHub-35981](https://github.com/magento/magento2/issues/35981)
...
* 當 **顯示無庫存產品** 組態設定已啟用。 [GitHub-35899](https://github.com/magento/magento2/issues/35899)

<!-- AC-6384-->

* 若 **允許每頁所有產品** 設定。 [GitHub-35900](https://github.com/magento/magento2/issues/35900)

<!-- AC-6330-->

* 此 **顯示無庫存產品** 設定選項現在會在啟用時如預期般運作。 之前，在管理員上啟用此選項時，店面上的所有產品都會顯示為無存貨。 [GitHub-35898](https://github.com/magento/magento2/issues/35898)
...
* 作為可設定產品選項的無庫存簡單產品現在會在店面上顯示為停用，當 **顯示無庫存產品** 設為 **[!UICONTROL Yes]**. 以前，所有選項都顯示為可用，儘管其狀態和實際可用性都不同。
...
* 當頁面載入期間，產品詳細資料頁面上的主要產品影像不會再出現明顯的向下移動 `product_image_white_borders` 主題 `view.xml` 已停用。 之前，產品影像在頁面載入期間，會因為Fotorama JavaScript中的高度設定錯誤而明顯向下移動。
...
* 當可配置產品的可售數量為0時，在店面上不再顯示可用色票。 [GitHub-33697](https://github.com/magento/magento2/issues/33697)
...
* 「管理抵用券代碼」工作流程已新增驗證邏輯，以在產生抵用券時強制執行抵用券代碼設定。 之前，產生抵用券代碼會忽略 **程式碼長度** 設定時機 **抵用券數量** 值大於2500。
...
* 客戶電子郵件地址現在可以包含區分。 之前，前端驗證禁止建立電子郵件地址含有區分的使用者。 [GitHub-12075](https://github.com/magento/magento2/issues/12075)

<!-- ACP2E-1200-->

* 當 **電子郵件訂單確認** 設定已停用。 [GitHub-36256](https://github.com/magento/magento2/issues/36256)
...
* 使用 `mixed` 類型設為參數，或傳回類型現已支援。 先前。 Adobe Commerce使用此類型時擲回此PHP錯誤： `Type "mixed" cannot be nullable`. [GitHub-35579](https://github.com/magento/magento2/issues/35579)

<!-- AC-6223-->

* 此 `NewRelic` 包裝類現在捕獲所有可丟棄 `TypeError` 例外。 [GitHub-35854](https://github.com/magento/magento2/issues/35854)
...
* 中的內部伺服器錯誤 `/V1/products/{sku}/links` 已解決API端點。 [GitHub-35923](https://github.com/magento/magento2/issues/35923)

<!-- AC-6421-->

* 中的內部伺服器錯誤 `/V1/creditmemo/refund` 已解決API端點。 [GitHub-35925](https://github.com/magento/magento2/issues/35925)

<!-- AC-7497-->

* `get.php` 若 `resource_config.json` 已損壞： `Trying to access array offset on value of type null`. [GitHub-36640](https://github.com/magento/magento2/issues/36640)

<!-- AC-6440-->

* 從Magento2專案的主要 `composer.json` 檔案。 [GitHub-35994](https://github.com/magento/magento2/issues/35994)

<!-- AC-7300-->

* 移除 `version` 內鍵 `sampledata:deploy`. 之前，執行中 `bin/magento sampledata:deploy` 產生錯誤訊息，指出 `version` 缺少的密鑰 `composer.json`. 命令成功執行時，Adobe Commerce現在會確認成功，並指出後續步驟。 [GitHub-36554](https://github.com/magento/magento2/issues/36554)
...
* CMS頁面階層的階層連結在 **啟用階層功能** 已停用。 過去，會顯示階層連結，當購物者點按該連結時，便會顯示404頁面。 [GitHub-35513](https://github.com/magento/magento2/issues/35513)
...
* `base64` — 編碼URL不再包含RFC 3986所定義的保留字元。 [GitHub-36042](https://github.com/magento/magento2/issues/36042)
...
* New Relic報表模組現在符合CSP，且啟用追蹤網站資料時，為瀏覽器報表指定的New Relic網域會包含在CSP標題中。 [GitHub-34707](https://github.com/magento/magento2/issues/34707)
...
* Adobe Commerce不再在無效的表單輸入上複製錯誤訊息。 [GitHub-36201](https://github.com/magento/magento2/issues/36201)

<!-- AC-7508-->

* 管理員使用者登入現在可正確記錄在 `admin_user` 表格。 [GitHub-36642](https://github.com/magento/magento2/issues/36642)

<!-- AC-7507-->

* 已在 `Authorization/README.md`. [GitHub-36644](https://github.com/magento/magento2/issues/36644)

<!-- AC-7511-->

* 更新專案行為准則(`CODE_OF_CONDUCT.md`)。 [GitHub-36639](https://github.com/magento/magento2/issues/36639)

<!-- AC-7500-->

* 已重構Google Adwords程式碼範本，以改善可讀性，並以正確的函式取代過時的逸出函式。 [GitHub-36641](https://github.com/magento/magento2/issues/36641)
...
* 您現在可以從管理員將允許的錯誤計數設為0 **[!UICONTROL System]** > **[!UICONTROL Import]**. [GitHub-36252](https://github.com/magento/magento2/issues/36252)
...
* 此 `Fixex.io` 匯入服務現在如預期般運作。 現在已成功匯入匯率。 [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
* 被禁 `@author` 標籤已從整個程式碼基底的模組中移除。 [GitHub-36338](https://github.com/magento/magento2/issues/36338)

### Inventory management

此版本包含多項錯誤修正。 此處提供Inventory management的測試版發行說明。 您可以在以下位置找到Adobe Commerce 2.4.6 GA的Inventory management發行說明： [發行說明](https://experienceleague.adobe.com/docs/commerce-admin/inventory/release-notes.html).
...
* 現在，在單一來源模式中使用「庫存」時，搭配 **顯示無庫存產品** 設定啟用。 Adobe Commerce現在會在計算時檢查產品是否可售。
...
* 當實施多來源庫存的部署中，可設定產品的其中一個變數已手動傳回至庫存時，該店面上的可設定產品不再顯示為無存貨。
...
* 商家現在可以為從中刪除產品的訂單建立貸項通知單。 以前，在建立發票後從訂單中刪除產品時，商家無法建立貸項通知單。 應用程式顯示此錯誤： `Following products with requested skus were not found: s00001`.
...
* 現在預設會停用舊版媒體集。 以前， **啟用舊介質庫** 設為 **[!UICONTROL Yes]**.
...
* 狀態 **已關閉** 現在，在發出貸項通知單並退款客戶後，會如預期分配給虛擬訂單。 以前，虛擬產品顯示為 **完成** 中，但 **已關閉** 從管理員新增註解時顯示在訂單註解的頁尾中。 [GitHub-35283](https://github.com/magento/magento2/issues/35283)
...
* 購物者現在可以視需要，使用可選的自訂選項來重新訂購可設定的產品。 以前，Adobe Commerce會擲回此錯誤： `Some of the selected options are not currently available`. [GitHub-35409](https://github.com/magento/magento2/issues/35409)

<!-- ACP2E-1313-->

* 現在，當 **對價格應用折扣** 配置選項設定為 **包括稅**.
...
* 現在，當啟用BraintreePayPal時，使用Braintree付款方法的結帳已成功完成，並 **啟用卡付款** 已停用。 以前進度列從未解析，而Adobe Commerce顯示此主控台錯誤： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`. [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* Braintree付款資訊現在會按商店儲存，購物者現在只會看見儲存在其購物所在商店中的卡片資訊。 之前，購物者在結帳期間可以看到來自所有商店的儲存卡資訊。 [GitHub-34253](https://github.com/magento/magento2/issues/34253)
...
* 購物者現在可以在商店中使用Braintree付款方法， **啟用卡付款** 已被禁用。 之前，Adobe Commerce會擲出此JavaScript錯誤： `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* 引號內的字串搜尋現在可如預期般運作 **要符合的最低詞語** 組態設定已啟用。 以前，Adobe Commerce會擲回此錯誤： `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* 搜尋結果現在包含所有可用產品的無存貨選項輸出，當 **顯示無庫存產品** 已啟用 [GitHub-36055](https://github.com/magento/magento2/issues/36055)
...
* Adobe Commerce現在會遵照管理員客戶設定 **顯示公司** 設定。 禁用此設定後，發運公司將不再包含在發運、客戶地址或資料庫訂單記錄中。 以前，儘管設定了禁用，仍顯示資訊。 [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* 遺漏 `scope` 值已新增至 `isShippingCarrierAvailable`. [GitHub-36645](https://github.com/magento/magento2/issues/36645)

<!-- AC-7503-->

* 從購物車頁面提供的運送預估現已準確。 collectShippingRates現在只設定一次。 以前，發運估計是重複的。 [GitHub-36648](https://github.com/magento/magento2/issues/36648)
...
* 此 **顯示零稅小計** 在提供多種運送方法的部署中，現在會如預期套用設定設定。 以前，即使停用此設定，在選取運送方法之前，零稅欄位仍會顯示在購物車頁面上。 [GitHub-35561](https://github.com/magento/magento2/issues/35561)
...
* 當沒有與客戶地址關聯的增值稅ID時，自動客戶組分配現在可以工作。 如果地址不包含增值稅ID，則該地址將視為屬於非增值稅註冊客戶，而Adobe Commerce會將客戶組更改為非增值稅組。 以前，當 **啟用自動分配至客戶組** 啟用後，客戶帳戶不會自動指派給 `customerGroup` VAT ID從客戶帳戶中刪除時。
...
* 捷克共和國的ISO 3166地區已加入 `directory_country_region` 表格。 [GitHub-36346](https://github.com/magento/magento2/issues/36346)

### UI

<!-- ACP2E-1018-->

* 「管理產品」格線中的「特殊價格」、「成本」和MSRP欄位，現在會如預期般使用網站專用貨幣。 以前，當依據商店檢視篩選特定價格時，管理目錄產品格線中不會顯示正確的貨幣符號。
...
* 現在，使用自訂擴充功能的部署中，客戶格線會如預期般排序。 `Shared` 現在設為 `false` for `SortOrderBuilder`. 之前，此產生器會造成不想要的副作用，而按一下客戶格線的標題則沒有作用。 [GitHub-35939](https://github.com/magento/magento2/issues/35939)
...
* 現在，您可以為包含大於零且小於1的物料數量的訂單建立發運，當 **[!UICONTROL Quantity uses decimals]** 啟用。 以前，當輸入的產品數量小於1時，發票建立正確，但未建立發運。 [GitHub-35971](https://github.com/magento/magento2/issues/35971)

<!-- AC-7510-->

* 針對開發人員體驗問題和功能要求，GitHub問題範本已轉換為表單[GitHub-36651](https://github.com/magento/magento2/issues/36651)

<!-- AC-7509-->

* 冗餘 `getCacheTags` 方法已從 `Topmenu` 類別。 [GitHub-36643](https://github.com/magento/magento2/issues/36643)

<!-- AC-7504-->

* 已新增類別，以設定地址傳送強制回應的樣式。 若沒有此特定類別，前端開發人員將無法完成整個強制回應視窗的樣式，而不會影響結帳工作流程中的其他模組。 [GitHub-36647](https://github.com/magento/magento2/issues/36647)
...
* 將產品可見度變更為 **不會個別顯示** 現在，在全域「所有商店檢視」範圍中，只會移除未覆寫可見性屬性之商店檢視的URL重新寫入。 以此方式變更產品可見性也會為所有商店檢視產生URL重寫，包括已覆寫URL索引鍵屬性（但非可見性設定）的商店檢視。 以前，可見性設定變更時不會套用URL重新寫入規則。
