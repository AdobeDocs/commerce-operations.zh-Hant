...
description: 了解2.4.5 Adobe Commerce版本的新功能、增強功能、錯誤修正和已知問題。
...
Adobe Commerce改善了平台品質、付款方法、GraphQL快取效能和協助工具。 其中包括對整合的Google模組和升級相容性工具的更新，並推出Live Search B2B支援。

此版本包含超過400項品質修正和增強功能。
...
Adobe Commerce 2.4.5包括解決Adobe Commerce所提供的下列hotfix所解決的所有問題：

* `Braintree_disabled_partial_capture_for_applepay_googlepay.zip`. 此修補程式可解決使用Apple支付或Google支付Braintree作為付款方法時，使用者無法建立部分發票的問題。 請參閱 [Adobe Commerce 2.4.4:無法建立部分發票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知識庫文章。

* `AC-2655_2.4.4.patch.zip`. 此修補程式可解決取消選取「允許國家/地區」選項時，購物者無法將產品新增至購物車的問題。 請參閱 [若未在「允許國家/地區」中選取任何項目，則使用者無法將產品新增至購物車](https://support.magento.com/hc/en-us/articles/5377681213325-Users-not-able-to-add-product-to-cart-if-nothing-selected-in-Allow-Countries) 知識庫文章。

## Adobe Commerce 2.4.5重點

查看此版本中的下列重點。

### 安全性增強功能

此版本包含20項安全性修正和平台安全性改善。 此安全性修正已背向Adobe Commerce 2.4.3-p3和Adobe Commerce 2.3.7-p4。
...
* **Adobe Commerce內部部署**:Adobe Commerce 2.4.5已通過測試，並確認與Elasticsearch7.17(~7.17.0，含約束)相容。 在本地托管Adobe Commerce的商戶可以使用Elasticsearch或OpenSearch 1.2。 <!-- AC-2425-->

* **Adobe Commerce雲端托管部署**:Elasticsearch7.11或更新版本不支援Adobe Commerce 2.4.5雲端托管部署。 OpenSearch是Adobe Commerce 2.4.5雲端部署的預設搜尋引擎。

Adobe Commerce 2.4.5現已支援
...
主要索引效能已透過將 `catalog_product_index_price_tmp` 表格。 此增強功能可減少因共用目錄而產生的eSKU倍增，進而減少價格指數中建立的記錄數。 索引時間已大大縮短。 <!-- MCP-653 MC-43342-->
...
商家現在可以讓客戶在Adobe Commerce結帳期間，以電子方式簽署自訂協定，例如保修檔案、購買協定及條款與條件。

### Adobe Commerce與Adobe IMS整合

Adobe Commerce商戶若擁有Adobe ID且想要簡化Adobe Commerce登入與Adobe商業產品，可將商務驗證與Adobe IMS驗證工作流程整合。 為您的商務商店啟用此整合後，每位管理員使用者必須使用其Adobe憑證（而非其商務憑證）才能登入。  請參閱 [AdobeIdentity Management服務(IMS)整合概述](https://experienceleague.adobe.com/docs/commerce-admin/start/admin/ims/adobe-ims-integration-overview.html).

### 品牌和樣式變更

管理員已更新，以符合Adobe的品牌策略。 變更會影響頁首、頁尾、資料格線顏色更新和導覽元素。

### B2B

我們優化了實施共用目錄功能所需的標準化資料庫資料。 這種eSKU倍增的減少會導致效能提升，因為必須儲存的資料庫列數較少。 先前，Adobe Commerce會針對每個共用目錄複製目錄中的每個SKU。 Adobe Commerce現在會為直接指派給共用目錄的使用者建立唯一的eSKU。

啟用新 **啟用共用目錄直接產品價格分配** 配置選項還提高了產品價格索引器效能。 <!-- MCP-946-->
...
* 開發人員和管理員在部署時或在生產環境中變更屬性時，可更快重建統一店面GraphQL結構。 由於任何原因必須重建GraphQL結構時，購物者的頁面載入速度也會大幅加快。

* 新增在GraphQL API中使用JSON網頁代號(JWT)來使用授權代號的到期日/時間的功能。

* 此 `bin/magento config:set graphql/session/disable 1` 命令可讓商戶完全停用所有GraphQL作業的工作階段cookie建立作業。 依預設，Adobe Commerce會建立這些Cookie並依賴它們進行授權，進而影響效能。 日後，我們建議使用代號作為GraphQL請求的唯一授權形式。 我們不建議單獨使用工作階段Cookie或搭配授權Token使用。 請參閱 [GraphQL授權](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html). <!-- B2B-2204 -->

* 工作階段Cookie現在只會在需要時，使用類別Proxy在GraphQL作業中啟動。 <!-- B2B-2217 -->

* 工作階段使用情形已從 `http` GraphQL中的標題處理器，例如商店、客戶或貨幣。 <!-- B2B-2224 -->
...
此版本推出B2B客戶群組和自訂定價的支援。 「即時搜尋」現在會依循將產品指派給客戶群組，以及為特定客戶群組/共用目錄所設定的定價。
...
* 「管理員」現在提供結帳、迷你購物車、購物車和產品頁面的PayPal、Credit和Pay Later按鈕預覽。 預覽可顯示這些按鈕在店面上啟用和呈現時的外觀。

#### Braintree

* Braintree已停止KOUNT欺詐保護整合。 已從Adobe Commerce程式碼基底中移除。

* 此 **始終請求3DS** 「管理員」已新增選項。

### PWA Studio

PWA Studiov.12.5.x與Adobe Commerce 2.4.5相容。
...
如需增強功能和錯誤修正的詳細資訊，請參閱 [PWA Studio發行](https://github.com/magento/pwa-studio/releases). 請參閱 [版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 以取得PWA Studio版本及其相容Adobe Commerce核心版本的清單。

### 升級相容性工具

增強功能包括：

* 方法簽名驗證現在標識方法或建構子簽名中不相容的更改。 <!-- AC-2632-->

* 資料庫架構驗證現在可識別架構變更和不相容。 <!-- AC-58-->

* ID設定驗證現在會驗證中已移除或已棄用非API核心類別的參考 `di.xml` 和核心類和介面的首選項。  <!-- AC-438-->

* UCT現在可識別取代程式碼的情形，並提供解決每個問題的特定建議。  <!-- AC-3147-->
...
* 您現在可以更名資料修補程式，並將舊類名稱添加為 `patch_list` 資料庫表。 Adobe Commerce現在會在套用修補程式之前，檢查資料庫中是否已存在資料修補程式別名。 以前，Adobe Commerce會在這些條件下擲回錯誤。

<!-- AC-3036-->

* Adobe Commerce不再在您嘗試從管理員將管理URL變更為自訂URL時擲回例外狀況。 以前，在變更管理URL後，您無法登入。 [GitHub-35416](https://github.com/magento/magento2/issues/35416)
...
* Adobe Commerce Analytics的路徑不再以硬式編碼撰寫。 以前，此硬式編碼路徑會在單一伺服器上安裝多個Adobe Commerce執行個體時導致衝突。 [GitHub-29373](https://github.com/magento/magento2/issues/29373)
...
* Adobe Commerce現在會在套用「產品子選取」規則時，正確計算套件組合產品的購物車總計。

### 快取

<!-- ACP2E-684-->

* 當Adobe Commerce快取被清除，且 `use_stale_cache` 啟用。

<!-- AC-2410-->

* New Relic部署標籤現在會在快取刷新時如預期運作。 [GitHub-32649](https://github.com/magento/magento2/issues/32649)
...
* 現在會處理SKU所新增之受限制產品的權限例外。 店面現在會向購物者提供適當的訊息，而錯誤表格中的數量欄位會停用。 之前，Adobe Commerce會擲出例外狀況如下： `There has been an error processing your request`.
...
* 當訪客結帳停用時，具有全域帳戶共用的購物者不再需要以多網站部署重新登入次要網站。 購物者導覽至子網域時，現在會載入客戶資料。 購物者不再被要求重新登入，而會顯示先前的購物車內容。
...
* 此 `cart` 當產品無存貨時，查詢不再傳回null回應。 新 `errors` 將包含錯誤訊息的元素引入回應。 以前，當您使用無存貨產品執行查詢時，Adobe Commerce會顯示 `null` 值 `items` 區段。 請參閱 [購物車查詢](https://devdocs.magento.com/guides/v2.4/graphql/queries/cart.html).
...
* 您現在可以成功切換多頁產品清單的清單和格線檢視。 以前，在切換至格線檢視之前，您導覽至多頁產品清單檢視的最後一頁時，Adobe Commerce會顯示此錯誤： `Unfortunately there are no products in this category on our website currently`.
...
* 當類別規則將產品指派給類別（後續在多網站部署中排序）時，Adobe Commerce不再擲回錯誤。
...
* Adobe Commerce現在會在您嘗試新增產品URL索引鍵並加上尾端連字型大小時提供驗證錯誤訊息。 也提供資訊工具提示文字。

<!-- ACP2E-267-->

* 新 `ConfigurableWishlistItem.configured_variant` 欄位已取代 `ConfigurableWishlistItem.child_sku` 欄位。 當客戶願望清單包含未配置的可配置產品時，後一欄位觸發了內部錯誤。
...
* 當 `Synchronize widget products with backend storage` 設定，Adobe Commerce會將最近檢視的產品資料新增至 `catalog_product_frontend_action` 資料庫表。 新增記錄時會包含客戶或訪客ID。 此 `recently_viewed_product` 如果客戶ID和訪客ID為null，回應中的區段現在為空。 因此，當 `customer/section/load` 已傳送Ajax請求，Adobe Commerce可以根據客戶或訪客ID正確篩選最近檢視的產品。 之前，回應會包含 `catalog_product_frontend_action` 資料庫表，因為沒有檢查空的客戶或訪客ID。

<!-- ACP2E-28-->

* 類別規則 `Quantity` 實作測試時，可設定產品的屬性現在可正常運作。 之前，當產品的值不同 `row_id` 和 `entity_id` 在 `catalog_product_entity` 由於測試更新，表格為可視採購程式 **依規則比對產品** 功能無法正確篩選產品。
...
* 產品清單現在可在「管理員」中正確轉譯。 之前，產品清單不會呈現，而Adobe Commerce會顯示此錯誤：  `Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "<ID>" already exists`. [GitHub-33145](https://github.com/magento/magento2/issues/33145)
...
* 您現在可以變更類別中顯示的每頁產品限制，當 **記住類別分頁**  啟用。 之前，Cookie `form_key` 和UI `form_key` 不同，而Adobe Commerce顯示此錯誤：  `Invalid Form Key. Please refresh the page`.
...
* Adobe Commerce現在會在您嘗試從產品頁面建立屬性而未完成「管理員」欄位時，如預期顯示錯誤訊息。 [GitHub-33099](https://github.com/magento/magento2/issues/33099)
...
* 具有受限權限的管理員現在可以在計畫更新後查看CMS層次結構中的CMS頁。

### 可配置產品

<!-- ACP2E-687-->

* Adobe Commerce現在會在產品詳細資料頁面上變更其數量後，使用選取的選項，顯示可設定產品的正確產品價格。 以前，在數量變更後，價格會重設為初始值。
...
* 現在，產品編輯期間會正確儲存多選項屬性。 以前，Adobe Commerce會在儲存產品時，儲存非選取屬性以及選取屬性的預設選項。

<!-- ACP2E-55-->

* Adobe Commerce現在會在建立全域期間如預期顯示可設定屬性 `select` 屬性。 以前，會隱藏符合條件的全域屬性。

<!-- AC-2521-->

* 此 `addConfigurableProductsToCart` 變異現在可用來使用自訂選項新增可設定的產品。 以前，Adobe Commerce會擲回此錯誤： `Magento 2.3.4 graphql Notice: Undefined index: option_value in /var/www/html/mg234/vendor/magento/module-configurable-product-graph-ql/Model/Resolver/ConfigurableCartItemOptions.php on line 62`. [GitHub-28860](https://github.com/magento/magento2/issues/28860)

<!-- ACP2E-43-->

* 當可設定產品的其中一個子產品的可用性因排程更新而變更時，Adobe Commerce現在會正確顯示可設定產品的庫存狀態、可用設定和顯示價格。 以前，當可配置產品有兩個子產品，並且計畫更新禁用了一個子產品的可用性時，父產品的庫存狀態將錯誤顯示為 **無庫存** 更新生效時，更新完成後，其配置的顯示價格和可用性計算錯誤。
...
* Adobe Commerce現在會根據店面上的網站設定，正確顯示預先定義的EAV系統屬性。 之前，針對某個網站啟用而針對另一個網站停用的網站層級客戶屬性，會針對這兩個網站顯示為已啟用。

### 客戶區段

<!-- ACP2E-473-->

* 客戶區段特定相關產品規則現在可如預期般運作。 此問題已借由呼叫方法來為特定網站新增訪客區段關係而修正。 過去，此規則的區段只會根據註冊客戶和網站擷取。
...
* Adobe Commerce現在會在購物者點按後，購物者輸入無效的電子郵件格式時，在運送頁面上顯示錯誤訊息 **下單**. 之前，錯誤訊息會顯示在「付款」頁面上。 [GitHub-33590](https://github.com/magento/magento2/issues/33590)
...
* Adobe Commerce在為產品指派新來源並變更其數量後，不再擲回SQL錯誤。 [GitHub-35262](https://github.com/magento/magento2/issues/35262)
...
* 此 `updateCartCurrency` 函式現在會設定字串，而非購物車物件內的物件。 以前，Adobe Commerce未透過 `getQuote` 因為 `updateCartCurrency` 函式會設定物件，而非購物車物件內的字串。 [GitHub-34199](https://github.com/magento/magento2/issues/34199)
...
* Adobe Commerce不再顯示 `preg_replace()` 管理錯誤。 第三個引數(`$subject`)現在屬於類型 `array|string` 而非 `bool`.
...
* Adobe Commerce在您為任何類別方法建立外掛程式時，不會再擲回下列錯誤 `vendor/magento/module-backend/Model/Menu.php`: `Error: Call to undefined method ReflectionUnionType::getName()`. [GitHub-35292](https://github.com/magento/magento2/issues/35292)

<!-- AC-1518-->

* Adobe Commerce現在會在您導覽至 `/checkout/sidebar/updateItemQty/?item_qty=error` 在店面。 之前，系統擲回此錯誤：  `Warning: A non-numeric value encountered in /vendor/magento/module-checkout/Controller/Sidebar/UpdateItemQty.php on line 69`. [GitHub-34380](https://github.com/magento/magento2/issues/34380)

<!-- ACP2E-847-->

* Adobe Commerce不再觸發 `trim(): Passing null to parameter #1 ($string) of type string is deprecated` 在未設定SSL的情況下設定AMPQ連線時發生錯誤。
...
* 現在，「類別」頁面和「產品編輯」頁面都會封鎖受限制管理員的類別建立作業。 之前，類別建立在類別頁面上遭到封鎖，但仍可透過產品編輯頁面進行。

<!-- ACP2E-448-->

* Adobe Commerce不再在您透過Page Builder新增套件產品時擲回例外狀況。
...
* 此 `products` 查詢現在會傳回準確反映「顯示相關產品」設定的產品資訊。 此 `related_products`, `upsell_products`，和 `crosssell_products` 現在會分別根據「顯示相關產品」、「顯示向上銷售產品」和「顯示交叉銷售產品」設定來解析GraphQL ProductInterface中的欄位。
...
* 根據類別的目標規則只會顯示規則中宣告之類別的產品。 以前， 「相關產品」規則會從指派給產品且未在規則中定義的類別中顯示產品。
...
* Adobe Commerce現在會在您設定無效的Cookie網域時顯示錯誤訊息(**商店**  > **配置**  >  **Web**  >  **預設Cookie設定Cookie網域**)。 之前，網站崩潰了。 [GitHub-35048](https://github.com/magento/magento2/issues/35048)

<!-- AC-2765-->

* 當角色限定於單一網站的管理員使用PageBuilder將產品新增至「內容元素」時，Adobe Commerce不再擲回錯誤。 以前，Adobe Commerce擲回SQL錯誤。
...
* 升級後，Adobe Commerce在管理員中建立目錄規則期間不再擲回下列錯誤： `A technical problem with the server created an error. Try again to continue what you were doing. If the problem persists, try again later`.

<!-- AC-2445-->

* Adobe Commerce在您啟用  **請查看這裡，將RSS饋送連結到您的願望清單** 核取方塊，再點按 **共用願望清單**. [GitHub-34998](https://github.com/magento/magento2/issues/34998)
...
* `products` 查詢現在可以正確傳回包含禮品卡產品的產品資料，且 `gift-card` URL索引鍵。
...
* 此 `categories` 在提取類別清單時，查詢不再擲回異常，其中一個類別包含在檔案系統上找不到的影像。 以前，Adobe Commerce會擲出此例外： `Category image not found`. [GitHub-34266](https://github.com/magento/magento2/issues/34266)
...
* Adobe Commerce工作階段在提出GraphQL要求後不再結束。 之前， `ClearCustomerSessionAfterRequest` 外掛程式將購物者登出。 [GitHub-34550](https://github.com/magento/magento2/issues/34550)

<!-- AC-818-->

* 此 `products` 不再傳回 `price_range` 受已停用變體影響的可設定產品的值。 [GitHub-33629](https://github.com/magento/magento2/issues/33629)
...
* `setShippingAddressesOnCart` 請求現在可成功驗證地區ID。 之前，Adobe Commerce會在您使用地區ID而非地區代碼時擲回錯誤。
...
* GraphQL結構現在在自訂 `type` 產品屬性已定義。 之前，架構無效，因為 `type` 自訂 `type` 屬性。 [GitHub-34929](https://github.com/magento/magento2/issues/34929)
...
* Adobe Commerce現在會在匯出報表時篩選欄之前檢查自訂檢視。 以前，匯出的報表沒有考慮自訂檢視，且匯出的欄不正確。
...
* 新增在匯入期間驗證類別名稱的功能。 以前，Adobe Commerce未驗證類別名稱，當類別名稱超過255個字元時，就會導致錯誤。
...
* 客戶、客戶地址和訂單動作現在可以正確記錄在「管理員動作」報表中。 以前，若 `postDispatch` 未在配置設定中指定處理程式。
...
* 您現在可以透過從店面隱藏的自訂客戶屬性，成功從新訂單建立新客戶。 以前，Adobe Commerce未儲存自訂客戶屬性的正確值。
...
* 當您變更「管理員帳戶」編輯頁面上指派給客戶的主要電子郵件地址，Adobe Commerce不再變更指派給訂單的自訂電子郵件地址。 之前，當您編輯客戶的主要電子郵件地址時，會將新電子郵件地址指派給為該客戶建立的每個訂單。 [GitHub-34397](https://github.com/magento/magento2/issues/34397)

<!-- AC-2664-->

* Adobe Commerce現在會在執行PHP 7.4的部署中，從貸項通知單網格頁面上顯示請求儲存的記錄。之前，在您建立貸項通知單並嘗試查看後，Adobe Commerce擲回了以下錯誤： `The store that was requested wasn't found. Verify the store and try again`.

<!-- AC-2442-->

* Adobe Commerce現在會在貸項通知單格線頁面上，針對從商店檢視建立且名稱前面加上數字的訂單顯示貸項通知單。 以前，Adobe Commerce會顯示錯誤： `The store that was requested wasn't found. Verify the store and try again. Exception in /var/www/html/vendor/magento/module-store/Model/StoreRepository.php:75`. [GitHub-35122](https://github.com/magento/magento2/issues/35122)

<!-- ACP2E-31-->

* Adobe Commerce現在會在「管理員」和「店面發票」頁面總計中顯示免運費(0)。 以前，當訂單的發運為零時，Adobe Commerce不會在發票頁面發運合計上顯示發運金額合計。

<!-- ACP2E-740-->

* Adobe Commerce現在會在使用客戶商店評分時，於結帳期間顯示正確的訂單狀態。
...
* 管理員現在可以使用PayPal PayflowPro付款方法向管理員下單。 以前，Adobe Commerce顯示此錯誤： `No such entity with cartId = 0`.
...
* 商戶現在可以針對透過Braintree以Apple支付支付的訂單提交部分退款。 這是Adobe Commerce 2.4.4中的已知問題。

#### PayPal

<!-- ACP2E-296-->

* Adobe Commerce現在會以PayPal付費的訪客訂單顯示正確的客戶名稱。 以前，客戶名稱會顯示為訪客。
...
* 動態塊載入效能得到了改善。 過去，不會對每個網站快取訪客區段，這會導致對相同資料的資料庫進行重複查詢。
...
* 此 **使用時間** 當訪客客戶以折扣下單並隨後註冊帳戶時，折扣的值不再增加兩次。
...
* 此 `GET /V1/returnsAttributeMetadata` 現在，當端點正常運作時 `rma_item` 實體預設屬性集ID與預設安裝ID不同。 以前，如果這些ID不同，此端點會傳回空白結果。

<!-- ACP2E-247-->

* 此 **使用預設值** 用於在產品編輯頁上啟用RMA的複選框現在在預設商店中正常工作。 之前，儲存產品後會立即清除核取方塊。

### 獎勵

<!-- ACP2E-433-->

* 現在當商店信用功能停用時，可退回獎勵點數。

<!-- ACP2E-757-->

* 套用獎勵點時，結帳效能已有所改善。 當 `entity` 為null，則不會執行資料庫查詢，因為查詢 `magento_reward_history` 在這些情況下，表不是強制性的。

<!-- ACP2E-674-->

* 來賓訂單的貸項通知單不再檢查 `customer_id`，會略過查詢 `magento_reward_history` 獎勵點的資料庫表。 此變更已改善效能。

### 角色

* 此 **商店** >  **設定** > **服務** 「資源存取」設為時，頁面現在會如預期顯示Adobe Commerce Web API資訊  **自訂** 在「角色資源」頁簽上。 [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* Elasticsearch查詢現在可如預期運作，當 `int` 已設定為可搜尋的後端 `type` 屬性。 之前，Adobe Commerce `Elasticsearch\Common\Exceptions\BadRequest400Exception` 例外。
...
* Adobe Commerce現在會在部署的店面中顯示準確的搜尋結果建議計數，其中「搜尋建議」和 **顯示每個建議的結果計數** 設定。 過去，關鍵字旁顯示的計數為零。
...
* 修正 `catalogsearch/advanced/result` 和  `catalogsearch/advanced/index` 頁面。  以前，Adobe Commerce在任何進階搜尋字串中傳入陣列時，都會顯示此錯誤：  `Warning: trim() expects parameter 1 to be string, array given | magento/module-catalog-search`. [GitHub-33586](https://github.com/magento/magento2/issues/33586)

### 裝運

<!-- AC-2052-->

* 如果未選取允許的運送方法，Adobe Commerce在載入UPS運費時不再擲回錯誤。 以前，當購物者在這些條件下的結帳工作流程中輸入運送地址時，不會顯示其他運送方法，而Adobe Commerce在店面上顯示錯誤。 [GitHub-34411](https://github.com/magento/magento2/issues/34411)
...
* 針對快取頁面維持不變的銷售規則套用測試更新後，Adobe Commerce不再清除整頁快取。

<!-- ACP2E-304-->

* 內容測試控制面板不再顯示非作用中的永久更新。

<!-- ACP2E-257-->

* 從測試控制面板變更測試更新的結束日期現在已成功將這些變更套用至測試更新及其實體。 （已引入佇列以處理測試更新變更。）

<!-- ACP2E-238-->

* 商戶現在可以移除已排程更新的結束日期，或刪除並重新建立更新。 以前，移除結束日期時，系統會維持該時間的排程實體。 Adobe Commerce現在會移除已移除回滾的過時更新。

<!-- ACP2E-149-->

* CMS頁面的「已排程變更」區段現在會如預期顯示，顯示「CMS」頁面的「已排程變更」區段。

<!-- ACP2E-81-->

* 此 `custom_design_to` 變更排程更新時，屬性值現在會如預期更新。 以前，沒有結束時間的兩個不同類別設計更新會視為一次。 當第一個執行中的某個排程更新被刪除時， `custom_design_from` 時間值變大 `custom_design_to` 下次計畫更新中的時間值。

<!-- ACP2E-58-->

* Forms現在會在編輯排程的更新後，如預期般填入資料。

<!-- ACP2E-18-->

* 此 `vendor/magento/module-catalog-staging/Setup/Patch/Data/MigrateCatalogProducts.php` 當資料庫包含具有自訂選項的產品和 `special_from_date` 屬性。

<!-- ACP2E-741-->

* 現在，當執行中的更新新增變更時，會在第二次排程更新期間儲存變更。 以前不會儲存變更，而商戶需要重新輸入並儲存變更。

<!-- ACP2E-21-->

* 「大量動作」記錄現在會顯示有關失敗的資訊 `to be moved` 排程更新。 以前，有關這些故障的資訊，請參閱 `cron.log` 檔案。

<!-- ACP2E-420-->

* 在排程更新中編輯產品價格時，無論地區為何，商家現在都只能使用逗號分隔的價格。 以前，排程更新中包含逗號的價格會依地區設定以1000除以。

<!-- AC-3214-->

* 運行PHP 8.1的部署中，DHL國內航運的問題已通過僅要求國際運輸的應課稅標籤而得到解決。
...
* Adobe Commerce現在會在簡單產品的數量變更後，依預期更新產品頁面上簡單產品的排除稅層級價格。
...
* 您現在可以在 `Magento_LoginAsCustomerAdminUi` 啟用， **要登錄的儲存視圖** 設為手動選取。 以前，Adobe Commerce會擲回此錯誤： `(Magento\Framework\Exception\LocalizedException): Unable to get Customer ID`. [GitHub-33096](https://github.com/magento/magento2/issues/33096)
...
* Adobe Commerce現在會在管理員嘗試儲存街道過長的地址時，顯示資訊性錯誤訊息 **商店** > **屬性**  > **客戶地址**.  管理員現在可以刪除額外的地址資訊並成功保存該地址。 以前，Adobe Commerce會提交額外的行，但未儲存資料。
...
* Favicon圖示上傳表單現在支援 `.ico` 檔案類型。 以前，當您嘗試在「管理員」中上傳具有此副檔名類型的Favicon檔案時，Adobe Commerce會顯示此錯誤： `Warning: imagecreatefromstring(): one parameter to a memory allocation multiplication is negative or zero, failing operation gracefully in /var/www/html/vendor/magento/module-media-storage/Model/File/Validator/Image.php on line 64`. [GitHub-34858](https://github.com/magento/magento2/issues/34858)
...
* 您現在可以設定Vimeo影片，在 `banner` 元素 `restrict mode`. 之前，Adobe Commerce擲回JavaScript錯誤。
...
* 從願望清單頁面更新項目數量現在會如預期般更新產品詳細資料頁面上的數量。 Adobe Commerce現在會從產品URL擷取更新的值，並填入 `qty` 願望清單本身的產品詳細資訊頁面的欄位。

## 已知問題

**問題**:啟用Braintree時，管理員使用者無法從管理員建立客戶訂單或重新訂購。 當管理員使用者點按 **順序** 或 **重新排序**,Adobe Commerce不會提交訂單，且  `system.log` 顯示此錯誤： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.  **因應措施**:  `BUNDLE-3137-composer.patch` 現已可用。 請參閱 [啟用Braintree付款時，管理員無法建立訂單/重新排序](https://support.magento.com/hc/en-us/articles/8322266352525) 知識庫文章，以討論此問題並訪問修補程式。 雲端商戶應下載Commerce適用的雲端修補程式v1.0.18。Adobe Commerce 2.4.5-p1也將提供修正。 <!-- AC-6222-->

**問題**: [!DNL RabbitMQ] 在Adobe Commerce 2.4.5的全新安裝（未升級）後，基礎架構元素（交換、佇列）可能未初始化。此設定不正確 [!DNL RabbitMQ] 執行非同步操作或產生效能設定檔時，可能會導致嚴重錯誤。 雲端部署不受影響。 **因應措施**:在內部執行全新Adobe Commerce 2.4.5安裝的商戶應執行 `bin/magento setup:upgrade --keep-generated` 使用 `bin/magento setup:config:set` 設定 [!DNL RabbitMQ] 或透過傳遞安裝Adobe Commerce [!DNL RabbitMQ] 設定選項 `setup:install` 命令。 請參閱 [[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md).
