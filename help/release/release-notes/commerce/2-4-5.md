...
* 現在，在將名稱包含「。」的新影像新增至產品，然後儲存產品並清除影像快取後，就能成功產生新產品快取。 [GitHub-32699](https://github.com/magento/magento2/issues/32699)
...
* 系統核發的電子郵件現在已成功傳送給其電子郵件地址中帶有「。 — 」的收件者。
...
* 修正`catalogsearch/advanced/result`和`catalogsearch/advanced/index`頁面上的PHP錯誤。  之前，在任何進階搜尋字串中傳遞陣列時，Adobe Commerce會顯示此錯誤： `Warning: trim() expects parameter 1 to be string, array given | magento/module-catalog-search`。 [GitHub-33586](https://github.com/magento/magento2/issues/33586)
...
* 計算表格費率運費金額現在不包含虛擬產品價格。 先前未正確計算這些產品的運費。[GitHub-35185](https://github.com/magento/magento2/issues/35185)
...
* 在名稱驗證程式中新增抑音符號(&amp;grave；)字元，以便可以為包含此抑音符號的名字或姓氏建立客戶帳戶。
...
**問題**：從2.4.5之前的產品版本升級至Adobe Commerce 2.4.6或更新版本後，暫存銷售規則標籤可能會不相符。 **因應措施**：在升級期間，在更新中繼資料步驟之前，套用ACSD-50625_2.4.5-P1.patch。 請參閱從版本&lt; 2.4.5](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27186)知識庫文章升級時的[salesRules標籤問題。

解決此問題的一般步驟：

1. 下載必要的修補程式。 請參閱從版本&lt; 2.4.5](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-27186)知識庫文章升級時的[salesRules標籤問題。
...
3. 在升級期間，在管理套件階段之後和更新中繼資料階段之前，套用此修補程式。請參閱[如何套用Adobe提供的撰寫器修補程式](https://experienceleague.adobe.com/en/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/how-to-apply-a-composer-patch-provided-by-magento) 。
4. 完成升級程式。
