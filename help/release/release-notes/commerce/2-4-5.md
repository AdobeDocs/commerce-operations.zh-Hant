...
* 西班牙和意大利的商家現在可以向購物者優惠方案PayPal Pay Later。  <!-- AC-2521-->
...
* 方法簽名驗證現在可識別方法或構造函数簽名中不兼容的更改。 <!-- AC-2632-->

* 資料庫綱要驗證現在可識別綱要更改和不兼容。 <!-- AC-58-->

* DI 配置驗證現在驗證對已刪除或棄用的非 API 核心類`di.xml`的引用以及核心類和介面的首選項。  <!-- AC-438-->

* UCT 現在可識別代碼棄用，並提供解決每個問題的具體建議。  <!-- AC-3147-->
...
* 在呼叫前 `collectQuoteTotals` 新增外掛程式，確保商店評分不會多次套用。
...
* 通過 `POST /V1/salesRules/` 端點創建的購物車價格規則現在在狀態從已禁用更改為已啟用后保留現有的抵用券代碼值。 [GitHub-35298](https://github.com/magento/magento2/issues/35298)
