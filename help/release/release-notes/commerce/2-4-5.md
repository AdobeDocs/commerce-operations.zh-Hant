...
Adobe Commerce改善了平台品質、付款方式、GraphQL快取效能和協助工具。 其中包括對整合的Google模組和升級相容性工具的更新，並推出Live Search B2B支援。
...
* 此 `bin/magento config:set graphql/session/disable 1` 命令允許商戶完全禁用所有GraphQL操作的會話cookie的建立。 依預設，Adobe Commerce會建立這些Cookie並依賴它們進行授權，進而影響效能。 日後，我們建議使用代號作為GraphQL請求的唯一授權形式。 我們不建議單獨使用工作階段Cookie或搭配授權Token使用。 請參閱 [GraphQL授權](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html). <!-- B2B-2204 -->
...
* Adobe Commerce工作階段在提出GraphQL請求後不再結束。 之前， `ClearCustomerSessionAfterRequest` 外掛程式將購物者登出。 [GitHub-34550](https://github.com/magento/magento2/issues/34550)
