...
* `AC-2655_2.4.4.patch.zip`。 此修補程式解決取消選取「允許國家/地區」選項時，購物者無法新增產品至購物車的問題。 若未在「允許國家/地區」[知識庫文章中選取任何專案，請參閱](https://support.magento.com/hc/en-us/articles/5377681213325-Users-not-able-to-add-product-to-cart-if-nothing-selected-in-Allow-Countries)使用者無法將產品加入購物車。
...
* **Adobe Commerce內部部署**： Adobe Commerce 2.4.5已經過測試，並已確認相容於Elasticsearch 7.17 （約7.17.0且有限制）。 內部部署Adobe Commerce的商家可以使用Elasticsearch或OpenSearch 1.2。<!-- AC-2425-->
...
如需有關增強功能和錯誤修正的資訊，請參閱[PWA Studio發行版本](https://github.com/magento/pwa-studio/releases)。 如需PWA Studio版本及其相容的Adobe Commerce核心版本的清單，請參閱[版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/)。
...
* 付款資訊信用卡選項文字元素或文字影像現在已符合WCAG 2.0的要求，標準文字的最低色彩對比率為4.5:1，即18pt (24px)或14pt (19px) （若是粗體字）。 先前，這些量度不符合預期的對比率。

<!-- AC-2483-->

* **通訊錄** > **通訊** > **帳戶資訊**&#x200B;自訂焦點指標現在提供的對比率至少為背景顏色3:1。
...
* 當產品無存貨時，`cart`查詢不再傳回null回應。 已將包含錯誤訊息的新`errors`元素匯入回應。 先前，當您使用無庫存產品執行查詢時，Adobe Commerce在回應中的`null`區段下顯示`items`值。 請參閱[購物車查詢](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/queries/cart/)。
...
* 在完整重新索引後，觸發程式現在會如預期般還原到`catalogrule_product_price`資料表。 先前，在`catalogrule_product_price`或`catalogrule_rule`完整重新索引後，從`catalogrule_product`資料表中移除觸發程式。
...
* 現在當執行暫存時，可設定產品之具有`Quantity`屬性的類別規則可正確運作。 先前，當產品在`row_id`資料表中的`entity_id`與`catalog_product_entity`因暫存更新而具有不同值時，Visual Merchandiser **依規則比對產品**&#x200B;功能無法正確篩選產品。
...
* `TypeError`中的`magento2/app/code/Magento/Security/Model/AdminSessionsManager.php`已從`int`更正為`string`。  [GitHub-34415](https://github.com/magento/magento2/issues/34415)
...
* 停用`products`在搜尋結果中使用&#x200B;**分層導覽設定時，**&#x200B;查詢不再傳回作為彙總的屬性。 [GitHub-33318](https://github.com/magento/magento2/issues/33318)
...
* `new_from_data`中的`new_to_datefields`和`ProductInterface`不再被取代。 [GitHub-34783](https://github.com/magento/magento2/issues/34783)
...
* 啟用`products`顯示缺貨產品&#x200B;**組態設定時，**&#x200B;查詢回應中可設定的產品價格範圍現在可正確計算。 過去，停用的選項會在計算最高和最低價格時列入考量。
...
* 停用`products`在搜尋結果階層導覽中使用&#x200B;**設定時，**&#x200B;查詢不再傳回作為彙總的屬性。 [GitHub-33318](https://github.com/magento/magento2/issues/33318)
...
* 當回應中指定了`customerOrders`物件，但禮品訊息不存在時，`gift_message`查詢現在會如預期般回應。 以前，查詢傳回此訊息： `Can't load gift message for order is returned`。 [GitHub-28957](https://github.com/magento/magento2/issues/28957)
...
* 已在`SelectMenu`中更正jQuery UI滑桿和`vendor/magento/module-theme/view/base/requirejs-config.js`對應。
...
* `increment_id`資料表中的`sales_order`資料行已增加。 先前，假設`sales_order.increment_id`長度為50個字元的第三方模組只儲存`increment_id`的前32個字元。 [GitHub-34521](https://github.com/magento/magento2/issues/34521)
...
* 當`GET /V1/returnsAttributeMetadata`實體預設屬性集ID與預設安裝ID不同時，`rma_item`端點現在可以正確運作。 以前，如果這些ID不同，此端點會傳回空白結果。
...
* 當資料庫包含具有自訂選項的產品且已設定`vendor/magento/module-catalog-staging/Setup/Patch/Data/MigrateCatalogProducts.php`屬性時，`special_from_date`資料修補程式不再失敗。
...
* 傳回`cart`時，`subtotal_with_discount_excluding_tax`查詢不再包含稅捐。 [GitHub-33905](https://github.com/magento/magento2/issues/33905)
...
* 當CSP設為`banner`時，您現在可以將Vimeo視訊設定為在`restrict mode`元素的背景中執行。 之前，Adobe Commerce擲回JavaScript錯誤。
...
**問題**：從2.4.5之前的產品版本升級至Adobe Commerce 2.4.6或更新版本後，暫存銷售規則標籤可能會不相符。 **因應措施**：在升級期間，在更新中繼資料步驟之前，套用ACSD-50625_2.4.5-P1.patch。 請參閱從版本&lt; 2.4.5[知識庫文章升級時的](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/salesrule-label-issues-when-upgrade-from-versions-under-2.4.5.html)salesRules標籤問題。

解決此問題的一般步驟：

1. 下載必要的修補程式。 請參閱從版本&lt; 2.4.5[知識庫文章升級時的](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/salesrule-label-issues-when-upgrade-from-versions-under-2.4.5.html)salesRules標籤問題。
...
**問題**： Adobe Commerce 2.4.5的全新安裝（非升級）後，[!DNL RabbitMQ]基礎結構元素（交換、佇列）可能未初始化。這個不正確的[!DNL RabbitMQ]組態可能會在執行非同步作業或產生效能設定檔時導致嚴重錯誤。 雲端部署不受影響。 **因應措施**：執行Adobe Commerce 2.4.5內部部署全新安裝的商戶應於使用`bin/magento setup:upgrade --keep-generated`設定`bin/magento setup:config:set`或將[!DNL RabbitMQ]設定選項傳遞至[!DNL RabbitMQ]命令以安裝Adobe Commerce後執行`setup:install`。 請參閱[[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md)。
