...
**問題**:在Adobe Commerce 2.4.5的全新安裝（非升級）後，RabbitMQ基礎架構元素（交換、佇列）可能未初始化。如果執行非同步操作或產生效能設定檔，RabbitMQ的這個錯誤配置可能會導致嚴重錯誤。 雲端部署不受影響。 **因應措施**:在內部執行全新Adobe Commerce 2.4.5安裝的商戶應執行 `bin/magento setup:upgrade --keep-generated` 使用 `bin/magento setup:config:set` 將RabbitMQ配置選項傳遞至 `setup:install` 命令。 請參閱 [RabbitMQ](../../../installation/prerequisites/rabbitmq.md).
