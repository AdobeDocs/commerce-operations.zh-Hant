...
# Adobe Commerce2.4.5發行說明

Adobe Commerce對平台質量、支付方式、GraphQL快取效能和可訪問性進行了改進。 它包括對整合的Google模組和升級相容性工具的更新，並引入Live Search B2B支援。
...
Adobe Commerce2.4.5包括解決下列熱點解決的所有問題，這些熱點解決為Adobe Commerce提供：

* `Braintree_disabled_partial_capture_for_applepay_googlepay.zip`. 此補丁解決了在將「Apple支付」或「通過Braintree支付」作為付款方法時用戶無法建立部分發票的問題。 查看 [Adobe Commerce2.4.4:無法建立部分發票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知識庫文章。

* `AC-2655_2.4.4.patch.zip`. 此補丁程式可解決在取消選擇「允許國家/地區」選項時購物者無法將產品添加到購物車中的問題。 查看 [如果未在「允許國家/地區」中選擇任何選項，則用戶無法將產品添加到購物車](https://support.magento.com/hc/en-us/articles/5377681213325-Users-not-able-to-add-product-to-cart-if-nothing-selected-in-Allow-Countries) 知識庫文章。
...
此版本包括20個安全修復和平台安全改進。 此安全修復程式已支援到Adobe Commerce2.4.3-p3和Adobe Commerce2.3.7-p4。

迄今未發生與這些問題相關的已確認攻擊。 但是，某些漏洞可能被利用來訪問客戶資訊或接管管理員會話。 大多數這些問題都要求攻擊者首先獲得對管理員的訪問權限。 因此，我們提醒您採取一切必要步驟保護您的管理員，包括但不限於以下工作：

* IP允許
* [雙因素身份驗證](https://devdocs.magento.com/guides/v2.4/security/two-factor-authentication.html)
* VPN的使用
* 使用唯一位置，而不是 `/admin`
* 良好的密碼衛生
...
* **Adobe Commerce內部部署**:Adobe Commerce2.4.5已經測試並證實與Elasticsearch7.17(~7.17.0帶約束)相相容。 在本地托管Adobe Commerce的商戶可以使用Elasticsearch或OpenSearch 1.2。 <!-- AC-2425-->

* **Adobe Commerce雲托管部署**:Elasticsearch7.11或更高版本在Adobe Commerce2.4.5雲托管部署中不受支援。 OpenSearch是Adobe Commerce2.4.5雲部署的預設搜索引擎。

Adobe Commerce2.4.5現在支援
...
主索引效能已通過將主索引數從3減少到1而得到改善 `catalog_product_index_price_tmp` 的子菜單。 通過減少共用目錄導致的eSKU倍增，此增強減少了在價格索引中建立的記錄數。 索引時間大大縮短。 <!-- MCP-653 MC-43342-->
...
商戶現在可以允許客戶在Adobe Commerce結帳期間以電子方式簽署自定義協定，如保修文檔、採購協定以及條款和條件。

### Adobe Commerce與Adobe IMS整合

Adobe Commerce商戶擁有Adobe ID，希望優化登錄Adobe Commerce和Adobe商務產品，他們可以將商務認證與Adobe IMS認證工作流整合。 在為您的Commerce儲存啟用此整合後，每個管理員用戶必須使用其Adobe憑據（而不是其Commerce憑據）才能登錄。  請參閱 [AdobeIdentity Management服務(IMS)整合概述](https://experienceleague.adobe.com/docs/commerce-admin/start/admin/ims/adobe-ims-integration-overview.html)。

### 品牌和樣式更改

管理員已更新，以與Adobe的品牌戰略保持一致。 更改會影響頁眉、頁腳、資料網格顏色更新和導航元素。

### B2B

我們優化了實現共用目錄功能所需的標準化資料庫資料。 eSKU倍增的減少會提高效能，因為必須儲存的資料庫行數較少。 以前，Adobe Commerce會複製每個共用目錄的目錄中的每個SKU。 Adobe Commerce現在為直接分配給共用目錄的用戶建立唯一的eSKU。

啟用新 **已啟用共用目錄直接產品價格分配** 配置選項還提高了產品價格索引器效能。 <!-- MCP-946-->
...
* 的 `bin/magento config:set graphql/session/disable 1` 命令允許商家完全禁用所有GraphQL操作的會話cookie的建立。 預設情況下，Adobe Commerce會建立這些Cookie並依賴它們進行授權，這會影響效能。 今後，我們建議使用令牌作為GraphQL請求的唯一授權形式。 我們不建議單獨使用會話cookie或與授權令牌一起使用。 請參閱 [GraphQL授權](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html)。 <!-- B2B-2204 -->
...
本版本引入了對B2B客戶組和自定義定價的支援。 Live Search現在尊重客戶組的產品分配以及為特定客戶組/共用目錄設定的定價。

### 頁面生成器

Page Builder v.1.7.2與Adobe Commerce2.4.5相容。
...
* PayPal、Credit和Pay Later按鈕的預覽現在可在管理員中查看結帳、迷你購物車、購物車和產品頁。 預覽可顯示這些按鈕在店面上啟用和呈現時的外觀。

#### Braintree

* Braintree已經終止了KOUNT欺詐保護一體化。 它已從Adobe Commerce庫底酶中去除。

* 的 **始終請求3DS** 選項已添加到管理中。

### PWA Studio

PWA Studiov.12.5.x與Adobe Commerce2.4.5相容。
...
有關增強和錯誤修復的資訊，請參見 [PWA Studio版本](https://github.com/magento/pwa-studio/releases)。 請參閱 [版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 清單中的「PWA Studio」版本及其相容的Adobe Commerce核心版本。
...
* 資料庫架構驗證現在可標識架構更改和不相容。 <!-- AC-58-->

* DI配置驗證現在驗證對中已刪除或不建議使用的非API核心類的引用 `di.xml` 以及核心類和介面的首選項。  <!-- AC-438-->

* UCT現在發現代碼棄用，並為解決每個問題提供特定建議。  <!-- AC-3147-->

## 已修復問題

我們解決了Adobe Commerce核心代碼中的數2.4.5問題。

### 安裝、升級、部署

<!-- ACP2E-49-->

* 現在，您可以更名資料修補程式，並將舊類名作為別名添加到 `patch_list` 資料庫表。 Adobe Commerce現在在應用補丁程式之前檢查資料庫中是否已存在資料補丁程式別名。 此前，Adobe Commerce在這些情況下拋出了一個錯誤。

<!-- AC-3036-->

* Adobe Commerce在嘗試將管理URL從管理更改為自定義URL時不再引發異常。 以前，更改管理URL後，無法登錄。 [GitHub-35416](https://github.com/magento/magento2/issues/35416)
...
* 通往Adobe Commerce分析的路徑不再是硬編碼。 以前，在一台伺服器上安裝多個Adobe Commerce實例時，此硬編碼路徑會導致衝突。 [GitHub-29373](https://github.com/magento/magento2/issues/29373)
...
* Adobe Commerce現在可以在應用「產品子選擇」規則時正確計算捆綁產品的購物車總數。

### 快取

<!-- ACP2E-684-->

* 當刷新並禁用Adobe Commerce快取時，全頁快取不再顯示為禁用 `use_stale_cache` 的子菜單。
...
* 現在，SKU添加的受限產品的權限例外已處理。 現在，購物者會在店面上收到相應消息，並且錯誤表中的quantity欄位被禁用。 此前，Adobe Commerce曾提出過這樣的例外： `There has been an error processing your request`。
...
* 的 `cart` 當產品庫存不足時，查詢不再返回空響應。 新 `errors` 已將包含錯誤消息的元素引入響應。 以前，當您使用庫存不足的產品運行查詢時，Adobe Commerce顯示 `null` 值 `items` 的子菜單。 請參閱 [購物車查詢](https://devdocs.magento.com/guides/v2.4/graphql/queries/cart.html)。
...
* 現在，您可以成功在多頁產品清單的清單視圖和網格視圖之間切換。 以前，在切換到網格視圖之前導航到多頁產品清單視圖的最後一頁時，Adobe Commerce顯示了以下錯誤： `Unfortunately there are no products in this category on our website currently`。
...
* 當類別規則將產品分配給隨後在多站點部署中排序的類別時，Adobe Commerce不再拋出錯誤。
...
* Adobe Commerce現在在嘗試添加帶尾隨連字元的產品URL鍵時提供驗證錯誤消息。 還提供資訊工具提示文本。

<!-- ACP2E-267-->

* 新 `ConfigurableWishlistItem.configured_variant` 欄位已替換 `ConfigurableWishlistItem.child_sku` 的子菜單。 當客戶願望清單包含未配置的可配置產品時，後一個欄位觸發了內部錯誤。
...
* 當 `Synchronize widget products with backend storage` 設定已啟用，Adobe Commerce最近將產品資料添加到 `catalog_product_frontend_action` 資料庫表。 添加記錄時，它包括客戶或訪問者ID。 的 `recently_viewed_product` 如果客戶ID和訪問者ID為空，則響應中的節現在為空。 因此，當 `customer/section/load` Ajax請求已發送，Adobe Commerce可以根據客戶或訪問者ID正確過濾最近查看的產品。 以前，響應包含了 `catalog_product_frontend_action` 資料庫表，因為沒有檢查空的客戶或訪問者ID。

<!-- ACP2E-28-->

* 類別規則 `Quantity` 實施轉移時，可配置產品的屬性現在可以正常工作。 以前，當產品的值不同時 `row_id` 和 `entity_id` 的 `catalog_product_entity` 表由於轉移更新，Visual Stlocker **按規則匹配產品** 功能未正確篩選產品。
...
* 現在，產品清單在管理員中正確呈現。 以前，產品清單未呈現，Adobe Commerce顯示了以下錯誤：  `Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "<ID>" already exists`。 [GitHub-33145](https://github.com/magento/magento2/issues/33145)
...
* 現在，您可以更改類別中顯示的每頁產品限制 **記住類別分頁**  的子菜單。 以前， `form_key` 和UI `form_key` 不同，Adobe Commerce顯示了這個錯誤：  `Invalid Form Key. Please refresh the page`。
...
* Adobe Commerce現在會在您嘗試從產品頁面建立屬性而未完成「管理」欄位時顯示一條預期的錯誤消息。 [GitHub-33099](https://github.com/magento/magento2/issues/33099)
...
* Adobe Commerce現在在產品詳細資訊頁面上更改其數量後，顯示具有選定選項的可配置產品的正確產品價格。 以前，在數量更改後，價格被重置為初始值。
...
* 現在，在產品編輯期間可正確保存多選屬性。 以前，Adobe Commerce在保存產品時保存了非選定屬性和選定屬性的預設選項。

<!-- ACP2E-55-->

* Adobe Commerce現在按建立全球 `select` 屬性。 以前，合格的全局屬性被隱藏。

<!-- AC-2521-->

* 的 `addConfigurableProductsToCart` 現在，突變可用於添加具有自定義選項的可配置產品。 此前，Adobe Commerce拋出了這個錯誤： `Magento 2.3.4 graphql Notice: Undefined index: option_value in /var/www/html/mg234/vendor/magento/module-configurable-product-graph-ql/Model/Resolver/ConfigurableCartItemOptions.php on line 62`。 [GitHub-28860](https://github.com/magento/magento2/issues/28860)

<!-- ACP2E-43-->

* Adobe Commerce公司現在可以正確顯示可配置產品的庫存狀態、可用配置和顯示的價格，因為某個可配置產品的子產品的可用性已通過計畫更新更改。 以前，當可配置產品有兩個子產品，並且使用計畫更新禁用了一個子產品的可用性時，父產品的庫存狀態錯誤地顯示為 **缺貨** 當更新處於活動狀態時，更新完成後，顯示的價格及其配置的可用性計算錯誤。
...
* Adobe Commerce現在根據店面網站設定正確顯示預定義的EAV系統屬性。 以前，為一個網站啟用的網站級客戶屬性和為另一個網站禁用的網站級客戶屬性都顯示為為兩個網站啟用。

### 客戶細分

<!-- ACP2E-473-->

* 特定於客戶的相關產品規則現在可以按預期工作。 已通過調用為特定網站添加訪問者段關係的方法來解決該問題。 以前，此規則的段僅基於註冊客戶和網站獲取。
...
* Adobe Commerce現在在購物者點擊購物者後輸入無效的電子郵件格式時，在「送貨」頁面上顯示一條錯誤消息 **下單**。 以前，錯誤消息顯示在「付款」頁上。 [GitHub-33590]https://github.com/magento/magento2/issues/33590
...
* Adobe Commerce在為產品分配新源並更改其數量後不再拋出SQL錯誤。 [GitHub-35262](https://github.com/magento/magento2/issues/35262)
...
* 的 `updateCartCurrency` 函式現在設定字串，而不是cart對象內的對象。 以前，Adobe Commerce未使用 `getQuote` 因為 `updateCartCurrency` 函式在cart對象中設定對象，而不是字串。 [GitHub-34199](https://github.com/magento/magento2/issues/34199)
...
* Adobe Commerce不再展示 `preg_replace()` 管理員上出錯。 第三個參數(`$subject`)現在的類型 `array|string` 而不是 `bool`。
...
* Adobe Commerce在為任何類方法建立插件時不再拋出以下錯誤 `vendor/magento/module-backend/Model/Menu.php`: `Error: Call to undefined method ReflectionUnionType::getName()`。 [GitHub-35292](https://github.com/magento/magento2/issues/35292)

<!-- AC-1518-->

* Adobe Commerce現在在導航到時返回404錯誤，而不是500錯誤 `/checkout/sidebar/updateItemQty/?item_qty=error` 在店面。 以前引發了此錯誤：  `Warning: A non-numeric value encountered in /vendor/magento/module-checkout/Controller/Sidebar/UpdateItemQty.php on line 69`。 [GitHub-34380](https://github.com/magento/magento2/issues/34380)

<!-- ACP2E-847-->

* Adobe Commerce不再引發 `trim(): Passing null to parameter #1 ($string) of type string is deprecated` 配置AMPQ連接時未配置SSL時出錯。
...
* 現在，在「類別」頁和「產品編輯」頁上，已阻止為受限管理員建立類別。 以前，在類別頁面上阻止了類別建立，但仍然可以通過「產品編輯」頁面進行。

<!-- ACP2E-448-->

* Adobe Commerce在通過頁面生成器添加捆綁產品時不再引發異常。

<!-- ACP2E-427-->

* 現在可以從類別樹中選擇類別作為客戶段的條件。
...
* 基於類別的目標規則僅顯示規則中聲明的類別中的產品。 以前，「相關產品」規則顯示的產品來自分配給產品且未在規則中定義的類別。
...
* Adobe Commerce現在在設定無效的cookie域時顯示錯誤消息(**儲存**  > **配置**  >  **Web**  >  **預設Cookie設定Cookie域**)。 此前，網站崩潰了。 [GitHub-35048](https://github.com/magento/magento2/issues/35048)

<!-- AC-2765-->

* 當具有作用域到單個網站的角色的管理員使用PageBuilder將產品添加到內容元素時，Adobe Commerce不再拋出錯誤。 以前，Adobe Commerce拋出SQL錯誤。
...
* Adobe Commerce在升級後在管理員中建立目錄規則時不再拋出以下錯誤： `A technical problem with the server created an error. Try again to continue what you were doing. If the problem persists, try again later`。

<!-- AC-2445-->

* Adobe Commerce在您激活時不再拋出錯誤  **按一下這裡將RSS源連結到您的願望清單** 複選框，然後按一下 **共用願望清單**。 [GitHub-34998](https://github.com/magento/magento2/issues/34998)
...
* `products` 查詢現在可正確返回包含禮品卡產品的產品資料 `gift-card` URL鍵。
...
* 的 `categories` 在獲取類別清單時，查詢不再引發異常，其中一個類別包含在檔案系統上找不到的映像。 此前，Adobe Commerce提出了這一例外： `Category image not found`。 [GitHub-34266](https://github.com/magento/magento2/issues/34266)
...
* Adobe Commerce會議在提出GraphQL請求後不再結束。 以前， `ClearCustomerSessionAfterRequest` 插件已註銷購物者。 [GitHub-34550](https://github.com/magento/magento2/issues/34550)

<!-- AC-818-->

* 的 `products` 查詢不再返回 `price_range` 受禁用變型影響的可配置產品的值。 [GitHub-33629](https://github.com/magento/magento2/issues/33629)
...
* `setShippingAddressesOnCart` 請求現在已成功驗證區域ID。 以前，Adobe Commerce在您使用區域ID而不是區域代碼時拋出錯誤。
...
* Adobe Commerce現在在導出報告時在篩選列之前檢查自定義視圖。 以前，導出的報告未考慮自定義視圖，導出的列不正確。

<!-- ACP2E-502-->

* Adobe Commerce現在成功導入具有長檔案名的影像。 以前，Adobe Commerce未導入映像並拋出此錯誤： `Imported resource (image) could not be downloaded from external resource due to timeout or access permissions in row(s):`。
...
* 在導入期間為類別名稱添加了驗證。 以前，Adobe Commerce沒有驗證類別名稱，當類別名稱超過255個字元時，這會導致錯誤。
...
* 客戶、客戶地址和訂單操作現在正確記錄在「管理」操作報告中。 以前，Adobe Commerce沒有記錄 `postDispatch` 未在配置設定中指定處理程式。
...
* 現在，您可以從新訂單中成功建立新客戶，該訂單具有隱藏在店面中的自定義客戶屬性。 以前，Adobe Commerce未保存自定義客戶屬性的正確值。
...
* Adobe Commerce不再更改在您更改管理員帳戶編輯頁面上分配給客戶的主電子郵件地址時分配給訂單的自定義電子郵件地址。 以前，當您編輯客戶的主電子郵件地址時，新電子郵件地址會分配給為該客戶建立的每個訂單。 [GitHub-34397](https://github.com/magento/magento2/issues/34397)

<!-- AC-2664-->

* Adobe Commerce現在在運行PHP 7.4的部署中在貸項通知單網格頁面上顯示請求儲存的記錄。以前，在建立貸項通知單並嘗試查看後，Adobe Commerce拋出了以下錯誤： `The store that was requested wasn't found. Verify the store and try again`。

<!-- AC-2442-->

* Adobe Commerce現在在貸項通知單網格頁上顯示從名稱前面帶有數字的商店視圖建立的訂單的貸項通知單。 以前，Adobe Commerce顯示了錯誤： `The store that was requested wasn't found. Verify the store and try again. Exception in /var/www/html/vendor/magento/module-store/Model/StoreRepository.php:75`。 [GitHub-35122](https://github.com/magento/magento2/issues/35122)

<!-- ACP2E-31-->

* Adobe Commerce現在在「管理」和店面發票頁合計中顯示免費發運成本(0)。 以前，當訂單的發運為零時，Adobe Commerce未在發票頁發運合計上顯示發運金額合計。

<!-- ACP2E-740-->

* Adobe Commerce現在在使用客戶商店信用時在結帳期間顯示正確的訂單狀態。
...
* 管理員現在可以使用PayPal PayflowPro付款方法在管理員上下單。 以前，Adobe Commerce顯示了以下錯誤： `No such entity with cartId = 0`。
...
* 商戶現在可以提交通過Braintree向Apple支付支付的訂單的部分退款。 這是Adobe Commerce已知的問2.4.4。

#### 貝帕爾

<!-- ACP2E-296-->

* Adobe Commerce現在在PayPal支付的來賓訂單中顯示正確的客戶名稱。 以前，客戶名稱顯示為Guest。
...
* 動態塊載入效能得到了改善。 以前，每個網站都沒有快取訪問者段，這導致對同一資料的資料庫進行冗餘查詢。
...
* 的 **使用時間** 當客戶用折扣下單並隨後註冊帳戶時，折扣的值不再增加兩次。
...
* 的 `GET /V1/returnsAttributeMetadata` 現在，當終結點 `rma_item` 實體預設屬性集ID與預設安裝ID不同。 以前，如果這些ID不同，此終結點返回空結果。

<!-- ACP2E-247-->

* 的 **使用預設值** 複選框（用於在產品編輯頁上啟用RMA）現在可以按預期對預設商店使用。 以前，在保存產品後立即清除該複選框。

### 獎勵

<!-- ACP2E-433-->

* 現在，當儲存信用功能被禁用時，獎勵積分可以退還。

<!-- ACP2E-757-->

* 當應用獎勵積分時，結賬表現得到改善。 當 `entity` 為空，則不會執行資料庫查詢，因為查詢 `magento_reward_history` 在這種情況下，表不是強制的。

<!-- ACP2E-674-->

* 來賓訂單的貸項通知單不再檢查 `customer_id`，跳過查詢 `magento_reward_history` 獎勵積分資料庫表。 這種變化提高了效能。

### 角色

* 的 **儲存** >  **配置** > **服務** 頁面現在顯示「資源訪問」設定為時預期的Adobe CommerceWeb API資訊  **自定義** 在「角色資源」頁籤上。 [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* Elasticsearch查詢現在按預期工作 `int` 配置為可搜索的後端 `type` 屬性。 以前，Adobe Commerce `Elasticsearch\Common\Exceptions\BadRequest400Exception` 例外。
...
* Adobe Commerce現在在部署中顯示準確的搜索結果建議計數，其中搜索建議和 **顯示每個建議的結果計數** 設定。 以前，關鍵字旁邊顯示的計數為零。
...
* 已修復上的PHP錯誤 `catalogsearch/advanced/result` 和  `catalogsearch/advanced/index` 頁。  以前，Adobe Commerce在任何高級搜索字串中傳遞陣列時都顯示此錯誤：  `Warning: trim() expects parameter 1 to be string, array given | magento/module-catalog-search`。 [GitHub-33586](https://github.com/magento/magento2/issues/33586)

### 裝運

<!-- AC-2052-->

* 如果未選擇允許的發運方法，Adobe Commerce在載入UPS發運費率時不再引發錯誤。 以前，當購物者在這些條件下在結帳工作流中輸入送貨地址時，不會顯示其他送貨方法，而Adobe Commerce在店面顯示錯誤。 [GitHub-34411](https://github.com/magento/magento2/issues/34411)
...
* Adobe Commerce在應用銷售規則的暫存更新後不再清除全頁快取，在該規則中快取頁保持不變。
...
* 現在，從轉移控制面板更改轉移更新的結束日期已成功將這些更改應用於轉移更新及其實體。 （已引入隊列以處理暫存更新更改。）

<!-- ACP2E-238-->

* 商家現在可以刪除計畫更新的結束日期，或刪除並重新建立更新。 以前，刪除結束日期時，實體仍計畫該時間。 Adobe Commerce現在刪除已刪除回滾的過時更新。

<!-- ACP2E-149-->

* CMS頁的「計畫更改」部分現在可以像預期的那樣顯示CMS頁的活動計畫更新。

<!-- ACP2E-81-->

* 的 `custom_design_to` 當計畫更新更改時，屬性值現在按預期更新。 以前，兩個沒有結束時間的單獨類別設計更新被視為一個。 當第一個計畫更新運行時刪除其中一個計畫更新時， `custom_design_from` 時間值變大 `custom_design_to` 下次計畫更新的時間值。

<!-- ACP2E-58-->

* 現在，在編輯計畫更新後，Forms將按照預期填充資料。

<!-- ACP2E-18-->

* 的 `vendor/magento/module-catalog-staging/Setup/Patch/Data/MigrateCatalogProducts.php` 當資料庫包含具有自定義選項的產品和 `special_from_date` 屬性。

<!-- ACP2E-741-->

* 現在，在運行的更新添加更改時，將在第二次計畫更新期間保存更改。 以前，更改未保存，商家需要重新輸入和保存更改。

<!-- ACP2E-21-->

* 批量操作日誌現在顯示有關失敗的資訊 `to be moved` 計畫更新。 以前，有關這些故障的資訊可在 `cron.log` 檔案。

<!-- ACP2E-420-->

* 現在，商家在計畫更新中編輯產品價格時只能使用逗號分隔的價格，而不考慮區域設定。 以前，在計畫更新中包含逗號的價格會根據區域設定以1000除以。

<!-- AC-3214-->

* 運行PHP 8.1的部署中DHL國內運輸的問題已通過僅要求國際運輸的應課稅品標籤而得到解決。
...
* Adobe Commerce現在會在簡單產品的數量發生更改後，按預期更新產品頁面上簡單產品的排除稅層價格。
...
* 現在，您可以在 `Magento_LoginAsCustomerAdminUi` 已啟用並 **儲存視圖以登錄到** 設定為手動選擇。 此前，Adobe Commerce拋出了這個錯誤： `(Magento\Framework\Exception\LocalizedException): Unable to get Customer ID`。 [GitHub-33096](https://github.com/magento/magento2/issues/33096)
...
* Adobe Commerce現在在管理員嘗試在管理員中保存街道行過多的地址時顯示資訊錯誤消息 **儲存** > **屬性**  > **客戶地址**。  管理員現在可以刪除額外的地址資訊並成功保存地址。 以前，Adobe Commerce提交了額外的行，但沒有保存資料。
...
* 「查看」表徵圖上載表單現在支援 `.ico` 檔案類型。 以前，當您嘗試在管理員中上載具有此擴展類型的favicon檔案時，Adobe Commerce顯示了以下錯誤： `Warning: imagecreatefromstring(): one parameter to a memory allocation multiplication is negative or zero, failing operation gracefully in /var/www/html/vendor/magento/module-media-storage/Model/File/Validator/Image.php on line 64`。 [GitHub-34858](https://github.com/magento/magento2/issues/34858)
...
* 現在，您可以將Vimeo視頻設定為在 `banner` 元素 `restrict mode`。 以前，Adobe Commerce拋出JavaScript錯誤。
...
* 現在，從希望清單頁更新物料數量時，將按照預期更新產品詳細資訊頁上的數量。 Adobe Commerce現在從產品URL中提取更新值並填充 `qty` 從wishlist本身獲取產品詳細資訊頁面的欄位。

## 已知問題

**問題**:啟用Braintree後，管理員用戶無法從管理員為客戶建立訂單或重新訂購。 當管理員用戶按一下 **訂單** 或 **重新排序**,Adobe Commerce不提交命令，  `system.log` 顯示此錯誤： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`。  **解決方法**:  `BUNDLE-3137-composer.patch` 的子菜單。 查看 [啟用Braintree付款時，管理員無法建立訂單/重新排序](https://support.magento.com/hc/en-us/articles/8322266352525) 有關此問題的討論和對修補程式的訪問的知識庫文章。 雲商應下載Commerce v1.0.18的雲修補程式。Adobe Commerce2.4.5-p1中也將包括修復。 <!-- AC-6222-->

**問題**: [!DNL RabbitMQ] 在新安裝（而非升級）Adobe Commerce2.4.5後，基礎架構元素（交換、隊列）可能未初始化。此配置不正確 [!DNL RabbitMQ] 在執行非同步操作或生成效能配置檔案時可能會導致錯誤。 雲部署不受影響。 **解決方法**:進行Adobe Commerce全新安裝的商2.4.5應運行 `bin/magento setup:upgrade --keep-generated` 使用 `bin/magento setup:config:set` 配置 [!DNL RabbitMQ] 或安裝Adobe Commerce [!DNL RabbitMQ] 配置選項 `setup:install` 的子菜單。 請參閱 [[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md)。
