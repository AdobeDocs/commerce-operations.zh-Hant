...
# Adobe Commerce2.4.2發行說明

Adobe Commerce2.4.2對效能和安全性進行了增強，並對平台進行了重大改進。 安全增強包括擴展對 `SameSite` 屬性。 B2B的改進重點是支援採購訂單的線上付款。 Elasticsearch7.9.x和Redis 6.x現在受支援。
...
在此版本中，配置安裝以使用剝離資料庫的功能已棄用。 當前使用拆分資料庫的商家應開始計畫還原到單個資料庫或遷移到單個資料庫，或使用替代方法。 查看 [棄用Adobe Commerce的拆分資料庫功能](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) 有關此問題的概述的DevBlog帖子。  請參閱 [從拆分資料庫還原到單個資料庫](../../../configuration/storage/revert-split-database.md) 的子菜單。
...
* 已添加對多個願望清單的支援。 你可以用GraphQL [建立](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-wishlist.html)。 [刪除](https://devdocs.magento.com/guides/v2.4/graphql/mutations/delete-wishlist.html), [更名](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-wishlist.html) 願望清單 [移動](https://devdocs.magento.com/guides/v2.4/graphql/mutations/move-products-between-wishlists.html) 或 [複製](https://devdocs.magento.com/guides/v2.4/graphql/mutations/copy-products-between-wishlists.html) 之間的項目。 <!-- MC-32356-->

* 增加了對退回商品授權(RMA)的支援。 購物者可 [請求返回](https://devdocs.magento.com/guides/v2.4/graphql/mutations/request-return.html)。 如果商家接受請求，購物者可以執行諸如 [添加註釋](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-return-comment.html) 和 [添加跟蹤資訊](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-return-tracking.html)。 <!-- MC-34596-->

* 添加了對以下B2B功能的支援：
   * 公司。 您可以添加公司 [管理員](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-company.html)。 [用戶](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-company-user.html)。 [角色](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-company-role.html), [團隊](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-company-team.html)。 <!-- MC-33861-->
   * 公司貸方。 的 [公司查詢](https://devdocs.magento.com/guides/v2.4/graphql/queries/company.html) 包括有關公司信用記錄的詳情。 <!-- MC-33862-->
   * 申請清單。 你可以 [建立](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-requisition-list.html)。 [刪除](https://devdocs.magento.com/guides/v2.4/graphql/mutations/delete-requisition-list.html), [更新](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-requisition-list.html) 申請清單。 支援還包括 [添加](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-products-to-requisition-list.html)。 [更新](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-products-in-wishlist.html)。 [刪除](https://devdocs.magento.com/guides/v2.4/graphql/mutations/delete-requisition-list-items.html)。 [複製](https://devdocs.magento.com/guides/v2.4/graphql/mutations/copy-items-between-requisition-lists.html), [移動](https://devdocs.magento.com/guides/v2.4/graphql/mutations/move-items-between-requisition-lists.html) 申請清單中的項目以及添加申請清單項目 [進車裡](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-requisition-list-items-to-cart.html)。 <!-- MC-34312-->
...
2.4.2推出B2B 1.3.1.此版本包括對採購訂單的線上付款以及多個錯誤修復的支援。

**現在可以使用聯機付款方法完成採購訂單**。 提示B2B買家選擇他們針對每個客戶的首選付款方法 [採購訂單](https://docs.magento.com/user-guide/stores/b2b-purchase-order-flow.html) 在初始簽出時。 在採購訂單獲得批准後，系統將提示採購員輸入付款詳細資訊以將採購訂單轉換為最終訂單。

要支援接受線上付款的完整工作流，此功能還：

* 在初始結帳期間覆蓋現有付款方法模板以維護PCI合規性。

* 保持與第三方、自定義和社區開發的付款方法的相容性。

* 在需要將付款詳細資訊添加到批准的採購訂單時通過電子郵件通知採購員。

* 允許自定義管理員的電子郵件，允許商家使用與其品牌一致的模板。

* 介紹 `Approved – Pending Payment` 狀態，以明確顯示何時需要採取行動。

* 防止在最終付款步驟中添加或刪除折扣代碼，這可確保批准的採購訂單的訂單總額保持不變。

* 允許買家在最終付款步驟期間更改付款方式，以保持靈活性並增加轉換。
...
此版本包括對頁面生成器內容遷移和頁面生成器CSS自定義功能的增強：

* 開發人員現在可以根據每個視區對內容類型輸出進行不同的樣式，而無需使用 `!important` 指令。

* 遷移到頁面生成器的內容不再使用預設的內聯樣式填充。

* 頁面生成器不再要求將所有內容類型放在一行中。 的 [頁面生成器階段](https://docs.magento.com/user-guide/cms/page-builder-workspace.html#stage) 現在為空，支援將以下內容類型直接添加到舞台：行、列、制表符、HTML代碼、塊、動態塊。

* 內容類型不再需要預定義的邊距和畫面。

* 新移動 [視埠切換](https://docs.magento.com/user-guide/cms/page-builder-workspace.html#viewports) 和表單域值的viewport作用域允許用戶和開發人員執行以下操作：

   * 創作時查看不同視圖埠上的內容。

   * 針對每個視區的不同內容類型優化最小高度欄位參數。 (機箱外僅支援一個參數（最小高度）。 需要自定義開發，以便為其他內容參數啟用移動優化。)

   * 將欄位範圍添加到自定義欄位和欄位集（開發人員）。

### 互動式產品內指導

互動式產品內指南向商家提供使用提示和管理員內部有關新功能公告、瀏覽指南、上門資訊和工具提示的資訊。  **如果未啟用此功能，管理員必須從管理員中選擇加入以接收產品內指導**。 請參閱 [使用情況資料收集](https://docs.magento.com/user-guide/stores/admin.html) 和 [管理員使用情況](https://docs.magento.com/user-guide/configuration/advanced/admin.html#admin-usage)。
...
對AWSS3的支援已添加到所有模組中，包括B2B、PageBuilder和Adobe Stock整合。
...
MCOM連接器與2.4.2完全相容。有關連接器的錯誤修復和增強，請參見 [連接器發行說明](https://omsdocs.magento.com/release-notes/connector/index.html)。 此版本包括對OMS命令行介面的改進，該介面現在提供了合作夥伴和支援團隊可用於排除故障的詳細資訊 `retry`。 `queue_prune`。 `order_sync`。 `message process`, `catalog export` 進程。

### 供應商開發的擴展

有關此版本的功能和更改的更新，請參閱以下文章：

* [Amazon](https://docs.magento.com/user-guide/payment/amazon-pay.html)。 Amazon支付已在此版本中棄用，將在2.5.0Magento中刪除。版本2.4.3及更高版本將僅包含相容性更新和主要錯誤修復。
...
* 在此版本中，配置安裝以使用剝離資料庫的功能已棄用。 查看 [棄用Adobe Commerce的拆分資料庫功能](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog帖子。
...
* 的 **添加新評級** 按鈕 **保存**。 **刪除**。 **重置** 按鈕在 **商店** > **屬性** > **評級** 頁面。

<!-- MC-35804-->

* 管理員現在只顯示登錄管理員有權訪問的網站中的資料。 以前，管理員顯示了部署中所有網站的資料。

<!-- MC-35027-->

* 所有選定網站現在都儲存在 `gws_websites` 無論網站ID大小。 以前，網站ID被截斷，並且所選網站的清單未儲存在資料庫中。 未正確保存用戶角色，當商家嘗試更改和保存角色時，應用程式顯示了以下錯誤： `The "X" store ID is incorrect. Verify the store ID and try again`。

<!-- MC-37223-->

* 應用程式現在在管理員網格中為受限管理用戶顯示正確的項目計數。 以前，這些網格顯示的計數反映了所有國家的資料，而網格本身僅顯示特定國家的資料。 （此修復程式可更正「客戶段」、「目錄價格規則」、「購物車價格規則」、「所有用戶」、「鎖定的用戶」和「用戶角色」網格的問題。）

<!-- MC-34156-->

* 權限排除的管理員 `Magento_Catalog::edit_product_design` （編輯產品設計）現在可以通過在所選商店視圖中使用新名稱保存現有產品來建立新產品。 以前，應用程式顯示以下錯誤：  `Not allowed to edit the product's design attributes`。 [GitHub-28106 ](https://github.com/magento/magento2/issues/28106)

<!-- MC-36406-->

* 應用程式不再顯示 **添加屬性** 按鈕 **商店** >管理員缺少建立這些實體的相應權限時的屬性。 以前，當沒有相應權限的網站管理員嘗試建立 **屬性集** 或 **客戶** 屬性。

<!-- ENGCOM-8230 -->

* 嘗試更改時，應用程式不再引發錯誤 `backend-frontname` 使用 `ssh` 裝了Adobe Commerce。 [GitHub-26762](https://github.com/magento/magento2/issues/26762)
...
* 管理員可以通過SKU將產品從管理員添加到購物車中，而無需從管理員重定向。 管理員不再重定向到店面頁面，應用程式將按照預期在「管理購物車項目」部分顯示產品。

<!-- MC-36750-->

* 當您上傳包含多種錯誤類型的CSV檔案時，應用程式現在會在按SKU將產品添加到購物車時顯示資訊性錯誤消息。 以前，在您按一下時，應用程式會顯示空白頁和一般錯誤消息 **選擇檔案** 上 **管理購物車** > **按SKU添加到購物車**。
...
* 配置目錄權限的順序不再阻止具有正確權限的管理員將產品添加到購物車。 以前，應用程式未驗證 `addToCart` 當前客戶組的配置設定。 因此，當管理員嘗試將產品添加到購物車時，應用程式拋出錯誤。
...
* 在重複訪問類別頁時，啟用管理員的類別權限不再導致在全頁快取中快取類別頁失敗。 以前，未按照預期從快取中提供對頁面的後續訪問，因此效能降低。
...
* 現在，「我的帳戶」區域和「管理訂單詳細資訊」頁中正確顯示了自定義地址屬性。
...
* 應用程式現在按預期保存B2B用戶的自定義客戶屬性值。 以前，為添加或編輯公司用戶而建立的自定義客戶屬性未與「添加新用戶」表單綁定，因此，屬性值未保存。 此外，自定義日期屬性是從資料庫中派生的字串形式，未轉換為適當的日期格式以用於店面顯示。
...
* 在可下載產品已部分退款後，應用程式不再在「我的下載產品」頁籤中列出可下載的產品。 [GitHub-28388](https://github.com/magento/magento2/issues/28388)

### 動態塊（以前的標題）

<!-- MC-23938-->

* 在您刪除與該塊關聯的客戶段後，應用程式不再在店面上顯示動態塊。

<!-- MC-37322-->

* 現在，在確定是否顯示動態塊時，應用程式會正確考慮目錄價格規則。
...
* 當更改儲存視圖時，應用程式不再將客戶重定向到首頁。 的 `\Magento\VersionsCmsUrlRewrite\Model\StoreSwitcher::switch()` 方法現在僅在以下情況下才將購物者重定向到首頁 `$urlPath` 找不到第二個儲存視圖。
...
* 的 `FlushCacheByTags` 類已更新為使用插件後代替插件。 [GitHub-29558](https://github.com/magento/magento2/issues/29558)

<!-- ENGCOM-8358 -->

* `MessageValidator` 不再在不包含哈希的陣列上失敗 `0` 的子菜單。 [GitHub-30563](https://github.com/magento/magento2/issues/30563)
...
* URL重寫生成器現在設定類別對象 `url_key`。 `url_path`,  `store_id` 在保存全局作用域的類別URL重寫時預設儲存值。 以前，它保存了 `url_key`。 `url_path`, `store_id` 上次處理的儲存ID的值。 [GitHub-29585](https://github.com/magento/magento2/issues/29585)
...
* 包含禮品卡代碼的電子郵件現在僅在訂單成功保存後才發送 **訂購訂單物料時建立禮品卡帳戶** 配置設定已啟用。 以前，如果在訂購物料時生成禮品卡帳戶，但在發送虛擬禮品卡後發生異常，則會重新使用禮品卡代碼併發送給不同的客戶。

<!-- MC-36496-->

* 應用程式不再複製禮品卡訂單。 以前，當購物者訂購一張禮品卡時，申請人會交付兩份禮品代碼。 這是因為 `cron` 作業調用 `\Magento\GiftCard\Observer\GenerateGiftCardAccountsInvoice::execute()`，並且此方法未檢查在建立發票期間是否已生成禮品卡代碼。
...
* Google標籤經理 `productClick` 函式現在按預期工作。 以前， `productClick` 未在清單頁上的產品上觸發，Google標籤管理器無法捕獲該事件。

<!-- MC-38781-->

* Google標籤管理器可配置產品選項在您編輯購物車中的可配置產品選項後，現在將按照預期在資料層中更新。 以前， `removeFromCart` 在更改product選項時錯誤改寫了事件，並且數量保持不變。
...
* 將產品添加到購物車的突變現在具有目錄權限。 [GitHub-30179](https://github.com/magento/magento2/issues/30179)
...
* 在按預期上載後，目錄事件傳送構件現在將顯示影像。 以前， `catalog_event_status_checker` cron作業刪除了上載的映像。
...
* 當CSV檔案還包含不正確的條目時，計畫的產品導入流程現在會使用正確的資料欄位更新產品。 以前，如果CSV檔案包含不正確的條目，則不更新任何條目 **出錯時：繼續處理** 已啟用。
...
* Adobe Commerce通過消除每個索引器實例多次刷新標籤而提高了快取效能。 [GitHub-29890](https://github.com/magento/magento2/issues/29890)
...
* 問題 `StorefrontProductListWidgetWithSharedCatalogTest` 已解決導致跳過test的問題。 [GitHub-322](https://github.com/magento/partners-magento2b2b/issues/322)
...
* MFTFtest中已添加有意義的標題和說明 `Company`。 `CompanyCredit`。 `CompanyPayment`。 `CompanyShipping`, `ConfigurableSharedCatalog` 模組。 [GitHub-30164](https://github.com/magento/magento2/issues/30164)
...
* 應用程式不再顯示 `Purchase Order number is a required field` 購物者在將採購訂單配置為唯一可用付款選項的部署中輸入任何資訊之前，在優惠券代碼欄位上方顯示一條消息。
...
* 現在，客戶購物者可在 **自動分配給客戶組** 的子菜單。 付款在PayPal網關上處理，訂單以Magento方式建立，購物者看到訂單感謝頁。 以前，訂單在PayPal網關上已成功處理，但訂單未在Adobe Commerce建立。 反之，購物者被重定向到訂單審閱頁，應用程式在該頁顯示以下錯誤： `Failed address validation: %1" error`。 [GitHub-25399](https://github.com/magento/magento2/issues/25399)
...
* Adobe Commerce在編輯和保存包含超過3,000,000個客戶的客戶細分市場的操作過程中的效能已得到改善。
...
* 連續的非同步價格更新不再相互干擾，並且每個操作都分配了正確的狀態。 以前，如果非同步價格更新失敗，則所有以下非同步價格更新都將失敗，直到相應隊列使用者重新啟動。 [GitHub-27865](https://github.com/magento/magento2/issues/27865)
...
* 現在，在您分解RMA時，選定選項將應用於分解行項目。 以前，未保存「返回原因」、「物料條件」和「解析度」的值。

<!-- MC-35360-->

* 現在，您可以在啟用JavaScript捆綁後刪除RMA的跟蹤和發運資訊。

<!-- MC-36757-->

* 對特定網站範圍內的產品具有限制訪問權限的管理員現在可以將項目添加到返回中，即使該項目不再位於其網站範圍內。

### 評論

<!-- MC-37145-->

* 上顯示的評級數 **商店** > **屬性** > **評級** 現在與多站點部署中的分級計數匹配。 [GitHub-30258](https://github.com/magento/magento2/issues/30258)
...
* 現在，當部分訂單完成時，將按照對應稅訂單的預期計算獎勵積分。

<!-- MC-37244-->

* 商家現在可以成功地為已購買的帶有獎勵積分的訂單發放部分退款，並且隨後仍會退還剩餘的訂單。 此前，商家無法退回剩餘訂單的獎勵積分。

<!-- MC-37113-->

* 當購物者刪除獎勵積分後重新應用獎勵積分時，申請不再顯示此錯誤： `You removed the reward points from this order`。

<!-- MC-37032-->

* 現在，當客戶在支援多種貨幣的商店中使用非基礎貨幣下單時，將正確計算獎勵積分。
...
* Elasticsearch在購物者搜索搜索同義詞時不再引發未知修飾符錯誤。 以前，擒縱者在規則運算式中缺少轉義斜槓符號，這會導致致命錯誤。
...
* 現在，當您檢查首頁計畫更新時，「編輯首頁」頁將按預期顯示所有計畫更新。 以前，應用程式只顯示多個計畫更新中的一個。

<!-- MC-34988 -->

* 使用 `/rest/V1/products/special-price-information` 現在，特殊價格終結點將保存價格並按預期返回計畫。

<!-- MC-35362 -->

* 當導入CSV檔案包含位元組順序標籤(BOM)時，計畫導入現在按預期運行。
...
* 當您嘗試為類別安排更新時，應用程式不再引發錯誤。 以前，當您按一下時，應用程式拋出了此錯誤 **保存** 在「計畫新更新」頁上： `Something went wrong while saving the Magento\Catalog\Api\Data\CategoryInterface`。

<!-- MC-38455-->

* 的 `rest/V1/products/special-price` API現在可用於為多個商店計畫產品特殊價格。 以前，當商家為多個商店計畫價格更新時，應用程式引發錯誤 `from` 和 `to` 在新計畫的開始時間可用時回滾的時間。
...
* 相關產品的目標規則現在在 `is one of` 用於「要顯示的產品」(Products to Display)特徵的條件包含多個條件。 以前，當此條件包含多個值時，不會分析它。
...
* 現在，包含URL的標頭值將在中正確分析 `ApiFunctional TestFramewor`k [GitHub-26425](https://github.com/magento/magento2/issues/26425)
...
* 已將游標的位置從中間調整到 **消息** 欄位 **我的帳戶** > **禮品註冊表** > **共用禮品註冊表**。

<!-- MC-33497-->

* 已解決「添加計畫導出」頁上的列佈局顯示問題。
...
* 此時，應用程式將在 **日期** 在「建立帳戶」頁面中，輸入欄位應為。
...
* 購物者現在可以成功地將產品從一個願望清單移動到另一個願望清單。
...
**問題**:的 `[magento_root]/index.php` 檔案已刪除，應用程式現在從 `/pub` 預設情況下。 從子資料夾提供的儲存將無法按預期工作，並可能顯示404個錯誤。 **解決方法**:使用符號連結將安裝模擬到子資料夾中。 以下示例使用兩個儲存(`https://shop01.com/shop/` 和 `https://shop02.com/shop/`)以說明如何使用symlink模擬子資料夾中的安裝。
...
**問題**:商家必須從管理員激活「快速強制TLS」功能，以便為所有儲存頁啟用全局HTTP到HTTPS重定向。 查看 [將雲上所有頁的HTTP重定向到HTTPS（強制TLS）](https://support.magento.com/hc/en-us/articles/360006296953) 知識庫文章。 <!-- MC-39988-->
...
**問題**:當購物者點擊Braintree時，應用程式會按預期建立訂單 **使用Venmo付款**，但不在管理員中建立訂單。 **解決方法**:沒有。 查看 [2.4.2已知問題：BraintreeVenmo付款無效](https://support.magento.com/hc/en-us/articles/360054662652/)  知識庫文章。  <!-- BUNDLE-2894-->

### B2B已知問題

**問題**:B2B採購員可以使用線上付款方法繞過通常的採購訂單流。 如果買家可以將其全部結帳總額減少到0（例如，通過促銷代碼或禮品卡），然後刪除代碼或禮品卡，則會發生這種情況。 即使在這種情況下，申請人仍然根據所分配目錄中的商品價格，訂出正確數量的訂單。 **解決方法**:啟用聯機付款方法以審批採購訂單時禁用禮品卡和贈券代碼。 <!-- B2B-1603-->

**問題**:當嘗試使用PayPal Express結帳從採購訂單下訂單時，將將採購員重定向到購物車 **上下文模式** 已禁用。 <!-- B2B-1604-->

**問題**:當採購員建立採購訂單，然後導航到結帳頁時，應用產品有時會顯示404錯誤。 當採購員在導航到結帳頁而不完成先前的採購之前使用聯機付款方法建立了不同的採購訂單時，會發生此錯誤。 採購員仍然可以下達採購訂單。 **解決方法**:沒有。 <!-- B2B-1605-->

**問題**:特定付款方法的折扣在採購訂單的結帳期間持續存在，即使在買方在最終結帳期間更改其付款方法時也是如此。 因此，客戶可能獲得他們無權獲得的折扣。 發生此情況的原因是，儘管付款方法發生了更改，但仍然應用了原始付款方法的購物車規則。 **解決方法**:沒有。 查看 [2.4.2 B2B已知問題：更改付款方法後聯機採購訂單的折扣保留](https://support.magento.com/hc/en-us/articles/360054667312) 知識庫文章。 <!-- B2B-1012 -->

**問題**:的 `deleteRequisitionListOutput` 查詢返回有關已刪除申請清單的詳細資訊，而不是剩餘的申請清單。 <!-- MC-39894-->
