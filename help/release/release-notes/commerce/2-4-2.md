...
Adobe Commerce 2.4.2引進了效能和安全性的增強功能，以及重要的平台改善。 安全性增強功能包括擴大對 `SameSite` 所有Cookie的屬性。 B2B改善著重於支援採購單的線上付款。 現在支援Elasticsearch7.9.x和Redis 6.x。
...
此版本已棄用設定安裝以使用分割資料庫的功能。 目前使用分割資料庫的商家，應開始計畫回覆或移轉至單一資料庫，或使用替代方法。 請參閱 [淘汰Adobe Commerce中的分割資料庫功能](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog文章以概略瞭解此問題。  另請參閱 [從分割資料庫還原為單一資料庫](../../../configuration/storage/revert-split-database.md) 以取得移轉指示。
...
* 已新增對下列B2B功能的支援：
   ...
   * 公司評價。 此 [公司查詢](https://devdocs.magento.com/guides/v2.4/graphql/queries/company.html) 包括公司信用記錄的詳細資訊。 <!-- MC-33862-->
...
**現在可以使用線上付款方式完成採購單**. 系統會提示B2B購買者為各個，選取其偏好付款方式 [purchase order — 採購單](https://docs.magento.com/user-guide/stores/b2b-purchase-order-flow.html) 初次結帳期間。 核准採購單之後，系統會提示採購員輸入付款明細，以將採購單轉換為最終訂單。
...
* 推出 `Approved – Pending Payment` 採購單的狀態，以清楚顯示何時需要動作。
...
* 開發人員現在可以根據檢視區以不同方式設定內容型別輸出的樣式，而不需使用 `!important` 指令。

* 移轉至頁面產生器的內容不再以預設內嵌樣式填入。

* Page Builder不再要求將所有內容型別置入列。 此 [頁面產生器階段](https://docs.magento.com/user-guide/cms/page-builder-workspace.html#stage) 現在最初是空白的，並支援將下列內容型別直接新增到舞台：列、欄、標籤、HTML程式碼、區塊、動態區塊。
...
互動式產品內指導透過新功能公告、逐步說明指南、上線資訊和工具提示，為商家提供管理員內的使用提示和資訊。  **如果未啟用此功能，管理員必須從管理員選擇加入，才能收到產品內指南**. 另請參閱 [使用資料集合](https://docs.magento.com/user-guide/stores/admin.html) 和 [管理員使用情況](https://docs.magento.com/user-guide/configuration/advanced/admin.html#admin-usage).
...
MCOM聯結器與2.4.2完全相容。有關聯結器的錯誤修正和增強功能的說明，請參閱 [聯結器發行說明](https://omsdocs.magento.com/release-notes/connector/index.html). 此版本包含改進OMS命令列介面，現在提供合作夥伴和支援團隊可用於疑難排解的詳細資訊 `retry`， `queue_prune`， `order_sync`， `message process`、和 `catalog export` 程式。

### 廠商開發的擴充功能

請參閱下列文章，瞭解此版本功能和變更的更新：

* [Amazon Pay](https://marketplace.magento.com/amzn-amazon-pay-magento-2-module.html). Amazon Pay在此版本中已過時，將在Magento2.5.0中移除。2.4.3版及更新版本將僅包含相容性的更新及主要錯誤的修正。
...
* 此版本已棄用設定安裝以使用分割資料庫的功能。 請參閱 [淘汰Adobe Commerce中的分割資料庫功能](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog文章。

<!-- MC-36472-->

* 商家現在可以成功地將執行MySQL 8.x的開放原始碼部署升級為Commerce部署。 之前，應用程式擲回例外狀況， `AUTO_INCREMENT` 值已恢復為所有表格的初始值，其中 `row_id` 已在升級期間新增。
...
* 此 **新增評等** 按鈕和 **儲存**， **刪除**， **重設** 按鈕不再可用於 **商店** > **屬性** > **評等** 許可權受限制的管理員頁面。
...
* 許可權排除的管理員 `Magento_Catalog::edit_product_design` （編輯產品設計）現在可以透過在所選商店檢視中以新名稱儲存現有產品來建立新產品。 之前，應用程式會顯示此錯誤：  `Not allowed to edit the product's design attributes`. [GitHub-28106](https://github.com/magento/magento2/issues/28106)

<!-- MC-36406-->

* 應用程式不再顯示 **新增屬性** 下的頁面上的按鈕 **商店** >管理員缺乏建立這些實體的適當許可權時的屬性。 之前，當沒有適當許可權的網站管理員嘗試建立 **屬性集** 或 **客戶** 屬性。

<!-- ENGCOM-8230 -->

* 當您嘗試變更時，應用程式不會再擲回錯誤 `backend-frontname` 使用 `ssh` 安裝Adobe Commerce後的容器。 [GitHub-26762](https://github.com/magento/magento2/issues/26762)
...
* 透過SKU將產品新增到購物車時，當您上傳包含多種錯誤型別的CSV檔案時，應用程式現在會顯示資訊性錯誤訊息。 以前，當您按一下時，應用程式會顯示空白頁面和一般錯誤訊息 **選擇檔案** 於 **管理購物車** > **依SKU新增至購物車**.
...
* 目錄許可權的設定順序不會再防止具有正確許可權的管理員將產品新增到購物車。 之前，應用程式不會驗證 `addToCart` 目前客戶群組的組態設定。 因此，當管理員嘗試將產品新增到購物車時，應用程式擲回錯誤。
...
* 變更商店檢視時，應用程式不再將客戶重新導向首頁。 此 `\Magento\VersionsCmsUrlRewrite\Model\StoreSwitcher::switch()` 方法現在只會在將購物者重新導向至首頁時 `$urlPath` 找不到第二個存放區檢視的。
...
* URL重寫產生器現在會設定類別物件 `url_key`， `url_path`、和  `store_id` 以儲存全域範圍的類別URL重寫時預設儲存值。 之前已儲存 `url_key`， `url_path`、和 `store_id` ，並使用上次處理的存放區ID的值。 [GitHub-29585](https://github.com/magento/magento2/issues/29585)
...
* 包含禮品卡代碼的電子郵件現在只會在訂單成功儲存後，才傳送，當 **訂購訂單料號時，建立禮品卡帳戶** 已啟用組態設定。 先前，如果訂購商品時產生禮卡帳戶，但傳送虛擬禮卡後發生例外狀況，則會重複使用禮卡代碼並傳送給不同客戶。

<!-- MC-36496-->

* 應用程式不再複製禮品卡訂單。 以前，當購物者訂購一張禮品卡時，應用程式會傳送兩個禮品代碼。 發生此狀況是因為 `cron` 已呼叫的工作 `\Magento\GiftCard\Observer\GenerateGiftCardAccountsInvoice::execute()`，此方法不會檢查是否已在建立發票期間產生禮品卡代碼。
...
* Google標籤管理員 `productClick` 函式現在如預期般運作。 先前， `productClick` 並未在產品清單頁面上觸發，且Google Tag Manager無法擷取該事件。
...
* 如果CSV檔案也包含不正確的專案，排程產品匯入程式現在會以正確的資料欄位更新產品。 以前，如果CSV檔案包含不正確的專案，則 **發生錯誤時：繼續處理** 已啟用。
...
* 應用程式不再顯示 `Purchase Order number is a required field` 在購物者輸入部署中的任何資訊之前，優惠券代碼欄位上方的訊息，其中採購單已設定為唯一可用的付款選項。
...
* 訪客購物者現在可以在以下情況下下達PayPal訂單： **自動指定至客戶群組** 已啟用。 付款會在PayPal閘道處理，訂單會在Magento中建立，而購物者會看到訂單感謝頁面。 之前，訂單已在PayPal閘道成功處理，但訂單並非在Adobe Commerce中建立。 相反地，購物者被重新導向至訂單複查頁面，應用程式在該頁面顯示以下錯誤： `Failed address validation: %1" error`. [GitHub-25399](https://github.com/magento/magento2/issues/25399)
...
* 傳送0和 `/rest/V1/products/special-price-information` 特殊價格端點現在會儲存價格，並如預期傳回排程。
...
* 現在，重新排程的產品變更在 `cron` 執行。

<!-- MC-38759-->

* 當您嘗試排程類別的更新時，應用程式不再擲回錯誤。 之前，應用程式在您按一下時擲回此錯誤 **儲存** 在「排程新更新」頁面上： `Something went wrong while saving the Magento\Catalog\Api\Data\CategoryInterface`.

<!-- MC-38455-->

* 此 `rest/V1/products/special-price` API現在可用來排程多個商店的產品特別價格。 之前，當商家針對相同商品排程多間商店的價格更新時，應用程式會擲回錯誤 `from` 和 `to` 在新排程的開始時間提供復原的時間。
...
* 現在，相關產品的Target規則可在 `is one of` 「要顯示的產品」功能所使用的條件包含多個條件。 先前，當此條件包含多個值時，不會進行剖析。
...
* 包含URL的標頭值現在會在中正確剖析 `ApiFunctional TestFramewor`k. [GitHub-26425](https://github.com/magento/magento2/issues/26425)
...
* 調整游標從中間到開始的位置 **訊息** 欄位於 **我的帳戶** > **贈品登入** > **共用贈品登入**.
...
* 應用程式現在會在旁邊顯示行事曆圖示 **日期** 輸入欄位，如預期在店面建立帳戶頁面。
...
**問題**：此 `[magento_root]/index.php` 檔案已移除，應用程式現在執行自 `/pub` Apache設定的預設值。 從子資料夾提供的存放區將無法如預期運作，並可能顯示404錯誤。 **因應措施**：使用symlink將安裝模擬至子資料夾。 下列範例使用兩個存放區(`https://shop01.com/shop/` 和 `https://shop02.com/shop/`)以說明如何使用symlink來模擬子資料夾中的安裝。
...
**問題**：商家必須從管理員啟用Fastly Force TLS功能，以針對所有商店頁面啟用全域HTTP至HTTPS重新導向。 請參閱 [將雲端上所有頁面的HTTP重新導向至HTTPS （強制TLS）](https://support.magento.com/hc/en-us/articles/360006296953) 知識庫文章。 <!-- MC-39988-->
...
**問題**：當購物者按一下時，應用程式會如預期在Braintree中建立訂單 **以Venmo付款**，但不會在管理員中建立訂單。 **因應措施**：無。 請參閱 [2.4.2已知問題：BraintreeVenmo付款無法運作](https://support.magento.com/hc/en-us/articles/360054662652/)  知識庫文章。  <!-- BUNDLE-2894-->

### B2B已知問題

**問題**：B2B買家可使用線上付款方式，以略過一般的採購單流程。 如果買家將結帳總數減少到0 （例如，透過促銷代碼或禮品卡），然後移除代碼或禮品卡，就可能發生這種情況。 即使在這些條件下，應用程式仍會根據指定目錄中的專案價格，下達正確金額的訂單。 **因應措施**：當啟用線上付款方式以進行採購單核準時，請停用禮品卡和優惠券代碼。 <!-- B2B-1603-->

**問題**：當買家嘗試使用PayPal Express結帳功能從採購單下訂單時，系統會重新導向至購物車。 **內文中模式** 已停用。 <!-- B2B-1604-->

**問題**：當採購員建立採購單，然後導覽至結帳頁面時，應用模組有時會顯示404錯誤。 當採購員先前使用線上付款方式建立不同的採購單，而未完成先前的採購就瀏覽至結帳頁面時，就會發生此錯誤。 採購員仍然可以下採購單。 **因應措施**：無。 <!-- B2B-1605-->

**問題**：在採購單結帳期間，即使買方在最後結帳期間變更付款方式，特定付款方式的折扣仍會持續存在。 因此，客戶可能會獲得他們無權享有的折扣。 發生此狀況是因為儘管付款方式有所變更，原始付款方式的購物車規則仍會套用。 **因應措施**：無。 請參閱 [2.4.2 B2B已知問題：付款方式變更後，線上「採購單」的折扣仍會保留](https://support.magento.com/hc/en-us/articles/360054667312) 知識庫文章。 <!-- B2B-1012 -->

**問題**：此 `deleteRequisitionListOutput` 查詢會傳回已刪除的請購單清單（而非剩餘的請購單清單）的相關明細。 <!-- MC-39894-->
...
您可以使用以下專案安裝Adobe Commerce 2.4.2： [作曲者](../../../installation/composer.md).
