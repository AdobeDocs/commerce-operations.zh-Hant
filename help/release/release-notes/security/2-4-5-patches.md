...
>
>在此日期之後，Adobe Commerce 2.4.5將不會針對MySQL 8.0之後發行的任何MySQL版本提供相容性或>支援。Adobe將不會在此Adobe>Commerce發行版本中>驗證或支援較新的MySQL主要版本。
>
>強烈建議所有執行2.4.5版的Adobe Commerce內部部署客戶>將其資料庫伺服器移轉至相容的MariaDB版本。

{{extended-support}}

## 2.4.5-p17

Adobe Commerce 2.4.5-p17是[延伸支援安全性版本](../../lifecycle-policy.md#security-only-transitional-period)，針對舊版2.4.5中發現的漏洞提供安全性錯誤修正。
...
Adobe Commerce 2.4.5現在與Valkey 8.1 LTS相容，提供雲端基礎結構上的Adobe Commerce所支援的長期支援快取後端選項。

#### RabbitMQ 4.2支援

Adobe Commerce 2.4.5現在與RabbitMQ 4.2相容，後者處理RabbitMQ 4.1支援終止日期排定於2026年2月。 與Apache ActiveMQ Artemis的相容性得以保留，且ActiveMQ仍為此安全性專用發行版本的預設訊息佇列服務。

## 2.4.5-p16

Adobe Commerce 2.4.5-p16是[延伸支援安全性版本](../../lifecycle-policy.md#security-only-transitional-period)，針對舊版2.4.5中發現的漏洞提供安全性錯誤修正。
...
#### 與Apache ActiveMQ Artemis相容

由於RabbitMQ 4的相關支援終止風險，Adobe Commerce已展開策略移轉至Apache ActiveMQ Artemis，以確保訊息佇列基礎結構的長期穩定性及支援。

* Adobe Commerce 2.4.5現在支援最新版本的Apache ActiveMQ Artemis。
* 我們保留與RabbitMQ 4.1的相容性，以支援偏好繼續使用現有MQ服務的客戶。
* Adobe Commerce Cloud現在完全支援ActiveMQ，包括適用於雲端原生部署的AWS ActiveMQ。

#### 2.4.5-p16與MariaDB 10.11的相容性

已驗證Adobe Commerce 2.4.5-p16是否相容於MariaDB 10.11，同時繼續完全支援MariaDB 10.6。 目前執行2.4.5-px的商戶可安全地升級至2.4.5-p16，並從MariaDB 10.6轉換至MariaDB 10.11。

## 2.4.5-p15

Adobe Commerce 2.4.5-p15是[延伸支援安全性版本](../../lifecycle-policy.md#security-only-transitional-period)，針對舊版2.4.5中發現的漏洞提供安全性錯誤修正。
