...
* 新的全頁快取組態設定有助於減輕與HTTP `{BASE-URL}/page_cache/block/esi`端點相關的風險。 此端點支援來自Commerce配置控點和區塊結構的不受限制、動態載入的內容片段。 新的&#x200B;**[!UICONTROL Handles params size]**&#x200B;組態設定會設定此端點的`handles`引數值，此引數會決定每個API允許的控點數目上限。 此屬性的預設值為100。 商戶可從管理員(**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL System]** > **[!UICONTROL Full Page Cache]** > **[!UICONTROL Handles params size]**)變更此值。 請參閱[設定Commerce應用程式以使用Varnish](/help/configuration/cache/configure-varnish-commerce.md)。<!-- AC-9113 -->

* **已新增子資源完整性(SRI)支援**，以符合PCI 4.0對驗證付款頁面上指令碼完整性的要求。 子資源完整性(SRI)支援為駐留在本機檔案系統中的所有JavaScript資產提供完整性雜湊。 預設SRI功能僅在管理員和店面區域的付款頁面上實作。 不過，商家可以將預設設定延伸至其他頁面。 請參閱&#x200B;_Commerce PHP開發人員指南_.<!--AC-11513-->中的[子資源完整性](https://developer.adobe.com/commerce/php/development/security/subresource-integrity)
...
* [!DNL Google Pay]&#x200B;**的** 3DS支援 — 包含[!DNL Google Pay]非標籤化卡片的3DS驗證支援。 如需詳細資訊，請參閱[Braintree檔案](https://developer.paypal.com/braintree/docs/guides/3d-secure/advanced-options/javascript/v3#verify-google-pay-card-using-3ds)。
...
* 帳戶區域&#x200B;**中的**&#x200B;[!DNL Vaulting]&#x200B;付款 — 登入的客戶現在可以在客戶帳戶區域[儲存庫/商店](https://experienceleague.adobe.com/zh-hant/docs/commerce-admin/stores-sales/payments/braintree#step-3-complete-the-advanced-settings)新信用卡/借記卡和[!DNL PayPal]帳戶。 以前，客戶只能在儲存付款以供稍後在結帳時完成交易時進行儲存和儲存，現在他們可以在無需建立新交易的情況下儲存新的信用卡/借記卡和PayPal帳戶。
...
* **訂單取消支援**。 [cancelOrder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/)突變可讓客戶取消訂單，傳遞其識別碼和取消原因。<!-- LYNX-197 -->
  ...
  * 中的新`order_cancellation_enabled`和`order_cancellation_reasons.description`回應欄位 `storeConfig`
    查詢支援使用者啟動的訂單取消請求。 請參閱[查詢商店的訂單取消設定](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config#query-a-stores-order-cancellation-configuration)
...
* 在包含GraphQL區塊的CMS頁面上，來自CMS GET作業的回應現在會如預期在Fastly快取中清除。 後續的排程更新現在會顯示這些頁面的正確更新內容。
...
* 在購物車層級套用的固定折扣金額現在正確。[GitHub-37496](https://github.com/magento/magento2/issues/37496)
...
* 已在`app/code/Magento/CatalogRule/Controller/Adminhtml/Promo/Catalog/NewConditionHtml.php`.[GitHub-38093](https://github.com/magento/magento2/issues/38093)中將`reponseBody`變數的名稱變更為`responseBody`
...
* 使用&#x200B;**[!UICONTROL Customers and Addresses (single file)]**&#x200B;選項匯入庫存來源與客戶地址現在可成功完成。
...
* 針對具有&#x200B;**[!UICONTROL Special Price]**&#x200B;值的產品選取自訂選項時（當&#x200B;**[!UICONTROL Price Display Settings]**&#x200B;設為含稅時），Magento Open Source現在會顯示&#x200B;**[!UICONTROL Regular Price]**&#x200B;值的正確值。
...
* 已為`HEAD`個元素將`crossorigin`屬性新增至`linkTypes`。[GitHub-32509](https://github.com/magento/magento2/issues/32509)
...
* 產品詳細資料頁面的Cumulative Layout Shift (CLS)值已有所改善。 （此量度量化頁面配置的視覺穩定性。）[GitHub-35410](https://github.com/magento/magento2/issues/35410)
...
* 已將`taxes`、`itemized_taxes`和`additional_itemized_taxes`擴充功能屬性新增至`Magento\Sales\Api\Data\OrderInterface`。 這些屬性可讓您使用Order REST呼叫儲存套用的稅捐。
...
* Magento Open Source現在已成功在`GET V1/products/sku/media`產品媒體API回應中傳回`content`屬性（base64影像代碼）。 `content`屬性先前未出現在回應中。
