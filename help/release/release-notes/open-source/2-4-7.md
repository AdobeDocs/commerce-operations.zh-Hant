---
title: Magento Open Source 2.4.7 — 測試版1發行說明
description: 瞭解2.4.7-beta1Magento Open Source版本中的新功能、增強功能、錯誤修正和已知問題。
...
# Magento Open Source 2.4.7 — 測試版1發行說明
...
此版本提高GraphQL的自訂屬性涵蓋範圍並增強GraphQL快取功能。
...
## Magento Open Source2.4.7 - beta1重要功能
...
此版本的安全性改善專案會改善對最新安全性最佳實務的合規性，包括此變更，這會造成現有REST端點的預設行為變更：

**透過REST和GraphQL API傳輸的付款資訊的原生費率限制**. 商戶現在可以設定使用REST和GraphQL傳輸之付款資訊的費率限制。 此新增的保護層可支援防止梳理攻擊，並可能會減少同時測試許多信用卡號碼的梳理攻擊的數量。 另請參閱 [速率限制](https://developer.adobe.com/commerce/webapi/get-started/rate-limiting/). <!-- AC-1271 -->
...
* Composer 2.5.x.與Composer 2.2.x的相容性保持不變。  <!-- AC-8129 -->

* **支援清漆快取7.3**. 此版本相容於最新版的Varnish Cache 7.3。6.0.x和7.2.x版本仍維持相容性，但我們建議僅將Magento Open Source2.4.7-beta1與Varnish Cache 7.3或6.0版LTS搭配使用。

* **RabbitMQ 3.11支援**. 此版本與最新版RabbitMQ 3.11相容。RabbitMQ 3.9仍支援相容性至2023年8月，但我們建議僅將Magento Open Source2.4.7-beta1與RabbitMQ 3.11搭配使用。
...
此版本新增增強的GraphQL快取功能，並針對客戶EAV屬性支援GraphQL架構，包括：

* **增強對自訂屬性的支援**. GraphQL自訂屬性支援已透過豐富API資料而增強，因此現在支援所有屬性型別。 GraphQL EAV屬性方案現在支援在管理員中擴充客戶屬性和客戶位址物件，以及使用GraphQL擷取它們。 特定的增強功能包括： <!-- LYNX-61 110-->
...
* GraphQL剖析器的效能已藉由減少 `parse` 系統會根據要求呼叫方法。 現在只呼叫一次。 之前，至少呼叫過剖析器三次。 <!-- B2B-2606 -->
...
此發行版本推出Magento Open Source擴充功能中繼套件v1.0.0-beta1，自動套裝 [付款服務](https://experienceleague.adobe.com/docs/commerce-merchant-services/payment-services/guide-overview.html) 擴充功能搭配此核心版本。 此中繼資料中包含的此擴充功能版本，會在 `composer update` 會執行，以簡化升級至最新核心發行版本時升級擴充功能的程式。 此擴充功能會維護獨立的發行排程。
...
PWA Studio v.13.2.x與Magento Open Source 2.4.7-beta1相容。 其中包含多項改善協助工具的增強功能。 如需有關錯誤修正的資訊，請參閱 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 另請參閱 [版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 以取得PWA Studio版本及其相容Magento Open Source核心版本的清單。
...
* 此 `addProductsToCart` 查詢現在已成功新增禮品卡到購物車。 以前，此查詢需要禮品卡收件者名稱，但會傳回寄件者名稱。 Magento Open Source擲回此錯誤： `Please specify a recipient name`. [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
* 此 `customer` 變數現在只會傳回與特定商店檢視相關的客戶評論，如按商店ID篩選時所預期。
...
* 此 `searchProducts` 查詢現在會傳回所有可用的彙總（篩選器）及其正確轉譯的標籤。 之前，價格和類別彙總標籤並未如預期般轉譯。 [GitHub-36140](https://github.com/magento/magento2/issues/36140)
...
* 新增為環境變數的非預設設定（網站或商店範圍）不再中斷 `app:config:import` 處理序發生遞回錯誤。
...
* Orders REST API現在會依順序傳回可設定與簡單產品的資訊，無論簡單產品的庫存狀態為何。 只有當選取的專案（產品）有庫存或可供銷售時，管理員使用者現在才能下訂單。 以前，此API只傳回已有產品的相關資訊。
...
* 現在，每個商店都會儲存Braintree付款資訊，而購物者現在只會看到儲存在自己購物商店中的卡片資訊。 過去，購物者在結帳時可以看到所有商店儲存的卡片資訊。 [GitHub-34253](https://github.com/magento/magento2/issues/34253)
...
**問題**： UPS已於2023年6月5日停止使用UPS XML API，並以下列方式影響商家：

* 目前客戶（即已產生UPS存取金鑰的商家）要到2024年6月才能受到影響。
...
您可以使用以下專案安裝Magento Open Source2.4.7-beta1： [作曲者](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
