---
title: Magento Open Source 2.4.7-beta3發行說明
description: 瞭解2.4.7-beta3Magento Open Source版本中的新功能、增強功能、錯誤修正和已知問題。
...
# Magento Open Source 2.4.7-beta3發行說明
...
Magento Open Source2.4.7-beta3引入對PHP 8.3的支援。其中包含超過150項品質修正和增強功能。 核心撰寫器相依性和協力廠商程式庫已升級至最新可用版本。
...
Magento Open Source2.4.7-beta3已針對下列元件版本進行測試：
...
## Magento Open Source2.4.7 - beta3重要功能
...
Magento Open Source2.4.7-beta3包含下列平台升級：
...
Magento Open Source2.4.7-beta3仍與PHP 8.1相容，僅供升級之用。 PHP 8.1不受支援，也不建議使用。 Magento Open Source2.4.7-beta3核心程式碼、所有隨附的擴充功能，以及所有Adobe擁有的擴充功能和SaaS服務都與PHP 8.3相容。 <!-- AC-9668 -->

* **RabbitMQ 3.11支援**. 此版本與最新版RabbitMQ 3.11相容。RabbitMQ 3.9仍支援相容性至2023年8月，但我們建議僅將Magento Open Source2.4.7-beta3與RabbitMQ 3.11搭配使用。

* **Composer 2.6.x**. 與Composer 2.2.x的相容性保持不變。 <!-- AC-9663 -->

* **支援清漆快取7.4**. 此版本相容於最新版的Varnish Cache 7.4。6.0.x和7.2.x版本仍維持相容性，但我們建議僅將Magento Open Source2.4.7-beta3與Varnish Cache 7.4或6.0版LTS一起使用。 <!-- AC-9662 -->
...
* Commerce UPS XML API閘道已移轉至新的Commerce UPS REST API，以支援UPS對其進行的更新 [API安全性模型](https://developer.ups.com/oauth-developer-guide). (UPS正在針對所有API實作OAuth 2.0安全性模型（持有人權杖）。) 所有先前的Commerce UPS XML API已從Magento Open Source2.4.7程式碼基底中移除。 <!-- AC-8918 -->

* 與FedEx的Magento Open Source整合已從舊版FedEx WSDL Web Services移轉至最新的FedEx RESTful API。 FedEx網站服務追蹤、地址驗證和驗證郵遞區號WSDLS將於2024年5月淘汰。 <!-- AC-1470 -->
...
Magento Open Source2.4.7包含增強的GraphQL快取功能、支援GraphQL架構的自訂屬性、支援headless訂單取消，以及改善解析器快取。
...
此版本包含Magento Open Source擴充功能中繼套件v1.0.0-beta2，可自動將特定Magento Open Source擴充功能與此核心版本整合。 此中繼資料中包含的此擴充功能版本，會在 `composer update` 會執行，以簡化升級至最新核心發行版本時升級擴充功能的程式。 此擴充功能會維護獨立的發行排程。

Magento Open Source 2.4.7-beta3的Magento Open Source擴充功能中繼資料包含下列擴充功能：
...
* Magento Open Source不再將產品說明文字插入產品詳細資料頁面 **[!UICONTROL Meta Description]** 此欄位留空時的欄位。 先前，當產品說明包含PageBuilder或其他HTML編輯器產生的HTML程式碼時，HTML會顯示在中繼資料中 **[!UICONTROL Description]** 欄位，因此您無法將此欄位留空。
...
* 當您嘗試在下列情況下為產品URL建立301/302重新導向，Magento Open Source不再擲回錯誤 **產生類別/產品URL重寫** 已啟用。 建立產品並指派給類別時，就會建立自動化URL重新寫入：一個用於直接URL，另一個用於透過類別路徑傳送的產品。 建立新產品URL重寫時，會檢查產品、類別和商店之間的關聯。 此檢查現在以關聯的實體屬性（類別和產品）為基礎。 之前，這項檢查是在預先存在的URL重寫時執行的。
...
您可以使用以下工具安裝Magento Open Source2.4.7-beta3： [作曲者](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
