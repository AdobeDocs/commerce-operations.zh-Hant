...
儘管這些功能的代碼與Magento Open Source核心代碼的發佈捆綁在一起，但其中一些專案也是獨立發佈的。 這些專案的 Bug 修復記錄在每個專案的文件中提供的特定於專案的單獨發佈資訊中。

## Magento Open Source 2.4.7 亮點

請檢視此版本中的下列重點專案。

### 安全性增強功能

此發行版本包含與Adobe Commerce 2.4.6-p5、2.4.5-p7和2.4.4-p8中包含的相同安全性修正和平台安全性改善。 請參閱[Adobe安全性公告](https://helpx.adobe.com/security/products/magento/apsb24-18.html)，瞭解這些已修正問題的最新討論內容。
...
* **對自動生成抵用券代碼**&#x200B;數量的限制。 Magento Open Source現在會限制自動產生的優惠券代碼數量。 預設最大值為250,000。 商戶可以使用新的&#x200B;**[!UICONTROL Code Quantity Limit]**&#x200B;組態選項(**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL Customers]** > **[!UICONTROL Promotions]**)，以防止許多優惠券可能讓系統不知所措。<!-- AC-8753 -->
...
* **添加了子資源完整性 （SRI） 支援** ，以符合PCI 4.0要求，以驗證付款頁面上的腳本完整性。 子資源完整性 （SRI） 支持為駐留在本地文件系統中的所有JavaScript資產提供完整性哈希。 預設 SRI 功能僅在管理員和店面區域的付款頁面上實現。 但是，商家可以將預設配置擴展到其他頁面。 請參閱 [Commerce PHP 開發人員指南&#x200B;_中的_&#x200B;子資源完整性](https://developer.adobe.com/commerce/php/development/security/subresource-integrity/)。<!--AC-11513-->

* **內容安全性原則(CSP)的變更**—Adobe Commerce內容安全性原則(CSP)的組態更新和增強功能，以符合PCI 4.0的要求。 如需詳細資訊，請參閱&#x200B;_Commerce PHP Developer Guide_&#x200B;中的[內容安全性原則](https://developer.adobe.com/commerce/php/development/security/content-security-policies/)。<!--AC-11512-->
...
* **PHP 8.3相容性**。 此發行版本引入對PHP 8.3的支援。Magento Open Source現在同時支援PHP 8.3和8.2。PHP 8.2將受支援直到其服務結束(EOS)日期為2025年12月。 在2025年12月之後，所有執行2.4.7部署的商家都應移轉至PHP 8.3。<!-- AC-8988 -->
...
* **RabbitMQ 3.13支援**。 此版本與最新版RabbitMQ 3.13相容。相容性仍與RabbitMQ 3.11和3.12相容，分別支援至2024年8月和2024年12月，但Adobe建議僅將Magento Open Source2.4.7與RabbitMQ 3.13搭配使用。<!-- AC-9659 -->

* **Composer 2.7.x**。 與Composer 2.2.x的相容性保持不變。<!-- AC-9663 -->

* **清漆快取7.4支援**。 此版本相容於最新版的Varnish Cache 7.4。6.0.x和7.2.x版本仍維持相容性，但我們建議僅將Magento Open Source2.4.7與Varnish Cache 7.4或6.0版LTS搭配使用。<!-- AC-9662 -->
...
Magento Open Source核心程式碼中的所有JavaScript程式庫和NPM相依性均已更新至最新可用版本。 所有Laminas程式庫相依性都已更新為與PHP 8.3相容的最新版本。<!-- AC-9667 9665 -->

#### 其他升級

* Commerce UPS XML API閘道已移轉至新的[Commerce UPS REST API](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/ups)，以支援UPS對其[API安全性模型](https://developer.ups.com/oauth-developer-guide)進行的更新。 (UPS正在針對所有API實作OAuth 2.0安全性模型（持有人權杖）。) 所有先前的Commerce UPS XML API已從Magento Open Source2.4.7程式碼基底中移除。<!-- AC-8918 -->

* Magento Open Source[與FedEx的整合](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/fedex)已從舊版FedEx WSDL Web Services移轉至最新的FedEx RESTful API。 FedEx網站服務追蹤、地址驗證及驗證郵遞區號WSDLS將於2024年5月淘汰。<!-- AC-1470 -->
...
* Temando送貨模組已從核心Magento Open Source程式碼庫中移除。 Magento Open Source2.4.4已棄用此功能。<!-- AC-8130 -->

### 效能與擴充性增強功能

Magento Open Source2.4.7包含下列Magento Open Source效能和擴充性的增強功能：
...
* **銷售規則效能改善**。 透過許多（約100,000個）有效的銷售規則，改善企業部署的效能。 大量實作促銷活動的企業部署通常會部署許多作用中的購物車規則。 在結帳作業期間，這些執行Magento Open Source2.4.7的企業部署型別不會看到任何與已設定購物車價格規則數量相關的效能降低。<!-- ACPT-1198 -->

* **為具有許多存儲**&#x200B;的部署更快地保存商店級配置的操作。 在具有500個以上存儲的部署中保存配置設置可能非常耗時。 新的 [異步配置](../../../performance/configuration.md#asynchronous-configuration-save) 模組通過運行使用使用者處理消息佇列中的保存操作的 cron 作業來實現異步配置保存操作。 AsyncConfig 預設為停用。
...
* 在Admin Configuration **中的**[!DNL Braintree]&#x200B;發行說明和支援連結 — 商家現在可以直接從Commerce Admin連結，以線上Braintree支援和發行說明。
...
* **已擴充對解析程式快取的支援**。 下列GraphQL查詢解析器現在可在GraphQL解析器結果快取中快取，以改進隨POST請求提交查詢時的效能：
...
* **支援訂單取消**。 [cancelOrder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) 突變允許客戶取消訂單，傳遞其標識符和取消原因。<!-- LYNX-197 -->

   * 中的`storeConfig`新增`order_cancellation_enabled`欄位和`order_cancellation_reasons.description`回應欄位
查詢支持用戶發起的訂單取消請求。請參閱[查詢商店的訂單取消設定](https://developer-stage.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/#query-a-stores-order-cancellation-configuration)
...
* **增強的 GraphQL 快取功能可提高頁面載入速度**。 快取功能已新增到這些查詢，改善了大部分PWA頁面的頁面載入時間速度：
...
此版本包含Magento Open Source擴充功能中繼套件v1.0.0，此中繼套件會自動將特定Magento Open Source擴充功能與此核心版本整合。 此中繼資料中包含的此擴充功能版本會在執行`composer update`時安裝，以簡化升級至最新核心版本時擴充功能的程式。 此擴充功能會維護獨立的發行排程。
...
PWA Studio v14.0與Magento Open Source 2.4.7-beta1相容。 其中包含多項改善協助工具的增強功能。 如需有關錯誤修正的資訊，請參閱[PWA Studio版本](https://github.com/magento/pwa-studio/releases)。 如需PWA Studio版本及其相容的Magento Open Source核心版本的清單，請參閱[版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/)。
...
* 已從設定程式中移除不必要的快取操控。 以前，Magento Open Source在執行`bin/magento setup:db-data:upgrade`或`bin/magento setup:upgrade`時將其設定寫入磁碟是不必要的，這會在安裝期間導致某些模組發生問題。 [GitHub-38124](https://github.com/magento/magento2/issues/38124)
...
* 刪除`app/etc/config.php`後，`bin/magento setup:install`現在成功完成。 之前，在安裝期間不會重新產生遺失的檔案，且Magento Open Source擲回錯誤。 [GitHub-37805](https://github.com/magento/magento2/issues/37805)
...
* 資料庫還原不再因分隔符號錯誤而失敗。 之前，執行`bin/magento setup:rollback --db`時Magento Open Source擲回此錯誤： `Syntax error or access violation: 1064 You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'delimiter' at line 1, query was: delimiter ;;`。
...
* 現在，控制面板上的圖表預設已成功停用。 [GitHub-38430](https://github.com/magento/magento2/issues/38430)
...
* 現在，您可以使用包含斜線(`/`)字元的SKU，將套件產品成功新增至管理員訂單。 之前，管理員使用者無法使用此方法將產品新增至管理員訂單，且Magento Open Source擲回JavaScript錯誤。

<!-- ACP2E-1799 -->

* 為對應的簡單產品啟用&#x200B;**[!UICONTROL Qty Uses Decimal]**&#x200B;時，管理員使用者現在可以為新新增的套件組合選項設定十進位預設值。 以前，小數值只能針對儲存的選擇範圍設定。
...
* 啟用配置設置后&#x200B;**[!UICONTROL Show out-of-stock products]**，`categoryList`查詢現在會按預期返回所有分發包選項。以前，缺貨期權不包括在查詢回應中。

<!-- ACP2E-1345 -->

* 現在， `POST V1/products` 當目錄價格範圍設置為 時 `website`，建立的捆綁產品會成功。 以前，會發生完整性約束衝突。 [GitHub-35595](https://github.com/magento/magento2/issues/35595)
...
* 在包含GraphQL區塊的CMS頁面上，來自CMSGET作業的回應現在會如預期在Fastly快取中清除。 後續的排程更新現在會顯示這些頁面的正確更新內容。
...
* 外掛程式的用途是在儲存與可設定產品相關聯的簡單產品時新增可設定快取標籤，現在可如預期運作。 當`POST V1/products`更新簡單產品時，所有相關的可設定快取標籤都會如預期移除。 以前，並非所有價格都在店面更新，必須手動清除緩存。 [GitHub-36726](https://github.com/magento/magento2/issues/36726)

### 購物車和結帳

<!-- ACP2E-2658 -->

* 現在，Magento Open Source 在啟用持久購物車時生成完整頁面可緩存頁面（如產品詳細信息頁面 （PDP） 和 CMS 頁面）時，不載載特定於用戶的報價數據，從而優化性能。 以前，系統會在這些頁面的生成過程中不必要地載入特定於用戶會話和報價數據，從而影響性能。

<!-- ACP2E-2684 -->

* 自訂位址自定義屬性現在可正常儲存在資料庫中。 以前，保存在資料庫中的屬性值前面都附加了屬性代碼。

<!-- ACP2E-2517 -->

* 改善產品屬性的購物車規則篩選文字產生。 單個產品不再重複載入。
...
* Magento Open Source現在可在觸發「新增至購物車」動作時显示信息性訊息。 以前，`The requested qty is not available`並不一定都會視需要在店面顯示。

<!-- ACP2E-2166 -->

* 對於擁有大型通訊簿的客戶，結帳頁面載入時間已得到改進。 Magento Open Source現在只會處理&#x200B;**[!UICONTROL Customer Addresses Limit]**&#x200B;設定中指定的客戶地址數目。 之前，Magento Open Source會載入整個通訊錄。
...
* 現在，當從伺服器中刪除會話時，本地儲存中的小型購物車和客戶數據會重置。 以前，此數據仍然在刪除工作階段檔時顯示。

<!-- ACP2E-1763 -->

* 啟用永久性購物車后，離散購物車不再錯誤地從管理員合併購物車。

<!-- ACP2E-1580 -->

* 購物車稅和運費估算器現在可以準確反映預設目的地配置。 先前，當您設定預設稅捐目的地計算設定並指定州/地區和ZIP時，**[!UICONTROL Estimate Tax and Shipping]**&#x200B;下的購物車中只會顯示ZIP值。 這是由於在從管理員叫用設定預設地區ID的條件陳述式之前，Magento Open Source將地區ID設定為undefined所導致。
...
* `cart`當指定購物車中的產品缺貨時，查詢不再傳回錯誤。以前，使用此查詢進行購買的購物者在商品缺貨時無法完成購買，並且結帳頁面為空白。
...
* `If an item is FOUND/NOT FOUND in the cart with ALL/ANY of these conditions true`目錄價格規則條件現在可正常用于類別和SKU屬性。之前，此條件未正確應用於價格抵用券邏輯購物車且無效優惠券已套用至訂單。 [GitHub-37660](https://github.com/magento/magento2/issues/37660)

<!-- ACP2E-1932 -->

* 您現在可以在停用擴充功能的情況下，成功 `Magento_OfflineShipping` 儲存新的購物車價格規則。 之前，Magento Open Source拋出這個錯誤： `report.ERROR: Warning: Undefined array key "simple_free_shipping"`。 [GitHub-37580](https://github.com/magento/magento2/issues/37580)

<!-- ACP2E-1516 -->

* 現在，當為商店配置與一種配送方式相關的購物車價格規則，並且此配送方式在結帳時更改為另一種配送方式時，將按預期應用購物車價格規則。 以往，`sales_order_item`表格中的`applied_rule_ids`值從未變更，而且購物車價格規則未套用至訂單。

<!-- AC-2044 -->

* 現在，當購物者在選擇統一運費方式后應用抵用券時，優惠券已成功應用於訂單，其中購物車價格規則將送貨方式設置為條件。 以前，購物車頁面顯示`Coupon is Not valid`錯誤，並且未套用優惠券。 [GitHub-34866](https://github.com/magento/magento2/issues/34866)

### 目錄

<!-- AC-10746 -->

* 除非另有指定，否則正在運行`bin/magento cache:clean`或`bin/magento cache:flush`不再刷新整個內置或 Varnish 全頁面緩存。 [GitHub-38301](https://github.com/magento/magento2/issues/38301)

<!-- ACP2E-2257 -->

* 商家現在可以使用產品輪播創建CMS區塊，前提是 `catalog_product_price dimensions-mode` 設置為 。`website`以前，Magento Open Source沒有保存塊並拋出此SLQ錯誤： `report.CRITICAL: PDOException: SQLSTATE[42S02]: Base table or view not found: 1146 Table`。
...
* 當此欄位留空時，Magento Open Source不再將產品描述文字插入產品詳細資料頁面&#x200B;**[!UICONTROL Meta Description]**&#x200B;欄位中。 先前，當產品說明包含頁面產生器或其他HTML編輯器產生的HTML代碼時，HTML會顯示在中繼&#x200B;**[!UICONTROL Description]**&#x200B;欄位中，您無法將此欄位留空。
...
* 最近檢視的產品信息現在會按照商店視圖設定顯示。 之前，最近查看的產品數據不會根據商店視圖進行更新，包括名稱和首選語言。

<!-- ACP2E-2296 -->

* Vimeo 視頻現在可以在基於 Chromium 的網路瀏覽器中以移動視圖的產品頁面上正常播放。

<!-- ACP2E-1959 -->

* 該 `bin/magento catalog:images:resize` 命令現在可為分配給具有自定義主題的自定義網站的產品正確生成產品圖像。

<!-- ACP2E-1982 -->

* 現在`NULL`，根據“**使用預設值值**”複選框，可以按預期保存帶有 or `NOT NULL` 值的元數據，以供預設商店使用。以前，取消選取此複選框時，不會儲存具有值中繼資料 `NULL` 。
...
* REST `V1/products/<sku>/media`端點現在可以同時處理多個媒體影像的上傳。 先前，當同時處理多個請求時，會建立不一致的資料，而當此資料儲存至資料庫時，Magento Open Source會擲回錯誤。
...
* 當管理員用戶將商店視圖從產品創建期間使用視圖更改為另一個商店視圖時，從管理員訪問可下載產品不再導致錯誤。 現在，當管理員使用者將商店檢視從所有商店檢視切換為預設商店檢視時，產品價格已成功轉換為浮點數。 之前，Magento Open Source擲回錯誤。 [GitHub-37519](https://github.com/magento/magento2/issues/37519)
...
* 類別樹狀結構中的產品計數（管理員&#x200B;**[!UICONTROL Catalog]** > **[!UICONTROL Category]**）現在準確。 之前，無法從`catalog_category_product_index`資料表中擷取類別產品計數。 修正了引發此錯誤的錯字。 [GitHub-35417](https://github.com/magento/magento2/issues/35417)

<!-- ACP2E-1239 -->

* 當禁用主題時 `product_image_white_borders` 頁面產品詳細資訊上的主產品圖片在載入 `view.xml` 期間不再明顯向下移動頁面。 以前，由於 Fotorama JS 中的高度設置不正確，產品圖像在載入期間頁面明顯向下移動。

<!-- ACP2E-1361 -->

* 現在，當客戶訂閱價格下降警報時，客戶會收到有關產品價格下降的通知。 以前，由於應用程式層級快取，不一定會傳送降價通知。

<!-- ACP2E-1422 -->

* 伺服器端驗證已新增至透過網页 API 建立產品屬性的程序。 當&#x200B;**[!UICONTROL Catalog Input Type for Store Owner]**&#x200B;欄位值為 `Text Field`、`Text Area`、`Text Editor`、`Date`或 `Date and Time`時，無法啟用&#x200B;**[!UICONTROL Use in Layered Navigation Catalog]**&#x200B;該欄位。以前，類別和結果中省略搜尋產品。
...
* 商戶現在可以從Admin將GIF影像新增至產品的影像庫。 之前，Magento Open Source顯示此錯誤： `imagecolorsforindex(): Argument #2 ($color) is out of range`。
...
* Magento Open Source現在會傳回自訂日期`01/01/1970`的有效結果。 先前，自訂屬性傳回了以下錯誤： `Invalid input datetime format of value '1/01/1970`。 [GitHub-37274](https://github.com/magento/magento2/issues/37274)

<!-- AC-6507 -->

* `routes` 使用片段的查詢現在可正常返回類別信息。 之前，類別頁面上發生內部伺服器錯誤。 [GitHub-35906](https://github.com/magento/magento2/issues/35906)
...
* `aggregate_sales_report_bestsellers_data` cron 作業不再創建非常大的臨時 MySQL 表，現在成功完成。此cron作業現在按商店插入數據，而不是一次插入所有儲存的數據。 以前，此cron作業可能會導致 `No space left on device` 錯誤。

<!-- ACP2E-2427 -->

* `cron_schedule`資料表中的擱置記錄數已減少，因為防止cron工作嘗試取得不必要的cron工作鎖定。 以前，cron 作業在運行作業為時已晚時會嘗試獲取鎖。 `cron_schedule`中的Cron工作未標籤為已錯過，即使它們已經超過排定的時間。 Magento Open Source也在錯誤記錄檔中重複顯示此錯誤： `report.WARNING: Could not acquire lock for cron job: indexer_update_all_views`。
...
* Magento Open Source現在根據配置設置在编辑客戶頁面上显示&#x200B;**[!UICONTROL Date of Birth]****[!UICONTROL Tax/VAT Number]****[!UICONTROL Gender]****[!UICONTROL Telephone]****[!UICONTROL Company]**、和&#x200B;**[!UICONTROL Fax]**&#x200B;字段。[GitHub-36196](https://github.com/magento/magento2/issues/36196)

### 電子郵件

<!-- ACP2E-2125 -->

* 包含變音符號或重音符號的電子郵件（即utf8電子郵件）現在支援客戶和公司，以及來賓和註冊客戶的訂單下單。 之前，Magento Open Source未儲存包含變音符號且顯示此錯誤的客戶記錄： `Something went wrong while saving the customer`。
...
* 通過使用導出帶有 Varnish 的 HTTP 標頭時添加 `UNCACHEABLE` 值來改進調試。 以前，`HIT`只會輸出 OR `MISS` 值。 [GitHub-37912](https://github.com/magento/magento2/issues/37912)
...
* 該 `varnish:vcl:generate` 指令現在包括一個 `input-file argument`. 這支援將自定義 VCL 添加到 Git 存放庫（相對於商務根目錄）。 [GitHub-37363](https://github.com/magento/magento2/issues/37363)

<!-- AC-10739 -->

* 已從代碼庫中移除對 Varnish 4.x 和 5.x 的支援。 [GitHub-38304](https://github.com/magento/magento2/issues/38304)
...
* 不符合 [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html)  的字元已從基底程式代碼中移除。 [GitHub-38355](https://github.com/magento/magento2/issues/38355)

<!-- AC-10692 -->

* 修正了類別中 `Magento\Reports\Block\Adminhtml\Shopcart\Product\Grid`不正確的 PHP 文件標記。 [GitHub-38186](https://github.com/magento/magento2/issues/38186)

<!-- AC-10865 -->

* 在編碼實踐時創建的整個代碼庫中重構的代碼規定檔中包含的`app/etc/env.php`任何緩存部分都將包括前端部分。 此重構解決了升級 2.4.4 之前的部署Magento Open Source潛在問題。 [GitHub-38363](https://github.com/magento/magento2/issues/38363)

<!-- AC-8005 -->

* 闡明瞭在 API 調用期間提供值為 0 的股票 ID 時關聯的 `StockItemValidator.php` 錯誤消息。 [GitHub-31107](https://github.com/magento/magento2/issues/31107)
...
* 重構`Magento_CatalogWidget`模組，將轉義函數`$escaper`替換為`$block`轉義函數。[GitHub-37107](https://github.com/magento/magento2/issues/37107)

<!-- AC-9336 -->

* `Swaziland` 已更新至 `Eswatini` 整個程式代碼庫。 [GitHub-37873](https://github.com/magento/magento2/issues/37873)

<!-- ACP2E-2638 -->

* 將新的尼加拉瓜貨幣（NIO）添加到可用貨幣清單中。
...
* `User`模組中的JSON序列化程式已更換為更精確的新序列化程式(`JsonHexTag`)。 以前，當包含未逸出的元素時，嘗試使用由規則序列化程式序列化的資料時，Magento Open Source有時會當機。 [GitHub-31377](https://github.com/magento/magento2/issues/31377)
...
* AwsS3 驅動程序現在按預期工作，作為基本默認文件儲存實施的替代品。 以前，將此驅動程式配置為文件儲存時，數據可能會損壞。 [GitHub-37844](https://github.com/magento/magento2/issues/37844)
...
* 現在，當管理員使用者的Magento Open Source執行個體連線至S3儲存空間時，管理員使用者可以產生發票。 之前，Magento Open Source顯示此錯誤： `Cannot create image resource. File not found`。 [GitHub-35706](https://github.com/magento/magento2/issues/35706)
...
* 已移除不需要空白字元的`id`屬性。 (此錯誤是由Magento Open Source2.4.6程式碼變更所造成。)
...
* 當用戶嘗試打印發貨標籤時，Magento Open Source不再在記錄檔中引發異常。 以前，如果擴展修改包含附件的響應標頭，系統會在記錄檔中引發異常。

<!-- AC-7986 -->

* 已新增對更多HTML類別的支援到`elements.xsd`。 [GitHub-36891](https://github.com/magento/magento2/issues/36891)
...
* 包含表情符號的 CMS 內容塊在保存到資料庫時不再在表情符號之後截斷。 以前，內容 在表情符號后被截斷，因為默認資料庫配置不支持四位元組字元。

<!-- AC-8823 -->

* 現在當登入頁面上的複選框處於活動狀態時 **[!UICONTROL Show Password]** ，密碼會按預期顯示。
...
* 啟用 后 **[!UICONTROL RSS Feed Top Level Category]** ，在請求類別的 RSS 摘要時不再發生錯誤。 以前，流覽 RSS 摘要類別頁面會導致啟用時&#x200B;**[!UICONTROL RSS Feed Top Level Category]**&#x200B;日誌文件中出現Elasticsearch`CRITICAL`錯誤。
...
* 啟用該設定后， **[!UICONTROL Add Store Code to Urls]** 按兩下「管理員登入頁面的管理員重設密碼連結，現在可以訪問重設密碼頁面。 管理員重設密碼連結先前會開啟登入頁面或404頁面。

<!-- ACP2E-1274 -->

* 管理員統計信息儀錶板中的日期範圍現在根據配置設置中設置的時區計算，然後轉換為 UTC 以從資料庫中獲取數據。

<!-- ACP2E-1665 -->

* 篩選商店視圖 **[!UICONTROL Save]** 然後按兩下管理員 **[!UICONTROL Content]** > **[!UICONTROL Hierarchy]**“後，CMS 階層篩選器將按預期工作。 以前，按兩下 **[!UICONTROL Save]** 會刷新頁面，但內容會丟失，並且不再顯示所選商店視圖。

<!-- ACP2E-1631 -->

* 現在，按下 **[!UICONTROL Send invitation]** 按鈕（管理員 **[!UICONTROL Marketing]** > **[!UICONTROL Private sales]** > **[!UICONTROL Invitation]**）將提交POST 要求以及表單密鑰並成功發送邀請。 以前，當您按兩下此按鈕時，HTTP POST 方法和表單密鑰都不存在。

<!-- ACP2E-1608 -->

* 現在可以使用 AWS S3 將零位元組檔成功複製到遠端儲存。

<!-- AC-7402 -->

* 現在，當為其他網站（其商店ID與網站ID不符）建立該客戶時，管理員使用者可以從管理員新增客戶地址。 之前，Magento Open Source會顯示以下快顯視窗訊息： `Something went wrong`。 [GitHub-36582](https://github.com/magento/magento2/issues/36582)
...
* 標幟、其解析程序、緩存和關聯測試的 `is_subscribed` 聲明已從 `CustomerGraphQl` 模組移至 `NewsletterGraphQl` 模組。

<!-- AC-9362 -->

* 突變不再 `addProductsToCart` 報告 中 `user_errors`不相關的錯誤。 以前，與購物車相關的錯誤與預期的操作錯誤一起包含在內 `user_errors` 。 [GitHub-37908](https://github.com/magento/magento2/issues/37908)

<!-- ACP2E-2119 -->

* `products`查詢產品篩選器現在可以傳回部分匹配結果以及完全匹配結果。新 `match_type` （PART， FULL） 屬性支援指定符合類型。 之前，查詢只會傳回完整相符專案。
...
* 您現在可以在查詢`ProductInterface`上的`related_products`、`upsell_products`和`crossell_products`時使用片段`ProductCard`。 [GitHub-29769](https://github.com/magento/magento2/issues/29769)
...
* 當沒有要匯入的有效資料時，驗證檢查現在會如預期停止匯入程式，而Magento Open Source現在會顯示此錯誤： `There are no valid rows to import`。 先前在這些條件下通過驗證，但匯入程式失敗，出現此訊息： `entity values are mixed`。 [GitHub-32905](https://github.com/magento/magento2/issues/32905)
...
* Magento Open Source現在會在匯入客戶記錄時考慮&#x200B;**[!UICONTROL Disable Automatic Group Change Based on VAT ID]**&#x200B;設定。 先前已忽略匯入檔案中的`disable_auto_group_change`值。 [GitHub-36409](https://github.com/magento/magento2/issues/36409)
...
* 改進了未設置目錄規則時索引器的性能  `catalogrule_product` 。 [GitHub-34784](https://github.com/magento/magento2/issues/34784)
...
* `bin/magento indexer:reindex customer_grid` 當客戶網格包含新創建的自定義客戶屬性時，不再失敗並顯示 MySQL 錯誤。 [GitHub-36233](https://github.com/magento/magento2/issues/36233)
...
* 已改良為具有許多屬性選項的產品編製索引。 [GitHub-36386](https://github.com/magento/magento2/issues/36386)
...
* 當格式錯誤的HTTP請求傳送至伺服器時，Magento Open Source會顯示的錯誤訊息中，不再包含伺服器根目錄的完整路徑。 現在只有字串會作為第二個引數傳遞至`explode`方法。
...
* 已新增烏克蘭區域至`directory_country_region`表格。 [GitHub-35187](https://github.com/magento/magento2/issues/35187)
...
* 商戶現在可以在產品啟用&#x200B;**[!UICONTROL No Manage Stock]**&#x200B;設定時通知購物者，訂單已準備好取貨。 先前，啟用此設定時，Magento Open Source會顯示此錯誤訊息： `Your order is not ready for pickup`。

<!-- ACP2E-2227 -->

* 商戶現在可以為訂單建立銷退折讓單，其中已刪除與可設定產品相關聯的簡單產品。 之前，商家無法建立銷退折讓單，且Magento Open Source擲回`TypeError`例外狀況。
...
* 現在，在如預期啟動帳戶后，會將客戶重新導向至 登入 頁面。 在過去，客戶是自動登入的。

### 通訊

<!-- AC-10806 -->

* Commerce 現在會在接受新聞稿的新訂閱者之前檢查是否啟用了新聞稿訂閱功能。 [GitHub-33040](https://github.com/magento/magento2/issues/33040)

### 訂購

<!-- AC-8857 -->

* 從管理購物車移動到要訂購的專案清單的產品現在將按預期從購物車中刪除。 [GitHub-37538](https://github.com/magento/magento2/issues/37538)

<!-- ACP2E-2535 -->

* 商戶現在可以如預期建立部份退貨的銷退折讓單，而部份退貨是由獎勵點數所支付。 以前，當商家創建部分退貨時，訂單狀態被錯誤地標識為已關閉，並且管理員訂單頁面不顯示貸項通知單選項。

<!-- ACP2E-2583 -->

* Magento Open Source 現在會將正確的折扣金額套用至部分取消的訂單。 以前，更改訂單時，列中 `base_discount_cancelled` 計算的折扣金額計算不正確。 對負值的支援已添加到折扣訂單的發票中。

<!-- ACP2E-2412 -->

* Magento Open Source現在根據管理員用戶在管理員中創建訂單時提供的排序順序對自定義客戶地址屬性進行排序。

<!-- ACP2E-2400 -->

* 當商家從此頁面起始搜尋時，「管理銷售訂單網格」頁面現在會如預期般載入。 先前，當商家嘗試從這個表格搜尋訂單時，頁面凍結，且Magento Open Source在瀏覽器主控台中顯示`Request-URI Too Long`錯誤。
...
* 當出貨價格包含稅捐，且稅捐計算是在折扣後套用時，啟用免運費時，銷退折讓單頁面不再損毀。 之前，Magento Open Source擲回`Division by zero`例外狀況。 [GitHub-36800](https://github.com/magento/magento2/issues/36800)
...
* 停用`Magento_AsyncConfig`模組時，HTML轉換的內容現在會如預期顯示在電子郵件範本中。 以前，電子郵件範本僅包含原始 HTML。
...
* 當您編輯超連結並儲存範本時，頁面產生器不再將`tabindex`HTML屬性新增到超連結標籤`a`。
...
* 現在，將文本從一個 頁面 Builder 文本編輯者複製並粘貼到另一個 Builder 文本可以正常工作。 以前，您嘗試粘貼文本的頁面無法保存，並且Magento Open Source顯示此控制台錯誤： `[ERROR] Page Builder was rendering for 5 seconds without releasing locks.`。

<!-- ACP2E-1568 -->

* 頁面以前以旧版 頁面 Builder 建立的「產生器 `column-block` 」內容類型現已可正確顯示。

<!-- ACP2E-1234 -->

* 改進了在建立產品期間從管理員篩選產品中繼資料描述的功能。 以前，如果啟用了頁面生成器，則會根據預設` {{name}} {{description}}` 掩碼生成產品中繼資料描述。 默認產品元描述掩碼現在由 組成 `{{name}}`。 [GitHub-36107](https://github.com/magento/magento2/issues/36107)
...
* 已解決 頁面 Builder 內容 中欄的显示問題。 之前， `column-line` 內容類型不會呈現內容。 [GitHub-3949](https://github.com/magento/pwa-studio/issues/3949)
...
* **[!UICONTROL Card Security Code Does Not Match]**&#x200B;配置設置現在在 Payflow Pro 付款過程中按預期處理。以前，此評估過程從未完成，因此阻止了簽出。
...
* **[!UICONTROL Venmo]**&#x200B;現在可以在啟用PayPal按鈕時隱藏按鈕。按鈕無法 **[!UICONTROL Venmo]** 從購物車、迷你購物車、產品詳情或結帳页面移除。

<!-- BUNDLE-3313 -->

* 購物者現在可以在結帳時應用抵用券代碼時選擇送貨方式。 以前在這些條件下，Magento Open Source拋出錯誤併為訂單分配了統一運費。
...
* 解決了啟用本地付款時，由Ideal 付款支付的訂單的後援 URL 問題。
...
* 現在，當卡片付款保險庫被禁用時，購物者可以使用Braintree付款方式結帳運送到多個位址的訂單。 [GitHub-37615](https://github.com/magento/magento2/issues/37615)
...
* 啟用&#x200B;**[!UICONTROL Enable Vault for Card Payments]**&#x200B;時，為由Magento Open Source支付的訂單建立第二張部份發票時，Braintree不再擲回錯誤。 先前，當商家嘗試建立第二張商業發票時，Magento Open Source擲回此錯誤： `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`。

<!-- ACP2E-1229 -->

* 當購物者更新其帳單地址時，結帳工作流程中不再自動取消選取「Braintree儲存庫付款方式」選項。 以前，當購物者取消選取&#x200B;**[!UICONTROL My billing and shipping address are the same]**&#x200B;按鈕時，會取消選取付款方法。
...
* 在&#x200B;**銷售** > **訂單**&#x200B;頁面的整批動作下拉式功能表中按多下，就不會再產生多個POST要求。  [GitHub-37997](https://github.com/magento/magento2/issues/37997)

<!-- AC-10944 -->

* 只有當購物者導航到觸發 301 重新導向的頁面時，才會 `200` 緩存 或 `404` 回應，除非頁面的 Cache-Control 標頭包含 `private `。 以前，緩存緩存控制標頭中帶有 `private` 的 HTTP 狀態 200，並且緩存 HTTP 狀態 301，除非它包含 `private` 在其 Cache-Control 標頭中。 [GitHub-36492](https://github.com/magento/magento2/issues/36492)
...
* 使用許多自訂產品屬性執行時，`addProductToCart`突變的效能已改善。 之前，當使用許多產品屬性（約1000）執行此變異時，Magento Open Source會嘗試載入所有產品屬性。

<!-- ACP2E-1965 -->

* 已解決載入購物車規則使用的產品屬性時的效能問題。 現在只會載入使用中購物車規則的產品屬性。
...
* reCAPTCHA 接口工具集現在位於 **[!UICONTROL Place Order]** 付款頁面上的按鈕前面。 以前，介面工具集顯示在頁面頂部。

### 報表

<!-- ACP2E-1541 -->

* 當訂單由使用非預設貨幣的商店生成時，訂單報告（**[!UICONTROL REPORTS]** > **[!UICONTROL Sales]** > **[!UICONTROL Orders]**）現在包括正確的訂單值。 以前，此報表包含不正確的值。 [GitHub-36742](https://github.com/magento/magento2/issues/36742)

<!-- ACP2E-1477 -->

* 為清楚起見，已修訂銷售訂單報表表窗體中的誤導性資訊。

<!-- ACP2E-1435 -->

* 管理員用戶現在可以搜尋包含“放棄購物車報表”頁面中（加號）符號的電子郵件 `+` 的記錄。 [GitHub-36560](https://github.com/magento/magento2/issues/36560)

### 搜索 {#search-heading}

<!-- ACP2E-2272 -->

* 標頭的迷你搜尋前端上的自動完成搜尋建議現在可以按預期工作。 以前，啟用此功能時，自動完成功能不會填入此欄位。 [GitHub-37557](https://github.com/magento/magento2/issues/37557)

<!-- ACP2E-2101 -->

* 通過添加索引器，從管理搜尋欄進行的查詢的性能得到了提高。 以前，表格越大`search_query`，完成查詢所需的時間就越久。
...
* 當&#x200B;**[!UICONTROL Price Display Settings]**&#x200B;設定為含稅時，針對具有&#x200B;**[!UICONTROL Special Price]**&#x200B;值的產品選取自訂選項時，Magento Open Source現在會顯示&#x200B;**[!UICONTROL Regular Price]**&#x200B;值的正確值。
...
* 在「商店檢視」範圍中建立現有可設定產品的新變體時，新建立之簡單產品的`tax class`值現在已設定為父產品的值。 此值以前設為none。 因此，在購物車中，這些產品沒有增加任何稅款，並且向客戶少繳了稅款。 [GitHub-37180](https://github.com/magento/magento2/issues/37180)

### 測試

<!-- ACP2E-2501 -->

* 改進測試 `AdminCreateOrderWithVirtualProductFromDefaultSourceWithBackorders` ，以消除因過早填寫電子郵件欄位而導致的失敗。
...
* `AdminSimpleProductwithTextandVisualSwatch`通過引入一個額外的步驟來明確設置要在搜尋中使用的屬性，從而修復了測試。

<!-- AC-9194 -->

* 更正了單元測試中的 `SortbyTest` 錯別字。 [GitHub-37792](https://github.com/magento/magento2/issues/37792)

<!-- AC-9035 -->

* 刪除了對單元測試中不存在的類的引用。 [GitHub-37636](https://github.com/magento/magento2/issues/37636)
...
* `\Magento\Test\Integrity\Di\CompilerTest::testConstructorIntegrity`測試現在會驗證`ComponentRegistrar()`所登入之模組中的所有PHP類別。
...
* 現在，影像上傳程式可在動態列中正常運作。 現在以多國陣列形式傳送影像時，可以正確上載影像。 [GitHub-37957](https://github.com/magento/magento2/issues/37957)

<!-- AC-10877 -->

* 檔案上傳樣式不再出現在 Luma 主題中。 [GitHub-32373](https://github.com/magento/magento2/issues/32373)

<!-- AC-9984 -->

* 街道欄位客戶帳戶編輯地址表單中的街道欄位現在已正確儲存。 這些欄位現在已按表單中的指定鍵正確排序。 [GitHub-37973](https://github.com/magento/magento2/issues/37973)
...
* 現在，當 客戶 註冊 窗體包含多选客戶屬性時，它會按預期載入。 之前，Magento Open Source拋出此異常： `[2023-03-05T05:57:40.032602+00:00] report.CRITICAL: Exception: Deprecated Functionality: explode(): Passing null to parameter #2 ($string) of type string is deprecated in /magento2ee/app/code/Magento/CustomAttributeManagement/Block/Form/Renderer/Multiselect.php on line 53 in /lib/internal/Magento/Framework/App/ErrorHandler.php:62`. [GitHub-37189](https://github.com/magento/magento2/issues/37189)
...
* 當您嘗試在「管理員順序」頁面中的評論歷程記錄與傳回之間快速變更檢視時，Magento Open Source不再擲回錯誤。 之前，Magento Open Source顯示此一般錯誤： `A technical problem with the server created an error`。
...
* 管理員用戶現在可以在為商店 ID 與網站 ID 不匹配的另一個網站創建客戶時，從管理員添加新的 客戶 地址。 之前，Magento Open Source會顯示以下快顯視窗訊息： `Something went wrong`。 [GitHub-36582](https://github.com/magento/magento2/issues/36582)

<!-- ACP2E-1775 -->

* 與在訂單詳細資訊頁面上提交評論相關的錯誤訊息，現在可提供更多資訊。 先前，當購物者按一下&#x200B;**[!UICONTROL Submit comment]**&#x200B;而未變更訂單狀態或未在評論區中輸入文字時，Magento Open Source會顯示此訊息： `The comment is missing. Enter and try again`。

<!-- ACP2E-1659 -->

* 為一個欄位選取日期後，行事曆圖示現在在管理員&#x200B;**[!UICONTROL Marketing]** > **[!UICONTROL Cart Price Rules]**&#x200B;的日期範圍欄位中顯示為預期。 先前，您為一個欄位選取日期後，其他欄位不再顯示行事曆圖示，並Magento Open Source顯示預留位置文字。 [GitHub-36499](https://github.com/magento/magento2/issues/36499)
...
* 現在URL重寫是基於多商店部署中的商店級可見性配置生成的。 以前，當產品可見性設置為 **在預設商店範圍下不可見** 時，不會為非預設商店生成URL重寫。

<!-- AC-10464 -->

* `Category::getUrl()` 不再擷取 301 重新導向。 [GitHub-38152](https://github.com/magento/magento2/issues/38152)

<!-- ACP2E-2629 -->

* Magento Open Source 在為自定義商店生成缺失的URL重寫時，不再為根類別和在多商店設置中設置為 **[!UICONTROL Not Visible Individually]** 的產品生成產品URL重寫。

<!-- ACP2E-2481 -->

* 規範 URL 標記現在可在更新 類別 URL Key 屬性 后反映變更。 以前，使用過時的類別URL，這會立即觸發到新URL的重定向。
...
* 啟用&#x200B;**產生類別/產品URL重寫**&#x200B;時，當您嘗試為產品URL建立301/302重新導向時，Magento Open Source不再擲回錯誤。 創建產品並將其分配給類別時，將創建自動重寫URL：一個用於直接URL，另一個用於通過類別路徑的產品。 創建新產品URL重寫時，將檢查產品、類別和商店之間的關聯。 此檢查現在以關聯的實體屬性 （類別 和 產品） 為基礎。 以前，此檢查是在預先存在的URL重寫上執行的。

<!-- ACP2E-1737 -->

* 現在類別從CSV文件導入新類別時，會為所有商店視圖生成URL重寫。

<!-- ACP2E-1700 -->

* 產品URL後綴和類別URL後綴設定現已可與表格中的 `url_rewrite` 後綴相符。 以前，更新URL後綴配置時會出現爭用條件，並且配置的URL後綴和 `url_rewrite` 表格不匹配。

<!-- ACP2E-1703 -->

* Magento Open Source現在在創建類別URL重寫時在所有級別呈現類別樹。 以前，只會顯示類別的前三個層級。 [GitHub-36973](https://github.com/magento/magento2/issues/36973)
...
* 當&#x200B;**[!UICONTROL Callback URL]**&#x200B;和&#x200B;**[!UICONTROL Identity link URL]**&#x200B;欄位空白時，Magento Open Source不再在「管理員&#x200B;**[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]**」中顯示`Integration not secure`警告。 [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* 層價格 REST API 現在支援十進位數量。 以前，此 API 不允許在啟用該 **[!UICONTROL Use Decimal Quantity]** 設置時平均十進位數量。

<!-- AC-8998 -->

* 嘗試向待處理訂單的狀態添加註釋時，REST `V1/orders/{id}/comments` 端點不再更改訂單的狀態。 它現在給出了一個資訊性錯誤。 以前，此端點在添加註釋時更改訂單狀態。

<!-- ACP2E-2275 -->

* REST API `GET` 和`POST V1/products/attributes`終結點不再為`is_filterable`選項的屬性&#x200B;**[!UICONTROL Use in Layered Navigation]**&#x200B;和`Filterable(no results)`選項返回`Filterable(with results)`相同的值。以前，此端點返回相同的值，因為 `is_filterable` 屬性屬性的類型 `Boolean`為 ，不允許將此屬性 `Filterable(no results)`設置為 。 [GitHub-37979](https://github.com/magento/magento2/issues/37979)
...
* REST `V1/carts/mine/estimate-shipping-methods`方法現在會驗證所有層級的裝載欄位資料，並傳回400 HTTP程式碼中的無效資料。 之前，僅驗證第一層的API裝載，如果以未驗證的裝載層級傳回無效的資料，Magento Open Source會傳回500狀態代碼。
...
* `POST /V1/products/tier-prices`端點現在會在層級價格建立期間使用無效的客戶群組名稱時，傳回資訊更豐富的錯誤訊息。 之前，Magento Open Source傳回類似以下的錯誤： `Notice: Undefined index: XXXXX in ~/public_html/vendor/magento/module-catalog/Model/Product/Price/Validation/TierPriceValidator.php on line 474"`。 [GitHub-36431](https://github.com/magento/magento2/issues/36431)
...
* Magento Open Source現在已成功傳回`GET V1/products/sku/media`產品媒體API回應中的`content`屬性（base64影像代碼）。 `content`屬性先前未出現在回應中。
...
您可以使用[撰寫器](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html)安裝Magento Open Source2.4.7。
