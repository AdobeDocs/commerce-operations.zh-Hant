...
* **內容安全策略 （CSP） 的更改 — Adobe Systems商務內容安全策略 （CSP）** 的配置更新和增強功能，以符合 PCI 4.0 要求。 有關詳細資訊，請參閱[商務 _PHP 開發人員指南_&#x200B;中的內容安全策略](https://developer.adobe.com/commerce/php/development/security/content-security-policies/)。<!--AC-11512-->
...
* [Adobe Systems Commerce 與 Adobe Systems IMS 整合](https://experienceleague.adobe.com/docs/commerce-admin/start/admin/ims/adobe-ims-integration-overview.html?lang=zh-Hant)
...
* 現在對設計主題配置處理器中的陣列類型強制實施類型檢查。 [GitHub-34440](https://github.com/magento/magento2/issues/34440)

### 庫存管理

<!-- ACP2E-1883 -->

* 現在，在包含產品數量退貨的貸項通知單後，產品會自動返回到“有貨”狀態。 以前，如果產品缺貨，但貸項通知單退回了該產品的庫存，則商家必須手動更改產品庫存狀態。

<!-- ACP2E-2123 -->

* 通過刪除重新索引預設源項的冗餘請求，提高了清單索引器在異步模式下的性能。

<!-- AC-7023 -->

* 更正了影響可配置產品的店面庫存計數準確性的庫存索引器問題。 [GitHub-36421](https://github.com/magento/magento2/issues/36421)

<!-- ACP2E-2082 -->

* 現在，通過 REST `V1/inventory/source-items` 更新庫存源專案的速度更快。 此呼叫不再觸發對未更改的庫存產品執行重新編列索引或清理緩存作。
...
* 當您使用“從Stock移至底部”選項對產品進行排序時，類別頁面不再重定向到管理儀錶板。 問題已解決，方法是先確定條件陳述式尚未包含SQL聯結陳述式，再將SQL聯結陳述式放在條件陳述式內。 以前，頁面會重新導向至管理員儀表板，訊息為： `Invalid security or form key. Please refresh the page`。
...
* 現在，從一個Page Builder文字編輯器複製文字並貼上文字到另一個編輯器可以如預期運作。 Previously, the page into which you tried to paste text could not saved, and Magento Open Source displayed this console error: `[ERROR] Page Builder was rendering for 5 seconds without releasing locks.`.
...
* Magento Open Source現在會防止在此錯誤發生時建立訂單，以正確處理PayPal的502錯誤。 以前，儘管 PayPal 出現 502 錯誤，但仍會創建訂單，這銷售機會Magento Open Source記錄與PayPal記錄之間的不一致。

<!-- AC-9142 -->

* PayPal Express Checkout 交易現在包含子產品 SKU 和完整產品標題。 該文件 `Magento/Paypal/Model/Cart.php` 現在收集子產品 SKU 及其父產品標題，以標識帶有子SKU的產品標題。 [GitHub-36808](https://github.com/magento/magento2/issues/36808)
...
* 當購物者沒有保險庫令牌時，保險櫃付款方式選項不再顯示在管理員結帳工作流程中。 [GitHub-36273](https://github.com/magento/magento2/issues/36273)
...
* 重構 `Magento\GraphQl\Catalog\ProductSearchTest::testFilterProductsBySingleCategoryId`測試以處理查詢回應中的 `products` 隨機類別順序。

<!-- AC-6842 -->

* `\Magento\Test\Integrity\Di\CompilerTest::testConstructorIntegrity`測試 現在驗證由 註冊的`ComponentRegistrar()`模組中的所有 PHP 類。

<!-- AC-7098 -->

* 中 `TableNameSniff` 大型數位的性能問題已得到解決。
...
* 啟用&#x200B;**產生類別/產品URL重寫**&#x200B;時，當您嘗試為產品URL建立301/302重新導向時，Magento Open Source不再擲回錯誤。 創建產品並將其分配給類別時，將創建自動重寫URL：一個用於直接URL，另一個用於通過類別路徑的產品。 建立新產品URL重寫時，會檢查產品、類別和商店之間的關聯。 此檢查現在以關聯的實體屬性（類別和產品）為基礎。 之前，這項檢查會對預先存在的URL重寫執行。
