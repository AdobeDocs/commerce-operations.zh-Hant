---
title: Magento Open Source2.4.3發行說明
description: 瞭解2.4.3Magento Open Source版中的新功能、增強功能、錯誤修復和已知問題。
...
# Magento Open Source2.4.3發行說明

Magento Open Source2.4.3引入了效能和安全性的增強，並對平台進行了重大改進。 安全增強包括擴大reCAPTCHA覆蓋範圍和包括內置速率限制。 核心合成器依賴項和第三方庫已升級到與PHP 8.x相容的最新版本。頁面生成器現在可作為Magento Open Source中的捆綁擴展。 它現在是Adobe Commerce和Magento Open Source的預設內容編輯工具。

此版本包括370多個針對核心代碼的新修復和33個安全增強功能。 它包括由我們的社區成員解決近290個GitHub問題。 這些社區貢獻包括對核心代碼的輕微清理，以及GraphQL的重大改進。

此版本包括370多個針對核心代碼的新修復和33個安全增強功能。 本版本中已解決了Magento Open Source2.4.2發行說明中確定的所有已知問題。
...
升級到2.4.3Magento Open Source時可能出現以下致命錯誤：

```terminal
PHP Fatal error: Uncaught Error: Call to undefined function Magento\Framework\Filesystem\Directory\str_contains() in [...]/magento/vendor/magento/framework/Filesystem/Directory/DenyListPathValidator.php:74
```

此錯誤源於 `str_contains` 函式，即PHP 8.x函式。 應用程式Open Source 2.4.3不支援PHP 8.x。此修補程式將此函式替換為受支援的PHP 7.x函式。 查看 [Adobe Commerce升級2.4.3, 2.3.7-p1 PHP錯誤修補程式](https://support.magento.com/hc/en-us/articles/4408021533069-Adobe-Commerce-upgrade-2-4-3-2-3-7-p1-PHP-Fatal-error-Hotfix) 知識庫文章。
...
* Magento Open Source2.4.3已經測試並確認與Redis相容6.0.12。 （版本2.4.x仍與Redis 5.x相容。）

* Laminas庫依賴項已升級為PHP 8.x相容版本。 已從 `composer.json` 的子菜單。 **Magento Open Source2.4.3使用Laminas 3.4.0**。 <!-- MC-39513-->
...
此版本為共用路由添加了GraphQL覆蓋。 的 [路由查詢](https://devdocs.magento.com/guides/v2.4/graphql/queries/route.html) 和 [可路由介面](https://devdocs.magento.com/guides/v2.4/graphql/interfaces/routable-interface.html) 支援產品、類別和CMS頁上的路由請求。 的 `urlResolver` 查詢已被棄用，其功能已被 `route` 的子菜單。
...
* 當新可配置產品的建立失敗時，配置值現在將保留在表單重裝中。 以前，在重新載入表單期間丟失了值，應用程式顯示了以下錯誤： `The value specified in the URL Key field would generate a URL that already exists`。 [GitHub-32102](https://github.com/magento/magento2/issues/32102)
...
* 的 `updateProductsInWishlist` 現在，突變成功更新了屬於願望清單中捆綁產品的項目。 以前，此變異沒有更新願望清單項，而是刪除了項並建立了新項，從而更改了項ID。
...
* Magento Open Source在更改「管理相關產品」、「追加銷售」和「交叉銷售」清單的每頁值時，會按預期更新總頁數。 [GitHub-31059](https://github.com/magento/magento2/issues/31059)
...
* 全局Magento Open Source插件(例如， `webapi_rest` 和 `graphql` )不再觸發新自定義區域類型 `di.xml` 的插件。
...
* 應用程式現在將在CSV檔案導入期間刪除追加銷售、交叉銷售和相關產品之間的產品關係，如預期時間 `__EMPTY__VALUE__` 在CSV檔案中指定。
...
* 的效能  `catalog_product_alert` 在大型表（幾百萬行）上運行cron進程已得到改進。 以前， `catalog_product_alert` 已載入所有產品警報，導致記憶體不足異常。
...
* 使用Safari的管理員現在可以成功將產品添加到管理員的訂單中。 以前，當管理員按一下 **將所選產品添加到訂單** 按鈕，應用程式將顯示旋轉載入表徵圖，頁面掛起。
...
* 現在，您可以將自定義Elasticsearch欄位映射器添加到 `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`。

<!-- ENGCOM-9029 MC-24026-->

* 開發人員現在可以更改Elasticsearch映射。 以前，在預設映射中發送到Elasticsearch伺服器的動態模板是硬編碼的，這防止了開發人員Elasticsearch與自定義模組關聯的任何額外資料而進行索引。

<!-- ENGCOM-8464 -->

* 禁用「搜索建議」時，搜索欄位（窗體部件搜索）現在可以按預期工作。
...
* 受限於特定網站的管理員用戶現在可以為同一網站上的訂單建立發運。 以前，當管理員用戶沒有預設儲存視圖的權限試圖發送在管理員用戶有權訪問的儲存視圖中放置的訂單時，應用程式會引發此異常： `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`。
...
* 應用程式不再取消檢查 **追加註釋** 複選框 **獲取發運方法和費率** 並在從管理員建立訂單時選擇發運方法。
...
* 購物者在「發貨至多個地址」頁上更改數量並按一下瀏覽器「後退」按鈕後，產品數量現在仍保持預期不變。
...
* 當商戶嘗試使用DHL發運訂單時，應用程式不再引發錯誤 **建立發運標籤** 複選框，並且產品名稱包含unicode字元。 以前，應用程式在請求建立標籤時顯示以下錯誤： `The response is in wrong format`。

<!-- ENGCOM-8952 -->

* 您可以在多站點部署中成功地從管理員下單， `United States` 在一個網站上啟用，`Disable all countries` 作為其他網站上的預設範圍啟用。 以前，應用程式未下訂單並顯示以下錯誤： `Please check the shipping address information. "regionId" is required. Enter and try again`。
...
* 應用程式現在顯示 **增值稅編號** 在 `customer/create_account/vat_frontend_visibility` 的子菜單。
...
* 已刪除 `CacheCleaner::cleanAll();` 從整合test。

<!-- ENGCOM-8898-->

* 已為此方案添加test:管理員用戶可以在客戶訂閱排隊的新聞稿時編輯客戶帳戶。

<!-- ENGCOM-8484-->

* 已刪除 `cache:flush` test命令以縮短執行時間 `Catalog`。 `CatalogUrlRewrite`, `LoginAsCustomer` 模組。
...
* 應用程式現在在使用標準Magento Open Source主題的店面頁面上一致地顯示頁面元素。 以前，並非所有樣式都應用在「空白」主題中，因此並非所有頁面元素都顯示在使用此主題的頁面中。 （例如，「我的訂單」頁搜索欄位中沒有放大鏡表徵圖。）

### 翻譯和區域設定

<!-- MC-23553-->

* 配置內聯轉換時，添加到字串的方括弧不再轉義。 以前， `escapeHtmlAttr` 方法將方括弧轉換為HTML實體代碼。
...
* 模板中用於管理儲存的付款方法的文本字串(**我的帳戶**  >  **儲存的付款方法** )現在可翻譯。

<!-- MC-30798 ENGCOM-8717-->

* 從管理員發送的訂購電子郵件現在使用儲存區域設定，而不是與管理員帳戶關聯的區域設定。
...
* PayPal Express Checkout在結帳期間顯示的訂單複查頁現在已成功載入。 以前，模板包含未使用的 **更新傳遞方法** 按鈕，該按鈕僅在頁面呈現期間被指令碼部分隱藏。

<!-- MC-37941-->

* 現在，當存在冗長的評級名稱時，產品頁面「客戶評論」部分中的評級星和審閱文本會被適當地隔開。
...
* 應用程式現在在為新管理員用戶生成電子郵件時使用正確的自定義電子郵件模板。 以前，應用程式使用舊的預設模板，該模板省略了管理員的姓和名。
...
* 對於具有多個願望清單的客戶，願望清單中的產品總數現在與願望清單中的項目數相匹配。 以前，庫存不足的產品已包括在產品總數中。
