...
## 套用MC-__set_rate_limits__2.4.3.patch以解決API速率限制問題
...
* Laminas程式庫相依性已升級為PHP 8.x相容版本。 已從`composer.json`檔案中移除部分多餘的相依性。 **Magento Open Source 2.4.3使用Laminas 3.4.0**。<!-- MC-39513-->
...
* REST GET Cart API呼叫現在會傳回帳單和運送地址的正確自訂屬性值。 以前，自訂地址屬性在「我的帳戶店面」頁面和「管理員」的訂單詳細資訊頁面中顯示不正確。
...
* 相依性`pelago/emogrifier`已從版本3.1.0更新至5.0.0。 此更新導致對`Magento\Email\Model\Template\Filter`類別進行不相容的回溯變更。 已變更的程式碼會在電子郵件範本轉譯期間執行。 請參閱[BIC參考](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/reference/)。
...
* `Magento\Widget\Block\Adminhtml\Widget\Instance\Edit\Chooser\Container`中的版面配置不再採用硬式編碼。  因此，`getPageLayouts()`函式現在會依預期傳回不同模組宣告的實際版面配置清單。 以前只傳回硬式編碼版面。 [GitHub-31168](https://github.com/magento/magento2/issues/31168)
...
* `ArrayIterator` PHP物件已更新，可如預期般搭配PHP 7.4使用。 [GitHub-32088](https://github.com/magento/magento2/issues/32088)
...
* 開發人員現在可以變更Elasticsearch對應。 先前，以預設對應傳送至Elasticsearch伺服器的動態範本採用硬式編碼，如此可防止開發人員在Elasticsearch中編制與自訂模組相關的任何額外資料的索引。
...
* 已移除多餘引數，且`\Magento\Logging\Model\ProcessorTest::testLoggingProcessorLogsActionShipping`中的POST已變更為GET （視需要而定）。
...
您可以使用[Composer](../../../installation/composer.md)安裝Magento Open Source 2.4.3。
