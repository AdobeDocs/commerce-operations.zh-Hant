...
# Magento Open Source 2.4.3 發行說明

Magento Open Source 2.4.3 引入了對性能和安全性的增強以及重大的平台改進。 安全性增強功能包括擴大 reCAPTCHA 覆蓋範圍和納入內建速率限制。 核心撰寫器依賴項和協力廠商資料庫已升級到與 PHP 8.x 相容的最新版本。頁面 Builder 現在在 Magento Open Source 中以捆綁擴展的形式提供。 現在是 Adobe Systems Commerce 和 Magento Open Source 的預設內容编辑工具。

此版本包含370多項核心程式碼的新修正及33項安全性增強功能。 其中包括由我們的社群成員解決近290個GitHub問題。 這些社群貢獻的內容包括核心程式碼的少量清理，以及GraphQL中的重大增強功能。

此版本包括 370 多項核心代碼的新修正和 33 項安全性增強功能。 Magento Open Source 2.4.2 發行說明中識別的所有已知問題在此版本中均已修正。
...
此版本包含33項安全性修正和平臺安全性改進。 其中許多安全修復已向後移植到 2.4.2-p2 和 2.3.7-p1。
...
* **新的 Composer 外掛程式**&#x200B;有助於防止依賴項混淆，並識別與公共包存放庫上的內部包同名的惡意包。[請參閱 Adobe Systems Release新 Composer Plugin with 2.4.3 Release](https://magento.com/blog/best-practices/adobe-releases-new-composer-plugin-magento-243-release) 部落格貼文。

* **速率限制現已內置於** API 中，以防止拒絕服務 （DoS） 攻擊。 Web API 現在對用戶端可以請求的資源的大小或數量施加限制（預設最大值設置為 20，可以根據業務需求配置為不同的值）。 有關配置這些限制的資訊，請參閱 [速率限制](https://developer.adobe.com/commerce/webapi/get-started/api-security/) 。 <!-- MC-35358-->
...
**包含PayPal的部署現在支援** PayPal稍後付款。 此功能可讓購物者以雙週分期付款的方式支付訂單，而不需在購買時支付全額。<!-- MC-40556-->

**`use_application_lock`新索引模式**。該 `use_application_lock` 模式允許您通過使用環境變數或配置文件 `app/etc/env.php` 來啟用重新索引。 啟用此模式後，您就不需要在失敗後手動重設索引器。  請參閱[使用應用程式鎖定模式重新索引處理序](https://developer.adobe.com/commerce/php/development/components/indexing/#using-application-lock-mode-for-reindex-processes)。

### 平台增強功能

2.4.3版尚未與PHP 8.x相容，但後續的平台升級使我們更接近未來與PHP 8.x的相容性。

* 核心 Composer 依賴項和協力廠商資料庫已升級到與 PHP 8.x 相容的最新版本。 <!-- MC-39514-->

* KnockoutJS 資料庫已經升級到 v3.5.1 （最新版本）。 <!-- MC-40694-->

* 已棄用的 TinyMCE v3 資料庫 已被移除。 `Magento_Tinymce3Banner`與TinyMCE v3.x相關的模組和MFTF測試已從Adobe Systems Commerce中刪除。<!-- MC-42199 42170 -->

* Magento Open Source 2.4.3 已經過測試並確認與 Redis 6.0.12 相容。 （2.4.x 版仍與 Redis 5.x 相容。）

* Laminas 資料庫 依賴項已升級到 PHP 8.x 相容版本。 已從`composer.json`檔案中移除部分多餘的相依性。 **Magento Open Source 2.4.3 使用 Laminas 3.4.0**。 <!-- MC-39513-->
...
此版本新增共用路由的GraphQL涵蓋範圍。 [路由查詢](https://developer.adobe.com/commerce/webapi/graphql/queries/route.html)和[RoutableInterface](https://developer.adobe.com/commerce/webapi/graphql/interfaces/routable-interface.html)支援產品、類別和CMS頁面上的路由要求。 `urlResolver`查詢已過時，其功能已由`route`查詢取代。
...
有關增強功能和錯誤修復的資訊，請參閱 [PWA Studio版本](https://github.com/magento/pwa-studio/releases)。 如需PWA Studio版本及其相容版本的清單，請參閱[相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/)。
...
* 如果價格在下訂單之前已更改，則包含捆綁商品的訂單的訂單詳細信息現在會顯示捆綁商品的正確價格。

<!-- MC-40035-->

* 捆綁產品庫存狀態現在根據其子產品的庫存狀態進行更新。 以前，當從產品中刪除一個選項時，捆綁產品顯示為缺貨，並且捆綁產品具有兩個具有相同SKU的選項。

<!-- MC-39276-->

* 將套件產品移至其他屬性群組後，管理員現在可以變更套件產品`Shipment Type`屬性的值。 以前，如果將此屬性移動到屬性集中預設群組以外的屬性群組，則此屬性始終與值一起 `Together` 保存。
...
* 現在，您可以從客戶購物車存取捆綁產品，成功設定捆綁產品。 以前，「設定產品」頁面從未完全載入，因此您無法儲存設定。

<!-- MC-30317 ENGCOM-7141-->

* 商家現在可以在多商店部署的每個商店視圖為捆綁產品分配唯一的價格。 特定於網站的價格保存在`catalog_product_bundle_selection_price` 表中。 以前，當「商店>**配置」>目錄**“> ******目錄>”價格**“>****”目錄價格範圍&#x200B;**“設置為 時`Website`，應用程式不會將捆綁產品的價格基於網站範圍平均。******`catalog_product_bundle_selection_price`中未儲存任何網站特定價格。 [GitHub-12584](https://github.com/magento/magento2/issues/12584)
...
* 自定義主題佈局更新現在可如預期套用。 以前會忽略自訂佈景主題版面更新。
...
* 頁面生成器產品小部件現在預覽當每個網站上匹配的產品具有不同的價格時，在多網站部署中按預期工作。

<!-- MC-40344 ENGCOM-8629-->

* 按產品搜尋位置排序現在如預期般運作GET `/rest/V1/products/?searchCriteria[filterGroups]` 。 以前，產品集合沒有用於排序的欄位 `position` 值。 [GitHub-31591](https://github.com/magento/magento2/issues/31591)
...
* 購物者現在可以將可配置的產品從非預設的商店視圖添加到他們的購物車中。 以前，當非預設商店上的購物者視圖嘗試添加可配置產品時，應用程式显示此錯誤： `Could not add item to cart. Please check required options and try again`。 [GitHub-31660](https://github.com/magento/magento2/issues/31660)

### 內容安全原則 CSP）

<!-- ENGCOM-8577-->

* 內容安全策略現在支持通過 載入 `data: scheme`base64 編碼的圖像和字體。
...
* `indexer_update_all_views`  cron 作業現在在上一次失敗后按預期運行。 失敗的運行在 中 `cron_schedule schedule`標記為失敗，後續運行不會自動失敗。 以前， `cron_schedule` 表中填滿了掛起的作業， `indexer_update_all_views` 並且cron作業未運行。 [GitHub-23054](https://github.com/magento/magento2/issues/23054)
...
* 店面客戶帳戶通訊簿上的 狀態 **** 字段現在載入為 並保留為下拉頁面元素。**提交按鈕**&#x200B;現在會停用，直到所有頁面元素都已完全載入。以前，應用程式將此欄位載入為文字框，然後再將其呈現為下拉元素，購物者可以在文本字段中輸入和保存值，這後來會導致結帳時出錯。

<!-- MC-40329-->

* 上傳文件時，在管理員客戶地址字段中保存帶有文件附件的客戶地址屬性時，應用程式不再引發錯誤。 這是由於控制器操作中缺少 return 語句而發生的。 之前，應用程式拋出此錯誤： `Something went wrong while saving the file`。

<!-- MC-39354-->

* REST GET購物車 API 呼叫現在會為帳單和送貨地址傳回正確的自定義屬性值。 以前，自定義地址屬性在“我的帳戶店面”頁面頁面的訂單詳細信息和“管理員”中顯示不正確。

<!-- MC-24495-->

* 應用程式 現在可成功處理包含具有輸入類型 `file (attachment)`客戶 地址屬性的文件。 以前，應用程式在上傳附件期間拋出此錯誤： `Something went wrong while saving the file`。
...
* 現在會檢查Admin中的父類別是否有docblock註解，以及原始類別和繼承的介面。 之前，由於未檢查父類別，因此在Admin中執行任何與客戶相關的動作時觸發事件，都會導致錯誤。 應用程式在異常日誌中記錄了此錯誤： `report.CRITICAL: Method's return type must be specified using @return annotation.`。

<!-- MC-41192-->

* 當 Redis 伺服器在啟用了 Redis 頁面 快取 的部署中停止時，應用程式不再引發致命錯誤。
...
* 應用程式 現在會按預期翻譯所有可翻譯的字符串，供管理員購物車頁面使用。 以前，轉換載入發生得太晚，並跳過了訂閱控制器操作預調度的所有觀察者。 [GitHub-31849](https://github.com/magento/magento2/issues/31849)
...
* 現在，當選取實體類型時，篩選功能會正常運作於已排程的導出清單。 之前，篩選器不起作用，並且應用程式拋出JavaScript錯誤。 [GitHub-361](https://github.com/magento/partners-magento2ee/issues/361)
...
* 新增阿爾巴尼亞、丹麥、希臘、冰島、葡萄牙和瑞典地區 `directory_country_region` 至此。 [GitHub-31040](https://github.com/magento/magento2/issues/31040)

<!-- MC-30104 magento/magento2#31480 -->

* 現在，當發生異常時，消息會在MysqlMQ消息佇列中標記為錯誤。 [GitHub-18140](https://github.com/magento/magento2/issues/18140)
...
* 按 篩選`user_id`時`Magento\Users\Model\ResourceModel\Users\Collection`，應用程式不再引發 SQL 異常。[GitHub-31216](https://github.com/magento/magento2/issues/31216)

### 禮品卡

<!-- MC-32651-->

* 價格範圍驗證邏輯已新增至贈品卡片建立頁面。 以前，管理員可以創建最小值超過最大值的卡片。 [GitHub-493](https://github.com/magento/partners-magento2ee/issues/493)
...
* 您現在可以在計劃的匯出中保存空值。 以前，應用程式在匯出創建或保存後更新空篩選器值。 在資料庫中，和 值均`no`由零表示，而的值`not selected`被 覆蓋`no``not specified`為 。

<!-- MC-41092-->

* 現在，導出過程在出口產品、庫存來源和客戶實體時要考慮帳戶用戶角色範圍。 以前，出口過程忽略了用戶角色範圍，這允許出口私營用戶角色出口實體。
...
* 該應用程式現在會在文件導入過程中刪除向上銷售、交叉銷售和相關產品之間的產品關係CSV如預期的那樣，當在CSV檔中指定時 `__EMPTY__VALUE__` 。

<!-- MC-40012-->

* 匯出許多客戶的自訂位址資料（管理員&#x200B;**系統** > **匯出**）不再會定期導致記憶體錯誤。 以前，導出自定義地址數據時，應用程式會嘗試載入所有客戶數據，從而導致記憶體耗盡，並且應用程式引發分配記憶體失敗錯誤。
...
* 您現在可以在匯入期間如預期從 客戶 地址刪除區域。 以前，使用空區域導入客戶地址時，分配的區域不會更改。
...
* 進程管理員現在在子進程失敗時退出並顯示錯誤。 以前，如果傳遞給「程式管理員」的函式數（例如索引器維度）低於`MAGE_INDEXER_THREADS_COUNT`環境變數的值，該管理員一律會成功結束。 [GitHub-30964](https://github.com/magento/magento2/issues/30964)
...
* 刪除現在不包含產品的已禁用類別對目錄搜尋和平面索引表類別沒有影響。 以前，刪除非活動中類別會觸發完全重新編列索引。 [GitHub-23297](https://github.com/magento/magento2/issues/23297)
...
* 將外掛程式新增到父類別時，應用程式不再擲回錯誤。 （`Assest.php`中的`optionsProvider`引數現在會在`getContentIdentities`之後宣告。） 之前，應用程式擲回此錯誤： `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`。 [GitHub-31291](https://github.com/magento/magento2/issues/31291)
...
* 應用程式不再將每個Cookie記錄為單獨的上下文。 `$_COOKIE`陣列也已轉換為字串。 在過去，由於每個Cookie都記錄為單獨的上下文，因此當 Cookie 數超過 50 個時，應用程式會記錄以下消息： `Unable to send the cookie. Maximum number of cookies would be exceeded`。 [GitHub-31334](https://github.com/magento/magento2/issues/31334)

### 發票

<!-- ENGCOM-8472 -->

* 在“管理”中創建新發票時，“ **通過電子郵件發送副本”** 複選框現在按預期工作。 以前，如果在銷售電子郵件&#x200B;**中**&#x200B;啟用了發送發票電子郵件的全域設置，則會忽略該複選框。它現在保持一致，並且操作方式與裝運和貸項通知單創建頁面相同。 [GitHub-28511](https://github.com/magento/magento2/issues/28511)
...
* 應用程式不再將新聞稿電子郵件發送給在新聞稿佇列創建和發送新聞稿之間的時段中已取消訂閱新聞稿的客戶。 [GitHub-32116](https://github.com/magento/magento2/issues/32116)
...
* 快取訂閱狀態已從新聞稿外掛程式中刪除。 [GitHub-19345](https://github.com/magento/magento2/issues/19345)
...
* 應用程式現在會打開付款塊 `frontend` ，無論電子郵件是從哪個區域發送的。 （現在類比當前區域，就像呈現付款塊之前一樣 `frontend` 。 以前，付款塊是在發送電子郵件的區域呈現的。 因此，無論銷售電子郵件是從管理員還是由 REST API 觸發，資產 的 URL 都會嘗試從錯誤的區域 （`webapi_rest` 或 `adminhtml`） 載入它們。
...
* 當僅需要一個編輯頁面來呈現Admin表單時，應用程式不再載入所有CMS頁面。  這些頁面現在載入速度更快。 [GitHub-30936](https://github.com/magento/magento2/issues/30936)
...
* 現在，當購物者更新店面上的產品時，應用程式會使用分組價格正確更新產品的價格數量。 [GitHub-32669](https://github.com/magento/magento2/issues/32669)

<!-- MC-39282-->

* 現在，當產品已分配層價並 `Magento\Framework\Api\ExtensibleDataObjectConverter` 用於轉換產品數據時，可以保存捆綁產品。 以前，當 `Magento\Framework\Api\ExtensibleDataObjectConverter` 用於在保存產品時將產品數據轉換數位時，應用程式不會保存產品並顯示此錯誤： `Notice: Undefined index: price in app/code/Magento/Catalog/Model/Product/Type/Price.php on line 382.`

<!-- MC-36811-->

* 計劃的價格更新現在適用於已在購物者購物車中的產品。 [GitHub-356](https://github.com/magento/partners-magento2ee/issues/356)
...
* 您現在可以使用&#x200B;**新增視訊**&#x200B;按鈕（管理員&#x200B;**目錄** > **產品**）來連續新增多個視訊。 以前，視頻欄位保留前一個視頻的詳細資訊。

<!-- MC-40817-->

* 在產品頁面上進入產品視訊的全屏模式，現在在行動裝置裝置上可以正常運作。 以前，進入全螢幕模式會導致視頻在退出全螢幕模式之前暫停。
...
* 現在，可在我的帳戶頁面的“評論詳細信息”部分中正確計算產品評論評級星。

<!-- MC-39763-->

* 產品評論詳細資訊的「平均產品評分」和「產品評分」區域現在頁面正確顯示。 以前，應用程式計算店面評分時未設置評論 ID，並且未正確呈現產品評論範本。
...
* 該應用程式現在可以正確計算基於PayPal Payment Pro 作為支付閘道的部署商店中的訂單或發票的貸項通知單的部分貸項通知單稅款總額。 以前，對於具有多張發票的訂單，應用程式在貸項通知單中將該訂單的全部稅款應用於部分發票取消。

<!-- MC-39521 ENGCOM-8528-->

* 應用程式不再在向客戶發送新訂單電子郵件時創建隨機資料庫死鎖。 以前，發生死鎖是因為應用程式儲存了整個物件及其相關物件，而不是更新的SQL資料行。 應用程式顯示此錯誤型別： `SQLSTATE[40001]: Serialization failure: 1213 Deadlock found when trying to get lock; try restarting transaction`。 [GitHub-31090](https://github.com/magento/magento2/issues/31090)
...
* 包含條件類別 **不是** “的購物車價格規則現在將按預期應用於未分配給類別但其父產品已分配父產品的可配置子/簡單產品。
...
* 現在Search屬性設定為可搜尋時，會如預期般包含 `weight` 屬性。

<!-- MC-40409-->

* 當您視圖啟用了Elasticsearch的空類別頁面時，應用程式不再引發錯誤。 相反，它會按預期呈現頁面並顯示資訊性消息。 以前，空的全文檢索會在類別頁面上觸發異常。
...
* 階層導覽篩選器現在會顯示精確的產品計數。 以前，Elasticsearch的產品計數值不按目錄許可權篩選。

<!-- MC-23881 ENGCOM-8618-->

* Elasticsearch 類別 URL 頁面参数超過分页時不再引發錯誤。 [GitHub-23843](https://github.com/magento/magento2/issues/23843)

<!-- MC-24033  magento/magento2#3274-->

* 現在，您可以將自訂Elasticsearch欄位映射器 `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`加入到 。
...
* 現在，當購物者在離開多配送頁面后，在結帳時返回到購物車頁面時，應用程式會显示正確的訂單小計。 [GitHub-31889](https://github.com/magento/magento2/issues/31889)

<!-- MC-40541 magento/magento2#31841 -->

* 在管理員訂單創建期間编辑帳單信息不再更改具有不同預設送貨和帳單地址的客戶的發貨信息。 [GitHub-31786](https://github.com/magento/magento2/issues/31786)
...
* 購物者現在可以使用「上一頁」瀏覽器按鈕，在結帳具有多個地址的訂單時返回「選取送貨方法」頁面。 以前，應用程式显示已損毀的選擇送貨方法頁面。 [GitHub-30268](https://github.com/magento/magento2/issues/30268)

<!-- MC-40021 ENGCOM-8983-->

* 在購物者數量更改“運送到多個地址地址”頁面並按擊瀏覽器返回按鈕后，產品數量現在按預期保持不變。
...
* 您可以在多網站部署中成功從管理員下訂單， `United States` 該網站在一個網站上啟用，`Disable all countries` 並在另一個網站上作為預設範圍啟用。 之前，應用程式未下訂單，且顯示這個錯誤： `Please check the shipping address information. "regionId" is required. Enter and try again`。

### 商店

<!-- MC-40538-->

* 顯示 商店 首頁時，應用程式不再將 的 `0` 字符串視為空值。 以前，應用程式將請求路徑開始處的整數值視為商店 ID，這會對SEO產生意外影響。

<!-- MC-40313-->

* 已刪除 under `lib/internal/Magento/Framework/App/Action/Plugin` 的`\Magento\Framework\App\ActionInterface`外掛程式，以符合外掛程式應用於自定義另一個模組的一個模組的行為的準則。[GitHub-28050](https://github.com/magento/magento2/issues/28050)
...
* 從測試中刪除了該`cache:flush`命令，以縮短、 `CatalogUrlRewrite`和`LoginAsCustomer`模組的`Catalog`執行時間。
...
* 在管理員貨件創建期間，來源的分頁現在按預期顯示。

<!-- MC-40240-->

* 當您使用自定義日期屬性篩選產品且管理員用戶區域設置為 時 `en_GB`，產品網格篩選器現在可正常工作。
...
* 現在，您可以在管理 **內容** > **介面工具集**&#x200B;上建立新介面工具集後移除配置更新。  [GitHub-29936](https://github.com/magento/magento2/issues/29936)
...
* 現在，在通過批量更新將產品分配給網站後，會按預期生成多網站部署中特定網站的產品URL重寫。

<!-- MC-40780 magento/magento2#32598-->

* 現在，當為父類別啟用URL鍵的使用預設值值&#x200B;**複選框時**，應用程式將正確生成子類別的URL路徑。先前，在多存放區檢視部署中使用不同的URL索引鍵時，在階層中移動類別會導致`url_path`值不正確。 [GitHub-16202](https://github.com/magento/magento2/issues/16202)
...
* 使用自訂URL從管理員建立的URL重新導向現在可成功運作。 以前，GraphQL 緩存了來自 GraphQL `urlResolver` 查詢 的回應，並在 URL 重寫更新後返回舊值。

<!-- MC-38931  magento/magento2#31106-->

* 現在從網站移除產品時，會按預期移除產品URL重寫功能。 [GitHub-24184](https://github.com/magento/magento2/issues/24184)
...
* 屬於可設定產品之簡單產品的YouTube影片現在可如預期運作。 在過去，影像是按ID排序，而非依據位置排序。 因此，部分影片被視為影像。 [GitHub-29690](https://github.com/magento/magento2/issues/29690)
...
**問題**： _內容安全性原則錯誤_。 店面在主控台記錄檔中顯示下列錯誤： `The Content-Security-Policy directive frame-ancestors does not support the source expression unsafe-inline`。 店面性能不受影響。 <!-- MC-42613-->

**問題**： _贈與卡片金額_&#x200B;的PayPal信用顯示異常。 啟用 PayPal 積分並配置多個禮品卡片金額時，如果購物者更改禮品卡片值的金額，店面不會更新分期付款的金額。 Adobe Systems Commerce 2.4.4 和 Magento Open Source 2.4.4 將包含此問題的修正程式。<!-- MC-42499 AC-344-->
