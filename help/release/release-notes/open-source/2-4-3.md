...
description: 瞭解2.4.3Magento Open Source版本的新功能、增強功能、錯誤修正和已知問題。
...
Magento Open Source2.4.3引進了效能和安全性增強功能，並大幅改善了平台。 安全性增強功能包括擴充reCAPTCHA涵蓋範圍及納入內建速率限制。 核心撰寫器相依性和協力廠商程式庫已升級至與PHP 8.x相容的最新版本。Page Builder現在可作為Magento Open Source中的套件擴充功能使用。 它現在是Adobe Commerce和Magento Open Source的預設內容編輯工具。

此版本包含超過370項核心程式碼的新修正和33項安全性增強功能。 其中包括由我們的社群成員解決近290個GitHub問題。 這些社群貢獻的內容包括從微幅清理核心程式碼到GraphQL的重大增強功能。

此版本包含超過370項核心程式碼的新修正和33項安全性增強功能。 Magento Open Source2.4.2發行說明中確認的所有已知問題已在此版本中修正。
...
此錯誤是由於使用 `str_contains` 函式，它是一個PHP 8.x函式。 Open Source 2.4.3應用程式不支援PHP 8.x。此Hotfix會以支援的PHP 7.x函式取代此函式。 請參閱 [Adobe Commerce升級2.4.3、2.3.7-p1 PHP嚴重錯誤Hotfix](https://support.magento.com/hc/en-us/articles/4408021533069-Adobe-Commerce-upgrade-2-4-3-2-3-7-p1-PHP-Fatal-error-Hotfix) 知識庫文章。
...
* Magento Open Source2.4.3已經過測試，並已確認與Redis 6.0.12相容。 （2.4.x版仍與Redis 5.x相容。）

* Laminas程式庫相依性已升級至PHP 8.x相容版本。 已從移除一些多餘的相依性 `composer.json` 檔案。 **Magento Open Source2.4.3使用Laminas 3.4.0**. <!-- MC-39513-->
...
此版本新增共用路由的GraphQL涵蓋範圍。 此 [路由查詢](https://devdocs.magento.com/guides/v2.4/graphql/queries/route.html) 和 [可路由介面](https://devdocs.magento.com/guides/v2.4/graphql/interfaces/routable-interface.html) 支援產品、類別和CMS頁面上的路由請求。 此 `urlResolver` 查詢已過時，其功能已由 `route` 查詢。
...
* 此 `updateProductsInWishlist` 突變現在可成功更新願望清單中屬於套件產品的專案。 以前，此變異不會更新願望清單專案，而是刪除該專案並建立新專案，這會變更專案ID。
...
* 的效能  `catalog_product_alert` 改善在大型表格（數百萬列）上執行時的cron程式。 先前， `catalog_product_alert` 已載入所有產品警報，導致記憶體不足例外狀況。
...
* 使用Safari的管理員現在可以從管理員成功新增產品至訂單。 之前，當管理員按一下 **將所選產品新增至訂單** 按鈕，應用程式會顯示旋轉載入圖示，頁面隨即掛起。
...
* 開發人員現在可以變更Elasticsearch對應。 先前，在預設對應中傳送至Elasticsearch伺服器的動態範本會加上硬式編碼，使開發人員無法在Elasticsearch中索引與自訂模組相關聯的任何額外資料。

<!-- ENGCOM-8464 -->

* 當搜尋建議停用時，搜尋欄位（表單小型搜尋）現在會如預期般運作。
...
* 僅限特定網站的管理員使用者現在可以為同一網站上的訂單建立出貨。 之前，當缺少預設商店檢視許可權的管理員使用者嘗試將訂單傳送至該管理員使用者有權存取的商店檢視時，應用程式會擲回此例外狀況： `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.
...
* 應用程式不再取消勾選 **附加註解** 購物者點按時的核取方塊 **取得送貨方式與費率** 並從管理員建立訂單時選取送貨方法。
...
* 當購物者變更「送貨地點」多重地址頁面上的數量，並按一下瀏覽器的「上一步」按鈕後，產品數量現在會依預期維持不變。
...
* 當商家嘗試使用DHL傳送訂單時，應用程式不再擲回錯誤。 **建立送貨標籤** 核取方塊已啟用，且產品名稱包含unicode字元。 以前，應用程式在請求建立標籤時會顯示此錯誤： `The response is in wrong format`.
...
* 應用程式現在會顯示 **VAT編號** 欄位(若 `customer/create_account/vat_frontend_visibility` 已啟用。
...
* 新增此情況的測試：當客戶訂閱佇列電子報時，管理員使用者可以編輯客戶帳戶。

<!-- ENGCOM-8484-->

* 已移除 `cache:flush` 來自測試的命令，以改善的執行時間 `Catalog`， `CatalogUrlRewrite`、和 `LoginAsCustomer` 模組。
...
* 應用程式現在會在使用標準Magento Open Source主題的店面頁面上以一致的方式顯示頁面元素。 先前，並非所有樣式都套用至空白主題，因此並非所有頁面元素都會顯示在使用此主題的頁面中。 （例如，「我的訂單」頁面搜尋欄位中沒有放大鏡圖示。）
...
* 範本中用來管理儲存之付款方法的文字字串(**我的帳戶**  >  **儲存的付款方法** )現已可翻譯。

<!-- MC-30798 ENGCOM-8717-->

* 從管理員傳送的訂單電子郵件現在使用商店地區設定，而不是與管理員帳戶關聯的地區設定。
...
* 使用PayPal Express簽出時顯示的訂單稽核頁面現在已成功載入。 以前，範本包含未使用的 **更新傳遞方法** 按鈕，但在頁面呈現期間被指令碼只隱藏了一部分。

<!-- MC-37941-->

* 當存在冗長的評等名稱時，產品頁面「客戶評論」區段中的評等星級和評論文字現在會有適當的間距。
...
您可以使用以下專案安裝Magento Open Source2.4.3： [Composer](../../../installation/composer.md).
