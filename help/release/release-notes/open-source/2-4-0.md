...
如需安全性修補程式的一般資訊，請參閱[安全性修補程式發行版本](../security/overview.md)。 如需有關下載和套用安全性修補程式（包括修補程式2.3.5-p2）的說明，請參閱[快速入門內部部署](../../../installation/composer.md)。 安全性修補程式僅包含安全性錯誤修正，不包含完整修補程式中包含的其他安全性增強功能。
...
* **移除第三方付款方法的核心整合**。 在此版本中，已從核心程式碼中移除Authorize.Net付款方法整合。 商戶應移轉至Commerce Marketplace上提供的正式擴充功能。 檢視付款整合的[淘汰](https://experienceleague.adobe.com/en/docs/commerce-admin/start/compliance/payments/compliance-payment-services-directive).<!-- MC-29029-->
...
此擴充功能取代了核心的Braintree整合。 它提供與Braintree核心整合相同的功能。 請參閱[Braintree付款](https://commercemarketplace.adobe.com//paypal-module-braintree.html)。
...
* 完成的訂單現在會同時出現在付款系統和Magento Open Source中。 以前，訂單出現在付款系統中，但不出現在Magento Open Source中。 [GitHub-25862](https://github.com/magento/magento2/issues/25862)
...
* Elasticsearch 7.6.x現在是Adobe Commerce和Magento Open Source的預設目錄搜尋引擎。 您必須同時安裝Elasticsearch 7.6.x，才能安裝或升級至2.4.0版。 Elasticsearch 2.x版本的程式碼已移除。 Elasticsearch版本5.x和6.x已過時，不再支援。 請參閱[搜尋引擎必要條件](../../../installation/prerequisites/search-engine/overview.md)。
...
* Magento Open Source `cron`現在用於在建立測試資料後重新索引，並在適用時，`cache:flush config`取代`cache:flush`。 _Lukasz Bajsarowicz在提取要求[26990](https://github.com/magento/magento2/pull/26990)_&#x200B;中提交的修正。 [GitHub-26989](https://github.com/magento/magento2/issues/26989)
...
**問題**：在安裝PHP 7.4.2的部署中，店面錯誤訊息發生異常。 當使用PHP 7.4.2部署2.4.0時，店面錯誤訊息中的空格符號會被替換為加(+)字元。  此錯誤是PHP 7.4.2的原生錯誤，無法由Magento Open Source更正。 **因應措施**：建議您使用其他版本的PHP 7.4.x。 請參閱店面](https://support.magento.com/hc/en-us/articles/360045804332)知識庫文章上的[原始訊息資料顯示。<!-- MC-34170-->
...
您可以使用撰寫器安裝Magento Open Source 2.4.0。
