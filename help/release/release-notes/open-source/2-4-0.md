...
Magento Open Source2.4.0引入了對PHP 7.4 、 7.6.xElasticsearch和MySQL 8.0的支援。大量安全更改包括預設情況下在管理員中啟用雙因素身份驗證。

**在此版本中，Authorize.Net方法整合已從核心代碼中刪除。 商戶應遷移到Commerce Marketplace上可用的官方分機**。
...
雖然這些功能的代碼與季度版本捆綁在一起，但其中幾個項目(例如，Inventory management和Progressive Web Application(PWA)Studio)也是獨立發佈的。 這些項目的錯誤修復程式都記錄在每個項目文檔中提供的獨立、特定於項目的發行資訊中。
...
* **支援新的security.txt檔案**。 此檔案是伺服器上的行業標準檔案，可幫助安全研究人員向站點管理員報告潛在的安全問題。
...
* **MariaDB 10.4支援**。 對MySQL 8.0的支援為商戶提供了部署MariaDB 10.4的Magento Open Source。 儘管商家仍可以使用帶2.4.0的MariaDB 10.2，但我們建議升級至MariaDB 10.4，以提高效能和可靠性。 不再支援MariaDB 10.0和10.1（因為此版本中取消了對MySQL 5.6的支援）。
...
此擴展取代了我們的核心Braintree整合。 它提供與Braintree核心整合相同的功能。 請參閱 [Braintree付款](https://marketplace.magento.com/paypal-module-braintree.html)。
...
Magento Open Source安裝現在成功完成，並且當在中預定義儲存配置時，將按預期建立儲存 `config.php`。
...
* 現在，您可以使用「產品」(Products)網格中的「操作」(Actions)下拉菜單來指定在從「管理」(Admin)中更新多個產品的屬性時產品是否具有重量。  _修復由Bartomiej Szubert在拉入請求中提交 [26075](https://github.com/magento/magento2/pull/26075)_。 [GitHub-6310](https://github.com/magento/magento2/issues/6310)
...
* `php bin/magento setup:cron:run` 命令現在已被刪除。 升級時應刪除crontabs中對此的引用。
...
* 所有導出的網格資料(Magento Open Source和自定義資料)現在都有選項標籤，而不是所有類型列的選項值 `Select` 或 `Multiselect`。 以前，導出此資料時，導出的電子錶格包含ID為的列，而不是網格中顯示的值。 _修復由Maksym Novik在拉請求中提交 [26523](https://github.com/magento/magento2/pull/26523)_。 [GitHub-25963](https://github.com/magento/magento2/issues/25963)
...
* 改進了客戶資料段失效邏輯。 此版本引入了一種新的方法，使所有客戶部分資料失效，避免在自定義時本地儲存出現已知問題 `sections.xml` 無效活動。 (以前，在您有自定義內容時，私有內容（本地儲存）未正確填充 *etc/frontend/sections.xml* 操作無效。) 請參閱 [專用內容](https://developer.adobe.com/commerce/php/development/cache/page/private-content/)。
...
* 已完成的訂單現在同時顯示在付款系統和Magento Open Source中。 以前，訂單出現在支付系統中，但沒有Magento Open Source。 [GitHub-25862](https://github.com/magento/magento2/issues/25862)
...
* Elasticsearch7.6.x現在是Adobe Commerce和Magento Open Source的預設目錄搜索引擎。 如果不安裝7.6.xElasticsearch，則無法安裝或升級到2.4.0版。Elasticsearch2.x版代碼已刪除。 Elasticsearch5.x和6.x版已棄用，不再受支援。 請參閱 [搜索引擎先決條件](../../../installation/prerequisites/search-engine/overview.md)。
...
* Magento Open Source `cron` 現在用於在建立test資料後重新索引，如果適用， `cache:flush config` 替換 `cache:flush`。 _Fix由Lukasz Bajsarowicz在拉式請求中提交 [26990](https://github.com/magento/magento2/pull/26990)_。 [GitHub-26989](https://github.com/magento/magento2/issues/26989)
...
**問題**:在安裝PHP 7.4.2的部署中，會出現店面錯誤消息中的異常。 使用PHP 7.4.2部署2.4.0時，儲存前面錯誤消息中的空間符號將替換為加號(+)字元。  此錯誤是PHP 7.4.2的固有錯誤，無法通過Magento Open Source更正。 **解決方法**:我們建議使用其他版本的PHP 7.4.x。請參閱 [在儲存前顯示原始消息資料](https://support.magento.com/hc/en-us/articles/360045804332)  知識庫文章。 <!-- MC-34170-->
...
* 如果社區成員已為此版本提供了修復程式，我們將在這些注釋的「已修復問題」部分中標識該修復程式，並使用短語「」*由社區成員提供的修復@member_name*。
...
可以使用Composer安裝Magento Open Source2.4.0。
