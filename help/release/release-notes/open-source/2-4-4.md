...
description: 瞭解2.4.4Magento Open Source版中的新功能、增強功能、錯誤修復和已知問題。
...
Magento Open Source2.4.4引入對PHP 8.1的支援。已更新所有項目庫和依賴項，以與PHP 8.1相容。Core Composer依賴項和第三方庫也已升級到與PHP 8.1相容的最新版本。此版本還支援OpenSearch 1.2。
...
>[!NOTE]
>
>除Braintree外，所有供應商捆綁的擴展都已從Magento Open Source2.4.4中刪除。
...
Magento Open Source2.4.4包括解決下列熱門解決辦法處理的所有問題，這些熱門解決辦法是為Adobe Commerce和第2.4.3、2.4.3-p1和2.3.7-p2號Magento Open Source提供的：
...
此版本包括一個安全修復和平台安全改進。 此安全修復程式已備份到2.4.3-p2Magento Open Source和2.3.7-p3Magento Open Source。
...
* 現在，當Magento Open Source處於生產模式時，預設情況下禁用Swagger。 <!-- AC-1450-->

* 現在，預設情況下，HTTPS已為Magento Open Source店面啟用。 的 **在店面上使用安全URL** 和 **在管理中使用安全URL** 預設情況下啟用設定，所有內置cookie現在都設定為安全。  <!-- AC-1173-->

* 現在，所有Magento Open Source安裝都需要依賴項混淆插件。 以前，此插件僅對基於Composer的安裝是必需的。 現在它允許受信任的版本。 商家可以繞過此插件中設定的禁止某些Composer安裝組合的約束。 可以繞過受信任版本的約束，Magento Open Source現在在繼續安裝之前顯示警告。  <!-- AC-501 970-->

* 開發人員現在可以配置每個端點Magento Open SourceREST風格端點所接受的陣列大小限制。 請參閱 [API安全](https://developer.adobe.com/commerce/webapi/get-started/api-security/)。 <!-- AC-465-->
...
Magento Open Source2.4.4現在支援8.1菲律賓比索。已更新所有項目庫和依賴項，以與PHP 8.1相容。其他平台增強功能包括：

* Magento Open Source2.4.4支援Elasticsearch7.16和OpenSearch 1.2。部署在本地的應用程式開源商戶可以使用任一解決方案。 但是，OpenSearch現在是托管在雲中的Magento Open Source2.4.4部署的預設搜索引擎。 所有升級到2.4.4版的雲部署客戶都必須使用OpenSearch。 請參閱 [在雲上切換到OpenSearch for 2.4.4](https://support.magento.com/hc/en-us/articles/4419942355725-Switching-to-OpenSearch-for-Adobe-Commerce-on-Cloud-2-4-4)
...
除非 [Braintree](https://docs.magento.com/user-guide/payment/braintree.html)，已從Magento Open Source2.4.4代碼庫中刪除所有供應商捆綁的擴展。 商戶應遷移到官方分機， [Commerce Marketplace](https://marketplace.magento.com/extensions.html)。
...
此版本增強了對標準輔助功能指南的遵守。 它包括改進的工具提示、螢幕元素的可訪問命名和標籤以及重新設計的表徵圖和按鈕。 80%以上的修復有助於改善無視覺或有限視覺用戶的購物體驗。
...
我們正在解決Magento Open Source核心代碼中的數2.4.4問題。 下面對這些已修復問題進行了說明。

### 安裝、升級、部署

<!-- MC-42026-->

* Magento Open Source現在按預期在生成日誌檔案中記錄靜態內容部署錯誤。
...
* 現在，可以從命令行成功更改站點地圖生成器配置。 以前，當您嘗試更改管理員以外的站點地圖配置時，applicationOpen Source顯示了此錯誤： `Import failed: Notice: Trying to access array offset on value of type null in app/code/Magento/Cron/Model/Config/Backend/Sitemap.php on line 78`。  [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* 管理員現在可以在配置了自定義管理員路徑並啟用了密鑰的部署中登錄到管理員。 以前，applicationOpen Source顯示以下錯誤： `Invalid security or form key. Please refresh the page`。

<!-- MC-41949-->

* Magento Open Source在多儲存層次結構更改時不再重定向到「所有儲存」視圖範圍。 以前，在保存層次結構後，購物者被重定向到錯誤的商店URL，並且在多商店部署中選擇了「所有商店視圖」範圍。

### 捆綁產品

<!-- MC-42885-->

* 購物者現在可以將捆綁產品添加到購物車中，其中包含固定產品稅和兩個選項。 以前，applicationOpen Source未添加產品並顯示以下錯誤： `We can't add this item to your shopping cart right now`。
...
* `cron` 現在，按照預期清除產品類別快取 `indexer_update_all_views` 執行。 以前，在成批更新類別中的許多產品後，應用程式開放源在類別頁上顯示了不正確的產品計數。

<!-- MC-42811-->

* Magento Open Source現在會在捆綁產品重新出現在庫存中時按預期方式更新類別產品快取。
...
* 在啟用持久購物車的部署中，會話超時後，購物者現在可以無錯誤地下單。 以前，applicationOpen Source在會話超時後顯示付款錯誤。
...
* Magento Open Source現在將購物車產品數量還原為上一個值（如果更新的數量無效）。
...
* Magento Open Source現在在啟用表費率時正確將免費發運閾值應用於訂單。 [GitHub-21832](https://github.com/magento/magento2/issues/21832)
...
* Magento Open Source在購物者點擊時不再引發錯誤 **更新購物車** 更改迷你車中的產品數量。 以前，applicationOpen Source顯示以下錯誤：  `The quote item isn't found. Verify the item and try again`。
...
* Magento Open Source在購物者添加缺少街道欄位的帳單地址時不再引發此錯誤： `Uncaught TypeError: Unable to process binding "text: function(){return currentBillingAddress().street.join(', ') }"`。 [GitHub-33826](https://github.com/magento/magento2/issues/33826)

<!-- AC-1625-->

* Magento Open Source在從中刪除Mini Cart的商店中籤出時不再引發控制台錯誤。 [GitHub-34513](https://github.com/magento/magento2/issues/34513)
...
* Magento Open Source現在可以在滿足以下條件的訂單上應用兩個購物車規則時正確計算折扣：一個購物車規則與優惠券代碼一起應用 **整個購物車的固定價格**  條件和第二個購物車價格規則沒有優惠券和 **產品價格折扣百分比** 的子菜單。 以前，當前的購物車規則已對某些產品應用折扣時，相關算法無法將折扣按比例地分攤到購物車產品中。
...
* Magento Open Source使用 `addConfigurableProductsToCart` 更改商店視圖後將可配置產品直接添加到購物車中。

<!-- ACP2E-176-->

* Magento Open Source使用 `addConfigurableProductsToCart` 用於將可配置產品的禁用子級添加到購物車的變異。 以前，applicationOpen Source拋出完整性約束衝突錯誤。
...
* 已將驗證添加到結帳工作流中，以將客戶的街道地址與預設值進行比較。 如果驗證流程標識了更改，則它將設定 `customer_address_id` 至 `null` 將其視為新客戶地址。 以前，如果客戶在簽出時未進行驗證而保存了地址，並且同一客戶嘗試在將驗證應用於簽出欄位後使用同一地址下單，則應用程式開放源不接受編輯的地址並引發異常。

### 購物車價格規則

<!-- MC-42288-->

* 現在，在將相關購物車價格規則與 `auto` 選項。 如果嘗試保存此規則而未啟用 `auto` 選項，應用程式Open Source將顯示以下消息： `Rule is not saved with auto generate option enabled. Please save the rule and try again`。
...
* 現在，在配置Magento Open Source時，價格為0的產品可以成功添加到購物車 **整個購物車的固定金額折扣** 購物車價格規則。 以前，applicationOpen Source顯示以下錯誤：  `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`。
...
* Magento Open Source在可配置產品屬性更改時不再修改相關產品價格。 以前，可配置產品的最低廣告價格(MAP)會覆蓋店面相關產品的價格。
...
* 當路由器無法實例化從URL收集的操作類時，添加了案例驗證。 應用程式「開放源」現在顯示標準錯誤消息。 以前，applicationOpen Source顯示以下錯誤：  `PHP Error: Cannot instantiate abstract class Magento\Catalog\Controller\Product\Compare`。
...
* 更新子產品不再刪除在部分索引過程中從其父可配置產品繼承的目錄規則折扣。 以前，當我們將可配置產品分配給某個類別，而不是其關聯的簡單產品，然後使用該類別建立目錄規則時，應用程式開放源會在部分索引過程中刪除分配給簡單產品的目錄規則。
...
* Magento Open Source在部分重新索引期間不再刪除可配置產品的目錄價格規則。 以前，只有子產品在可配置產品重新編製索引時被重新編製索引。
...
* 可配置產品選項標籤名稱現在基於在建立管理訂單期間所選的商店視圖。 以前，applicationOpen Source使用預設儲存視圖中的標籤名稱。
...
* Magento Open Source在嘗試將可配置產品的禁用子項添加到購物車時不再引發內部錯誤。

### 客戶

<!-- MC-24204-->

* Magento Open Source現在在建立訂單期間考慮管理區域設定的網站範圍。 以前，訂單詳細資訊（如客戶地址屬性）僅在一個網站上在多站點部署中正常工作。 [GitHub-23254](https://github.com/magento/magento2/issues/23254)
...
* Magento Open Source在從管理員添加發運注釋時不再在發運詳細資訊電子郵件中包含錯誤消息 **通知客戶** 複選框。 以前，發運確認電子郵件包含以下消息： `We're sorry, an error has occurred while generating this content`。
...
* Magento Open Source現在包括 `Content-Disposition: inline` 電子郵件中的標題。 [GitHub-29258](https://github.com/magento/magento2/issues/29258)
...
* 按一下任何電子郵件模板的預覽，現在將按照預期在單獨的窗口中開啟模板預覽。  以前，applicationOpen Source會同時從電子郵件模板網格和彈出窗口中開啟模板預覽。
...
* 幫助程式 `Magento\Payment\Helper\Data` 不再在建構子中建立新佈局。 以前，當在自定義命令中使用此幫助程式而未指定區域代碼時，Magento Open Source拋出錯誤。 [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* Magento Open Source現在在引導檔案中出現異常時返回500響應代碼。 以前，applicationOpen Source返回了200 OK狀態代碼。 [GitHub-22196](https://github.com/magento/magento2/issues/22196)
...
* 已將新的必需選項添加到 `.htaccess` 和 `nginx.conf` 與PHP 8.x支援Magento Open Source一起分發的檔案。 已刪除過時的選項。 [GitHub-34358](https://github.com/magento/magento2/issues/34358)

<!-- MC-42448-->

* 商家現在可以上傳多種產品的視頻。 現在，僅當上載的檔案名包含副檔名時，檔案生成過程才會將副檔名附加到檔案名。 以前，當商家嘗試上載然後為兩種不同的產品保存同一視頻時，應用程式Open Source拋出了以下錯誤： `Notice: Undefined index: extension in /app/7ha7zds7wvqys_stg/vendor/magento/framework/File/Uploader.php on line 699`。 出現此情況是因為Magento Open Source嘗試將檔案副檔名追加到名稱中，而不管上載的檔案名是否具有副檔名。

<!-- magento/magento2/pull/34106-->

* 已更正問題 `sprintf(__())` 和 `sprintf(Magento\Framework\Phrase())` 構造以在整個代碼庫中與PHP 8.x相容。 以前，applicationOpen Source拋出了以下類型的錯誤： `Expected parameter of type 'string', 'Magento\Framework\Phrase' provided`。 [GitHub-34085](https://github.com/magento/magento2/issues/34085)
...
* 正在執行 `bin/magento support:backup:code` 現在按預期建立備份。 以前，applicationOpen Source建立了備份，但引發了錯誤。
...
* Magento Open Source現在在複製產品時，會根據其定義的範圍複製所有產品欄位。 以前， **名稱** 和 **描述** 欄位重置為 `global` 範圍。

<!-- MC-42612-->

* 當產品價格為0時，Magento Open Source不再對折扣應用增量捨入。 以前，applicationOpen Source計算了負折扣額。

<!-- MC-42215-->

* 具有 **設定為新** 現在可以成功保存分配了空開始日期和過期結束日期的屬性。 以前，在您保存具有以下設定的產品時，applicationOpen Source拋出了此錯誤： `Make sure the To Date is later than or the same as the From Date`。
...
* 類 `BundleDiscountPrice` 在 `magento2/app/code/Magento/Bundle/Pricing/Price/ConfiguredPrice.php` 現在已按預期聲明。 以前，applicationOpen Source拋出了以下錯誤： `Class Magento\Bundle\Pricing\Price\BundleDiscountPrice not found`。 [GitHub-33334](https://github.com/magento/magento2/issues/33334)
...
* 的 `setBillingAddressToCart` 變異現在正確使用 `same_as_shipping` 參數，以設定與發運地址匹配的開單地址。 以前，applicationOpen Source顯示以下錯誤： `The shipping method is missing. Select the shipping method and try again`。 [GitHub-30924](https://github.com/magento/magento2/issues/30924)
...
* 的 `setShippingAddressesOnCart` 變異現在支援在購物車上設定送貨地址，當 **顯示電話** 管理配置設定設定為可選。 以前，applicationOpen Source拋出了以下錯誤： `Field CartAddressInput.telephone of required type String! was not provided`。
...
* Magento Open Source在刷新映像快取的同時在儲存面上載入映像時不再引發錯誤。
...
* Magento Open Source現在在計畫導出後將導出檔案名中的時間戳轉換為用戶的時區。 以前，這些值未轉換，Magento Open Source顯示UTC時間戳。

<!-- MC-42330-->

* 當多次導入同一CSV檔案時，Magento Open Source不再在遠程儲存中建立重複的映像。
...
* Magento Open Source現在在重新編製索引後，按預期在店面顯示產品。 此前，當前被重新編製索引的前500種產品處於庫存狀態，而後500種產品處於庫存狀態時，店面沒有顯示任何附加產品。
...
* 不支援 `str_contains` 方法已替換為支援的函式。 這是Magento Open Source2.4.3的已知問題。 [GitHub-33680](https://github.com/magento/magento2/issues/33680)
...
* Magento Open Source現在在管理員具有只讀屬性時顯示資訊錯誤 `pubs/media` 嘗試訪問包含映像的產品的產品詳細資訊頁面。 以前，applicationOpen Source引發PHP錯誤。 [GitHub-32819](https://github.com/magento/magento2/issues/32819)
...
* 幫助程式 `Magento\Payment\Helper\Data` 不再在建構子中建立新佈局。 以前，當在自定義命令中使用此幫助程式而不指定區域代碼時，應用程式開源引發錯誤。 [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* 已刪除以下庫，因為支援2.4.xMagento Open Source的所有瀏覽器都內置了對此功能的支援：
...
* Magento Open Source不再為執行批量操作的失敗API調用建立日誌條目 **系統** > **批量操作**。 以前，失敗的API調用的永久條目已添加到批量操作日誌中。
...
* 現在，購物者可以成功重新訂購現有訂單，該訂單包含一個產品，該產品包含從店面和管理員中選擇的檔案類型和類型下拉清單的自定義選項。 以前，applicationOpen Source拋出錯誤，未處理重新訂單。

<!-- MC-42332-->

* 現在將按預期載入「管理訂單詳細資訊」頁。 以前，applicationOpen Source在為具有特定稅的訂單載入訂單詳細資訊頁時拋出以下錯誤： `Call to a member function getId() on array`。

<!-- MC-38521-->

* 具有限制權限（包括訂單和發運權限）的管理員現在可以按預期查看訂單頁。 以前，當具有這些權限的管理員嘗試查看訂單時，applicationOpen Source拋出錯誤。 [GitHub-14633](https://github.com/magento/magento2/issues/14633)
...
* `bin/magento setup:upgrade` 現在從2.4.2-p1Magento Open Source升級時按預期運行。 以前，applicationOpen Source拋出了以下錯誤： `Unable to apply data patch Magento\Paypal\Setup\Patch\Data\UpdateBmltoPayLater for module Magento_Paypal`。 [GitHub-33678](https://github.com/magento/magento2/issues/33678)

<!-- MC-42830-->

* 現在，正確的儲存ID用於在管理訂單發放期間檢索已保存的信用卡。 現在，根據網站範圍配置設定，在訂購期間顯示儲存的信用卡。 以前，當管理員嘗試從管理員為客戶建立訂單並選擇「儲存的信用卡」方法時，儲存的信用卡沒有可用的選項。

<!-- AC-344-->

* PayPal信用顯示禮品卡金額的異常現象已得到解決。 以前，當啟用PayPal信用並配置了多張禮品卡金額時，如果購物者更改了禮品卡價值的金額，則店面不會更新分期付款金額。 這是Magento Open Source2.4.3中的已知問題。

### 貝帕爾

<!-- MC-42154-->

* Magento Open Source在使用PayPal Express付款方法結帳時不再顯示錯誤。 以前，雖然簽出過程已完成，但應用程式開源顯示了以下錯誤： `Something went wrong`。

<!-- AC-722-->

* PayPal Express現在可以按預期從購物車中使用。 以前，當您按一下 **貝帕爾** 按鈕以從購物車開始快速簽出，應用程式「開放源」拋出此錯誤：  `To check out, please sign in with your email address`。 [GitHub-33445](https://github.com/magento/magento2/issues/33445)
...
* Magento Open Source預設情況下在產品保存期間同步調整影像大小。 商家現在可以通過對影像進行少量編輯，將影像調整為背景非同步過程 `di.xml` 的子菜單。
...
* 新 `product_alert` consumer通過建立隊列消息、運行consumer和縮短執行時間來改進客戶警報的發送。 以前，在發送超過100,000個產品警報時，applicationOpen Source拋出了記憶體不足異常。 應用程式Open Source還花費了20多小時來發送所有警報。

<!-- ACP2E-87-->

* 如果「類別」頁上的價格篩選器不包含第二個值，則Magento Open Source不再引發錯誤。 以前，applicationOpen Source在日誌檔案中引發異常。

### 產品視頻

<!-- MC-42105-->

* Magento Open Source現在啟用 **保存** 在添加視頻時輸入不完整的URL時，按鈕和自動完成欄位（管理） **目錄**  > **產品** > **添加視頻**)。 以前， **保存** 按鈕已禁用，且未填充欄位。
...
* Magento Open Source現在按預期從店面刪除HTML標籤 **帳戶** > **我的命令** > **查看訂單** 的子菜單。 以前，applicationOpen Source在店面客戶訂單注釋部分顯示HTML標籤。
...
* 價格範圍的分層導航選項現在與自定義價格屬性一樣適用。 應用程式開放源在篩選自定義價格屬性時使用價格導航步驟的配置。 以前，applicationOpen Source使用手動步驟配置。
...
* `category_name` 已添加到系統保留的產品屬性代碼中。 如果用戶嘗試使用屬性代碼建立產品屬性，則應用程式「開放源」現在會顯示錯誤 `category_name`。 以前，產品未列在預期類別中，並且記錄了Elasticsearch錯誤。
...
* Magento Open Source現在將正確的價格欄位添加到Elasticsearch價格指數映射中。 以前，將錯誤的價格欄位添加到具有儲存ID而不是網站ID的索引中。
...
* 現在，當Magento Open Source在多發運模式下從購物車中刪除產品時，購物者會成功更新迷你購物車，然後切換到單個發運地址。 此前，產品價格未按預期更新。

<!-- AC-267-->

* 使用S3儲存適配器時，現在可以按預期導入表速率。 以前，applicationOpen Source顯示以下錯誤： `File "https://[bucket].s3.eu-central-1.amazonaws.com/[prefix]/tmp/phpLjGmHf" not found`。 [GitHub-33072](https://github.com/magento/magento2/issues/33072)
...
* Magento Open Source現在顯示管理和店面發票頁合計的免費發運成本(0)。 以前，當訂單的發運為零時，應用產品開放來源不會在發票頁發運合計上以合計顯示發運金額。

<!-- ACP2E-120-->

* 對線上運輸承運人的費率請求已從幾個請求減少到一個。 現在，可以正確計算每個項目/包的包裝保險和手續費。 以前，applicationOpen Source顯示了FedEx發運方法的不正確發運費率。

<!-- ACP2E-174-->

* Magento Open Source現在在建立從波多黎各發運的發運標籤時顯示UPS費率。 以前，applicationOpen Source未顯示這些UPS速率。
...
* 「作為客戶登錄」功能現在可以在包含不同URL上的多個儲存的部署中按預期工作。 以前，即使訪問正確的儲存域，applicationOpen Source也未載入正確的儲存。

### 稅

<!-- MC-41924-->

* Magento Open Source現在可以在客戶選擇將物料發運至多個地址後刪除物料時正確計算迷你購物車小計。

<!-- MC-41945-->

* Magento Open Source現在在購物車僅包含虛擬產品時，所有結帳頁上顯示相同的稅率。
...
* 現在，當Magento Open Source在其基URL中配置了子路徑時，將正確顯示通知消息。
...
* Magento Open Source在載入具有預先存在的自定義主題的儲存的首頁時不再顯示錯誤消息。 以前，applicationOpen Source顯示以下消息：  `The store will not work correctly in the case when cookies are disabled`。

### 翻譯和區域設定

<!-- AC-285-->

* 的 `UserExpiration` 驗證程式不再失敗 `de_DE` 和 `uk_UA` 區域設定。 以前，當管理員嘗試在Admin中建立新用戶時，在將語言環境設定為時，applicationOpen Source拋出錯誤 `de_DE` 或 `uk_UA`。 [GitHub-32497](https://github.com/magento/magento2/issues/32497)
...
* 現在，在設定為葡萄牙語（葡萄牙）區域設定的商店中建立新帳戶時，您可以成功輸入客戶出生日期。 以前，Magento Open Source在嘗試建立帳戶時拋出錯誤。 [GitHub-34130](https://github.com/magento/magento2/issues/34130)
...
* Magento Open Source現在會從「聯繫我們」表單電子郵件輸入欄位中裁剪不斷空格字元。 以前，提交表單時未從電子郵件輸入中刪除非中斷空格字元（如果已提供），這會導致日誌檔案中出現錯誤。 的 **回復** 生成給儲存管理員的聯繫人電子郵件中也缺少欄位。

<!-- MC-42793-->

* Magento Open Source現在根據他們在管理員中的位置顯示相關產品、追加銷售產品和交叉銷售產品。
...
* 管理客戶網格現在顯示所有客戶資料，包括新添加的資料 `date` 自定義屬性（如預期）。 以前，applicationOpen Source拋出錯誤，在 `date` 屬性集已設定為列。
...
* Magento Open Source禁用Cookie限制模式後，升級後不再顯示此消息： `The store will not work correctly in the case when cookies are disabled`。 [GitHub-33811](https://github.com/magento/magento2/issues/33811)
...
* Magento Open Source在購物者點擊時不再顯示重複的地址 **更改地址** 部署中的「複查和付款」頁 **地址搜索** 配置設定已啟用。
...
* 查看斷點現在跨Magento Open Source一致。

<!-- ACP2E-199-->

* Magento Open Source現在可以在「管理客戶」頁上正確顯示具有自定義區域設定的自定義客戶日期屬性。

<!-- MC-41760-->

* 使用頁面生成器添加的影像在以768像素的瀏覽器窗口寬度顯示時不再複製到店面。 以前，使用此瀏覽器窗口寬度，應用程式開源會通過顯示移動和案頭版本的影像在店面上複製影像。
...
* 現在，可以使用的子類建立對象 `\Magento\Framework\Api\AbstractSimpleObjectBuilder` 七點三菲律賓比索。以前， `preg_match` 在Redhat上承載帶PHP 7.3的Magento Open Source時拋出此警告消息： `Warning: preg_match(): Compilation failed: unrecognized character follows...`。
...
* Magento Open Source現在生成 `customertoken` 按GraphQL或REST API請求執行，如多次連續失敗登錄嘗試後預期的那樣。 以前， applicationOpen Source未檢查 `lock_expires_at` 在 `oauth_token_request_log` 大於當前日期和時間，並始終返回失敗嘗試的次數，這使客戶無法登錄。 [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* Magento Open Source在 **在提要欄中顯示** 「wish list（希望清單）」選項已禁用。 以前，applicationOpen Source忽略了此選項。
...
**問題**:在「管理」中未選擇任何選項時，購物者無法將產品添加到購物車 **儲存** >  **配置** > **常規** > **國家/地區選項** > **允許國家/地區**。 而應用程式Open Source會顯示以下控制台錯誤： `Failed to load resource: the server responded with a status of 400 (Bad Request)`。 <!-- AC-2564-->
