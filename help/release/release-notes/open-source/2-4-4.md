...
請參閱[Adobe安全性公告](https://helpx.adobe.com/tw/security/products/magento/apsb22-13.html)，瞭解這些已修正問題的最新討論內容。
...
* 新增機制來限制使用者可在系統範圍內透過Web API要求的大小和資源數量，以及覆寫個別模組的預設值。 如此可解決`MC-43048__set_rate_limits__2.4.3.patch`所解決的問題。 請參閱 [API 安全性](https://developer.adobe.com/commerce/webapi/get-started/api-security/)。 <!-- AC-1120-->
...
* 現在支援 TinyMCE 5.8.1。 TinyMCE 4 已從代碼庫中刪除。 <!-- AC-41-->
...
* **新店面相關的管理員配置設置**。 `storeConfig`查詢現在返回零小計結帳和支票/匯票付款方式的配置設置。<!-- PWA-1576-->

* **更新核心 GraphQL 資料庫**。 使 `webonyx` 核心 GraphQL 能夠運行的 資料庫 已升級到版本 ^14.9。 <!-- PWA-2137 2184 -->

* **修正 GraphQL 中多網站、多語言商店**&#x200B;的翻譯問題。 GraphQL 解析器現在會按預期根據商店範圍返回已翻譯的字符串。 <!-- PWA-1946-->

* GraphQL 現在為 新 Relic 提供了描述性的事務名稱，這有助於調試。 [GitHub-30915](https://github.com/magento/magento2/issues/30915) <!-- PWA-1311-->

[有關這些增強功能的詳細資訊，請參閱 GraphQL 開發人員指南](https://developer.adobe.com/commerce/webapi/graphql/)。

### 供應商捆綁的擴展

除了[Braintree](https://experienceleague.adobe.com/zh-hant/docs/commerce-admin/stores-sales/payments/braintree)之外，所有廠商隨附的擴充功能已從Magento Open Source2.4.4程式碼基底中移除。 商戶應移轉至可在[Commerce Marketplace](https://marketplace.magento.com/extensions.html)上取得的正式副檔名。
...
* 現在，當 `addProductsToCart` 用于添加具有多個選擇選項的捆綁產品時，突變會將所有選定的選項添加到購物車。 以前，突變僅添加最後一個選擇。 [GitHub-33123](https://github.com/magento/magento2/issues/33123)
...
* 页面建立工具現在可以使用所見即所得的編輯者正確呈現自定義介面工具集。

## 已知問題

**問題**：從 Adobe Systems Commerce 2.4.4 升級到 Adobe Systems Commerce 2.4.4-p1 的過程中，商家可能會注意到軟體包版本降級通知。 可以忽略這些消息。 包版本的差異是由於包生成期間的異常造成的。 沒有任何產品功能受到影響。 請參閱從2.4.4升級為2.4.4-p1[&#128279;](https://support.magento.com/hc/en-us/articles/8214752983949)知識庫文章後降級的套件，瞭解受影響的案例和因應措施。
