...
Magento Open Source改善了平台品質、付款方法、GraphQL快取效能和協助工具。 其中包括整合Google模組的更新。
...
* 此 `bin/magento config:set graphql/session/disable 1` 命令可讓商戶完全停用所有GraphQL作業的工作階段cookie建立作業。 依預設，應用程式開放原始碼會建立這些Cookie，並依賴它們進行授權，進而影響效能。 日後，我們建議使用代號作為GraphQL請求的唯一授權形式。 我們不建議單獨使用工作階段Cookie或搭配授權Token使用。 請參閱 [GraphQL授權](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html). <!-- B2B-2204 -->
...
* Magento Open Source工作階段在提出GraphQL要求後不再結束。 之前， `ClearCustomerSessionAfterRequest` 外掛程式將購物者登出。 [GitHub-34550](https://github.com/magento/magento2/issues/34550)
