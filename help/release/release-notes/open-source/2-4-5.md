...
* ACL 資源已添加到清單中。 <!-- AC-1622-->

* 庫存範本安全性已得到增強。

* 篩選器 `MaliciousCode` 已升級為使用該 `HtmlPurifier` 資料庫。 <!-- AC-1498-->
...
* DHL 整合綱要已從 v6.0 更新至 v6.2。 <!-- AC-3022-->

* USPS 運輸的預設閘道URL已更新為使用 `https` `http`而不是 。 <!-- AC-2426-->

* 資料庫 `Froogaloop` 已被 Vimeo `Player.js` 資料庫 （2.16.4） 取代。 <!-- AC-2527 2262-->

* `grunt-eslint` （NPM） 資料庫已升級到最新版本。<!-- AC-2700-->

* 已將`jQuery Storage`資料庫取代為`julien-maurel/js-storage`。<!-- AC-2627-->

* 靜態`phpcs`代碼分析工具現在`php-cs-fixer`與 PHP 8.x 相容。<!-- AC-2259-->
...
* 該 `bin/magento config:set graphql/session/disable 1` 命令允許商家完全禁用為所有 GraphQL 操作創建工作階段 cookie。 默認情況下，應用程式 Open 來源 會創建這些 Cookie 並依賴它們進行授權，這會影響性能。 日後，我們建議使用權杖作為GraphQL要求的唯一授權形式。 我們不建議單獨使用工作階段Cookie或將其與授權Token搭配使用。 請參閱[GraphQL授權](https://developer.adobe.com/commerce/webapi/graphql/authorization-tokens.html)。<!-- B2B-2204 -->
