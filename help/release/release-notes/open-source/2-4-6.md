...
迄今為止，尚未發生與這些問題相關的確認攻擊。 但是，某些漏洞可能會被利用來訪問客戶信息或接管管理員會話。 其中大多數問題要求攻擊者首先獲取對管理員的訪問許可權。 因此，我們提醒您採取一切必要措施來保護您的管理員，包括但不限於以下工作：
...
請參閱[Adobe安全性公告](https://helpx.adobe.com/security/products/magento/apsb23-17.html)，瞭解這些已修正問題的最新討論內容。
...
* **新系統配置，以便在管理員用戶更改其電子郵件**&#x200B;時要求電子郵件確認。 為了減少網站漏洞，添加了一個新的系統配置設置（預設禁用），以便在管理員用戶更改其電子郵件時要求電子郵件確認： **[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** >> **[!UICONTROL Customers]** **[!UICONTROL Customer Configuration]** > **[!UICONTROL Account Information Options]** > **[!UICONTROL Require email confirmation if an email has been changed]**。 啟用此選項后，每次管理員用戶更改其電子郵件地址時，應用程式都將請求電子郵件確認。 以下模組受此變更影響： `Magento_Customer`。 <!-- AC-3064 -->
...
* OpenSearch 現已支持作為 Magento Open Source 本地和雲端部署的預設搜尋引擎。 支援作為Magento Open Source的獨立搜尋引擎。 此版本支援 OpenSearch v2.x，並已使用 OpenSearch 2.5 進行測試。儘管與 OpenSearch 1.x 的相容性仍然存在，但我們建議將此版本與 OpenSearch 2.x 一起使用。您可以使用 OpenSearch 2.x 在雲和本地 [Search引擎先決條件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html)上安裝和運行Magento Open Source和Magento Open Source 2.4.6。 <!-- AC-449 3555 -->

* Elasticsearch8.x。請參閱[升級Elasticsearch](../../../upgrade/prepare/prerequisites.md#upgrade-elasticsearch)。<!-- AC-6597 -->

* MariaDB 10.6 （LTS 版本）。 此版本仍與MariaDB 10.4相容，但建議您升級至MariaDB 10.6。<!-- AC-6921 -->

#### 其他升級和更換

* DHL 綱要 已從 v6.2 遷移至 v10.0。 <!-- AC-3023-->

* 過時的JavaScript資料庫已更新到最新版本，並且已刪除過時的依賴項。 這些更改可向後相容。 <!-- AC-2036-->
...
* 已從Commerce和Magento Open Source程式碼庫中移除`jquery-migrate`。 [GitHub-21406](https://github.com/magento/magento2/issues/21406) <!-- AC-1199 -->

* 已將`jQuery/fileUpload`資料庫升級至v10.32。<!-- AC-2658-->

* 已從程式碼基底中移除已到期的Zend架構(ZF1)元件。 請參閱[不向後相容的變更標示](https://developer-stage.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/).<!-- AC-1512 -->

* Laminas 依賴項已更新為支援 PHP 8.2。 <!-- AC-7019-->

* 已移除`laminas/laminas-dependency-plugin`「撰寫器」外掛程式。 [GitHub-36515](https://github.com/magento/magento2/issues/36515) <!-- AC-7100-->
...
新的`adobe-ims-metapackage` v2.2.0與Adobe Commerce 2.4.6和Magento Open Source2.4.6搭配。此版本未變更任何功能。<!-- CABPI-505-->

### Braintree

<!-- BUNDLE-3107-->

* 義大利和西班牙的&#x200B;**[!UICONTROL Pay Later]**&#x200B;按鈕和橫幅（訊息）已啟用Braintree。
...
* 新增詐騙防護和ACH網頁鉤點。 當Braintree中做出風險決策時，就會觸發詐騙防護Web勾點。 當 ACH 付款的狀態更新為 `settled` 或 `settlement decline`時，將觸發 ACH 網路挂鉤。
...
**改進了在啟用類別**&#x200B;許可權的情況下查詢類別時的響應時間。 查詢已啟用類別許可權且包含許多客戶群組或共用類別之類別中的產品時，回應時間已有所改善。 支援此效能改善的程式碼變更包括： <!-- B2B-2179 2404 2329-->
...
* 改進類別 ID 擷取和 `getChildren` 減少呼叫。 <!-- B2B-2464-->
* 改進了類別子項負載。<!-- B2B-2469-->

**减少批量購物車操作查詢響應時間**。 當新增最多500個簡單且可設定的產品時，「新增產品」變動已經過最佳化，以提升查詢效能。 套裝產品不受影響。 最佳化的變動包括`addProductsToCart`、`addConfigurableProductsToCart`、`addDownloadableProductsToCart`、`addSimpleProductsToCart`、`addVirtualProductsToCart`、`addWishlistItemsToCart`和`updateCartItems`。<!-- B2B-2331 2423-->
...
PWA Studio v.13.0.x 與 Magento Open Source 2.4.6 相容。其中包含多項改良功能，以改善輔助功能。 有關錯誤修復的資訊，請參閱 [PWA Studio版本](https://github.com/magento/pwa-studio/releases)。 有關 PWA Studio 版本及其兼容Adobe Systems Commerce 核心版本的清單，請参閱 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 。
...
* 當您在執行資料移轉工具的部署中執行`bin/magento setup:upgrade`命令時，Magento Open Source不再擲回下列例外狀況： `PHP Fatal error:  Declaration of Migration\Logger\Logger::addRecord(int $level, string $message, array $context = []): bool must be compatible with Monolog\Logger::addRecord(int $level, string $message, array $context = [], ?Monolog\DateTimeImmutable $datetime = null): bool in /var/www/html/vendor/magento/data-migration-tool/src/Migration/Logger/Logger.php on line 34`。 [GitHub-35604](https://github.com/magento/magento2/issues/35604)

<!-- AC-5970-->

* 該 `bin/magento setup:db:status` 命令在運行MySQL 8.0.29的環境中不再失敗。以前，命令輸出錯誤地指出聲明性綱要已過時。 [GitHub-35671](https://github.com/magento/magento2/issues/35671)
...
* 現在使用荷蘭文地區設定時，可以從管理員 **[!UICONTROL Catalog]** > **[!UICONTROL Products]** 成功篩選捆綁產品。 之前，當您嘗試篩選 `Type: Bundle Product`時，Magento Open Source 顯示此錯誤： `Something went wrong with processing the default view and we have restored the filter to its original state`。 [GitHub-35704](https://github.com/magento/magento2/issues/35704)

<!-- ACP2E-1038-->

* 當捆綁產品的特殊價格超過 1000 時，產品網格不再引發異常。 以前，當您按 SKU 搜尋產品或導航到 **[!UICONTROL Catalog]** > **[!UICONTROL Products]**&#x200B;時，Magento Open Source拋出此錯誤： `Non Numeric value encountered`。 [GitHub-36284](https://github.com/magento/magento2/issues/36284))
...
* 訂單履行后不再刷新全頁面緩存，除非訂購的產品缺貨。 以前，每當建立出貨時，就會清除類別的快取。
...
* 匯出 **[!UICONTROL Maximum Qty Allowed in Shopping Cart]** 的值現在與管理員配置設置中的預設值匹配。

<!-- ACP2E-889-->

* Magento Open Source現在可以正確計算具有運費折扣的訂單退款總額。
...
* 現在，當購物者更改購物車頁面上的產品數量而不更新購物車，然後嘗試導航到下一個頁面時，Magento Open Source會按預期顯示確認彈出窗口，然後將購物者重定向到下一個頁面。

<!-- AC-5987-->

* 購物者現在可以在管理中降低產品購物車后，從降低該產品的數量。 先前，當購物者嘗試減少產品數量時，Magento Open Source擲回下列錯誤： `The requested qty is not available`。 [GitHub-35780](https://github.com/magento/magento2/issues/35780)

<!-- AC-6744-->

* 當產品數量在多重送貨結帳期間超過設定的上限時，Magento Open Source不再在JavaScript主控台中擲回錯誤。 Magento Open Source現在改為顯示資訊性錯誤訊息。 [GitHub-36184](https://github.com/magento/magento2/issues/36184)
...
* 現在當送貨位址包含擴展屬性時，可以成功完成結帳。 之前，Magento Open Source拋出這個錯誤： `Object of class Magento\Quote\Api\Data\AddressExtension could not be converted to string`。 [GitHub-34202](https://github.com/magento/magento2/issues/34202)
...
* 啟用后，配置 **[!UICONTROL Display Out of Stock Products]** 選項現在可如預期般運作。 以前，在管理員上啟用此選項時，所有產品在店面上都顯示為缺貨。 [GitHub-35898](https://github.com/magento/magento2/issues/35898)
...
* 包含所有缺貨子產品的可配置產品現在在店面上顯示為缺貨。

<!-- AC-6852-->

* 可精確顯示可配置的產品 SKU 和產品名稱。 以前，當您將屬性群組移到>**[!UICONTROL Attribute set]**&#x200B;上的&#x200B;**[!UICONTROL Stores]**“產品詳細資訊”群組上方後，名稱和 SKU 顯示為未定義。[GitHub-36309](https://github.com/magento/magento2/issues/36309)
...
* 現在，當使用 PayPal Express 付款方式結帳時應用獎勵抵用券時，獎勵積分將按預期添加到客戶的帳戶中。

<!-- ACP2E-1225-->

* 現在，當購物車價格規則已權重為條件時，您可以在管理員訂單創建期間應用抵用券代碼。 以前，如果沒有為產品選擇其他字段，則不會設置抵用券代碼。 [GitHub-36271](https://github.com/magento/magento2/issues/36271)
...
* 自訂貨幣符號和價格之間缺少的空格已新增到店面和管理產品價格。 添加了適當的條件和正則表達式運算式，以允許在貨幣符號中自定義 NBSP。 例如，IDRx 105.00 先前顯示為IDRx105.00。
...
* 現在，在執行Magento Open Source2.4.3-p2的部署中，可如預期重新授權OAuth整合。 先前重新授權失敗，錯誤為： `The attempt to post data to consumer failed due to an unexpected error. Please try again later`。 Magento Open Source也嘗試解密未加密的值。 [GitHub-35313](https://github.com/magento/magento2/issues/35313)
...
* `getFilteredTimerIds`方法現在可如預期般運作。  類別`Magento\Framework\Profiler\Driver\Standard\Stat`現在與PHP 8.1相容。之前，呼叫此方法時Magento Open Source擲回例外狀況。 [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* 當管理員使用者從管理員統計儀表板選取&#x200B;**[!UICONTROL Last 24 Hours]**&#x200B;選項時，Magento Open Source現在會顯示準確的時間範圍。 以前，Magento Open Source顯示錯誤的時間範圍。 發生此狀況是因為資料庫的時區保持不變。

<!-- ACP2E-1066-->

* 現在可以為整數和十進位數量生成貸項通知單。 以前僅支援整數數量。

<!-- ACP2E-1001-->

* 僅類型 **數值 的客戶郵遞區編碼Code輸入驗證規則現在在欄位中按預期&#x200B;**[!UICONTROL Edit Customer Address]**工作（**[!UICONTROL Store]**>**[!UICONTROL Attribute]**>**[!UICONTROL Customer Address]**）。**&#x200B;先前儲存客戶地址資訊時不會進行郵遞區號驗證。
...
* `recordLogin`該方法已更新，可在用戶模型上設置登入值，並在表的 `admin_user` 和 `logdate` `lognum` 字段中保留正確的值。以前，不會記錄更新管理員使用者的登入嘗試。
...
* `products`查詢現在只會傳回與指定範圍關聯的評論。 以前，查詢會從所有商店返回評論。

<!-- ACP2E-882-->

* 現在在 GraphQL 請求期間會遵守類別許可權。
...
* 現在，當將 AWS S3 用作遠端儲存時，完全匯出可以正常工作。 之前，導出會因上傳檔案大小而失敗。

<!-- ACP2E-982-->

* 現在，當缺少CSV檔中的庫存數據列時，產品狀態將正確導入。 先前，在產品匯入後，預設來源會自動指派給產品，其數量設為零，且當CSV檔案中的存貨資料欄遺失時，其狀態會列為無庫存。
...
* `Fixer.io`導入服務現在按預期工作。現在已成功匯入貨幣匯率。 [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
* 協力廠商 `colinmollenhour/php-redis-session-abstract` 模組 和 PHP 8.1 的問題已得到解決。 [GitHub-35634](https://github.com/magento/magento2/issues/35634)
...
* 啟用非同步索引時，為免費產品下自動開立商業發票的訂單現在可如預期運作。 這些訂單的`sales_order_grid or sales_invoice_grid`資料表中未新增任何專案。 之前，Magento Open Source擲回例外狀況，因為應用程式嘗試在排序時索引格線，而不是依賴`cron`。 [GitHub-36334](https://github.com/magento/magento2/issues/36334)
...
* 啟用BraintreePayPal並停用&#x200B;**[!UICONTROL Enable Card Payments]**&#x200B;時，使用Braintree付款方式的結帳現在會順利完成。 先前，進度列從未解析，且Magento Open Source顯示此主控台錯誤： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`。 [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* 當購物者更新其帳單地址時，Braintree保險櫃付款方式選項在結帳工作流程中保持選中狀態。 以前，當購物者取消選擇 **[!UICONTROL My billing and shipping address are the same]** 按鈕時，會取消選擇付款方式。

<!-- BUNDLE-3184-->

* 現在當管理員啟用配置設定時， **[!UICONTROL Send Line Items]** 會為PayPal交易傳遞行專案。

<!-- BUNDLE-3187-->

* ACH 付款方式 **[!UICONTROL Account Number]** 欄位上的驗證邏輯現在支援 4 到 17 位文字。
...
* 價格索引器現在在大型目錄的部分重新索引期間替換記錄后，按預期刪除過時的數據。 以前，重新索引過程會在替換過時的記錄之前將其刪除，這會導致產品偶爾會暫時從店面消失。 [GitHub-35616](https://github.com/magento/magento2/issues/35616)
...
* 已優化包含目錄微件產品清單的頁面載入。 以前，非全域屬性是直接從資料庫載入的。
...
* 中的`Braintree/Ui/Component/Report/Listing/Column/PaymentType.php`常`ANDROID_PAY_CARD`量已取代 `GOOGLE_PAY_CARD`。此功能可解決 Braintree 報表的問題。

### 評論

<!-- ACP2E-1214-->

* 對特定範圍具有受限訪問許可權的管理員用戶現在可以按預期刪除產品評論。
...
* Magento Open Source現在在显示送貨或客戶地址時，會遵循管理員客戶 **[!UICONTROL Show Company]** 配置設置。 禁用此設置后，發貨公司將不再包含在發貨、客戶地址或資料庫訂單記錄中。 以前，儘管設置了禁用，仍會顯示資訊。 [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* 當沒有與客戶地址相關聯的VAT ID時，自動客戶群組指派現在可運作。 若地址不包含VAT識別碼，則會將其視為屬於非VAT註冊客戶，而Magento Open Source會將客戶群組變更為非VAT群組。 先前，啟用&#x200B;**[!UICONTROL Enable Automatic Assignment to Customer Group]**&#x200B;時，從客戶帳戶刪除VAT ID時，不會自動將客戶帳戶指派給`customerGroup`。
...
* 內聯翻譯現在可以正確處理特殊字元，包括位於 SCRIPT 標籤中的短語中的擴展 Unicode 字元。 以前，使用內嵌翻譯時，日文字符在編輯后的腳本中會不正確地呈現。

<!-- ACP2E-844-->

* `ñ`產品名稱中的字元現在可在產生金鑰時轉換為`n`URL。
...
* 現在，網站特定貨幣可如預期般用於管理產品網格中的特殊價格、成本和MSRP欄。 以前，按特殊價格商店視圖篩選時，管理目錄產品網格中不會顯示正確的貨幣符號。

<!-- ACP2E-980-->

* 產品自定義選項的拖放功能現在如預期般運作。 現在，當可自訂選項格線中有多個頁面可用時，您可以將可自訂選項從一個頁面移動到另一個頁面。

<!-- ACP2E-866-->

* 管理儀錶板圖表現在按預期顯示從當前日曆年開始到當前日期的 YTD 時間間隔。 以前，日期範圍包含超過12個月。

<!-- AC-3088-->

* 已新增店面首頁上遺失的默認圖示。 [GitHub-35493](https://github.com/magento/magento2/issues/35493)

<!-- ACP2E-1025-->

* 管理員現在可成功顯示每個管理用戶的最新登入日期。 以前，當管理員首次登錄到管理員用戶時，管理員用戶網格上會顯示一個空的登入日期值。
...
* 結帳期間新增的地址現在會保持其預期格式。 以前，空白行被忽略。

<!-- ACP2E-897-->

* 更改產品網格的列網格視圖和為自定義視圖選擇新列不再觸發“選擇所有網格列”操作。 以前，欄框會消失，而網格線中的產品是選取或取消選取。 [GitHub-36302](https://github.com/magento/magento2/issues/36302)
...
* 購物車中的產品名稱現在會如預期使用指定的商店檢視值。 以前，購物車中的產品名稱一律會傳回 REST API 購物車回應中的預設商店視圖值。

<!-- AC-5986-->

* 現在可以使用 REST API 更新購物車專案自訂選項值。 [GitHub-35768](https://github.com/magento/magento2/issues/35768)

<!-- AC-1330-->

* 超過 20 `sourceItems` 個的批量 API 請求現已在啟用了清單的部署中成功完成。

<!-- ACP2E-1044-->

* Fixer 貨幣服務現在使用通過 ApiLayer 生成的金鑰按預期工作，並且正確匯出貨幣匯率。 之前，當商家嘗試透過管理匯入貨幣匯率時，Magento Open Source會引發錯誤。
...
**問題**：若已啟用免運費(**[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL Sales]** > **[!UICONTROL Delivery Methods]**)，且顯示的運費含稅，則您無法建立銷退折讓單。 當您嘗試創建貸項通知單時，Adobe Systems商務會引發以下異常： `"Division by zero".vendor/magento/module-sales/Model/Order/Creditmemo/Total/Tax.phpLine 139`。 [GitHub-36800](https://github.com/magento/magento2/issues/36800)

**解決辦法**：現已提供此問題的 Hot Fix。 轉到 [品質補丁工具：補丁](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) Search和補丁 ACSD-50814 搜尋。
...
您可以使用 Composer](../../../installation/composer.md) 安裝 [Magento Open Source 2.4.6。
