...
description: 了解2.4.6版Magento Open Source的新功能、增強功能、錯誤修正和已知問題。
...
Magento Open Source2.4.6引入了對PHP 8.2的支援。PHP 8.1仍完全受支援，但已取消對PHP 7.4的支援。 其中包括顯著的效能和可擴充性增強功能。 已最佳化大量購物車作業和類別樹狀轉譯回應的GraphQL作業。

此版本包含品質修正和增強功能。 核心撰寫器相依性和第三方程式庫已升級至與PHP 8.2相容的最新版本。

{{bics}}

## 其他發行資訊

雖然這些功能的程式碼與Magento Open Source核心程式碼的發行搭配，但其中數個專案也是獨立發行。 這些專案的錯誤修正會記錄在每個專案的檔案中，提供個別專案的發行資訊。
...
Magento Open Source2.4.6引入了對PHP 8.2的支援。PHP 8.1仍完全受支援。 <!-- AC-6598 -->

此版本已移除PHP 7.4支援。 不能在PHP 7.4上運行Magento Open Source2.4.6。
...
* 現在支援OpenSearch作為Magento Open Source內部部署和雲端部署的預設搜尋引擎。 支援作為獨立的Magento Open Source搜尋引擎。 此版本支援OpenSearch v2.x，並已與OpenSearch 2.5進行測試。雖然與OpenSearch 1.x的相容性仍然存在，但建議將此版本與OpenSearch 2.x搭配使用。您可以在雲端和內部部署上，透過OpenSearch 2.x安裝並執行Magento Open Source和Magento Open Source2.4.6 [搜尋引擎必要條件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html). <!-- AC-449 3555 -->

* Elasticsearch8.x。請參閱 [升級Elasticsearch](../../../upgrade/prepare/prerequisites.md#upgrade-elasticsearch). <!-- AC-6597 -->
...
* PHP 7.4相容性已從商務和Magento Open Source代碼庫中刪除。 <!-- AC-2841-->
...
* `jquery-migrate` 已從商務和Magento Open Source程式碼基底中移除。 [GitHub-21406](https://github.com/magento/magento2/issues/21406) <!-- AC-1199 -->
...
下列Adobe IMS模組已與Adobe Commerce和Magento Open Source核心程式碼脫鈎，現在已封裝為個別的元包：

* `adobe-ims`
* `adobe-ims-api`
* `admin-adobe-ims`

新 `adobe-ims-metapackage` v2.2.0與Adobe Commerce 2.4.6和Magento Open Source2.4.6搭配。此版本中沒有任何功能變更。<!-- CABPI-505-->
...
頁面產生器1.7.3版與Magento Open Source2.4.6相容。

### PWA Studio

PWA Studiov.13.0.x與Magento Open Source2.4.6相容。它包含多項增強功能，可改善協助工具。 如需錯誤修正的詳細資訊，請參閱 [PWA Studio發行](https://github.com/magento/pwa-studio/releases). 請參閱 [版本相容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 以取得PWA Studio版本及其相容Adobe Commerce核心版本的清單。
...
* Magento Open Source在執行 `bin/magento setup:upgrade` 部署中已運行資料遷移工具的命令： `PHP Fatal error:  Declaration of Migration\Logger\Logger::addRecord(int $level, string $message, array $context = []): bool must be compatible with Monolog\Logger::addRecord(int $level, string $message, array $context = [], ?Monolog\DateTimeImmutable $datetime = null): bool in /var/www/html/vendor/magento/data-migration-tool/src/Migration/Logger/Logger.php on line 34`. [GitHub-35604](https://github.com/magento/magento2/issues/35604)

<!-- AC-5970-->

* 此 `bin/magento setup:db:status` 命令在運行MySQL 8.0.29的環境中不再失敗。以前，命令輸出錯誤地指出聲明性架構已過時。 [GitHub-35671](https://github.com/magento/magento2/issues/35671)
...
<!-- AC-2681-->
現在，當您使用命令列來設定不含密碼的Redis連線時，Magento Open Source會擲回參數驗證錯誤。
...
* 現在，您可以從管理員成功篩選套件產品 **[!UICONTROL Catalog]** > **[!UICONTROL Products]** 使用荷蘭語地區設定時。 之前，當您嘗試篩選時，Magento Open Source會顯示此錯誤 `Type: Bundle Product`: `Something went wrong with processing the default view and we have restored the filter to its original state`. [GitHub-35704](https://github.com/magento/magento2/issues/35704)

<!-- ACP2E-1038-->

* 當捆綁產品的特殊價格超過1000時，產品網格不再擲回例外。 以前，當您依SKU搜尋產品或導覽至 **[!UICONTROL Catalog]** > **[!UICONTROL Products]**,Magento Open Source擲出此錯誤： `Non Numeric value encountered`. [GitHub-36284](https://github.com/magento/magento2/issues/36284))
...
* 在GraphQL中包含 `BundleProduct` 片段， `items.options.product` 物件包含預期的資料。 以前，某些情況下物件可能為null。
...
* GraphQL產品查詢現在只會傳回查詢之商店範圍內的套件組合產品選項。
...
* Magento Open Source現在可正確計算帶有運費折扣的訂單退款的總金額。

<!-- ACP2E-876-->

* Google reCAPTCHA現在可如預期般運作於店面訂單。 以前，當購物者首次嘗試下訂單之前未先選取Google ReCAPTCHA時，Magento Open Source會顯示以下錯誤： `ReCaptcha validation failed, please try again` 和 `No such cart with id = ID`.
...
* Magento Open Source現在會在購物者變更購物車頁面上的產品數量而未更新購物車，然後嘗試導覽至下一頁時，如預期般先顯示確認快顯視窗。

<!-- AC-5987-->

* 購物者現在可以在管理員中減少產品後，從購物車減少其數量。 之前，當購物者嘗試減少產品數量時，Magento Open Source擲回下列錯誤： `The requested qty is not available`. [GitHub-35780](https://github.com/magento/magento2/issues/35780)

<!-- AC-6744-->

* 在多次運送結帳期間，當產品數量超過設定的上限時，Magento Open Source不再在JavaScript主控台中擲回錯誤。 現在，Magento Open Source會顯示資訊性錯誤訊息。 [GitHub-36184](https://github.com/magento/magento2/issues/36184)
...
* 購物者無法再從類別新增產品至購物車，在變更購物者所屬的客戶群組後，該類別現在不允許。 Magento Open Source現在會在權限組態設定變更時，檢查特定類別的客戶群組權限存取權。
...
* 當運送地址包含擴充功能屬性時，結帳現在可以成功完成。 以前，Magento Open Source擲回此錯誤： `Object of class Magento\Quote\Api\Data\AddressExtension could not be converted to string`. [GitHub-34202](https://github.com/magento/magento2/issues/34202)

<!-- AC-2793-->

* 此 `addBundleProductsToCart` 如果所需的產品選項沒有值，突變不再將產品添加到報價中。 [GitHub-25676](https://github.com/magento/magento2/issues/25676)

<!-- B2B-2458-->

* Magento Open Source現在會在 `addProductsToCart` 在權限不足的情況下將產品新增至購物車時，變異會失敗。

<!-- ACP2E-1242-->

* 當使用快速訂購來新增數量超過可用庫存的產品時，Magento Open Source不再在購物車頁面上擲回錯誤。
...
* Magento Open Source重新載入產品詳細資料頁面時，不再顯示上一頁的資料。 之前，重新載入產品詳細資料頁面時，第一個索引標籤的資料會與新載入的資料合併，並持續一秒。
...
* Magento Open Source不再錯誤地假設某個類別是根據類別路徑而另一個類別的父類別。 以前，當兩個類別相鄰，且一個類別ID以另一個類別的類別ID開頭時，Adobe Commerce會假設這些類別具有相同的父類別。 [GitHub-32464](https://github.com/magento/magento2/issues/32464)
...
* 當管理員使用者手動將產品新增至已指派許多簡單產品至其的可設定產品時，Magento Open Source不會再擲回錯誤。 以前，Magento Open Source顯示此錯誤： `Something went wrong with processing the default view and we have restored the filter to its original state`.

<!-- AC-2877-->

* 在部署中，購物者無法再將可售數量為0的可設定產品新增至購物車，同時停用後期訂單。 Magento Open Source現在會指出產品無存貨。 [GitHub-35319](https://github.com/magento/magento2/issues/35319)
...
* 此 `catalog_index_refresh_price` cron作業現在會成功執行。 以前，Magento Open Source在cron執行期間記錄了此錯誤： `report.ERROR: Cron Job catalog_index_refresh_price has an error: SQLSTATE[HY093]: Invalid parameter number: number of bound variables does not match number of tokens`.
...
* Magento Open Source從管理員建立訂單後，不會再傳送客戶重複的電子郵件。
...
* 執行Magento Open Source2.4.3-p2的部署時，OAuth整合現在可如預期重新授權。 以前，重新授權失敗，此錯誤為： `The attempt to post data to consumer failed due to an unexpected error. Please try again later`. Magento Open Source還試圖解密未加密的值。 [GitHub-35313](https://github.com/magento/magento2/issues/35313)
...
* AMQP消費者不再因已過時而失敗 `trim()` 在運行PHP 8.1的部署上運行。以前，當AMQP使用者在安裝後啟動時，Magento Open Source擲回了此錯誤： `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in .../vendor/magento/framework-amqp/Config.php on line 143`. [GitHub-35380](https://github.com/magento/magento2/issues/35380)

<!-- AC-2882-->

* 此 `getFilteredTimerIds` 方法現在可如預期般運作。  類別 `Magento\Framework\Profiler\Driver\Standard\Stat` 現在與PHP 8.1相容。以前，調用此方法時Magento Open Source引發異常。 [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* Magento Open Source現在會在管理員使用者選取 **[!UICONTROL Last 24 Hours]** 選項（在「管理統計資料」控制面板中）。 以前，Magento Open Source顯示的時間範圍不正確。 發生此情況是因為資料庫的時區保持不變。
...
* 此 `categoryList` 查詢現在會針對錨記和取消錨記的父類別，傳回精確的產品計數。 新 `is_anchor` 屬性已新增至類別集合的「選取的欄」清單。 以前，請求傳回0個產品計數。 [GitHub-36300](https://github.com/magento/magento2/issues/36300)
...
* 在 `products` 查詢現在包括價格類型屬性的正確值 `attribute_code` 和 `label`. [GitHub-28878](https://github.com/magento/magento2/issues/28878)
...
* 匯入與現有產品具有相同數值URL索引鍵，但沒有其終端字母字元的產品，在 **[!UICONTROL Product URL Suffix]** 在「管理」中設為empty 。
...
* 啟用非同步索引時，為免費產品下單自動開票的訂單現在可如預期運作。 不會將任何項目新增至 `sales_order_grid or sales_invoice_grid` 表格。 以前，Magento Open Source會引發例外，因為應用程式嘗試在訂單放置期間索引網格，而不是依賴 `cron`. [GitHub-36334](https://github.com/magento/magento2/issues/36334)
...
* Magento Open Source在商戶點按 **[!UICONTROL Reports]** > **[!UICONTROL Braintree Settlement]**: `Error: Undefined constant Braintree\PaymentInstrumentType::ANDROID_PAY_CARD in /var/www/html/vendor/paypal/module-braintree-core/Ui/Component/Report/Listing/Column/PaymentType.php:49`. [GitHub-35434](https://github.com/magento/magento2/issues/35434)
...
* 管理員使用者現在可以在啟用Braintree付款方法時建立訂單或重新排序。 以前，Magento Open Source擲回此錯誤： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.

<!-- BUNDLE-3139-->

* 現在，當啟用BraintreePayPal時，使用Braintree付款方法的結帳已成功完成，並 **[!UICONTROL Enable Card Payments]** 已停用。 以前，進度列從未解析過，Magento Open Source顯示了此控制台錯誤： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`. [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* 現在啟用Braintree付款方法時，管理員使用者可以為客戶建立訂單或重新排序。 以前，Magento Open Source擲回此錯誤： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`

<!-- BUNDLE-3139-->

* 購物者現在可以在商店中使用Braintree付款方法， **[!UICONTROL Enable Card Payments]** 已被禁用。 以前，Magento Open Source擲回此JavaScript錯誤： `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* 引號內的字串搜尋現在可如預期般運作 **[!UICONTROL Minimum Terms to Match]** 組態設定已啟用。 以前，Magento Open Source擲回此錯誤： `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Magento Open Source現在會接受管理員客戶 **[!UICONTROL Show Company]** 設定。 禁用此設定後，發運公司將不再包含在發運、客戶地址或資料庫訂單記錄中。 以前，儘管設定了禁用，仍顯示資訊。 [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* 將折扣套用至發運成本時，Magento Open Source不再擲回最小訂單金額錯誤。
...
* 新增 `price_excl_tax` 和 `price_excl_tax` 輸出的類型 `setShippingMethodsOnCart` 變異。 [GitHub-31206](https://github.com/magento/magento2/issues/31206)
...
* Magento Open Source現在可以在運行PHP 8.1的部署中，在店麵類別頁面上顯示帶稅的分組產品。以前，Magento Open Source未顯示產品清單，並引發此異常： `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`. [GitHub-35500](https://github.com/magento/magento2/issues/35500)
...
* 當沒有與客戶地址關聯的增值稅ID時，自動客戶組分配現在可以工作。 如果地址不包含增值稅ID，則該地址被視為屬於非增值稅註冊客戶，而Magento Open Source會將客戶組更改為非增值稅組。 以前，當 **[!UICONTROL Enable Automatic Assignment to Customer Group]** 啟用後，客戶帳戶不會自動指派給 `customerGroup` VAT ID從客戶帳戶中刪除時。
...
* 您現在可以指定 `vat_id` 使用GraphQL來新增或更新訪客的地址時。
...
* 用戶角色網格的篩選函式(管理員 **[!UICONTROL System]** > **[!UICONTROL Permissions]** > **[!UICONTROL User roles]** > **[!UICONTROL Administrators]**)現在可如預期般運作。 陣列值現在會個別處理。 以前，當您嘗試在此格線中輸入查詢時，Magento Open Source擲回此主控台錯誤，且頁面未載入： `Uncaught TypeError: Cannot read properties of null`. [GitHub-35993](https://github.com/magento/magento2/issues/35993)
...
* 主題標題設定 `Welcome Text` 現在可包含單引號字元。 以前，Magento Open Source擲回此JavaScript錯誤： `Uncaught SyntaxError: Unable to process binding "ifnot: function(){return customer().fullname }"`.
...
* 商家現在可以使用REST API為零總量的訂單建立貸項通知單。 以前，Magento Open Source顯示此錯誤： `The order does not allow a credit memo to be created. The credit memo's total must be positive. We can't create a credit memo for the invoice/order`.
...
* Fucher貨幣服務現在可如預期般運作，透過ApiLayer產生的金鑰，且貨幣匯率可正確匯出。 以前，當商家嘗試透過管理員匯入貨幣匯率時，Magento Open Source擲回錯誤。
...
**問題**:啟用免運費時，您無法建立貸項通知單(**[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL Sales]** > **[!UICONTROL Delivery Methods]**)，且顯示的出貨價格包含稅。 當您嘗試建立貸項通知單時，Adobe Commerce會擲回此例外： `"Division by zero".vendor/magento/module-sales/Model/Order/Creditmemo/Total/Tax.phpLine 139`. [GitHub-36800](https://github.com/magento/magento2/issues/36800)

**因應措施**:此問題的Hotfix現已可用。 前往 [質量修補工具：搜索修補程式](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) 並搜索修補程式ACSD-50814。
