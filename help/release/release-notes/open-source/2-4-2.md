---
title: Magento Open Source 2.4.2 發行說明
description: 了解 2.4.2 Magento Open Source 版本中的新功能、增強功能、錯誤修正及已知問題。
...
# Magento Open Source 2.4.2 發行說明

Magento Open Source2.4.2引進了效能與安全性的增強功能，以及重大的平台改良。 安全性增強功能包括擴展對所有 Cookie 的屬性支援`SameSite`。 現在支援 Elasticsearch 7.9.x 和 Redis 6.x。

此版本包含超過 280 項核心代碼的新修正和 35 項安全性增強功能。 它包括我們的社群成員解決的近 290 個 GitHub 問題。 這些社群貢獻的範圍從核心代碼的微小清理到 GraphQL 的重大增強。

2.4.1 中識別的所有已知問題均已在此版本中修正。
...
此版本包含超過35項安全修正和平臺安全性改進。 所有安全性修正已向後移植到 2.4.1-p1 和 2.3.6-p1。

#### 超過 35 項安全增強功能，可説明消除遠端代碼執行 （RCE） 和跨網站指令碼 （XSS） 漏洞

迄今為止，尚未發生與這些問題相關的確認攻擊。 但是，某些漏洞可能會被利用來訪問客戶信息或接管管理員會話。 其中大多數問題要求攻擊者首先獲取對管理員的訪問許可權。 因此，我們提醒您採取一切必要措施來保護您的管理員，包括但不限于以下工作：IP 允許列表、 [雙因素驗證](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication/)、使用 VPN、使用唯一位置 `/admin`而不是 以及良好的密碼衛生。 有關 [這些已修復問題的討論Adobe Systems](https://helpx.adobe.com/security/products/magento/apsb21-08.html) 請參閱安全性公告。

#### 其他安全增強功能

此版本的安全性改進包括：

* 現在所有核心 Cookie 都支援該`SameSite`屬性。

* 當用戶嘗試保存這些欄位中的值時，應用程式現在會顯示標識產品和類別描述字段中潛在惡意內容的消息。

* 跨元件的檔案系統操作已經過標準化和強化，以防止惡意上傳。

* 已修正核心內容安全策略 （CSP） 違規問題。
...
* [**現已支援**](../../../installation/system-requirements.md) Elasticsearch 7.9.x。 儘管我們建議運行 Elasticsearch 7.9.x，但版本 2.4.x 仍與 Elasticsearch 7.4.x 相容。 <!-- MC-36867-->

* 2.4.2 已使用 Varnish 6.4](../../../installation/system-requirements.md) 進行了[測試。2.4.x版仍與Varnish 6.x相容。
...
此版本已棄用設定安裝以使用分割資料庫的功能。 當前使用拆分資料庫的商家應開始計劃恢復或遷移到單個資料庫或使用替代方法。 [有關此問題的概述，請參閱Magento Open Source](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog 貼文中的拆分資料庫功能的棄用。有關遷移說明，請參閱 [從拆分資料庫還原到單個資料庫](../../../configuration/storage/revert-split-database.md) 。

### 效能增強

此版本包括代碼增強功能，可提高具有大型目錄的部署的API性能和管理員回應時間。 多個擴充性增強功能使 2.4.2 能夠本機支援比以前版本大 20 倍的複雜目錄。
...
* 新增對[比較清單](https://developer.adobe.com/commerce/webapi/graphql/queries/compare-list.html)的支援。 購物者可以[建立](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-compare-list.html)與[刪除](https://developer.adobe.com/commerce/webapi/graphql/mutations/delete-compare-list.html)比較清單，以及[新增](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-products-to-compare-list.html)與[移除](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-products-from-compare-list.html)專案至比較清單。 此外，以訪客身分建立比較清單的購物者可以客戶身分登入，並[保留](https://developer.adobe.com/commerce/webapi/graphql/mutations/assign-compare-list-to-customer.html)他們的比較清單。

* 新增[`generateCustomerTokenAsAdmin`](https://developer.adobe.com/commerce/webapi/graphql/mutations/generate-customer-token-as-admin.html)突變並更新`Customer`物件以支援遠端購買協助。
...
* 已將`staging`屬性新增至[`ProductInterface`](https://developer.adobe.com/commerce/webapi/graphql/interfaces/product-interface.html)和[`CategoryInterface`](https://developer.adobe.com/commerce/webapi/graphql/interfaces/category-interface.html)，以判斷產品是否已暫存，並檢視其相關的行銷活動資訊。<!-- MC-33897-->
...
內容&#x200B;**中的**&#x200B;網頁最佳化影像。 商戶現在可以在內容中使用[網頁最佳化的影像轉譯](https://experienceleague.adobe.com/en/docs/commerce-admin/content-design/wysiwyg/gallery/media-gallery-image-optimization)，而非高解析度的影像。 原始影像在「媒體集」中保持未修改狀態，且影像插入內容時會動態產生影像轉譯。
...
* [在 AWS S3 上存儲媒體](../../../configuration/remote-storage/remote-storage.md) 文件
...
* 此版本已棄用設定安裝以使用分割資料庫的功能。 請參閱Magento Open Source](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog文章中的[取代分割資料庫功能。

<!-- MC-36472-->

* 商戶現在可以成功地將執行MySQL 8.x的Open Source部署升級為Commerce部署。 之前，在升級期間新增`row_id`的所有資料表的`AUTO_INCREMENT`值回覆為初始值時，應用程式擲回例外狀況。
...
* 執行`bin/magento/setup:upgrade`後，`block_html`、`full_page`和`layout`快取現在如預期般停用。  [GitHub-28186](https://github.com/magento/magento2/issues/28186)
...
* 現在可以在安裝範例數據後執行 `bin/magento/setup:upgrade` 。 以前，當您嘗試執行 `bin/magento/setup:upgrade`時，應用程式顯示此錯誤： `unable to apply data patch magento\catalogrulesampledata\setup\patch\data\installcatalogrulesampledata for module magento_catalogrulesampledata`。 應用程式還在系統日誌中顯示此錯誤： `main.ERROR: Sample Data error: Unable to unserialize value. Error: Syntax error`。 [GitHub-30685](https://github.com/magento/magento2/issues/30685)

<!-- MC-36785-->

* 您現在可以如預期般地從命令行設定 YouTube API 金鑰。 以前，當您嘗試執行 `bin/magento config:sensitive:set catalog/product_video/youtube_api_key`時，應用程式傳回此錯誤： `There are no sensitive configurations to fill`。

<!-- ENGCOM-8163 -->

* 應用程式現在會遵循`queue_consumer.xml`中定義的`maxMessages`值。 以前，應用程式僅使用部署設定值。 [GitHub-29522](https://github.com/magento/magento2/issues/29522)
...
* 當您在安裝Magento Open Source後嘗試使用 容器 `ssh` 進行更改`backend-frontname`時，應用程式不再引發錯誤。[GitHub-26762](https://github.com/magento/magento2/issues/26762)

<!-- ENGCOM-8230 -->

* 當您運行`bin/magento setup:install`以連接到現有資料庫時，應用程式不再顯示此問題： `Overwrite the existing configuration for db-ssl-verify?[Y/n]`。 [GitHub-29612](https://github.com/magento/magento2/issues/29612)

### Adobe Stock整合

<!-- ENGCOM-8020-->

* 新增對讀取 `exif_image.png` 或 `exif-image.jpeg` 中繼資料的支持。 [GitHub-1449](https://github.com/magento/adobe-stock-integration/issues/1449)
...
* 現在，使用預設或非預設連線埠連線到`env.php`中的MySQL時，`analytics_collect_data`作業會順利執行。 以前，`analytics_collect_data`在您使用非預設連線埠時擲回錯誤。
...
* 購物者現在可以使用Apple Pay成功下單虛擬產品。 之前，應用程式拋出此錯誤： `There are no shipping methods available for you right now. Please try again or use an alternative payment method`。

* 默認的 PayPal 快速結帳付款方式現在與 Braintree 信用卡付款方式配合使用。 以前，啟用Braintree信用卡付款方式時，應用程式不會顯示 **PayPal** 按鈕。
...
* 現在，當捆綁商品包含缺貨商品時，價格排序會正常運作。 先前，依降序排序的價格不會依降序顯示產品。

<!-- MC-38576-->

* 當存在許多選項時，應用程式現在會自動對捆綁產品選項應用預設數量 1。 以前，商家必須手動為每個選項分配預設數量。

<!-- ENGCOM-8159 -->

* 訂單發票創建頁面現在按預期顯示捆綁產品的子產品。 [GitHub-27350](https://github.com/magento/magento2/issues/27350)

<!-- MC-38460 -->

* 現在，當您重新訂購屬於具有自定義價格的捆綁產品的簡單產品時，應用程式會分配正確的產品價格。 以前，在捆綁商品上設置自定義價格會導致在重新訂購時附加的簡單商品價格不正確。 [GitHub-30343](https://github.com/magento/magento2/issues/30343)

<!-- MC-30348 -->

* 現在，呼叫POST `/V1/order/{orderId}/ship` 發貨包含捆綁產品的訂單會將訂單標記為按預期完成。 之前，應用程式會顯示以下錯誤消息： `You can't create a shipment without products`。 [GitHub-9762](https://github.com/magento/magento2/issues/9762)

### 購物車和結帳

<!-- MC-34264-->

* 當結帳工作流程的送貨步驟上存在自定義地址屬性時，送貨方法估算器現在按預期工作。 之前，應用程式會顯示此錯誤消息： `{"message":"Error occurred during \"custom_attributes\" processing. A custom attribute is specified with a missing attribute code. Verify the code and try again."}` [GitHub-27505](https://github.com/magento/magento2/issues/27505)

<!-- MC-35783-->

* 現在，當購物者刪除其購物車中的唯一專案時，應用程式會按預期在迷你購物車中显示 `You have no items in your shopping cart` 消息。 以前，當購物者移除單個專案時，購物車數據不會更新 `checkout/cart/removeFailed` ，應用程式會显示以下訊息： `1 product requires your attention`.

<!-- MC-36088-->

* 該應用程式不再顯示不適用配送方式的訂單的運費。 以前，應用程式在結帳工作流程中顯示不適用送貨方式的價格為 0。

<!-- MC-36598-->

* 當未選中結帳工作流程上的“ **我的帳單和送貨地址相同** ”複選框時，現在將選中客戶的默認帳單地址。

<!-- MC-37351-->

* 當購物者在不同網域中從一個商店切換到另一個時，購物車內容不再丟失。

<!-- MC-38000-->

* 購物者現在可以在使用多種運輸方式時使用多個位址結帳成功完成訂單。 之前，應用程式會顯示此錯誤： `There has been an error processing your request`。 [GitHub-30197](https://github.com/magento/magento2/issues/30197)

<!-- MC-38048-->

* 現在，當購物者更改購物車的商店視圖時，應用程式會在結帳工作流程的送貨頁面上显示正確的預設國家/地區。
...
* 刪除了對購物車上終結点的 `/rest/V1/guest-carts/cart_id/totals-information` 冗餘调用，從而提高了購物車性能。 先前，當購物者新增產品至購物車，然後檢視購物車時，應用程式會多次呼叫`collectAddressTotals`方法。

<!-- MC-38833-->

* 當禁用「**如果國家/地區**&#x200B;可選，則允許選擇狀態」設置時，應用程式不再在結帳工作流程中顯示國家/地區的“**區域**”字段。[GitHub-30747](https://github.com/magento/magento2/issues/30747)
...
* 當禁用「**如果國家/地區**&#x200B;為可選，則允許選擇狀態」設置時，應用程式不再在結帳工作流程上顯示&#x200B;**「區域**」字段。[GitHub-30747](https://github.com/magento/magento2/issues/30747)

<!-- ENGCOM-8442 -->

* 現在，當購物者使用多個地址結帳時按兩下“ **更新購物車”** 時，應用程式會正確更新購物車的專案小計。 [GitHub-30408](https://github.com/magento/magento2/issues/30408)

<!-- ENGCOM-8283 -->

* 當當前未安裝的付款方式存在節點時， `config.xml` 應用程式不再引發異常。 之前，應用程式在結帳時擲出異常。 [GitHub-29555](https://github.com/magento/magento2/issues/29555)
...
* 現在，當計算訂單小計而不含稅時，會按預期套用購物車價格規則。 新的`Subtotal (Incl. Tax)`選項已新增為購物車價格規則條件。
...
* 已解決店面的產品排序順序問題。 先前，當應用程式將產品價格編列索引時，它偶爾會在`catalog_product_index_price`資料表中將可設定產品的`min_price`和`max_price`設定為0，這會影響店面的價格排序順序。

<!-- MC-35608-->

* 應用程式現在已成功更新標籤為`Product Type`的屬性。 以前，由於該屬性是在代碼中保留的，因此 `product_type` 當您嘗試更新 `Product Type` 屬性時，應用程式會顯示以下消息： `An attribute with the same code (product_type) already exists.`

<!-- MC-36214-->

* 當您保存商品時，應用程式不再更改已上傳視頻在商品詳情頁面的圖片和視頻部分中的位置。 以前，新圖像位置被分配了一個索引值（清單中的位置），如果現有圖像位置與其索引值不一致，則新圖像在清單中的位置不正確。

<!-- MC-36548-->

* 現在只要&#x200B;**可點按「**&#x200B;新增到購物車」按鈕，「新增到購物車&#x200B;****」功能就會如預期般運作。以前，如果您在等待產品頁面載入時多次點擊此按鈕，應用程式會拋出此錯誤： `Invalid Form Key. Please refresh the page`。

<!-- MC-37006 36835-->

* 當商家在類別中添加或刪除禁用的產品時類別應用程式不再刷新緩存。 之前，應用程式會針對相關類別排清快取，無論產品狀態為何。 儲存類別時也會取消指派類別，導致清除類別快取。
...
* 現在，應用程式在創建層價格期間在多網站部署中顯示正確的貨幣。

<!-- MC-38914-->

* 產品影像自定義屬性現在可以正確逸出。 正如預期的那樣，該 `data-src` 屬性具有與屬性相同的 `src` 值。 以前，URL特殊符號會被逸出。

<!-- MC-37665-->

* 通過 REST API 更新類別不再禁用“啟用類別”、“包含在功能表中”和“URL鍵”屬性上的“ **使用預設值值** ”設置。

<!-- MC-35363-->

* 現在，保存類別僅刷新與此類別相關的塊緩存。 以前，應用程式會清空所有 類別 塊的緩存。

<!-- MC-30626-->

* 具有百分比價格的自訂選項價格，現在會在多商店部署中，依預期轉換為作用中商店的基本貨幣。 以前，帶有百分比價格的自定義選項的價格轉換不正確。 [GitHub-26432](https://github.com/magento/magento2/issues/26432)

<!-- MC-38890-->

* 應用程式現在已成功重新建立影像，當商家使用POST`rest/all/V1/products`傳送媒體集資料時，這些影像會如預期在「管理員」中顯示。 以前，如果影像在`pub/media`中被刪除，但媒體中的記錄仍然包含這些影像的記錄，當商家透過POST`rest/all/V1/products`傳送媒體集資料時，應用程式會重新建立這些影像，但會刪除這些影像。 因此，商家在「管理員」中只看到預留位置，因為影像是在更新過程中建立和刪除的。
...
* `/V1/categories/{categoryId}/products`呼叫現在會如預期般返回屬於父類別的所有產品的清單。在過去，如果產品屬於父類別的多個子類別，應用程式會拋出這種類型的錯誤： `Internal Error. Details are available in the application log file. Report ID: webapi-5f8579e37db54`。  [GitHub-30461](https://github.com/magento/magento2/issues/30461)
...
* 已從中刪除 `app/code/Magento/Catalog/Helper/Product/View.php`多餘的空格。 [GitHub-30601](https://github.com/magento/magento2/issues/30601)

<!-- ENGCOM-8181 -->

* 更正了 中 `app/code/Magento/Ui/view/base/web/js/form/element/ui-select.js`函數名稱中的拼寫錯誤。 [GitHub-29987](https://github.com/magento/magento2/issues/29987)

<!-- ENGCOM-8478 -->

* 糾正了小型購物車和結帳訂單摘要中“item”的複数形式。 [GitHub-29920](https://github.com/magento/magento2/issues/29920)
...
* 已在`\Magento\Framework\Filter\Template\Tokenizer\Parameter`中將`rl`更正為`url`。 [GitHub-29185](https://github.com/magento/magento2/issues/29185)
...
* 已為`\Magento\Framework\View\Element\Template`類別的前端`viewModel`參考範例新增正確的區塊類別。 [GitHub-30450](https://github.com/magento/magento2/issues/30450)
...
* 在`EntityAbstract.php`中將`@param string $attribute`變更為`@param AbstractAttribute|string[]|string $attribute`。 [GitHub-30191](https://github.com/magento/magento2/issues/30191)

<!-- ENGCOM-8142 -->

* 無效標記（例如， `@package` 和 `@subpackage`）已從整個代碼庫中的文檔塊中刪除。 [GitHub-30061](https://github.com/magento/magento2/issues/30061)

<!-- ENGCOM-8117 -->

* 已刪除類結構 `AdvancedPricing` 中未使用的參數。 [GitHub-29531](https://github.com/magento/magento2/issues/29531)
...
* 應用程式現在可成功導出包含自定義性別屬性值的 客戶 数据。

### 客戶

<!-- MC-35771-->

* 店面客戶帳戶頁面出生日期字段的日曆微件現在使用指定的商店區域設置。

<!-- MC-38498-->

* 應用程式不再重複管理員簽出重新訂購的現有客戶時保存客戶地址的条目。
...
* 現在，“ **出生** 日期”字段的“日期格式”驗證在購物者按兩下 **建立帳戶** 按鈕之前起作用。 以前，購物者可以在無效格式中輸入值，當他們按兩下 **帳戶按鈕** 建立時，購物者被重定向到建立新客戶帳戶頁面，並顯示無效日期錯誤。

<!-- MC-33867-->

* 當管理員嘗試保存包含屬性無效數據的新客戶帳戶時，應用程式不再引發致命錯誤。
...
* 解決了 doposer 升級中因 dotdigital 模組 的`magento/module-authorization`依賴性而產生的問題。

### 下載

<!-- ENGCOM-7679 -->

* 應用程式不再在「我的下載產品」中列出可下載產品，標籤該產品所屬的訂單已部分退款。 [GitHub-28388](https://github.com/magento/magento2/issues/28388)
...
* 未在`toOptionArray()`之前呼叫`setPath()`時，`\Magento\Config\Model\Config\Source\Email\Template::toOptionArray`不再擲回錯誤。 [GitHub-29315](https://github.com/magento/magento2/issues/29315)
...
* 應用程式現在會使用有效的電子郵件位址向所有相關客戶發送電子郵件提醒。 以前，應用程式在遇到一個無效位址后停止向客戶發送提醒電子郵件。
...
* 現在，商家將購物車價格規則儲存到部署后，DatePicker 可以显示正確的日期。 [GitHub-30382](https://github.com/magento/magento2/issues/30382)
...
* 已對`LoginAsCustomer`模組進行下列改良：
   ...
   * 取代為 `(bool)->getValue()` `isSetFlag`。 [GitHub-29689](https://github.com/magento/magento2/issues/29689)
...
* 應用程式現在會將HTML列印檢視的&#x200B;**標誌組態設定**&#x200B;如預期套用至店面發票PDF。 [GitHub-24730](https://github.com/magento/magento2/issues/24730)
...
* 現在，當商家在產品創建過程中按兩次&#x200B;****&#x200B;時，應用程式會按預期打開新屬性頁面。之前，應用程式显示空白頁面並引發錯誤。 [GitHub-30361](https://github.com/magento/magento2/issues/30361)

<!-- ENGCOM-8426 -->

* 拿掉抵用券後，應用程式不再顯示 `The coupon code has been accepted` 該訊息。 [GitHub-30255](https://github.com/magento/magento2/issues/30255)

<!-- ENGCOM-8396 -->

* 管理員現在可以將介面工具集指派至特定類別。 [GitHub-30009](https://github.com/magento/magento2/issues/30009)
...
* 當選擇性模組`Magento_MessageQueue`停用時，`Magento\Framework\MessageQueue\ConfigInterface`現在可以具現化。 之前，應用程式擲回此錯誤： `PHP Fatal error:  Uncaught Error: Cannot instantiate interface Magento\Framework\MessageQueue\ConfigInterface in /var/www/magento2/magento2/lib/internal/Magento/Framework/ObjectManager/Factory/AbstractFactory.php:121`。 [GitHub-26080](https://github.com/magento/magento2/issues/26080)
...
* 應用程式現在檢查類中的私有常量，並在調用`static::`它們時顯示警告。 之前，應用程式拋出此錯誤： `Undefined class constant`。

<!-- ENGCOM-8326 -->

* 新增在錯誤頁面和維護模式中符號連結 `root` 的支援。 先前，如果`root`或`magento/pub`已符號連結至不同的目錄，則錯誤處理器(`Magento\Framework\Error\Processor`)不會產生正確的檢視檔案URL。 [GitHub-30296](https://github.com/magento/magento2/issues/30296)

<!-- ENGCOM-8087 -->

* 已將遺失的`order_data`陣列新增至`EmailSender`類別。 這使得這些類別符合Magento的建議，即使用純量變數而不是電子郵件範本中的物件。 [GitHub-29604](https://github.com/magento/magento2/issues/29604)
...
* 產品`categories.breadcrumbs`資料的查詢不再傳回父類別已停用的類別的階層連結資料。 [GitHub-30468](https://github.com/magento/magento2/issues/30468)
...
* 當`category_id`時，`products`查詢現在會傳回預期的結果  使用`in`關鍵字指定篩選器。 [GitHub-30349](https://github.com/magento/magento2/issues/30349)
...
* `FlushCacheByTags`類別已更新為使用外掛程式，而非外掛程式。 [GitHub-29558](https://github.com/magento/magento2/issues/29558)

<!-- ENGCOM-8370 -->

* 已從`messages`、`authentication`、`progressBar`、`estimation`及`sidebar checkout`元件中移除`sortOrder`，以增強`checkout_index_index.xml`檔案中的程式碼。 [GitHub-30550](https://github.com/magento/magento2/issues/30550)
...
* URL重寫產生器現在會在儲存全域範圍的類別URL重寫時，將類別物件`url_key`、`url_path`和`store_id`設定為預設的存放區值。 之前，它使用上次處理的存放區ID的值儲存`url_key`、`url_path`和`store_id`。 [GitHub-29585](https://github.com/magento/magento2/issues/29585)###群組產品
...
* 分組產品現在在產品編輯頁面上列為缺貨，當所有子簡單產品都缺貨時。

<!-- MC-37249-->

* 分組產品的狀態現在在其子簡單產品的狀態更改為無存貨時，在產品編輯頁面上正確更新。 以前，分組產品的父項的庫存物料在其子項更改庫存狀態后不會更新。

<!-- ENGCOM-8250 -->

* `\Magento\Widget\Controller\Adminhtml\Widget\Instance\Validate` 現在可將短語結果值識別為錯誤消息，並在 Widget 驗證 期間將其輸出給 用戶。 [GitHub-27397](https://github.com/magento/magento2/issues/27397)
...
* `acl.xml`已啟用但`Magento_LoginAsCustomerAdminUi`已禁用或`Magento_LoginAsCustomerLog`已禁用時`Magento_LoginAsCustomerAssistance`發生的不一致已得到糾正。以前，當具有這些憑據的管理員嘗試登錄到管理員時，應用程式顯示空白頁面並在以下情況下拋出此錯誤：。 `Exception #0 (LogicException): Could not create an acl object: Invalid Document Element 'resource': The attribute 'title' is required but missing. Line: 13`[GitHub-30680](https://github.com/magento/magento2/issues/30680)

<!-- ENGCOM-8157 -->

* 在建立外掛程式期間產生 時`Interceptor`，應用程式不再引發`Undefined class constant`錯誤。[GitHub-28981](https://github.com/magento/magento2/issues/28981)，[GitHub-29879](https://github.com/magento/magento2/issues/29879)
...
* 現在，當他們在重設密碼电子郵件&#x200B;****&#x200B;中按兩下“設置新密碼&#x200B;**”時，**&#x200B;購物者將按預期重定向到“設置新頁面密碼”商店。之前，購物者被重定向回 **登入頁面上的“忘記密碼** ”按鈕。 [GitHub-27954](https://github.com/magento/magento2/issues/27954)
...
* 基本檔案收集器現在尊重 `AppState->emulateAreaCode()`. [GitHub-39656](https://github.com/magento/magento2/issues/29656)

<!-- ENGCOM-8441 -->

* 新 `BlockByIdentifier` 類支援基於 CMS 塊識別子檢索佈局塊。 因此，從緩存中刪除 CMS 塊時，應用程式也會清除佈局塊緩存。 [GitHub-28309](https://github.com/magento/magento2/issues/28309)

<!-- ENGCOM-8385 -->

* 修正佇列中數值引數轉換的問題。 以前，當您在 中使用 `queue_topology.xml`數值 參數聲明 佇列 `type` 時，參數值被強制轉換為字串。[GitHub-29615](https://github.com/magento/magento2/issues/29615)

### GraphQL

<!-- MC-37615-->

* 查詢 `urlResolver` 現在可以正確處理查詢參數。

<!-- ENGCOM-8422 -->

* `urlResolver`將自定義 CMS 節點和頁面指定為輸入時，查詢不再返回 NULL。[GitHub-30474](https://github.com/magento/magento2/issues/30474)
...
* 當使用`in`關鍵字指定`category_id`篩選器時，`products`查詢現在會傳回預期的結果。 [GitHub-30349](https://github.com/magento/magento2/issues/30349)
...
* 查詢`products`現在會按預期返回層價格值。 [GitHub-29168](https://github.com/magento/magento2/issues/29168)

<!-- ENGCOM-7528 -->

* 已從`lib/internal/Magento/Framework/GraphQl/Query/EnumLookup.php`中的回圈移除多餘的`getMappedNums`。 [GitHub-30031](https://github.com/magento/magento2/issues/30031)
...
* 已復原`SelectedCustomizableOption.type`屬性。 此外， `customizable_options` 各種 `CartItemInterface` 實現中的屬性現在為非空值。

<!-- MC-23860-->

* 查詢的 `products` 搜尋功能不再返回已啟用類別許可權的專案。
...
* CMYK影像現在可以正確呈現色彩。 當 Imagick 適配器現在檢測到打開的圖像具有CMYK的色彩空間時，該色彩空間將轉換為 SRGB。 以前，這些影像會以反轉的顏色呈現。 [GitHub-22375](https://github.com/magento/magento2/issues/22375)

<!-- ENGCOM-8251 -->

* 不再為`image_with_border.phtml`設定布林值。 已從`.product-image-photo`移除多餘的`max-width`和`height`設定。 （這些值已透過重設樣式在影像上全域設定。） [GitHub-30186](https://github.com/magento/magento2/issues/30186)

### 匯入/匯出

<!-- MC-35081-->

* 當您使用CSV檔案（`entity type = "customer address"`和`import behavior = "add/update"`）匯入客戶地址（某些欄位值已從其中刪除）時，客戶地址`region_id`不再獲派`NULL`值。
...
* 我們已經解決了手動索引器的問題，這些問題導致類別頁面為空或類別頁面上的產品計數減少。 以下情況會發生類別頁或產品計數不完整的問題：

   * `catalogsearch_fulltext`和`catalog_product_price`部分索引器在大型目錄上執行，直到這些索引器完成執行。
...
* 已在`\Magento\MediaGalleryCatalogUi\Ui\Component\Listing\Columns\Thumbnail::prepareDataSource`中將`DataObject`取代為產品模型。[GitHub-1711](https://github.com/magento/adobe-stock-integration/issues/1711)
...
* 目前的 cURL 用戶端現在採用不區分大小寫的標頭名稱。 以前比較 `Set-Cookie` 是區分大小寫，這可能會導致某些 HTTP 伺服器出現問題。 [GitHub-29524](https://github.com/magento/magento2/issues/29524)

<!-- ENGCOM-8360 -->

* 更新 XSD URN 格式 `api-functional` 和測試案例 XML 檔案。 [GitHub-30552](https://github.com/magento/magento2/issues/30552)

<!-- ENGCOM-8295 -->

* 事件前綴和事件物件已添加到目錄產品選項值中，集合以改進 集合 `\Magento\Catalog\Model\ResourceModel\Product\Option\Value\Collection`載入的自定義。 [GitHub-29621](https://github.com/magento/magento2/issues/29621)
...
* 型別`\Magento\Framework\View\Element\Text\ListText`的`head.additional`區塊已新增至`adminhtml` `default`配置（由`magento2/module-base`）。 [GitHub-29165](https://github.com/magento/magento2/issues/29165)

<!-- ENGCOM-8312 -->

* 新增遺漏的逸出URL方法至配置範本模組。 [GitHub-30036](https://github.com/magento/magento2/issues/30036)

<!-- ENGCOM-8127 -->

* 該方法 `\Magento\Framework\Filesystem\Io\File::read()` 現在與相容 `\Magento\Framework\Filesystem\Io\IoInterface::read()`。 [GitHub-27866](https://github.com/magento/magento2/issues/27866)

### 克拉納

* 拿掉對 PayPal 模組的相依性。 [GitHub-29421](https://github.com/magento/magento2/issues/29421)

### 記錄

<!-- ENGCOM-8443 -->

* `Magento_Logging`模組現在已正確登入新值以及之前的值。 以前，“更改前值列中的值顯示值 N/A，無論這是第一次更改還是下一次更改，或者何時返回預設值。 [GitHub-26943](https://github.com/magento/magento2/issues/26943)

### 媒體庫

<!-- ENGCOM-8014 -->

* **>系統********頁面已更新>進階**>**系統**，可顯示正確的產品名稱（媒體集）。包括對用戶介面的輕微重新設計。 [GitHub-1738](https://github.com/magento/adobe-stock-integration/issues/1738)

<!-- ENGCOM-8019 -->

* 媒體庫圖像描述的“使用於”部分中的鏈接現在顯示已按預期按所選圖像篩選的網格。 以前，顯示的網格線的標題不正確，清單條目也不準確。 [GitHub-1694](https://github.com/magento/adobe-stock-integration/issues/1694)

<!-- ENGCOM-8019 -->

* 您現在可以在媒體庫中成功編輯多個圖像。 以前，在向一個影像增加標記后，無法為第二個影像新增標記。 [GitHub-1755](https://github.com/magento/adobe-stock-integration/issues/1755)

<!-- ENGCOM-8015 -->

* 從媒體庫中的圖像中刪除標籤后，“標籤”下拉功能表現在按預期為空。  [GitHub-1703](https://github.com/magento/adobe-stock-integration/issues/1703)

<!-- ENGCOM-8146 -->

* 現在支持媒體庫同步佇列消息的批次處理。 [GitHub-1724](https://github.com/magento/adobe-stock-integration/issues/1724)
...
* 不再裁切長媒體集文字字串和檔案夾名稱。 之前，應用程式會顯示裁切的文字和影像，並顯示以下訊息：  `We couldn't find any records`. [GitHub-1763](https://github.com/magento/adobe-stock-integration/issues/1763)、 [GitHub-1758](https://github.com/magento/adobe-stock-integration/issues/1758)

<!-- ENGCOM-8042 -->

* 编辑新頁面時，若未選取任何影像，則應用程式不再顯示 **「新增所選** 內容按鈕。 [GitHub-1769](https://github.com/magento/adobe-stock-integration/issues/1769)

<!-- ENGCOM-8047 -->

* 篩選器現在按預期應用於「 **內容**  >  **媒體庫** 」類別網格。 [GitHub-1763](https://github.com/magento/adobe-stock-integration/issues/1763)

<!-- ENGCOM-8183 -->

* 庫乘斷點選項配置現在按預期工作。 以前，斷點選項配置無法正常工作，因為在 `mediaCheck` 頁面載入時觸發了退出媒體查詢函數。 [GitHub-29933](https://github.com/magento/magento2/issues/29933)
...
* `AdminUpdateCategoryAndMakeInactiveTest` 並 `VerifyChildCategoriesShouldNotIncludeInMenuTest` 已重構以符合 MFTF 最佳做法。 [GitHub-30058](https://github.com/magento/magento2/issues/30058)

<!-- ENGCOM-8458 -->

* 刪除重複的 `AdminOpenCMSBlocksGridActionGroup`。 [GitHub-30896](https://github.com/magento/magento2/issues/30896)

<!-- ENGCOM-8156 -->

* 已由 AdminOpenCmsBlockActionGroup 取代 `AdminOpentCmsBlockActionGroup` 。 [GitHub-29839](https://github.com/magento/magento2/issues/29839)
...
* 由於冗餘而刪除 `AdminNavigateToPageGridActionGroup` 。 [GitHub-29838](https://github.com/magento/magento2/issues/29838)

<!-- ENGCOM-8122 -->

* 實施 `ActionGroup` 以流覽至結帳頁面。 [GitHub-29843](https://github.com/magento/magento2/issues/29843)
...
* 商家現在可以在使用信用卡片的付款方式切換到非信用卡片付款方式后，從管理員成功下訂單。 以前，當商家按兩下 **提交訂單** 按鈕時，載入過程開始但從未完成，並且不會下訂單。

<!-- MC-37649-->

* 使用 PayPal Express 下訂單時，應用程式不再建立重複訂單。 以前，當購物者嘗試使用 PayPal Express 支付訂單但PayPal返回錯誤時，應用程式仍會創建訂單。 這導致該商家網站上的訂單重複。 [GitHub-13952](https://github.com/magento/magento2/issues/13952)

<!-- MC-38829-->

* PayPal結算報表現在可按預期在「銷售報表」>**PayPal結算**>**取得****。**&#x200B;之前，應用程式未在PayPal結算報表格線中列出這些報表，且會顯示下列訊息： `We couldn't find any records`。
...
* 當啟用&#x200B;**自動指派至客戶群組**&#x200B;時，訪客購物者現在可以下達PayPal訂單。 付款會在PayPal閘道處理，訂單會在Magento中建立，而購物者會看到訂單感謝頁面。 以前，訂單在 PayPal 閘道上成功處理，但訂單未在Magento Open Source中創建。 相反，購物者被重定向到訂單審核頁面，應用程式在其中显示此錯誤： `Failed address validation: %1" error`。 [GitHub-25399](https://github.com/magento/magento2/issues/25399)

### 性能

<!-- MC-37705-->

* 中的 `module-catalog-import-export/Model/Import/Product/Option.php` 效能問題已得到解決。 匯出或匯入超過 100,000 條記錄所需的時間已縮短。 請參閱匯入 [/匯出花費的時間比預期長，有大量產品選項](https://support.magento.com/hc/en-us/articles/360050695772) 知識庫文章。

<!-- MC-37799-->

* `Magento_Csp/Model/BlockCache` 已重構以減少記憶體消耗和生成不必要的大型緩存字串。 以前，包含動態列入白名單的指令碼和樣式的頁面經常會因效能問題而發生問題。 [GitHub-29964](https://github.com/magento/magento2/issues/29964)

<!-- MC-37816 -->

* 我們改進了包含超過 100,000 種使用消費者的產品的目錄的計劃導出性能。

<!-- MC-38306 -->

* 當您將禁用的產品添加或保存到類別時，應用程式不再刷新類別緩存。 以前，儘管產品狀態會影響伺服器性能，但應用程式會刷新相關類別的緩存。

<!-- MC-38518 -->

* 頁面購物車中更新產品數量的載入時間已改善。 以前，應用程式在 購物車 中對每個產品執行多個單獨的查詢，這增加了頁面載入時間。

<!-- MC-39700 -->

* 索引器已添加到 `Magento_Bulk`  MySQL 表中，這提高了批量操作的性能。
...
* 現在，在購物者登錄到啟用了持久購物車的商店后，應用程式會按預期显示歡迎消息。 之前，應用程式顯示「 **不是你？」** 記錄后的消息。

<!-- MC-38269-->

* 應用程式不再顯示「 **不是你？」** 當客戶記錄到啟用了永久性購物車的部署時選擇“ **記住我** ”選項時的文本。
...
* 在「購物車中的產品」報告中顯示產品貨幣時，應用程式現在使用生成報告的商家的角色範圍中指定的貨幣。
...
* 應用程式現在會在使用者按一下Admin儀表板&#x200B;**移至進階報告**&#x200B;按鈕時，驗證權杖是否存在。  如果令牌存在，應用程式將显示進階報告頁面。 如果未配置令牌，應用程式將在使用手冊中显示信息性頁面。 [GitHub-25411](https://github.com/magento/magento2/issues/25411)

### 評論

<!-- MC-37117-->

* 現在，當您按兩下&#x200B;**管理員**&#x200B;行銷&#x200B;**「>**」所有評論&#x200B;**「上的」新審核**「按鈕時，新審核頁面將按預期載入」。按兩下此頁面上的產品，將打開該產品的預期新審核表單。

<!-- ENGCOM-8383 -->

* 當您嘗試從「管理&#x200B;**行銷**>**所有評論**」 中編輯訂單時，應用程式不再引發此錯誤。 `Uncaught TypeError: Event.observe is not a function`[GitHub-30388](https://github.com/magento/magento2/issues/30388)

### 銷售

<!-- MC-36219-->

* `sales_shipment_grid`的`order_created_at`資料行現在會在每次更新後依預期更新。
...
* 目錄搜尋的性能已提高。 停用&#x200B;**啟用Search**&#x200B;建議 （**儲存**>****&#x200B;設定>**目錄**」>**目錄Search** ） 會如預期般運作。以前，應用程式查詢 MySQL `search_query` 表而不是自動填充搜尋建議的 Elasticsearch。 [GitHub-25534](https://github.com/magento/magento2/issues/25534)

<!-- MC-35256-->

* Elasticsearch現在篩選器產品價格與MySQL相同。 Elasticsearch索引中的欄位類型 `price_*`  已從 `float` 更改為 ， `double` 以提高精度並更正浮點誤差。
...
* 當搜尋字詞包含`/`時，Elasticsearch不再擲回例外狀況。  [GitHub-25886](https://github.com/magento/magento2/issues/25886)，[GitHub-25110](https://github.com/magento/magento2/issues/25110)
...
* 當網路中斷中斷與 DHL 運輸方式的連接時，結帳程式不再失敗。 相反，應用程式會顯示其他可用的送貨方式。 以前，當與 DHL 運輸的連接中斷時，結帳程序會停止，並且應用程式顯示此錯誤： `Sorry, no quotes are available for this order at this time`。 [GitHub-29902](https://github.com/magento/magento2/issues/29902)
...
* UPS 貨件追蹤現在僅當包裹已遞送時才指示狀態 `Delivered On` 。 [GitHub-30032](https://github.com/magento/magento2/issues/30032)
...
* 店面商品詳情頁面現在按預期顯示含稅和不含稅的新舊價格。 [GitHub-27500](https://github.com/magento/magento2/issues/27500)、 [GitHub-11998](https://github.com/magento/magento2/issues/11998)

### 測試

<!-- MC-37364-->

* `bin/magento dev:tests:run` 現在按預期啟動以下測試：全部、單元、集成、全部集成、靜態、全部靜態、完整性、舊版和預設值。
...
* 現在已在`ApiFunctional TestFramework`中正確剖析包含URL的標頭值。 [GitHub-26425](https://github.com/magento/magento2/issues/26425)
...
* 優化了主題的網頁字型載入。  [GitHub-29526](https://github.com/magento/magento2/issues/29526)
...
* 新增對[ISO 3166-2：UY](https://en.wikipedia.org/wiki/ISO_3166-2:UY)中定義的烏拉圭地區的支援。 [GitHub-29729](https://github.com/magento/magento2/issues/29729)
...
* 抵用券代碼文字欄位現在在 Internet Explorer/EDGE 瀏覽器中會以適當的寬度顯示。

<!-- ENGCOM-7630 -->

* 變數`@button__border-radius`現已在資料庫中定義`lib/web/css/source/lib/variables/_buttons.less`。  `border-radius` 預設值 `3px`為 。 以前， `border-radius` 是硬式編碼的。  [GitHub-28674](https://github.com/magento/magento2/issues/28674)

<!-- ENGCOM-8067 -->

* 迷你搜尋欄位中的提交搜尋按鈕（放大鏡）現在處於禁用狀態，直到達到最小搜尋字串長度。 [GitHub-29704](https://github.com/magento/magento2/issues/29704)
...
* UI選取的元件選項在設定時 `this.disabled(true)` 不再可見。 [GitHub-29098](https://github.com/magento/magento2/issues/29098)
...
* 模組現在可 `Vault` 識別付款資訊管裡服務請求中的付款方式代碼。 以前，當購物者使用已保存的Braintree信用卡片下訂單時，當使用有效的付款方式時，應用程式平均拋出此錯誤： `The requested Payment Method is not available`。
...
* 現在，當將運費配置為含稅金額時免費會考慮頂點計算的稅費。

### Web API框架

<!-- MC-23904 ENGCOM-8421-->

* 現在，您可以使用 POST `V1/invoice/:invoiceId/refund` 為產品零數量和零運費的發票退款（例如， `{"items": [{"qty": 0, "orderItemId": 6, "extensionAttributes": {}}], "appendComment": false, "notify": true, "isOnline": true, "arguments": {"adjustment_negative": 0.0, "adjustment_positive": 0.99, "shipping_amount": 0}}.` ） 以前，應用程式拋出此錯誤： `You can't create a creditmemo without products`。 [GitHub-23069](https://github.com/magento/magento2/issues/23069)

<!-- MC-33732 -->

* 現在您可以使用 POST `V1/categories` 建立或更新類別。 以前，如果值設定為數位，應用程式 `default_sort_by` 不會儲存值。 當值 `default_sort_by` 設為字串時，應用程式會拋出以下錯誤： `Error occurred during \"custom_attributes\" processing. Attribute \"default_sort_by\" has invalid value. The \"string\" value's type is invalid. The \"string[]\" type was expected. Verify and try again.`

<!-- MC-35740 -->

* 現在如預期般地使用 POST `V1/invoices/:id/capture` 擷取付款資訊。 以前，應用模組會授權訂單，但只會在付款閘道的網站上擷取訂單。

<!-- MC-37794 -->

* 當您使用 POST `V1/order/:orderId/ship` 建立分批貨件時，端點會返回正確數量的已發貨產品。

<!-- MC-38425-->

* 使用POST`V1/order/:orderID/invoice`建立的商業發票現在會正確反映商店信用額度所支付的部分款項。 先前，當部份訂單付款給含商店信用額的訂單時，建立發票時不會將商店信用額列入考量。

<!-- MC-36830-->

* GET`rest/all/V1/categories`和GET`rest/all/V1/categories?rootCategoryId=2`呼叫現在會如預期傳回樹狀結構中所有類別的`name`和`product_count`欄位。 以前，類別欄位值為空。 資料表名稱解析程式外掛程式傳回不正確的資料表名稱，無法依類別擷取產品計數。

<!-- ENGCOM-8286 -->

* PUT `V1/categories/:id` 終結點現在存儲在提供自定義屬性時 `save_rewrites_history` 為類別URL鍵創建 301 重定向所需的數據。 [GitHub-29174](https://github.com/magento/magento2/issues/29174)、 [GitHub-30240](https://github.com/magento/magento2/issues/30240)
...
* 已為`\Magento\Wishlist\Controller\Shared\Allcart`實作`ActionInterface`。 [GitHub-29537](https://github.com/magento/magento2/issues/29537)

## 已知問題

**問題**： `[magento_root]/index.php`檔案已移除，Apache設定預設會從`/pub`執行應用程式。 從子資料夾提供的存放區將無法如預期運作，並可能顯示404錯誤。 **因應措施**：使用symlink將安裝模擬至子資料夾。 下面的示例用於 `https://shop01.com/shop/` 說明如何使用符號連結類比子資料夾中的安裝。
...
1. 在新目錄(`magento_root/pub/shop01/index.php`)中建立`index.php`檔案，並新增此內容：
...
**問題**：當購物者按一下&#x200B;**使用Venmo**&#x200B;付款時，應用程式會按預期在Braintree中建立訂單，但未在Admin中建立訂單。 請參閱[2.4.2已知問題：BraintreeVenmo付款無法運作](https://support.magento.com/hc/en-us/articles/360054662652/)知識庫文章。 <!-- BUNDLE-2894-->
...
社區工程團隊 [Magento貢獻者](https://magento.biterg.io/app/kibana) 按月、季度和年維護清單貢獻最大的個人和合作伙伴。 從該貢獻者頁面，您可以前往GitHub上的合併個人資料連結。
...
您可以使用[撰寫器](../../../installation/composer.md)安裝Magento Open Source2.4.2。
