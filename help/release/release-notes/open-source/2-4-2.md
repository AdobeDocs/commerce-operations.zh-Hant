...
description: 了解2.4.2版Magento Open Source的新功能、增強功能、錯誤修正和已知問題。
...
Magento Open Source2.4.2引入效能和安全性增強功能，並大幅改善平台。 安全性增強功能包括擴充 `SameSite` 屬性。 Elasticsearch7.9.x和Redis 6.x現已受支援。
...
此版本包含核心品質的增強功能，可改善架構品質和這些功能領域：客戶帳戶、目錄、CMS、OMS、匯入/匯出、促銷和鎖定，以及購物車和結帳。
...
此版本已淘汰將安裝設定為使用分割資料庫的功能。 當前使用拆分資料庫的商家應開始計畫恢復或遷移到單個資料庫或使用替代方法。 請參閱 [在Magento Open Source中取代拆分資料庫功能](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) 有關此問題的概述的DevBlog文章。  請參閱 [從拆分資料庫還原到單個資料庫](../../../configuration/storage/revert-split-database.md) 以取得移轉指示。
...
* GraphQL架構已經過增強，可針對具有許多變體的可設定產品，最佳化產品資料擷取。 <!-- MC-36138-->
...
* 此版本已淘汰將安裝設定為使用分割資料庫的功能。 請參閱 [在Magento Open Source中取代拆分資料庫功能](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog文章。
...
* 當您嘗試變更時，應用程式不再擲回錯誤 `backend-frontname` 使用 `ssh` 容器時顯示Magento Open Source。 [GitHub-26762](https://github.com/magento/magento2/issues/26762)
...
* 使用GraphQL搭配抵用券套用價格規則，現在可如預期般運作 **整個購物車的固定金額折扣** 動作。
...
* 在其所屬的訂單已部分退貨後，應用程式不再在「我的下載產品」索引標籤中列出可下載的產品。 [GitHub-28388](https://github.com/magento/magento2/issues/28388)
...
* GraphQL URL解析程式現在可以正確處理查詢參數。 以前，使用參數處理SEO友好型URL時傳回NULL。

<!-- ENGCOM-8422 -->

* GraphQL URL解析程式現在可以正確處理CMS頁面階層。 以前它返回NULL。 [GitHub-30474](https://github.com/magento/magento2/issues/30474)

<!-- MC-38209-->

* 產品的查詢 `categories.breadcrumbs` 若父類別停用，資料不再傳回階層連結資料。 [GitHub-30468](https://github.com/magento/magento2/issues/30468)

<!-- MC-38407-->

* 停用URL重新寫入時，現在可正確傳回類別影像的路徑。

<!-- MC-36648-->

* 現在，當GraphQL密碼重設操作期間發生錯誤時，應用程式會傳回資訊更豐富的訊息。 [GitHub-30179](https://github.com/magento/magento2/issues/30179)

<!-- MC-38078-->

* 快取查詢結果時，產品位置現在會如預期更新GraphQL查詢結果。
...
* 此 `FlushCacheByTags` 類別已更新，以使用外掛程式，而非使用外掛程式。 [GitHub-29558](https://github.com/magento/magento2/issues/29558)
...
* URL重寫產生器現在會設定類別物件 `url_key`, `url_path`，和  `store_id` 為全域範圍儲存類別URL重新寫入時，預設儲存值。 之前，它會儲存 `url_key`, `url_path`，和 `store_id` 和上次處理的儲存ID的值。 [GitHub-29585](https://github.com/magento/magento2/issues/29585)###分組產品
...
* GraphQL現在會執行目錄權限。 應用程式會限制針對 `products` 根據購物者的客戶群組進行查詢。
...
* Magento Open Source通過消除每個索引器實例多次的標籤刷新而提高了快取效能。 [GitHub-29890](https://github.com/magento/magento2/issues/29890)
...
* 訪客購物者現在可以在 **自動分配給客戶組** 啟用。 付款會在PayPal閘道上處理，訂單以Magento建立，購物者會看見訂單感謝頁面。 以前，訂單在PayPal閘道上已成功處理，但訂單未在Magento Open Source中建立。 反之，購物者已重新導向至訂單檢閱頁面，應用程式在此顯示此錯誤： `Failed address validation: %1" error`. [GitHub-25399](https://github.com/magento/magento2/issues/25399)
...
* Elasticsearch在搜尋同義字元上搜尋時，不會再擲回未知的修飾字元錯誤。 之前，逸出器在規則運算式中缺少逸出斜線符號，導致錯誤。
...
* 包含URL的標題值現在會在 `ApiFunctional TestFramework`. [GitHub-26425](https://github.com/magento/magento2/issues/26425)
...
**問題**:此 `[magento_root]/index.php` 檔案已移除，應用程式現在可從 `/pub` 依預設，適用於Apache設定。 從子資料夾提供的儲存將無法如預期運作，且可能會顯示404錯誤。 **因應措施**:使用symlink將安裝模擬到子資料夾中。 下列範例使用 `https://shop01.com/shop/` 以說明如何使用symlink模擬子資料夾中的安裝。
...
**問題**:當購物者點按時，應用程式會以預期的Braintree建立訂單 **使用Venmo支付**，但不會在管理員中建立順序。 請參閱 [2.4.2已知問題：BraintreeVenmo付款無法運作](https://support.magento.com/hc/en-us/articles/360054662652/) 知識庫文章。  <!-- BUNDLE-2894-->

**問題**:您無法使用 `addConfigurableProductToCart` GraphQL變異，可在執行Inventory的多商店部署中，以非預設的商店視圖將可配置產品添加到購物車。 （路馬店不受影響。） 應用程式顯示此錯誤： `Could not add item to cart. Please check required options and try again`. **因應措施**:使用 `addProductsToCart` 而是變異。 [GitHub-31660](https://github.com/magento/magento2/issues/31660)<!-- PWA-1298-->
