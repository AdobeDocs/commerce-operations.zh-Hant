...
badgePaas: label="僅限雲端上的Adobe Commerce" type="Informative" url="https://experienceleague.adobe.com/zh-hant/docs/commerce/user-guides/product-solutions" tooltip="僅適用於雲端專案上的Adobe Commerce 。"
hide: true
...
Adobe企業安全性原則要求雲端上Adobe Commerce的所有Adobe代管環境都必須在安全且相容的軟體上執行。

1. 所有協力廠商軟體相依專案(PHP、MariaDB、Elasticsearch/OpenSearch、Redis、RabbitMQ)的支援版本

1. 雲端上的Adobe Commerce （版本2.4.8、2.4.9或最新版本）

這是為了降低您的電子商務環境中的安全性風險。 未在[表1](#determine-your-required-actions)的截止日期前符合這些要求的環境將暫停輸入流量，使店面離線。 請將此通知視為包含執行日期的安全性和合規性要求。
...
在下表中，尋找您在雲端上的Adobe Commerce版本，以瞭解您的需求。
...
| 2.4.8或2.4.9 | 視軟體而定，需在2026年10月30日或2027年5月31日之前採取必要動作。 | 目前不需要採取任何動作 |

## 不需要採取動作的使用者

本通知不適用於：

* 使用[!DNL Adobe Commerce as a Cloud Service]的客戶
* 在所有環境中使用具有受支援軟體相依性的Cloud 2.4.8或2.4.9版上Adobe Commerce的客戶

### 驗證您目前的版本

您需要電子商務管理員的協助，才能進行以下步驟，以檢查您在雲端環境的每個Adobe Commerce中執行的版本。

#### 步驟1：檢查雲端上的Adobe Commerce版本
...
>[!WARNING]
>
>若您的環境未符合[必要動作與截止日期表格](#determine-your-required-actions)中詳述的截止日期的要求，Adobe將被強制暫停受影響環境的傳入流量。 您的電子商務店面將會離線，不會為購物者提供服務。

## 每個動作需要什麼

### 動作1：升級協力廠商軟體相依性

視軟體而定，所有不支援的軟體相依性都必須根據下表所共用的時間表進行升級。 您可以在[雲端主控台](https://console.adobecommerce.com/)中檢視環境。 若要驗證每個環境中執行的相依性版本，請參閱[檢查軟體相依性版本](#check-software-dependency-versions)。 軟體相依性升級適用於Cloud 2.4.4到2.4.9版的所有Adobe Commerce。
...
* **快速入門：**&#x200B;開啟支援票證，其中列出您需要升級的環境和相關的相依性。 請在執法日期前至少30天開啟票證，以便我們的團隊能排程工作。
...
### 動作2：升級至支援的Adobe Commerce版本

如果您需要升級雲端上的Adobe Commerce版本，您有兩個選項：

1. [升級至雲端上支援的Adobe Commerce版本](#upgrade-to-adobe-commerce-on-cloud-version-249)
1. [移轉至Adobe Commerce as a Cloud Service （SaaS平台）](#migrate-to-adobe-commerce-as-a-cloud-service)

無論您選擇哪個選項，目前版本的強制實施日期皆適用。

**表3：在雲端版本**&#x200B;上升級至受支援Adobe Commerce的准則與截止日期
...
若要決定哪個選項適合您的需求，請參閱下表比較Adobe Commerce as a Cloud Service在Cloud 2.4.9版上的Adobe Commerce。

**表4：雲端上的Adobe Commerce與Adobe Commerce as a Cloud Service的比較**
...
如果環境在[中的執行日期尚未符合這些要求，請決定您的必要動作](#determine-your-required-actions)，Adobe將會採取適當的動作。 這包括暫停流向受影響基礎結構的流量，因此，您的電子商務店面將會離線。
...
### 升級至Adobe Commerce on Cloud 2.4.9版
...
Adobe提供的工具可減少移轉至Adobe Commerce as a Cloud Service的成本和時間。 這些工具僅適用於移轉。 它們不用於雲端上的Adobe Commerce版本升級。 如需完整的移轉指南，包括移轉路徑和階段，請參閱[移轉概觀](https://experienceleague.adobe.com/zh-hant/docs/commerce/cloud-service/migration/overview)。
...
* **店面與擴充功能移轉：** Adobe的[AI輔助移轉與開發人員工具](https://developer.adobe.com/commerce/extensibility/developer-agent/) （包括[!DNL Adobe Developer App Builder]與[!DNL Commerce Storefront powered by Edge Delivery Services]）可協助加速店面現代化與擴充功能重新平台。
