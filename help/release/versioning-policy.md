...
Hotfix是包含高影響力安全性或品質修正的修補程式，例如影響許多商家的零日漏洞修正。 Adobe會視需要發行支援Adobe Commerce版本的Hotfix，在這些版本發生重大安全性或品質問題時提供支援。 Hotfix是透過[品質修補程式工具](../tools/quality-patches-tool/usage.md)傳遞。 這些修正包含在下一個計畫的修補程式發行版本中。
...
個別修補程式包含特定問題的低影響品質修正。 這些修正會套用至支援的Adobe Commerce次要版本。 根據[軟體生命週期原則](lifecycle-policy.md)，Adobe會根據Adobe Commerce的需要發行個別修補程式。 如需法律條款，請參閱[Adobe Commerce軟體生命週期原則(PDF)](https://www.adobe.com/content/dam/cc/en/legal/terms/enterprise/pdfs/Adobe-Commerce-Software-Lifecycle-Policy.pdf)。 它們會透過[品質修補工具](../tools/quality-patches-tool/usage.md)傳遞。
