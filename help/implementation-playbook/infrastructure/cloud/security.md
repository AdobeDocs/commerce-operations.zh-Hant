...
所有可執行代碼都部署到只讀檔案系統映像中，這大大減少了可用於攻擊的表面。 部署過程建立Swach-FS映像。 此方法大大減少了將PHP或JavaScript代碼注入系統或修改Adobe Commerce應用程式檔案的機會。
...
客戶擁有其資料，並控制該資料的位置。 客戶指定其生產和開發實例所在的位置。 它們還指定了與Commerce一起用於Adobe Commerce報告環境的位置，以及該Adobe Commerce報告應用程式是否有權訪問個人身份資訊。 生產實例可以位於大多數AWS地區，而開發環境和Adobe Commerce報告環境則可以在美國或歐盟境內找到。
